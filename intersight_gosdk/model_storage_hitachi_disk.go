...
type StorageHitachiDisk struct {
	...
	// Drive type of the Hitachi Disk. * `N/A` - Drive Type is not available. * `SAS` - SAS. * `SSD(MLC)` - SSD (MLC). * `SSD(FMC)` - SSD (FMC). * `SSD(FMD)` - SSD (FMD). * `SSD(SLC)` - SSD (SLC). * `SSD` - SSD. * `SSD(RI)` - SSD (RI).
