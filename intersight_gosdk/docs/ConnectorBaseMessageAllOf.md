...
**EncryptionKey** | Pointer to **string** | The public key that was used to encrypt the values present in SecureProperties dictionary. If the given public key is not the same as device connector&#39;s public key, an error response with appropriate error message is thrown back. | [optional] 
