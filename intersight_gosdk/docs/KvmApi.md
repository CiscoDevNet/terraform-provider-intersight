...
[**GetKvmKvmSessionByMoid**](KvmApi.md#GetKvmKvmSessionByMoid) | **Get** /api/v1/kvm/KvmSessions/{Moid} | Read a &#39;kvm.KvmSession&#39; resource.
[**GetKvmKvmSessionList**](KvmApi.md#GetKvmKvmSessionList) | **Get** /api/v1/kvm/KvmSessions | Read a &#39;kvm.KvmSession&#39; resource.
...
func main() {
    ...
    resp, r, err := api_client.KvmApi.CreateKvmPolicy(context.Background(), kvmPolicy).IfMatch(ifMatch).IfNoneMatch(ifNoneMatch).Execute()
    if err != nil {
...
## GetKvmKvmSessionByMoid

> KvmKvmSession GetKvmKvmSessionByMoid(ctx, moid).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.KvmApi.GetKvmKvmSessionByMoid(context.Background(), moid).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `KvmApi.GetKvmKvmSessionByMoid``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `GetKvmKvmSessionByMoid`: KvmKvmSession
    fmt.Fprintf(os.Stdout, "Response from `KvmApi.GetKvmKvmSessionByMoid`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetKvmKvmSessionByMoidRequest struct via the builder pattern
...
[**KvmKvmSession**](kvm.KvmSession.md)
...
## GetKvmKvmSessionList

> KvmKvmSessionResponse GetKvmKvmSessionList(ctx).Filter(filter).Orderby(orderby).Top(top).Skip(skip).Select_(select_).Expand(expand).Apply(apply).Count(count).Inlinecount(inlinecount).At(at).Tags(tags).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.KvmApi.GetKvmKvmSessionList(context.Background(), ).Filter(filter).Orderby(orderby).Top(top).Skip(skip).Select_(select_).Expand(expand).Apply(apply).Count(count).Inlinecount(inlinecount).At(at).Tags(tags).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `KvmApi.GetKvmKvmSessionList``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `GetKvmKvmSessionList`: KvmKvmSessionResponse
    fmt.Fprintf(os.Stdout, "Response from `KvmApi.GetKvmKvmSessionList`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetKvmKvmSessionListRequest struct via the builder pattern
...
[**KvmKvmSessionResponse**](kvm.KvmSession.Response.md)
...
func main() {
    moid := "moid_example" // string | The unique Moid identifier of a resource instance.
    kvmPolicy := openapiclient.kvm.Policy{AccountMoid: "AccountMoid_example", ClassId: "ClassId_example", CreateTime: "TODO", DomainGroupMoid: "DomainGroupMoid_example", ModTime: "TODO", Moid: "Moid_example", ObjectType: "ObjectType_example", Owners: []string{"Owners_example"), SharedScope: "SharedScope_example", Tags: []MoTag{), VersionContext: , Ancestors: []MoBaseMoRelationship{), Parent: , PermissionResources: []MoBaseMoRelationship{), DisplayNames: map[string]string{ "Key" = "Value" }, Description: "Description_example", Name: "Name_example", EnableLocalServerVideo: false, EnableVideoEncryption: false, Enabled: false, MaximumSessions: int64(123), RemotePort: int64(123), Organization: , Profiles: []PolicyAbstractConfigProfileRelationship{)} // KvmPolicy | The 'kvm.Policy' resource to update.
    ifMatch := "ifMatch_example" // string | For methods that apply server-side changes, and in particular for PUT, If-Match can be used to prevent the lost update problem. It can check if the modification of a resource that the user wants to upload will not override another change that has been done since the original resource was fetched. If the request cannot be fulfilled, the 412 (Precondition Failed) response is returned. When modifying a resource using POST or PUT, the If-Match header must be set to the value of the resource ModTime property after which no lost update problem should occur. For example, a client send a GET request to obtain a resource, which includes the ModTime property. The ModTime indicates the last time the resource was created or modified. The client then sends a POST or PUT request with the If-Match header set to the ModTime property of the resource as obtained in the GET request. (optional)

    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.KvmApi.PatchKvmPolicy(context.Background(), moid, kvmPolicy).IfMatch(ifMatch).Execute()
    if err != nil {
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.KvmApi.UpdateKvmPolicy(context.Background(), moid, kvmPolicy).IfMatch(ifMatch).Execute()
    if err != nil {
