...
**Cpu** | Pointer to **int64** | Number of vCPUs to be allocated to virtual machine. The upper limit depends on the hypervisor. | [optional] 
...
**Memory** | Pointer to **int64** | Virtual machine memory in mebi bytes (one mebibyte, 1MiB, is 1048576 bytes, and 1KiB is 1024 bytes). Input must be a whole number and scientific notation is not acceptable. For example, enter 1730 and not 1.73e03. No upper limit is enforced because hypervisors increase the limit in every release. | [optional] 
