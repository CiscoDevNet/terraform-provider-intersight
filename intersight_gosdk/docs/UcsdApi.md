...
## DeleteUcsdBackupInfo

> DeleteUcsdBackupInfo(ctx, moid).Execute()
...
func main() {
    ...
    resp, r, err := api_client.UcsdApi.DeleteUcsdBackupInfo(context.Background(), moid).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `UcsdApi.DeleteUcsdBackupInfo``: %v\n", err)
...
Other parameters are passed through a pointer to a apiDeleteUcsdBackupInfoRequest struct via the builder pattern
...
> UcsdBackupInfo GetUcsdBackupInfoByMoid(ctx, moid).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.UcsdApi.GetUcsdBackupInfoByMoid(context.Background(), moid).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `UcsdApi.GetUcsdBackupInfoByMoid``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `GetUcsdBackupInfoByMoid`: UcsdBackupInfo
    fmt.Fprintf(os.Stdout, "Response from `UcsdApi.GetUcsdBackupInfoByMoid`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetUcsdBackupInfoByMoidRequest struct via the builder pattern
...
## GetUcsdBackupInfoList

> UcsdBackupInfoResponse GetUcsdBackupInfoList(ctx).Filter(filter).Orderby(orderby).Top(top).Skip(skip).Select_(select_).Expand(expand).Apply(apply).Count(count).Inlinecount(inlinecount).At(at).Tags(tags).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.UcsdApi.GetUcsdBackupInfoList(context.Background(), ).Filter(filter).Orderby(orderby).Top(top).Skip(skip).Select_(select_).Expand(expand).Apply(apply).Count(count).Inlinecount(inlinecount).At(at).Tags(tags).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `UcsdApi.GetUcsdBackupInfoList``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `GetUcsdBackupInfoList`: UcsdBackupInfoResponse
    fmt.Fprintf(os.Stdout, "Response from `UcsdApi.GetUcsdBackupInfoList`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetUcsdBackupInfoListRequest struct via the builder pattern
