...
// WorkloadWorkloadDefinition ### Overview The WorkloadDefinition object represents the blueprint for defining reusable workload configurations that can be deployed across one or more organizations. It serves as the foundational template that captures the complete specification of a workload, including its associated blueprints, validation requirements, and deployment strategies. #### Purpose A WorkloadDefinition provides a versioned, reusable specification for workloads that can be deployed multiple times across different contexts organizations. It enables administrators to define complex multi-component workloads once and deploy them consistently, supporting both iterative development through versioning and controlled rollout through deployment strategies. #### Key Concepts - **Platform Support:** - Platform types can be specified to indicate which Intersight platforms support the workload, ensuring deployments target compatible infrastructure. - **Versioning and Preferred Versions:** - Each workload definition supports multiple versions, with the ability to designate a preferred (default) version. This allows for safe evolution of workload specifications while maintaining backward compatibility with existing deployments. - **Blueprint Composition:** - Workload definitions are composed of one or more blueprint references, enabling modular design where complex workloads are built from reusable blueprint components. - **Validation and Status Tracking:** - Each definition maintains validation information to ensure all required inputs and dependencies are properly configured before deployment, along with deployment summary aggregations across all instances. - **Override Inputs:** - Each workload definition can specify input overrides for the blueprints it references, allowing for customization within deployments.
type WorkloadWorkloadDefinition struct {
	...
	WorkloadMetadata     NullableWorkloadWorkloadMetadataRelationship `json:"WorkloadMetadata,omitempty"`
...
// GetWorkloadMetadata returns the WorkloadMetadata field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WorkloadWorkloadDefinition) GetWorkloadMetadata() WorkloadWorkloadMetadataRelationship {
	if o == nil || IsNil(o.WorkloadMetadata.Get()) {
		var ret WorkloadWorkloadMetadataRelationship
		return ret
	}
	return *o.WorkloadMetadata.Get()
}

// GetWorkloadMetadataOk returns a tuple with the WorkloadMetadata field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
...
// SetWorkloadMetadata gets a reference to the given NullableWorkloadWorkloadMetadataRelationship and assigns it to the WorkloadMetadata field.
func (o *WorkloadWorkloadDefinition) SetWorkloadMetadata(v WorkloadWorkloadMetadataRelationship) {
	o.WorkloadMetadata.Set(&v)
}

// SetWorkloadMetadataNil sets the value for WorkloadMetadata to be an explicit nil
func (o *WorkloadWorkloadDefinition) SetWorkloadMetadataNil() {
	o.WorkloadMetadata.Set(nil)
}
...
func (o WorkloadWorkloadDefinition) ToMap() (map[string]interface{}, error) {
	...
	if o.WorkloadMetadata.IsSet() {
		toSerialize["WorkloadMetadata"] = o.WorkloadMetadata.Get()
	}
...
func (o *WorkloadWorkloadDefinition) UnmarshalJSON(data []byte) (err error) {
	...
	if err == nil {
		...
		varWorkloadWorkloadDefinition.WorkloadMetadata = varWorkloadWorkloadDefinitionWithoutEmbeddedStruct.WorkloadMetadata
	...
	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		...
		delete(additionalProperties, "WorkloadMetadata")
