...
// IssueCondition Condition defines the set of criteria under which an issue exists.
...
func NewIssueCondition(classId string, objectType string) *IssueCondition {
	this := IssueCondition{}
...
func (o *IssueCondition) UnmarshalJSON(bytes []byte) (err error) {
	type IssueConditionWithoutEmbeddedStruct struct {
	}

	varIssueConditionWithoutEmbeddedStruct := IssueConditionWithoutEmbeddedStruct{}

	err = json.Unmarshal(bytes, &varIssueConditionWithoutEmbeddedStruct)
	if err == nil {
	...
	err = json.Unmarshal(bytes, &varIssueCondition)
	if err == nil {
		o.MoBaseComplexType = varIssueCondition.MoBaseComplexType
	} else {
		return err
	}
