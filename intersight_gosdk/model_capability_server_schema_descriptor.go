...
// checks if the CapabilityServerSchemaDescriptor type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CapabilityServerSchemaDescriptor{}

// CapabilityServerSchemaDescriptor The capability.ServerSchemaDescriptor is a hardware descriptor that identifies a server's Redfish capability for specific actions, such as controlling the locator LED, based on its CIMC firmware and Redfish schema version. #### Purpose The primary purpose of this object is to create a mapping between a server's firmware version and the specific Redfish properties used to control certain hardware features. For example, it defines the correct locatorLedName property to use for toggling the locator LED, which can vary between different Redfish schema versions. This allows for consistent management actions across a diverse range of server firmware. #### Key Concepts - **Redfish Capability Mapping:** Links server hardware actions to specific Redfish schema properties (redfishSchema, locatorLedName). - **Firmware-Dependent Logic:** The descriptor is identified by vendor, model, and version, making the mapping dependent on the server's firmware. - **Action Abstraction:** Allows the management system to use a consistent action (e.g., \"turn on locator LED\") while the descriptor provides the specific implementation detail for the target server. - **Extensible Catalog:** As a HardwareDescriptor, it is part of a catalog that can be updated to support new server models and Redfish schema versions without changing core management logic.
...
// GetDefaultClassId returns the default value "capability.ServerSchemaDescriptor" of the ClassId field.
func (o *CapabilityServerSchemaDescriptor) GetDefaultClassId() interface{} {
	return "capability.ServerSchemaDescriptor"
}
...
// GetDefaultObjectType returns the default value "capability.ServerSchemaDescriptor" of the ObjectType field.
func (o *CapabilityServerSchemaDescriptor) GetDefaultObjectType() interface{} {
	return "capability.ServerSchemaDescriptor"
}
...
func (o *CapabilityServerSchemaDescriptor) GetLocatorLedName() string {
	if o == nil || IsNil(o.LocatorLedName) {
		var ret string
		return ret
	}
...
func (o *CapabilityServerSchemaDescriptor) GetRedfishSchema() string {
	if o == nil || IsNil(o.RedfishSchema) {
		var ret string
		return ret
	}
...
func (o CapabilityServerSchemaDescriptor) ToMap() (map[string]interface{}, error) {
	...
	if !IsNil(o.LocatorLedName) {
		toSerialize["LocatorLedName"] = o.LocatorLedName
	}
	if !IsNil(o.RedfishSchema) {
...
func (o *CapabilityServerSchemaDescriptor) UnmarshalJSON(data []byte) (err error) {
	...
	err = json.Unmarshal(data, &varCapabilityServerSchemaDescriptorWithoutEmbeddedStruct)
	if err == nil {
	...
	err = json.Unmarshal(data, &varCapabilityServerSchemaDescriptor)
	if err == nil {
