...
// checks if the SyslogPolicy type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SyslogPolicy{}

// SyslogPolicy The Syslog Policy is a reusable policy for configuring how system log messages are handled on an endpoint. #### Purpose The purpose of this policy is to centralize and standardize the logging configuration for managed devices. It allows administrators to forward system logs to one or more remote syslog servers for aggregation, analysis, and long-term storage, which is essential for monitoring, troubleshooting, and security auditing. #### Key Concepts - **Remote and Local Logging:** The policy supports configuring both remote logging clients (to forward logs to a central server) and local logging clients (to control how logs are handled on the device itself). - **Multiple Destinations:** Administrators can define a list of remote syslog servers, each with its own specific configuration. - **Flexible Transport:** For remote logging, the policy supports both UDP and TCP as transport protocols. - **Severity Filtering:** It allows for the configuration of a minSeverity level for both local and remote logs, enabling administrators to filter out less important messages and reduce noise. - **Profile-Based Application:** The policy is attached to a Server Profile to apply a consistent logging configuration across multiple servers.
...
// GetDefaultClassId returns the default value "syslog.Policy" of the ClassId field.
func (o *SyslogPolicy) GetDefaultClassId() interface{} {
...
// GetDefaultObjectType returns the default value "syslog.Policy" of the ObjectType field.
func (o *SyslogPolicy) GetDefaultObjectType() interface{} {
	return "syslog.Policy"
}
...
func (o *SyslogPolicy) SetOrganizationNil() {
...
func (o SyslogPolicy) ToMap() (map[string]interface{}, error) {
...
func (o *SyslogPolicy) UnmarshalJSON(data []byte) (err error) {
	...
	err = json.Unmarshal(data, &varSyslogPolicyWithoutEmbeddedStruct)
	if err == nil {
	...
	err = json.Unmarshal(data, &varSyslogPolicy)
	if err == nil {
