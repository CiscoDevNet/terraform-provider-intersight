...
// IamEndPointUserInventory The IAM EndPointUser object represents a single local user account. It is a building block used within the EndPointUserPolicy to define the users that should be created on an endpoint. #### Purpose The purpose is to define the identity of a local user, primarily their username. It acts as a referenceable entity that can be associated with roles and a password within the broader EndPointUserPolicy. #### Key Concepts - **User Identity:** The core function of this is to specify the name (username) of the local user. - **Component of a Policy:** It does not function on its own but is created as part of an EndPointUserPolicy. - **Reusable within an Organization:** As an organization-owned object, a single EndPointUser can be referenced in multiple EndPointUserRole mappings across different policies, though it typically represents a unique user identity.
