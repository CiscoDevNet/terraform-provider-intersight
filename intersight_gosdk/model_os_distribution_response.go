...
func (dst *OsDistributionResponse) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["ObjectType"] == "mo.AggregateTransform" {
		...
		if err == nil {
			...
			return fmt.Errorf("failed to unmarshal OsDistributionResponse as MoAggregateTransform: %s", err.Error())
		}
	}
	...
	if jsonDict["ObjectType"] == "mo.DocumentCount" {
		...
		if err == nil {
			...
			return fmt.Errorf("failed to unmarshal OsDistributionResponse as MoDocumentCount: %s", err.Error())
		}
	}
	...
	if jsonDict["ObjectType"] == "mo.TagSummary" {
		...
		if err == nil {
			...
			return fmt.Errorf("failed to unmarshal OsDistributionResponse as MoTagSummary: %s", err.Error())
		}
	}

	// check if the discriminator value is 'os.Distribution.List'
	if jsonDict["ObjectType"] == "os.Distribution.List" {
		// try to unmarshal JSON data into OsDistributionList
		err = json.Unmarshal(data, &dst.OsDistributionList)
		if err == nil {
			return nil // data stored in dst.OsDistributionList, return on the first match
		} else {
			dst.OsDistributionList = nil
			return fmt.Errorf("failed to unmarshal OsDistributionResponse as OsDistributionList: %s", err.Error())
		}
	}
