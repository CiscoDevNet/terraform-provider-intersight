...
// checks if the IamEndPointUserRole type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &IamEndPointUserRole{}

// IamEndPointUserRole The IAM EndPointUserRole object is a mapping object that links a local user to their assigned roles and password within the context of an EndPointUserPolicy. #### Purpose The IAM EndPointUserRole objects purpose is to bind together three key pieces of information: the user, their password, and their permissions. It serves as the mechanism for granting specific roles (like admin or read-only) to an EndPointUser and securely storing the encrypted password for that user's account on the endpoint. #### Key Concepts - **User-Role-Password Mapping:** It connects an EndPointUser object with a collection of EndPointRole objects and contains the password for that user. - **Secure Password Handling:** The password property is write-only and encrypted, ensuring credentials are not exposed after being set. - **Component of a Policy:** It exists as part of the endPointUserRoles collection within an EndPointUserPolicy and is essential for defining a complete user account. - **Account Status:** Includes an enabled flag to control whether the user account is active on the endpoint.
type IamEndPointUserRole struct {
	...
	EndPointUserPolicy   NullableIamEndPointUserPolicyRelationship `json:"EndPointUserPolicy,omitempty"`
...
// GetDefaultClassId returns the default value "iam.EndPointUserRole" of the ClassId field.
func (o *IamEndPointUserRole) GetDefaultClassId() interface{} {
...
// SetEndPointUser gets a reference to the given NullableIamEndPointUserRelationship and assigns it to the EndPointUser field.
...
func (o *IamEndPointUserRole) SetEndPointUserNil() {
...
// SetEndPointUserPolicy gets a reference to the given NullableIamEndPointUserPolicyRelationship and assigns it to the EndPointUserPolicy field.
...
func (o *IamEndPointUserRole) SetEndPointUserPolicyNil() {
...
func (o IamEndPointUserRole) ToMap() (map[string]interface{}, error) {
...
func (o *IamEndPointUserRole) UnmarshalJSON(data []byte) (err error) {
	...
	err = json.Unmarshal(data, &varIamEndPointUserRoleWithoutEmbeddedStruct)
	if err == nil {
	...
	err = json.Unmarshal(data, &varIamEndPointUserRole)
	if err == nil {
