...
# Resource: intersight_software_hci_distributable
An HCI image distributed by Cisco.
## Argument Reference
...
* `meta_info`:(JSON as string)(ReadOnly) Additional information associated with the distributable object. This data is provided as a JSON blob by Nutanix, a partner vendor, and the format is not fixed. 
...
## Import
`intersight_software_hci_distributable` can be imported using the Moid of the object, e.g.
```
$ terraform import intersight_software_hci_distributable.example 1234567890987654321abcde
``` 
