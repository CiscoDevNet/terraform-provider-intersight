...
This complex property has following sub-properties:
  ...
  + `object_type`:(string) The fully-qualified name of the remote type referred by this relationship. 
  + `selector`:(string) An OData $filter expression which describes the REST resource to be referenced. This field maybe set instead of 'moid' by clients.1. If 'moid' is set this field is ignored.1. If 'selector' is set and 'moid' is empty/absent from the request, Intersight determines the Moid of theresource matching the filter expression and populates it in the MoRef that is part of the objectinstance being inserted/updated to fulfill the REST request.An error is returned if the filter matches zero or more than one REST resource.An example filter string is: Serial eq '3AA8B7T11'. 
* `cloud_resource`:(Array)
This complex property has following sub-properties:
  + `current_status`:(string) Currentstatus of the resource if applicable on the cloud. 
...
* `output`:(JSON as string) Terraform output of the entire execution. 
* `owners`:
...
This complex property has following sub-properties:
  + `object_type`:(string) The fully-qualified name of the instantiated, concrete type.The value should be the same as the 'ClassId' property. 
  + `run_id`:(string) Run identifier for every terraform execution. 
  + `state_file`:(string) StateFile identifier of terraform execution. 
...
* `stderr`:(JSON as string) Stderr of the terraform execution will be captured here. 
* `stdout`:(JSON as string) Stdout of the terraform execution will be captured here. 
* `tags`:(Array)
This complex property has following sub-properties:
  + `key`:(string) The string representation of a tag key. 
  + `value`:(string) The string representation of a tag value. 
* `task_id`:(string) TaskId of a pontem workflow is same as the MO. 
* `variables`:(JSON as string) Variables needed by the terraform configuration as a JSON object. 
...
## Import
`intersight_terraform_executor` can be imported using the Moid of the object, e.g.
```
$ terraform import intersight_terraform_executor.example 1234567890987654321abcde
``` 