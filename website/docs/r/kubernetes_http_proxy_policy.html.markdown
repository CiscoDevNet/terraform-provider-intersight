...
layout: "intersight"
page_title: "Intersight: intersight_kubernetes_http_proxy_policy"
description: |-
        A policy specifying HTTP and HTTPS proxy configuration.

---

# Resource: intersight_kubernetes_http_proxy_policy
A policy specifying HTTP and HTTPS proxy configuration.
## Argument Reference
...
* `http_proxy`:(HashMap) - The proxy configuration for HTTP communication. 
This complex property has following sub-properties:
...
* `https_proxy`:(HashMap) - The proxy configuration for HTTPS communication. 
This complex property has following sub-properties:
  + `hostname`:(string) HTTP/HTTPS Proxy server FQDN or IP. 
  + `is_password_set`:(bool)(ReadOnly) Indicates whether the value of the 'password' property has been set. 
  + `object_type`:(string) The fully-qualified name of the instantiated, concrete type.The value should be the same as the 'ClassId' property. 
...
## Import
`intersight_kubernetes_http_proxy_policy` can be imported using the Moid of the object, e.g.
```
$ terraform import intersight_kubernetes_http_proxy_policy.example 1234567890987654321abcde
``` 
