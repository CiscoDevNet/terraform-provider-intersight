...
* `holder`:(Array with Maximum of one item) -(Computed) A reference to a iamSecurityHolder resource.When the $expand query parameter is specified, the referenced resource is returned inline. 
