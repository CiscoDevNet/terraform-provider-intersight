...
* `last_notification_time`:(string)(ReadOnly) Holds the information on when last email notification was sent to the guest users. 
...
* `user_details`:(Array)
This complex property has following sub-properties:
  + `email_notification_status`:(string)(ReadOnly) Stores the information on whether user has been sent an email about access details or not.* `NotSent` - The Account Admin might not choose send email notification to any of their guest users.* `Sent` - An email notification sent to notified users who are configured per guest access link. 
  + `object_type`:(string) The fully-qualified name of the instantiated, concrete type.The value should be the same as the 'ClassId' property. 
  + `user_email`:(string) Stores the user email ID of the guest user. 
  + `user_name`:(string) Stores the user name of the guest user. 
...
## Import
`intersight_iam_user_qualifier` can be imported using the Moid of the object, e.g.
```
$ terraform import intersight_iam_user_qualifier.example 1234567890987654321abcde
``` 
