...
* `model_constraint`:(Array)
This complex property has following sub-properties:
  + `max_version`:(string) UCSM version above which the server model is unsupported. 
  + `name`:(string) Name of the platform constraint. 
...
## Import
`intersight_softwarerepository_category_unsupported_models` can be imported using the Moid of the object, e.g.
```
$ terraform import intersight_softwarerepository_category_unsupported_models.example 1234567890987654321abcde
``` 
