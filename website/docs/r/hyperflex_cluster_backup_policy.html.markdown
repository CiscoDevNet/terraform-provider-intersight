...
* `backup_target`:(Array with Maximum of one item) - A reference to a hyperflexCluster resource.When the $expand query parameter is specified, the referenced resource is returned inline. 
...
* `replication_schedule`:(Array with Maximum of one item) - Backup policy replication schedule. 
This complex property has following sub-properties:
  + `additional_properties`:
(Array with Maximum of one item) - Add additional properties in json format inside `jsonencode()` for this object.
