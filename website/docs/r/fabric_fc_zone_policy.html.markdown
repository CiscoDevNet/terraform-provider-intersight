...
# Resource: intersight_fabric_fc_zone_policy
List of target path entries of storage arrays that are used to configure zones on the switch.
## Argument Reference
...
* `fc_target_members`:(Array)
This complex property has following sub-properties:
  + `name`:(string) Name given to the target member. 
  + `object_type`:(string) The fully-qualified name of the instantiated, concrete type.The value should be the same as the 'ClassId' property. 
  + `switch_id`:(string) Unique identifier for the Fabric object.* `A` - Switch Identifier of Fabric Interconnect A.* `B` - Switch Identifier of Fabric Interconnect B. 
  + `vsan_id`:(int) VSAN with scope defined as Storage in the VSAN policy. 
  + `wwpn`:(string) WWPN that is a member of the FC zone. 
...
## Import
`intersight_fabric_fc_zone_policy` can be imported using the Moid of the object, e.g.
```
$ terraform import intersight_fabric_fc_zone_policy.example 1234567890987654321abcde
``` 
