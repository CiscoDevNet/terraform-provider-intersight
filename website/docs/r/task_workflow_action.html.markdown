...
# Resource: intersight_task_workflow_action
Start a test workflow using this object.
## Argument Reference
...
* `workflow_file`:(HashMap) - Path to workflow metadata file that will be published and started. 
This complex property has following sub-properties:
  + `contents`:(string) When the type of the download is inline, it will read the file from the contents here. 
  + `object_type`:(string) The fully-qualified name of the instantiated, concrete type.The value should be the same as the 'ClassId' property. 
  + `path`:(string) The path of the download from the specified source location for type S3, then this is the object key. 
  + `nr_source`:(string) The source of the download location and if type is S3, then this is the bucket name. In case of MoRef download type the source will have the Moid of the workflow definition. 
  + `type`:(string) The type of download location is captured in type.* `S3` - Download workflow from S3.* `Local` - Download workflow from local path.* `Inline` - Workflow is provided inline.* `MoRef` - Start an existing workflow registered with given Moid. 
* `workflow_info_moid`:(string)(ReadOnly) Returns the workflow info moid of the workflow started by workflow action api. workflowInfoMoid will be an empty sting if an error occurs while starting the workflow. 


## Import
`intersight_task_workflow_action` can be imported using the Moid of the object, e.g.
```
$ terraform import intersight_task_workflow_action.example 1234567890987654321abcde
``` 
