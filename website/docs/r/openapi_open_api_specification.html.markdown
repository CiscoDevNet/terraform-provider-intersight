...
# Resource: intersight_openapi_open_api_specification
An OpenAPI specification file uploaded to generate workflow tasks.
## Argument Reference
...
* `file_path`:(string)(ReadOnly) The path of the file in S3/minio bucket. 
* `file_upload_id`:(string)(ReadOnly) A unique tracking ID for the file being uploaded. 
...
## Import
`intersight_openapi_open_api_specification` can be imported using the Moid of the object, e.g.
```
$ terraform import intersight_openapi_open_api_specification.example 1234567890987654321abcde
``` 
