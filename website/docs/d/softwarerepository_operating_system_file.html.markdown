...
To access the ith object of the results obtained, use `data.intersight_softwarerepository_operating_system_file.<custom_name>.results[i].<propertyname>`.
...
* `import_progress`:(int) The progress percentage for the import operation. 
...
* `sample_hashes`:(string) File sample hashes at deterministic positions for efficient duplicate detection of large files. 
