...
* `resource_type`:(string) The type of resource present in the pool, such as 'server' can be a RackUnit or Blade.* `None` - The resource cannot consider for Resource Pool.* `Server` - Resource Pool holds the server kind of resources, example - RackServer, Blade. 
