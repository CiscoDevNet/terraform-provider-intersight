...
* `extended_power_capacity`:(string) Sets the Extended Power Capacity of the System. This option is only supported for Cisco UCS X series Chassis.* `Enabled` - Set the value to Enabled.* `Disabled` - Set the value to Disabled. 
