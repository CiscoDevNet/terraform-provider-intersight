...
To access the ith object of the results obtained, use `data.intersight_virtualization_vmware_network.<custom_name>.results[i].<propertyname>`.
...
* `vlan_id`:(int) VLAN id with which the network is associated. A value of 0 specifies that port is not associated with a VLAN. 
