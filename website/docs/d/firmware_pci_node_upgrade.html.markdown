...
layout: "intersight"
page_title: "Intersight: intersight_firmware_pci_node_upgrade"
description: |-
...
# Data Source: intersight_firmware_pci_node_upgrade
Firmware upgrade operation for PciNode that downloads the image located at Cisco/appliance/user provided HTTP repository or uses the image from a network share and upgrade. Direct download is used for upgrade that uses the image from a Cisco repository or an appliance repository.
## Argument Reference
The results of this data source are stored in `results` property.
All objects matching the filter criteria are fetched through pagination.
To access the ith object of the results obtained, use `data.intersight_firmware_pci_node_upgrade.<custom_name>.results[i].<propertyname>`.
