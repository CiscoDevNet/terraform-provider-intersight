...
# Data Source: intersight_software_ucsd_bundle_distributable
A UCSD connector pack image bundle distributed by Cisco for Private Appliance.
## Argument Reference
The results of this data source are stored in `results` property.
All objects matching the filter criteria are fetched through pagination.
To access the ith object of the results obtained, use `data.intersight_software_ucsd_bundle_distributable.<custom_name>.results[i].<propertyname>`.
