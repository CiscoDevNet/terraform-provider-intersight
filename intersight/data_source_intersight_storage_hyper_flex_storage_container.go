...
func dataSourceStorageHyperFlexStorageContainer() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStorageHyperFlexStorageContainerRead,
...
func dataSourceStorageHyperFlexStorageContainerRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageHyperFlexStorageContainerList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageHyperFlexStorageContainer: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			...
			if err := d.Set("last_modified_time", (s.GetLastModifiedTime()).String()); err != nil {
				return fmt.Errorf("error occurred while setting property LastModifiedTime: %+v", err)
			}
			...
			if err := d.Set("provisioned_capacity", (s.GetProvisionedCapacity())); err != nil {
				return fmt.Errorf("error occurred while setting property ProvisionedCapacity: %+v", err)
			}
			...
			if err := d.Set("un_compressed_used_bytes", (s.GetUnCompressedUsedBytes())); err != nil {
				return fmt.Errorf("error occurred while setting property UnCompressedUsedBytes: %+v", err)
			}
