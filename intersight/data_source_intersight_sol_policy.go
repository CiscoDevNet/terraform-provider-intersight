...
func dataSourceSolPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.SolApi.GetSolPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to SolPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewSolPolicyWithDefaults()
			...
			if err := d.Set("com_port", (s.GetComPort())); err != nil {
				return fmt.Errorf("error occurred while setting property ComPort: %+v", err)
			}
			...
			if err := d.Set("ssh_port", (s.GetSshPort())); err != nil {
				return fmt.Errorf("error occurred while setting property SshPort: %+v", err)
			}
