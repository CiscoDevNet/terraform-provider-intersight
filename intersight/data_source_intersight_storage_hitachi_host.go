...
func dataSourceStorageHitachiHost() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStorageHitachiHostRead,
...
func dataSourceStorageHitachiHostRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStorageHitachiHostWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageHitachiHostList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageHitachiHost: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageHitachiHostWithDefaults()
			...
			if err := d.Set("authentication_mode", (s.GetAuthenticationMode())); err != nil {
				return fmt.Errorf("error occurred while setting property AuthenticationMode: %+v", err)
			}
			...
			if err := d.Set("host_group_id", (s.GetHostGroupId())); err != nil {
				return fmt.Errorf("error occurred while setting property HostGroupId: %+v", err)
			}
			if err := d.Set("host_group_number", (s.GetHostGroupNumber())); err != nil {
				return fmt.Errorf("error occurred while setting property HostGroupNumber: %+v", err)
			}
			if err := d.Set("host_mode_options", (s.GetHostModeOptions())); err != nil {
				return fmt.Errorf("error occurred while setting property HostModeOptions: %+v", err)
			}

			if err := d.Set("initiators", flattenListStorageBaseInitiator(s.GetInitiators(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property Initiators: %+v", err)
			}
			if err := d.Set("is_chap_mutual", (s.GetIsChapMutual())); err != nil {
				return fmt.Errorf("error occurred while setting property IsChapMutual: %+v", err)
			}
			if err := d.Set("iscsi_name", (s.GetIscsiName())); err != nil {
				return fmt.Errorf("error occurred while setting property IscsiName: %+v", err)
			}
			...
			if err := d.Set("os_type", (s.GetOsType())); err != nil {
				return fmt.Errorf("error occurred while setting property OsType: %+v", err)
			}
