...
func dataSourceIamEndPointRole() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIamEndPointRoleRead,
...
func dataSourceIamEndPointRoleRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIamEndPointRoleWithDefaults()
	...
	res, _, err := conn.ApiClient.IamApi.GetIamEndPointRoleList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IamEndPointRole: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIamEndPointRoleWithDefaults()
			...
			if err := d.Set("end_point_privileges", flattenListIamEndPointPrivilegeRelationship(s.GetEndPointPrivileges(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property EndPointPrivileges: %+v", err)
			}
			...
			if err := d.Set("role_type", (s.GetRoleType())); err != nil {
				return fmt.Errorf("error occurred while setting property RoleType: %+v", err)
			}
