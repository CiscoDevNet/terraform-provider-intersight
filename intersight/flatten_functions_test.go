...
func TestFlattenListApplianceStatusCheck(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"appliance.StatusCheck","ObjectType":"appliance.StatusCheck","Result":"Result %d","Code":"Code %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "appliance.StatusCheck", "object_type": "appliance.StatusCheck", "result": "Result 1", "code": "Code 1"}, {"class_id": "appliance.StatusCheck", "object_type": "appliance.StatusCheck", "result": "Result 2", "code": "Code 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListAssetMeteringType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Unit":"Unit %d","ClassId":"asset.MeteringType","ObjectType":"asset.MeteringType","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"unit": "Unit 1", "class_id": "asset.MeteringType", "object_type": "asset.MeteringType", "name": "Name 1"}, {"unit": "Unit 2", "class_id": "asset.MeteringType", "object_type": "asset.MeteringType", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCapabilityFeatureConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"capability.FeatureConfig","ObjectType":"capability.FeatureConfig","FeatureName":"FeatureName %d","MinFwVersion":"MinFwVersion %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "capability.FeatureConfig", "object_type": "capability.FeatureConfig", "feature_name": "FeatureName 1", "min_fw_version": "MinFwVersion 1"}, {"class_id": "capability.FeatureConfig", "object_type": "capability.FeatureConfig", "feature_name": "FeatureName 2", "min_fw_version": "MinFwVersion 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCertificatemanagementCertificateBase(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"class_id": "certificatemanagement.CertificateBase", "object_type": "certificatemanagement.CertificateBase", "enabled": true, "is_privatekey_set": true}, {"class_id": "certificatemanagement.CertificateBase", "object_type": "certificatemanagement.CertificateBase", "enabled": true, "is_privatekey_set": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudCloudTag(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Value":"Value %d","ClassId":"cloud.CloudTag","ObjectType":"cloud.CloudTag","Key":"Key %d"}`
	...
	expectedOp := []map[string]interface{}{{"value": "Value 1", "class_id": "cloud.CloudTag", "object_type": "cloud.CloudTag", "key": "Key 1"}, {"value": "Value 2", "class_id": "cloud.CloudTag", "object_type": "cloud.CloudTag", "key": "Key 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudCustomAttributes(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AttributeValue":"AttributeValue %d","AttributeName":"AttributeName %d","AttributeType":"AttributeType %d","ClassId":"cloud.CustomAttributes","ObjectType":"cloud.CustomAttributes"}`
...
func TestFlattenListCloudNetworkInterfaceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NetworkId":"NetworkId %d","SubNetworkId":"SubNetworkId %d","NicIndex":32,"IpForwardingEnabled":true,"Identity":"Identity %d","MacAddress":"MacAddress %d","SubNetworkName":"SubNetworkName %d","ClassId":"cloud.NetworkInterfaceAttachment","ObjectType":"cloud.NetworkInterfaceAttachment","NetworkName":"NetworkName %d"}`
	...
	expectedOp := []map[string]interface{}{{"network_id": "NetworkId 1", "sub_network_id": "SubNetworkId 1", "nic_index": 32, "ip_forwarding_enabled": true, "identity": "Identity 1", "mac_address": "MacAddress 1", "sub_network_name": "SubNetworkName 1", "class_id": "cloud.NetworkInterfaceAttachment", "object_type": "cloud.NetworkInterfaceAttachment", "network_name": "NetworkName 1"}, {"network_id": "NetworkId 2", "sub_network_id": "SubNetworkId 2", "nic_index": 32, "ip_forwarding_enabled": true, "identity": "Identity 2", "mac_address": "MacAddress 2", "sub_network_name": "SubNetworkName 2", "class_id": "cloud.NetworkInterfaceAttachment", "object_type": "cloud.NetworkInterfaceAttachment", "network_name": "NetworkName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudSecurityGroupRule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"StartPort":32,"Description":"Description %d","ClassId":"cloud.SecurityGroupRule","Identity":"Identity %d","Index":32,"EtherType":"EtherType %d","Protocol":"Protocol %d","SourceSecurityGroup":"SourceSecurityGroup %d","Action":"Action %d","ObjectType":"cloud.SecurityGroupRule","EndPort":32,"Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"start_port": 32, "description": "Description 1", "class_id": "cloud.SecurityGroupRule", "identity": "Identity 1", "index": 32, "ether_type": "EtherType 1", "protocol": "Protocol 1", "source_security_group": "SourceSecurityGroup 1", "action": "Action 1", "object_type": "cloud.SecurityGroupRule", "end_port": 32, "name": "Name 1"}, {"start_port": 32, "description": "Description 2", "class_id": "cloud.SecurityGroupRule", "identity": "Identity 2", "index": 32, "ether_type": "EtherType 2", "protocol": "Protocol 2", "source_security_group": "SourceSecurityGroup 2", "action": "Action 2", "object_type": "cloud.SecurityGroupRule", "end_port": 32, "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudTfcWorkspaceVariables(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Category":"Category %d","Description":"Description %d","Value":"Value %d","Sensitive":true,"ObjectType":"cloud.TfcWorkspaceVariables","Hcl":true,"Key":"Key %d","Identity":"Identity %d","ClassId":"cloud.TfcWorkspaceVariables"}`
	...
	expectedOp := []map[string]interface{}{{"category": "Category 1", "description": "Description 1", "value": "Value 1", "sensitive": true, "object_type": "cloud.TfcWorkspaceVariables", "hcl": true, "key": "Key 1", "identity": "Identity 1", "class_id": "cloud.TfcWorkspaceVariables"}, {"category": "Category 2", "description": "Description 2", "value": "Value 2", "sensitive": true, "object_type": "cloud.TfcWorkspaceVariables", "hcl": true, "key": "Key 2", "identity": "Identity 2", "class_id": "cloud.TfcWorkspaceVariables"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Identity":"Identity %d","Index":32,"IsRoot":true,"AutoDelete":true,"ClassId":"cloud.VolumeAttachment","ObjectType":"cloud.VolumeAttachment","DeviceName":"DeviceName %d"}`
	...
	expectedOp := []map[string]interface{}{{"identity": "Identity 1", "index": 32, "is_root": true, "auto_delete": true, "class_id": "cloud.VolumeAttachment", "object_type": "cloud.VolumeAttachment", "device_name": "DeviceName 1"}, {"identity": "Identity 2", "index": 32, "is_root": true, "auto_delete": true, "class_id": "cloud.VolumeAttachment", "object_type": "cloud.VolumeAttachment", "device_name": "DeviceName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeInstanceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AutoDelete":true,"DeviceName":"DeviceName %d","InstanceId":"InstanceId %d","ClassId":"cloud.VolumeInstanceAttachment","ObjectType":"cloud.VolumeInstanceAttachment","State":"State %d"}`
	...
	expectedOp := []map[string]interface{}{{"auto_delete": true, "device_name": "DeviceName 1", "instance_id": "InstanceId 1", "class_id": "cloud.VolumeInstanceAttachment", "object_type": "cloud.VolumeInstanceAttachment", "state": "State 1"}, {"auto_delete": true, "device_name": "DeviceName 2", "instance_id": "InstanceId 2", "class_id": "cloud.VolumeInstanceAttachment", "object_type": "cloud.VolumeInstanceAttachment", "state": "State 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeIpAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"HttpPort":32,"KvmPort":32,"Name":"Name %d","Subnet":"Subnet %d","Address":"Address %d","HttpsPort":32,"ObjectType":"compute.IpAddress","KvmVlan":32,"Type":"Type %d","ClassId":"compute.IpAddress","Dn":"Dn %d","Category":"Category %d","DefaultGateway":"DefaultGateway %d"}`
	...
	expectedOp := []map[string]interface{}{{"http_port": 32, "kvm_port": 32, "name": "Name 1", "subnet": "Subnet 1", "address": "Address 1", "https_port": 32, "object_type": "compute.IpAddress", "kvm_vlan": 32, "type": "Type 1", "class_id": "compute.IpAddress", "dn": "Dn 1", "category": "Category 1", "default_gateway": "DefaultGateway 1"}, {"http_port": 32, "kvm_port": 32, "name": "Name 2", "subnet": "Subnet 2", "address": "Address 2", "https_port": 32, "object_type": "compute.IpAddress", "kvm_vlan": 32, "type": "Type 2", "class_id": "compute.IpAddress", "dn": "Dn 2", "category": "Category 2", "default_gateway": "DefaultGateway 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeServerOpStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ConfigState":"ConfigState %d","WorkflowType":"WorkflowType %d","ClassId":"compute.ServerOpStatus","ObjectType":"compute.ServerOpStatus"}`
	...
	expectedOp := []map[string]interface{}{{"config_state": "ConfigState 1", "workflow_type": "WorkflowType 1", "class_id": "compute.ServerOpStatus", "object_type": "compute.ServerOpStatus"}, {"config_state": "ConfigState 2", "workflow_type": "WorkflowType 2", "class_id": "compute.ServerOpStatus", "object_type": "compute.ServerOpStatus"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListConnectorpackConnectorPackUpdate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","NewVersion":"NewVersion %d","ClassId":"connectorpack.ConnectorPackUpdate","ObjectType":"connectorpack.ConnectorPackUpdate","CurrentVersion":"CurrentVersion %d"}`
...
func TestFlattenListContentParameter(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","ComplexType":"ComplexType %d","AcceptSingleValue":true,"Secure":true,"Name":"Name %d","ClassId":"content.Parameter","Path":"Path %d","ObjectType":"content.Parameter","ItemType":"ItemType %d"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "complex_type": "ComplexType 1", "accept_single_value": true, "secure": true, "name": "Name 1", "class_id": "content.Parameter", "path": "Path 1", "object_type": "content.Parameter", "item_type": "ItemType 1"}, {"type": "Type 2", "complex_type": "ComplexType 2", "accept_single_value": true, "secure": true, "name": "Name 2", "class_id": "content.Parameter", "path": "Path 2", "object_type": "content.Parameter", "item_type": "ItemType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListEquipmentIoCardIdentity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"equipment.IoCardIdentity","NetworkElementMoid":"NetworkElementMoid %d","SwitchId":32,"IoCardMoid":"IoCardMoid %d","ModuleId":32,"ClassId":"equipment.IoCardIdentity"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "equipment.IoCardIdentity", "network_element_moid": "NetworkElementMoid 1", "switch_id": 32, "io_card_moid": "IoCardMoid 1", "module_id": 32, "class_id": "equipment.IoCardIdentity"}, {"object_type": "equipment.IoCardIdentity", "network_element_moid": "NetworkElementMoid 2", "switch_id": 32, "io_card_moid": "IoCardMoid 2", "module_id": 32, "class_id": "equipment.IoCardIdentity"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricPortIdentifier(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AggregatePortId":32,"PortId":32,"SlotId":32,"ClassId":"fabric.PortIdentifier","ObjectType":"fabric.PortIdentifier"}`
	...
	expectedOp := []map[string]interface{}{{"aggregate_port_id": 32, "port_id": 32, "slot_id": 32, "class_id": "fabric.PortIdentifier", "object_type": "fabric.PortIdentifier"}, {"aggregate_port_id": 32, "port_id": 32, "slot_id": 32, "class_id": "fabric.PortIdentifier", "object_type": "fabric.PortIdentifier"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricQosClass(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","ClassId":"fabric.QosClass","Mtu":32,"AdminState":"AdminState %d","MulticastOptimize":true,"PacketDrop":true,"Weight":32,"BandwidthPercent":32,"ObjectType":"fabric.QosClass","Cos":32}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "class_id": "fabric.QosClass", "mtu": 32, "admin_state": "AdminState 1", "multicast_optimize": true, "packet_drop": true, "weight": 32, "bandwidth_percent": 32, "object_type": "fabric.QosClass", "cos": 32}, {"name": "Name 2", "class_id": "fabric.QosClass", "mtu": 32, "admin_state": "AdminState 2", "multicast_optimize": true, "packet_drop": true, "weight": 32, "bandwidth_percent": 32, "object_type": "fabric.QosClass", "cos": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFcpoolBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"fcpool.Block","ObjectType":"fcpool.Block","Size":32,"To":"To %d","From":"From %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "fcpool.Block", "object_type": "fcpool.Block", "size": 32, "to": "To 1", "from": "From 1"}, {"class_id": "fcpool.Block", "object_type": "fcpool.Block", "size": 32, "to": "To 2", "from": "From 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareBaseImpact(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"firmware.BaseImpact","ObjectType":"firmware.BaseImpact","ComputationError":"ComputationError %d","ComputationStatusDetail":"ComputationStatusDetail %d","FirmwareVersion":"FirmwareVersion %d","TargetFirmwareVersion":"TargetFirmwareVersion %d","VersionImpact":"VersionImpact %d","EndPoint":"EndPoint %d","Model":"Model %d","DomainName":"DomainName %d","ImpactType":"ImpactType %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "firmware.BaseImpact", "object_type": "firmware.BaseImpact", "computation_error": "ComputationError 1", "computation_status_detail": "ComputationStatusDetail 1", "firmware_version": "FirmwareVersion 1", "target_firmware_version": "TargetFirmwareVersion 1", "version_impact": "VersionImpact 1", "end_point": "EndPoint 1", "model": "Model 1", "domain_name": "DomainName 1", "impact_type": "ImpactType 1"}, {"class_id": "firmware.BaseImpact", "object_type": "firmware.BaseImpact", "computation_error": "ComputationError 2", "computation_status_detail": "ComputationStatusDetail 2", "firmware_version": "FirmwareVersion 2", "target_firmware_version": "TargetFirmwareVersion 2", "version_impact": "VersionImpact 2", "end_point": "EndPoint 2", "model": "Model 2", "domain_name": "DomainName 2", "impact_type": "ImpactType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareComponentMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RedfishUrl":"RedfishUrl %d","ClassId":"firmware.ComponentMeta","ComponentType":"ComponentType %d","Disruption":"Disruption %d","Model":"Model %d","ComponentLabel":"ComponentLabel %d","Vendor":"Vendor %d","ObjectType":"firmware.ComponentMeta","PackedVersion":"PackedVersion %d","ImagePath":"ImagePath %d","IsOobSupported":true,"Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"redfish_url": "RedfishUrl 1", "class_id": "firmware.ComponentMeta", "component_type": "ComponentType 1", "disruption": "Disruption 1", "model": "Model 1", "component_label": "ComponentLabel 1", "vendor": "Vendor 1", "object_type": "firmware.ComponentMeta", "packed_version": "PackedVersion 1", "image_path": "ImagePath 1", "is_oob_supported": true, "description": "Description 1"}, {"redfish_url": "RedfishUrl 2", "class_id": "firmware.ComponentMeta", "component_type": "ComponentType 2", "disruption": "Disruption 2", "model": "Model 2", "component_label": "ComponentLabel 2", "vendor": "Vendor 2", "object_type": "firmware.ComponentMeta", "packed_version": "PackedVersion 2", "image_path": "ImagePath 2", "is_oob_supported": true, "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareFirmwareInventory(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"firmware.FirmwareInventory","ObjectType":"firmware.FirmwareInventory","UpdateUri":"UpdateUri %d","Vendor":"Vendor %d","Version":"Version %d","Category":"Category %d","Label":"Label %d","Model":"Model %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "firmware.FirmwareInventory", "object_type": "firmware.FirmwareInventory", "update_uri": "UpdateUri 1", "vendor": "Vendor 1", "nr_version": "Version 1", "category": "Category 1", "label": "Label 1", "model": "Model 1"}, {"class_id": "firmware.FirmwareInventory", "object_type": "firmware.FirmwareInventory", "update_uri": "UpdateUri 2", "vendor": "Vendor 2", "nr_version": "Version 2", "category": "Category 2", "label": "Label 2", "model": "Model 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexFeatureLimitEntry(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Value":"Value %d","ClassId":"hyperflex.FeatureLimitEntry","ObjectType":"hyperflex.FeatureLimitEntry","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"value": "Value 1", "class_id": "hyperflex.FeatureLimitEntry", "object_type": "hyperflex.FeatureLimitEntry", "name": "Name 1"}, {"value": "Value 2", "class_id": "hyperflex.FeatureLimitEntry", "object_type": "hyperflex.FeatureLimitEntry", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHealthCheckScriptInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ScriptInput":"ScriptInput %d","Version":"Version %d","ClassId":"hyperflex.HealthCheckScriptInfo","AggregateScriptName":"AggregateScriptName %d","HyperflexVersion":"HyperflexVersion %d","ScriptExecuteLocation":"ScriptExecuteLocation %d","ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptName":"ScriptName %d"}`
	...
	expectedOp := []map[string]interface{}{{"script_input": "ScriptInput 1", "nr_version": "Version 1", "class_id": "hyperflex.HealthCheckScriptInfo", "aggregate_script_name": "AggregateScriptName 1", "hyperflex_version": "HyperflexVersion 1", "script_execute_location": "ScriptExecuteLocation 1", "object_type": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 1"}, {"script_input": "ScriptInput 2", "nr_version": "Version 2", "class_id": "hyperflex.HealthCheckScriptInfo", "aggregate_script_name": "AggregateScriptName 2", "hyperflex_version": "HyperflexVersion 2", "script_execute_location": "ScriptExecuteLocation 2", "object_type": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHxZoneResiliencyInfoDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HxZoneResiliencyInfoDt","Name":"Name %d","ClassId":"hyperflex.HxZoneResiliencyInfoDt"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "hyperflex.HxZoneResiliencyInfoDt", "name": "Name 1", "class_id": "hyperflex.HxZoneResiliencyInfoDt"}, {"object_type": "hyperflex.HxZoneResiliencyInfoDt", "name": "Name 2", "class_id": "hyperflex.HxZoneResiliencyInfoDt"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseKeysInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Count":32,"ExpirationDate":"ExpirationDate %d","LicenseId":"LicenseId %d","ClassId":"iaas.LicenseKeysInfo","ObjectType":"iaas.LicenseKeysInfo","Pid":"Pid %d"}`
	...
	expectedOp := []map[string]interface{}{{"nr_count": 32, "expiration_date": "ExpirationDate 1", "license_id": "LicenseId 1", "class_id": "iaas.LicenseKeysInfo", "object_type": "iaas.LicenseKeysInfo", "pid": "Pid 1"}, {"nr_count": 32, "expiration_date": "ExpirationDate 2", "license_id": "LicenseId 2", "class_id": "iaas.LicenseKeysInfo", "object_type": "iaas.LicenseKeysInfo", "pid": "Pid 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseUtilizationInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"iaas.LicenseUtilizationInfo","ObjectType":"iaas.LicenseUtilizationInfo","Label":"Label %d","LicensedLimit":"LicensedLimit %d","Sku":"Sku %d","ActualUsed":32}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "iaas.LicenseUtilizationInfo", "object_type": "iaas.LicenseUtilizationInfo", "label": "Label 1", "licensed_limit": "LicensedLimit 1", "sku": "Sku 1", "actual_used": 32}, {"class_id": "iaas.LicenseUtilizationInfo", "object_type": "iaas.LicenseUtilizationInfo", "label": "Label 2", "licensed_limit": "LicensedLimit 2", "sku": "Sku 2", "actual_used": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListInfraMetaData(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Value":"Value %d","ClassId":"infra.MetaData","ObjectType":"infra.MetaData","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"value": "Value 1", "class_id": "infra.MetaData", "object_type": "infra.MetaData", "name": "Name 1"}, {"value": "Value 2", "class_id": "infra.MetaData", "object_type": "infra.MetaData", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIppoolIpV6Block(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"object_type": "ippool.IpV6Block", "size": 32, "from": "From 1", "to": "To 1", "class_id": "ippool.IpV6Block"}, {"object_type": "ippool.IpV6Block", "size": 32, "from": "From 2", "to": "To 2", "class_id": "ippool.IpV6Block"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIqnpoolIqnSuffixBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"To":32,"From":32,"Size":32,"ClassId":"iqnpool.IqnSuffixBlock","ObjectType":"iqnpool.IqnSuffixBlock","Suffix":"Suffix %d"}`
	...
	expectedOp := []map[string]interface{}{{"to": 32, "from": 32, "size": 32, "class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "suffix": "Suffix 1"}, {"to": 32, "from": 32, "size": 32, "class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "suffix": "Suffix 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesEthernet(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.Ethernet","Name":"Name %d","ProviderName":"ProviderName %d","Gateway":"Gateway %d","Mtu":32,"ObjectType":"kubernetes.Ethernet"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "kubernetes.Ethernet", "name": "Name 1", "provider_name": "ProviderName 1", "gateway": "Gateway 1", "mtu": 32, "object_type": "kubernetes.Ethernet"}, {"class_id": "kubernetes.Ethernet", "name": "Name 2", "provider_name": "ProviderName 2", "gateway": "Gateway 2", "mtu": 32, "object_type": "kubernetes.Ethernet"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesNodeGroupLabel(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.NodeGroupLabel","Key":"Key %d","Value":"Value %d","ClassId":"kubernetes.NodeGroupLabel"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "kubernetes.NodeGroupLabel", "key": "Key 1", "value": "Value 1", "class_id": "kubernetes.NodeGroupLabel"}, {"object_type": "kubernetes.NodeGroupLabel", "key": "Key 2", "value": "Value 2", "class_id": "kubernetes.NodeGroupLabel"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesNodeGroupTaint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Key":"Key %d","ClassId":"kubernetes.NodeGroupTaint","ObjectType":"kubernetes.NodeGroupTaint","Value":"Value %d","Effect":"Effect %d"}`
	...
	expectedOp := []map[string]interface{}{{"key": "Key 1", "class_id": "kubernetes.NodeGroupTaint", "object_type": "kubernetes.NodeGroupTaint", "value": "Value 1", "effect": "Effect 1"}, {"key": "Key 2", "class_id": "kubernetes.NodeGroupTaint", "object_type": "kubernetes.NodeGroupTaint", "value": "Value 2", "effect": "Effect 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesTaint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.Taint","Effect":"Effect %d","Key":"Key %d","Value":"Value %d","ObjectType":"kubernetes.Taint"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "kubernetes.Taint", "effect": "Effect 1", "key": "Key 1", "value": "Value 1", "object_type": "kubernetes.Taint"}, {"class_id": "kubernetes.Taint", "effect": "Effect 2", "key": "Key 2", "value": "Value 2", "object_type": "kubernetes.Taint"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMacpoolBlock(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"from": "From 1", "to": "To 1", "size": 32, "class_id": "macpool.Block", "object_type": "macpool.Block"}, {"from": "From 2", "to": "To 2", "size": 32, "class_id": "macpool.Block", "object_type": "macpool.Block"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMemoryPersistentMemoryGoal(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MemoryModePercentage":32,"PersistentMemoryType":"PersistentMemoryType %d","SocketId":"SocketId %d","ClassId":"memory.PersistentMemoryGoal","ObjectType":"memory.PersistentMemoryGoal"}`
	...
	expectedOp := []map[string]interface{}{{"memory_mode_percentage": 32, "persistent_memory_type": "PersistentMemoryType 1", "socket_id": "SocketId 1", "class_id": "memory.PersistentMemoryGoal", "object_type": "memory.PersistentMemoryGoal"}, {"memory_mode_percentage": 32, "persistent_memory_type": "PersistentMemoryType 2", "socket_id": "SocketId 2", "class_id": "memory.PersistentMemoryGoal", "object_type": "memory.PersistentMemoryGoal"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMemoryPersistentMemoryLogicalNamespace(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"memory.PersistentMemoryLogicalNamespace","SocketId":32,"SocketMemoryId":"SocketMemoryId %d","Capacity":32,"Mode":"Mode %d","Name":"Name %d","ClassId":"memory.PersistentMemoryLogicalNamespace"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "memory.PersistentMemoryLogicalNamespace", "socket_id": 32, "socket_memory_id": "SocketMemoryId 1", "capacity": 32, "mode": "Mode 1", "name": "Name 1", "class_id": "memory.PersistentMemoryLogicalNamespace"}, {"object_type": "memory.PersistentMemoryLogicalNamespace", "socket_id": 32, "socket_memory_id": "SocketMemoryId 2", "capacity": 32, "mode": "Mode 2", "name": "Name 2", "class_id": "memory.PersistentMemoryLogicalNamespace"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaAccessPrivilege(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"meta.AccessPrivilege","Method":"Method %d","Privilege":"Privilege %d","ClassId":"meta.AccessPrivilege"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "meta.AccessPrivilege", "method": "Method 1", "privilege": "Privilege 1", "class_id": "meta.AccessPrivilege"}, {"object_type": "meta.AccessPrivilege", "method": "Method 2", "privilege": "Privilege 2", "class_id": "meta.AccessPrivilege"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaDisplayNameDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IncludeAncestor":true,"Name":"Name %d","Format":"Format %d","ClassId":"meta.DisplayNameDefinition","ObjectType":"meta.DisplayNameDefinition"}`
	...
	expectedOp := []map[string]interface{}{{"include_ancestor": true, "name": "Name 1", "format": "Format 1", "class_id": "meta.DisplayNameDefinition", "object_type": "meta.DisplayNameDefinition"}, {"include_ancestor": true, "name": "Name 2", "format": "Format 2", "class_id": "meta.DisplayNameDefinition", "object_type": "meta.DisplayNameDefinition"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaPropDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"meta.PropDefinition","OpSecurity":"OpSecurity %d","Type":"Type %d","IsCollection":true,"Name":"Name %d","Kind":"Kind %d","ClassId":"meta.PropDefinition","ApiAccess":"ApiAccess %d","IsComplexType":true,"SearchWeight":32.000000}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "meta.PropDefinition", "op_security": "OpSecurity 1", "type": "Type 1", "is_collection": true, "name": "Name 1", "kind": "Kind 1", "class_id": "meta.PropDefinition", "api_access": "ApiAccess 1", "is_complex_type": true, "search_weight": 32.000000}, {"object_type": "meta.PropDefinition", "op_security": "OpSecurity 2", "type": "Type 2", "is_collection": true, "name": "Name 2", "kind": "Kind 2", "class_id": "meta.PropDefinition", "api_access": "ApiAccess 2", "is_complex_type": true, "search_weight": 32.000000}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaRelationshipDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ApiAccess":"ApiAccess %d","Name":"Name %d","Type":"Type %d","Export":true,"Collection":true,"ClassId":"meta.RelationshipDefinition","ObjectType":"meta.RelationshipDefinition","PeerRelName":"PeerRelName %d","PeerSync":true,"ExportWithPeer":true}`
	...
	expectedOp := []map[string]interface{}{{"api_access": "ApiAccess 1", "name": "Name 1", "type": "Type 1", "export": true, "collection": true, "class_id": "meta.RelationshipDefinition", "object_type": "meta.RelationshipDefinition", "peer_rel_name": "PeerRelName 1", "peer_sync": true, "export_with_peer": true}, {"api_access": "ApiAccess 2", "name": "Name 2", "type": "Type 2", "export": true, "collection": true, "class_id": "meta.RelationshipDefinition", "object_type": "meta.RelationshipDefinition", "peer_rel_name": "PeerRelName 2", "peer_sync": true, "export_with_peer": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiaapiDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niaapi.Detail","Name":"Name %d","Chksum":"Chksum %d","Filename":"Filename %d","ObjectType":"niaapi.Detail"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "niaapi.Detail", "name": "Name 1", "chksum": "Chksum 1", "filename": "Filename 1", "object_type": "niaapi.Detail"}, {"class_id": "niaapi.Detail", "name": "Name 2", "chksum": "Chksum 2", "filename": "Filename 2", "object_type": "niaapi.Detail"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryDeploymentStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.DeploymentStatus","ObjectType":"niatelemetry.DeploymentStatus","Id":32,"Name":"Name %d","Status":"Status %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "niatelemetry.DeploymentStatus", "object_type": "niatelemetry.DeploymentStatus", "id": 32, "name": "Name 1", "status": "Status 1"}, {"class_id": "niatelemetry.DeploymentStatus", "object_type": "niatelemetry.DeploymentStatus", "id": 32, "name": "Name 2", "status": "Status 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryImageDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":"Version %d","ClassId":"niatelemetry.ImageDetail","ObjectType":"niatelemetry.ImageDetail","ImageName":"ImageName %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"nr_version": "Version 1", "class_id": "niatelemetry.ImageDetail", "object_type": "niatelemetry.ImageDetail", "image_name": "ImageName 1", "name": "Name 1"}, {"nr_version": "Version 2", "class_id": "niatelemetry.ImageDetail", "object_type": "niatelemetry.ImageDetail", "image_name": "ImageName 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryJobDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.JobDetail","JobId":32,"UpgStatus":"UpgStatus %d","ClassId":"niatelemetry.JobDetail"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "niatelemetry.JobDetail", "job_id": 32, "upg_status": "UpgStatus 1", "class_id": "niatelemetry.JobDetail"}, {"object_type": "niatelemetry.JobDetail", "job_id": 32, "upg_status": "UpgStatus 2", "class_id": "niatelemetry.JobDetail"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryLogicalLink(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LinkAddr2":"LinkAddr2 %d","IsPresent":true,"ObjectType":"niatelemetry.LogicalLink","DbId":32,"LinkType":"LinkType %d","Uptime":"Uptime %d","LinkAddr1":"LinkAddr1 %d","LinkState":"LinkState %d","ClassId":"niatelemetry.LogicalLink"}`
	...
	expectedOp := []map[string]interface{}{{"link_addr2": "LinkAddr2 1", "is_present": true, "object_type": "niatelemetry.LogicalLink", "db_id": 32, "link_type": "LinkType 1", "uptime": "Uptime 1", "link_addr1": "LinkAddr1 1", "link_state": "LinkState 1", "class_id": "niatelemetry.LogicalLink"}, {"link_addr2": "LinkAddr2 2", "is_present": true, "object_type": "niatelemetry.LogicalLink", "db_id": 32, "link_type": "LinkType 2", "uptime": "Uptime 2", "link_addr1": "LinkAddr1 2", "link_state": "LinkState 2", "class_id": "niatelemetry.LogicalLink"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetrySites(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","SiteType":"SiteType %d","Uuid":"Uuid %d","ClassId":"niatelemetry.Sites","ObjectType":"niatelemetry.Sites"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "site_type": "SiteType 1", "uuid": "Uuid 1", "class_id": "niatelemetry.Sites", "object_type": "niatelemetry.Sites"}, {"name": "Name 2", "site_type": "SiteType 2", "uuid": "Uuid 2", "class_id": "niatelemetry.Sites", "object_type": "niatelemetry.Sites"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryVniStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"VniState":"VniState %d","VniType":"VniType %d","ClassId":"niatelemetry.VniStatus","ObjectType":"niatelemetry.VniStatus","Vni":"Vni %d"}`
	...
	expectedOp := []map[string]interface{}{{"vni_state": "VniState 1", "vni_type": "VniType 1", "class_id": "niatelemetry.VniStatus", "object_type": "niatelemetry.VniStatus", "vni": "Vni 1"}, {"vni_state": "VniState 2", "vni_type": "VniType 2", "class_id": "niatelemetry.VniStatus", "object_type": "niatelemetry.VniStatus", "vni": "Vni 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryVpcDetails(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.VpcDetails","ObjectType":"niatelemetry.VpcDetails","PeerSwitchDbId":32,"SwitchDbId":32,"IsVpcConfigured":true}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "niatelemetry.VpcDetails", "object_type": "niatelemetry.VpcDetails", "peer_switch_db_id": 32, "switch_db_id": 32, "is_vpc_configured": true}, {"class_id": "niatelemetry.VpcDetails", "object_type": "niatelemetry.VpcDetails", "peer_switch_db_id": 32, "switch_db_id": 32, "is_vpc_configured": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNtpAuthNtpServer(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SymKeyId":32,"SymKeyValue":"SymKeyValue %d","KeyType":"KeyType %d","ServerName":"ServerName %d","ClassId":"ntp.AuthNtpServer","ObjectType":"ntp.AuthNtpServer"}`
...
func TestFlattenListOnpremImagePackage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"onprem.ImagePackage","PackageType":"PackageType %d","Name":"Name %d","ClassId":"onprem.ImagePackage","FileSize":32,"FilePath":"FilePath %d","Version":"Version %d","Filename":"Filename %d","FileSha":"FileSha %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "onprem.ImagePackage", "package_type": "PackageType 1", "name": "Name 1", "class_id": "onprem.ImagePackage", "file_size": 32, "file_path": "FilePath 1", "nr_version": "Version 1", "filename": "Filename 1", "file_sha": "FileSha 1"}, {"object_type": "onprem.ImagePackage", "package_type": "PackageType 2", "name": "Name 2", "class_id": "onprem.ImagePackage", "file_size": 32, "file_path": "FilePath 2", "nr_version": "Version 2", "filename": "Filename 2", "file_sha": "FileSha 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOsServerConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SerialNumber":"SerialNumber %d","InstallTarget":"InstallTarget %d","ClassId":"os.ServerConfig","ObjectType":"os.ServerConfig"}`
	...
	expectedOp := []map[string]interface{}{{"serial_number": "SerialNumber 1", "install_target": "InstallTarget 1", "class_id": "os.ServerConfig", "object_type": "os.ServerConfig"}, {"serial_number": "SerialNumber 2", "install_target": "InstallTarget 2", "class_id": "os.ServerConfig", "object_type": "os.ServerConfig"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOsValidationInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Status":"Status %d","StepName":"StepName %d","ErrorMsg":"ErrorMsg %d","ClassId":"os.ValidationInformation","ObjectType":"os.ValidationInformation"}`
	...
	expectedOp := []map[string]interface{}{{"status": "Status 1", "step_name": "StepName 1", "error_msg": "ErrorMsg 1", "class_id": "os.ValidationInformation", "object_type": "os.ValidationInformation"}, {"status": "Status 2", "step_name": "StepName 2", "error_msg": "ErrorMsg 2", "class_id": "os.ValidationInformation", "object_type": "os.ValidationInformation"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListPolicyActionParam(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"policy.ActionParam","Name":"Name %d","Value":"Value %d","ClassId":"policy.ActionParam"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "policy.ActionParam", "name": "Name 1", "value": "Value 1", "class_id": "policy.ActionParam"}, {"object_type": "policy.ActionParam", "name": "Name 2", "value": "Value 2", "class_id": "policy.ActionParam"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListPolicyPolicyStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"policy.PolicyStatus","ClassId":"policy.PolicyStatus","Moid":"Moid %d","Reason":"Reason %d","Status":"Status %d","Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "policy.PolicyStatus", "class_id": "policy.PolicyStatus", "moid": "Moid 1", "reason": "Reason 1", "status": "Status 1", "type": "Type 1"}, {"object_type": "policy.PolicyStatus", "class_id": "policy.PolicyStatus", "moid": "Moid 2", "reason": "Reason 2", "status": "Status 2", "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListResourcePerTypeCombinedSelector(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"resource.PerTypeCombinedSelector","ObjectType":"resource.PerTypeCombinedSelector","SelectorObjectType":"SelectorObjectType %d","CombinedSelector":"CombinedSelector %d","EmptyFilter":true}`
...
func TestFlattenListSnmpTrap(t *testing.T) {
	p := []models.SnmpTrap{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"snmp.Trap","ObjectType":"snmp.Trap","Version":"Version %d","Type":"Type %d","Enabled":true,"User":"User %d","Port":32,"Community":"Community %d","Destination":"Destination %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "snmp.Trap", "object_type": "snmp.Trap", "nr_version": "Version 1", "type": "Type 1", "enabled": true, "user": "User 1", "port": 32, "community": "Community 1", "destination": "Destination 1"}, {"class_id": "snmp.Trap", "object_type": "snmp.Trap", "nr_version": "Version 2", "type": "Type 2", "enabled": true, "user": "User 2", "port": 32, "community": "Community 2", "destination": "Destination 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpUser(t *testing.T) {
	p := []models.SnmpUser{}
	var d = &schema.ResourceData{}
	c := `{"AuthType":"AuthType %d","PrivacyType":"PrivacyType %d","SecurityLevel":"SecurityLevel %d","ObjectType":"snmp.User","IsAuthPasswordSet":true,"Name":"Name %d","IsPrivacyPasswordSet":true,"ClassId":"snmp.User"}`
	...
	expectedOp := []map[string]interface{}{{"auth_type": "AuthType 1", "privacy_type": "PrivacyType 1", "security_level": "SecurityLevel 1", "object_type": "snmp.User", "is_auth_password_set": true, "name": "Name 1", "is_privacy_password_set": true, "class_id": "snmp.User"}, {"auth_type": "AuthType 2", "privacy_type": "PrivacyType 2", "security_level": "SecurityLevel 2", "object_type": "snmp.User", "is_auth_password_set": true, "name": "Name 2", "is_privacy_password_set": true, "class_id": "snmp.User"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSoftwarerepositoryConstraintModels(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MinVersion":"MinVersion %d","ClassId":"softwarerepository.ConstraintModels","ObjectType":"softwarerepository.ConstraintModels","Name":"Name %d","PlatformRegex":"PlatformRegex %d"}`
	...
	expectedOp := []map[string]interface{}{{"min_version": "MinVersion 1", "class_id": "softwarerepository.ConstraintModels", "object_type": "softwarerepository.ConstraintModels", "name": "Name 1", "platform_regex": "PlatformRegex 1"}, {"min_version": "MinVersion 2", "class_id": "softwarerepository.ConstraintModels", "object_type": "softwarerepository.ConstraintModels", "name": "Name 2", "platform_regex": "PlatformRegex 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStorageBaseInitiator(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.BaseInitiator","ObjectType":"storage.BaseInitiator","Type":"Type %d","Wwn":"Wwn %d","Iqn":"Iqn %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "storage.BaseInitiator", "object_type": "storage.BaseInitiator", "type": "Type 1", "wwn": "Wwn 1", "iqn": "Iqn 1", "name": "Name 1"}, {"class_id": "storage.BaseInitiator", "object_type": "storage.BaseInitiator", "type": "Type 2", "wwn": "Wwn 2", "iqn": "Iqn 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStorageStorageContainerHostMountStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.StorageContainerHostMountStatus","ObjectType":"storage.StorageContainerHostMountStatus","Reason":"Reason %d","Accessibility":"Accessibility %d","HostName":"HostName %d","Mounted":true}`
...
func TestFlattenListStorageVirtualDriveConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"BootDrive":true,"ExpandToAvailable":true,"Name":"Name %d","Size":32,"ClassId":"storage.VirtualDriveConfiguration","ObjectType":"storage.VirtualDriveConfiguration"}`
...
func TestFlattenListSyslogRemoteClientBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Hostname":"Hostname %d","MinSeverity":"MinSeverity %d","Port":32,"Protocol":"Protocol %d","ObjectType":"syslog.RemoteClientBase","ClassId":"syslog.RemoteClientBase","Enabled":true}`
	...
	expectedOp := []map[string]interface{}{{"hostname": "Hostname 1", "min_severity": "MinSeverity 1", "port": 32, "protocol": "Protocol 1", "object_type": "syslog.RemoteClientBase", "class_id": "syslog.RemoteClientBase", "enabled": true}, {"hostname": "Hostname 2", "min_severity": "MinSeverity 2", "port": 32, "protocol": "Protocol 2", "object_type": "syslog.RemoteClientBase", "class_id": "syslog.RemoteClientBase", "enabled": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTamAction(t *testing.T) {
	p := []models.TamAction{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"tam.Action","Type":"Type %d","ObjectType":"tam.Action","AffectedObjectType":"AffectedObjectType %d","AlertType":"AlertType %d","Name":"Name %d","OperationType":"OperationType %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "tam.Action", "type": "Type 1", "object_type": "tam.Action", "affected_object_type": "AffectedObjectType 1", "alert_type": "AlertType 1", "name": "Name 1", "operation_type": "OperationType 1"}, {"class_id": "tam.Action", "type": "Type 2", "object_type": "tam.Action", "affected_object_type": "AffectedObjectType 2", "alert_type": "AlertType 2", "name": "Name 2", "operation_type": "OperationType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTamApiDataSource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","MoType":"MoType %d","ClassId":"tam.ApiDataSource","ObjectType":"tam.ApiDataSource","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "mo_type": "MoType 1", "class_id": "tam.ApiDataSource", "object_type": "tam.ApiDataSource", "name": "Name 1"}, {"type": "Type 2", "mo_type": "MoType 2", "class_id": "tam.ApiDataSource", "object_type": "tam.ApiDataSource", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTamS3DataSource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","ClassId":"tam.S3DataSource","ObjectType":"tam.S3DataSource","S3Path":"S3Path %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "class_id": "tam.S3DataSource", "object_type": "tam.S3DataSource", "s3_path": "S3Path 1", "name": "Name 1"}, {"type": "Type 2", "class_id": "tam.S3DataSource", "object_type": "tam.S3DataSource", "s3_path": "S3Path 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTerraformCloudResource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"terraform.CloudResource","CurrentStatus":"CurrentStatus %d","DesiredStatus":"DesiredStatus %d","ResourceId":"ResourceId %d","ClassId":"terraform.CloudResource"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "terraform.CloudResource", "current_status": "CurrentStatus 1", "desired_status": "DesiredStatus 1", "resource_id": "ResourceId 1", "class_id": "terraform.CloudResource"}, {"object_type": "terraform.CloudResource", "current_status": "CurrentStatus 2", "desired_status": "DesiredStatus 2", "resource_id": "ResourceId 2", "class_id": "terraform.CloudResource"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUcsdConnectorPack(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"State":"State %d","ConnectorFeature":"ConnectorFeature %d","DownloadedVersion":"DownloadedVersion %d","ObjectType":"ucsd.ConnectorPack","ClassId":"ucsd.ConnectorPack","Name":"Name %d","Version":"Version %d"}`
	...
	expectedOp := []map[string]interface{}{{"state": "State 1", "connector_feature": "ConnectorFeature 1", "downloaded_version": "DownloadedVersion 1", "object_type": "ucsd.ConnectorPack", "class_id": "ucsd.ConnectorPack", "name": "Name 1", "nr_version": "Version 1"}, {"state": "State 2", "connector_feature": "ConnectorFeature 2", "downloaded_version": "DownloadedVersion 2", "object_type": "ucsd.ConnectorPack", "class_id": "ucsd.ConnectorPack", "name": "Name 2", "nr_version": "Version 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUuidpoolUuidBlock(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"from": "From 1", "to": "To 1", "class_id": "uuidpool.UuidBlock", "object_type": "uuidpool.UuidBlock", "size": 32}, {"from": "From 2", "to": "To 2", "class_id": "uuidpool.UuidBlock", "object_type": "uuidpool.UuidBlock", "size": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationNetworkInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IpForwardingEnabled":true,"AdaptorType":"AdaptorType %d","ConnectAtPowerOn":true,"ObjectType":"virtualization.NetworkInterface","Name":"Name %d","PublicIpAllocate":true,"NicId":"NicId %d","Order":32,"MacAddress":"MacAddress %d","Ipv6Address":true,"Bridge":"Bridge %d","ClassId":"virtualization.NetworkInterface","PrivateIpAllocationMode":"PrivateIpAllocationMode %d","NetworkId":"NetworkId %d","SubnetId":"SubnetId %d","DirectPathIo":true}`
	...
	expectedOp := []map[string]interface{}{{"ip_forwarding_enabled": true, "adaptor_type": "AdaptorType 1", "connect_at_power_on": true, "object_type": "virtualization.NetworkInterface", "name": "Name 1", "public_ip_allocate": true, "nic_id": "NicId 1", "order": 32, "mac_address": "MacAddress 1", "ipv6_address": true, "bridge": "Bridge 1", "class_id": "virtualization.NetworkInterface", "private_ip_allocation_mode": "PrivateIpAllocationMode 1", "network_id": "NetworkId 1", "subnet_id": "SubnetId 1", "direct_path_io": true}, {"ip_forwarding_enabled": true, "adaptor_type": "AdaptorType 2", "connect_at_power_on": true, "object_type": "virtualization.NetworkInterface", "name": "Name 2", "public_ip_allocate": true, "nic_id": "NicId 2", "order": 32, "mac_address": "MacAddress 2", "ipv6_address": true, "bridge": "Bridge 2", "class_id": "virtualization.NetworkInterface", "private_ip_allocation_mode": "PrivateIpAllocationMode 2", "network_id": "NetworkId 2", "subnet_id": "SubnetId 2", "direct_path_io": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationNetworkPort(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.NetworkPort","Name":"Name %d","PortType":"PortType %d","Vlans":"Vlans %d","ClassId":"virtualization.NetworkPort"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "virtualization.NetworkPort", "name": "Name 1", "port_type": "PortType 1", "vlans": "Vlans 1", "class_id": "virtualization.NetworkPort"}, {"object_type": "virtualization.NetworkPort", "name": "Name 2", "port_type": "PortType 2", "vlans": "Vlans 2", "class_id": "virtualization.NetworkPort"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVirtualMachineDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Order":32,"Type":"Type %d","Bus":"Bus %d","Name":"Name %d","ClassId":"virtualization.VirtualMachineDisk","ObjectType":"virtualization.VirtualMachineDisk","VirtualDiskReference":"VirtualDiskReference %d"}`
	...
	expectedOp := []map[string]interface{}{{"order": 32, "type": "Type 1", "bus": "Bus 1", "name": "Name 1", "class_id": "virtualization.VirtualMachineDisk", "object_type": "virtualization.VirtualMachineDisk", "virtual_disk_reference": "VirtualDiskReference 1"}, {"order": 32, "type": "Type 2", "bus": "Bus 2", "name": "Name 2", "class_id": "virtualization.VirtualMachineDisk", "object_type": "virtualization.VirtualMachineDisk", "virtual_disk_reference": "VirtualDiskReference 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmDisk","Bus":"Bus %d","Type":"Type %d","Name":"Name %d","VirtualDiskReference":"VirtualDiskReference %d","ObjectType":"virtualization.VmDisk","BootOrder":32}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "virtualization.VmDisk", "bus": "Bus 1", "type": "Type 1", "name": "Name 1", "virtual_disk_reference": "VirtualDiskReference 1", "object_type": "virtualization.VmDisk", "boot_order": 32}, {"class_id": "virtualization.VmDisk", "bus": "Bus 2", "type": "Type 2", "name": "Name 2", "virtual_disk_reference": "VirtualDiskReference 2", "object_type": "virtualization.VmDisk", "boot_order": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Model":"Model %d","Name":"Name %d","Bridge":"Bridge %d","MacAddress":"MacAddress %d","ClassId":"virtualization.VmInterface","ObjectType":"virtualization.VmInterface"}`
	...
	expectedOp := []map[string]interface{}{{"model": "Model 1", "name": "Name 1", "bridge": "Bridge 1", "mac_address": "MacAddress 1", "class_id": "virtualization.VmInterface", "object_type": "virtualization.VmInterface"}, {"model": "Model 2", "name": "Name 2", "bridge": "Bridge 2", "mac_address": "MacAddress 2", "class_id": "virtualization.VmInterface", "object_type": "virtualization.VmInterface"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmwareResourceAllocationSystemTrafficTypes(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Reservation":32,"Shares":"Shares %d","SharesValue":32,"TrafficType":"TrafficType %d","ClassId":"virtualization.VmwareResourceAllocationSystemTrafficTypes","ObjectType":"virtualization.VmwareResourceAllocationSystemTrafficTypes","Limit":32}`
	...
	expectedOp := []map[string]interface{}{{"reservation": 32, "shares": "Shares 1", "shares_value": 32, "traffic_type": "TrafficType 1", "class_id": "virtualization.VmwareResourceAllocationSystemTrafficTypes", "object_type": "virtualization.VmwareResourceAllocationSystemTrafficTypes", "limit": 32}, {"reservation": 32, "shares": "Shares 2", "shares_value": 32, "traffic_type": "TrafficType 2", "class_id": "virtualization.VmwareResourceAllocationSystemTrafficTypes", "object_type": "virtualization.VmwareResourceAllocationSystemTrafficTypes", "limit": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmwareVlanRange(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareVlanRange","ObjectType":"virtualization.VmwareVlanRange","VlanRangeStart":32,"VlanRangeEnd":32}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "virtualization.VmwareVlanRange", "object_type": "virtualization.VmwareVlanRange", "vlan_range_start": 32, "vlan_range_end": 32}, {"class_id": "virtualization.VmwareVlanRange", "object_type": "virtualization.VmwareVlanRange", "vlan_range_start": 32, "vlan_range_end": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVmediaMapping(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"FileLocation":"FileLocation %d","IsPasswordSet":true,"RemoteFile":"RemoteFile %d","RemotePath":"RemotePath %d","Username":"Username %d","MountOptions":"MountOptions %d","HostName":"HostName %d","DeviceType":"DeviceType %d","ObjectType":"vmedia.Mapping","SanitizedFileLocation":"SanitizedFileLocation %d","VolumeName":"VolumeName %d","AuthenticationProtocol":"AuthenticationProtocol %d","ClassId":"vmedia.Mapping","MountProtocol":"MountProtocol %d"}`
	...
	expectedOp := []map[string]interface{}{{"file_location": "FileLocation 1", "is_password_set": true, "remote_file": "RemoteFile 1", "remote_path": "RemotePath 1", "username": "Username 1", "mount_options": "MountOptions 1", "host_name": "HostName 1", "device_type": "DeviceType 1", "object_type": "vmedia.Mapping", "sanitized_file_location": "SanitizedFileLocation 1", "volume_name": "VolumeName 1", "authentication_protocol": "AuthenticationProtocol 1", "class_id": "vmedia.Mapping", "mount_protocol": "MountProtocol 1"}, {"file_location": "FileLocation 2", "is_password_set": true, "remote_file": "RemoteFile 2", "remote_path": "RemotePath 2", "username": "Username 2", "mount_options": "MountOptions 2", "host_name": "HostName 2", "device_type": "DeviceType 2", "object_type": "vmedia.Mapping", "sanitized_file_location": "SanitizedFileLocation 2", "volume_name": "VolumeName 2", "authentication_protocol": "AuthenticationProtocol 2", "class_id": "vmedia.Mapping", "mount_protocol": "MountProtocol 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVnicVifStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.VifStatus","Name":"Name %d","Reason":"Reason %d","Status":"Status %d","ClassId":"vnic.VifStatus"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "vnic.VifStatus", "name": "Name 1", "reason": "Reason 1", "status": "Status 1", "class_id": "vnic.VifStatus"}, {"object_type": "vnic.VifStatus", "name": "Name 2", "reason": "Reason 2", "status": "Status 2", "class_id": "vnic.VifStatus"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowActionWorkflowDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.ActionWorkflowDefinition","CatalogMoid":"CatalogMoid %d","Version":32,"WorkflowDefinitionName":"WorkflowDefinitionName %d","ObjectType":"workflow.ActionWorkflowDefinition","Description":"Description %d","Name":"Name %d","Label":"Label %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.ActionWorkflowDefinition", "catalog_moid": "CatalogMoid 1", "nr_version": 32, "workflow_definition_name": "WorkflowDefinitionName 1", "object_type": "workflow.ActionWorkflowDefinition", "description": "Description 1", "name": "Name 1", "label": "Label 1"}, {"class_id": "workflow.ActionWorkflowDefinition", "catalog_moid": "CatalogMoid 2", "nr_version": 32, "workflow_definition_name": "WorkflowDefinitionName 2", "object_type": "workflow.ActionWorkflowDefinition", "description": "Description 2", "name": "Name 2", "label": "Label 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowApi(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ContentType":"ContentType %d","ObjectType":"workflow.Api","Label":"Label %d","StartDelay":32,"AssetTargetMoid":"AssetTargetMoid %d","ClassId":"workflow.Api","Name":"Name %d","Body":"Body %d","Timeout":32,"SkipOnCondition":"SkipOnCondition %d","Description":"Description %d","ErrorContentType":"ErrorContentType %d"}`
	...
	expectedOp := []map[string]interface{}{{"content_type": "ContentType 1", "object_type": "workflow.Api", "label": "Label 1", "start_delay": 32, "asset_target_moid": "AssetTargetMoid 1", "class_id": "workflow.Api", "name": "Name 1", "body": "Body 1", "timeout": 32, "skip_on_condition": "SkipOnCondition 1", "description": "Description 1", "error_content_type": "ErrorContentType 1"}, {"content_type": "ContentType 2", "object_type": "workflow.Api", "label": "Label 2", "start_delay": 32, "asset_target_moid": "AssetTargetMoid 2", "class_id": "workflow.Api", "name": "Name 2", "body": "Body 2", "timeout": 32, "skip_on_condition": "SkipOnCondition 2", "description": "Description 2", "error_content_type": "ErrorContentType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowBaseDataType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.BaseDataType","ObjectType":"workflow.BaseDataType","Name":"Name %d","Label":"Label %d","Required":true,"Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.BaseDataType", "object_type": "workflow.BaseDataType", "name": "Name 1", "label": "Label 1", "required": true, "description": "Description 1"}, {"class_id": "workflow.BaseDataType", "object_type": "workflow.BaseDataType", "name": "Name 2", "label": "Label 2", "required": true, "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowParameterSet(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ControlParameter":"ControlParameter %d","ClassId":"workflow.ParameterSet","ObjectType":"workflow.ParameterSet","Name":"Name %d","Value":"Value %d","Condition":"Condition %d"}`
	...
	expectedOp := []map[string]interface{}{{"control_parameter": "ControlParameter 1", "class_id": "workflow.ParameterSet", "object_type": "workflow.ParameterSet", "name": "Name 1", "value": "Value 1", "condition": "Condition 1"}, {"control_parameter": "ControlParameter 2", "class_id": "workflow.ParameterSet", "object_type": "workflow.ParameterSet", "name": "Name 2", "value": "Value 2", "condition": "Condition 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","SkipCondition":"SkipCondition %d","Description":"Description %d","TaskMoid":"TaskMoid %d","Version":32,"CatalogMoid":"CatalogMoid %d","ClassId":"workflow.RollbackTask","ObjectType":"workflow.RollbackTask"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "skip_condition": "SkipCondition 1", "description": "Description 1", "task_moid": "TaskMoid 1", "nr_version": 32, "catalog_moid": "CatalogMoid 1", "class_id": "workflow.RollbackTask", "object_type": "workflow.RollbackTask"}, {"name": "Name 2", "skip_condition": "SkipCondition 2", "description": "Description 2", "task_moid": "TaskMoid 2", "nr_version": 32, "catalog_moid": "CatalogMoid 2", "class_id": "workflow.RollbackTask", "object_type": "workflow.RollbackTask"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackWorkflowTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RefName":"RefName %d","RollbackCompleted":true,"TaskInfoMoid":"TaskInfoMoid %d","TaskPath":"TaskPath %d","RollbackTaskName":"RollbackTaskName %d","Status":"Status %d","ObjectType":"workflow.RollbackWorkflowTask","Description":"Description %d","Name":"Name %d","ClassId":"workflow.RollbackWorkflowTask"}`
	...
	expectedOp := []map[string]interface{}{{"ref_name": "RefName 1", "rollback_completed": true, "task_info_moid": "TaskInfoMoid 1", "task_path": "TaskPath 1", "rollback_task_name": "RollbackTaskName 1", "status": "Status 1", "object_type": "workflow.RollbackWorkflowTask", "description": "Description 1", "name": "Name 1", "class_id": "workflow.RollbackWorkflowTask"}, {"ref_name": "RefName 2", "rollback_completed": true, "task_info_moid": "TaskInfoMoid 2", "task_path": "TaskPath 2", "rollback_task_name": "RollbackTaskName 2", "status": "Status 2", "object_type": "workflow.RollbackWorkflowTask", "description": "Description 2", "name": "Name 2", "class_id": "workflow.RollbackWorkflowTask"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowServiceItemActionWorkflowDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.ServiceItemActionWorkflowDefinition","Description":"Description %d","WorkflowDefinitionName":"WorkflowDefinitionName %d","Version":32,"Label":"Label %d","ObjectType":"workflow.ServiceItemActionWorkflowDefinition","Name":"Name %d","CatalogMoid":"CatalogMoid %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.ServiceItemActionWorkflowDefinition", "description": "Description 1", "workflow_definition_name": "WorkflowDefinitionName 1", "nr_version": 32, "label": "Label 1", "object_type": "workflow.ServiceItemActionWorkflowDefinition", "name": "Name 1", "catalog_moid": "CatalogMoid 1"}, {"class_id": "workflow.ServiceItemActionWorkflowDefinition", "description": "Description 2", "workflow_definition_name": "WorkflowDefinitionName 2", "nr_version": 32, "label": "Label 2", "object_type": "workflow.ServiceItemActionWorkflowDefinition", "name": "Name 2", "catalog_moid": "CatalogMoid 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowWorkflowTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.WorkflowTask","ObjectType":"workflow.WorkflowTask","Label":"Label %d","Name":"Name %d","Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.WorkflowTask", "object_type": "workflow.WorkflowTask", "label": "Label 1", "name": "Name 1", "description": "Description 1"}, {"class_id": "workflow.WorkflowTask", "object_type": "workflow.WorkflowTask", "label": "Label 2", "name": "Name 2", "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListX509Certificate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PemCertificate":"PemCertificate %d","ObjectType":"x509.Certificate","SignatureAlgorithm":"SignatureAlgorithm %d","Sha256Fingerprint":"Sha256Fingerprint %d","ClassId":"x509.Certificate"}`
	...
	expectedOp := []map[string]interface{}{{"pem_certificate": "PemCertificate 1", "object_type": "x509.Certificate", "signature_algorithm": "SignatureAlgorithm 1", "sha256_fingerprint": "Sha256Fingerprint 1", "class_id": "x509.Certificate"}, {"pem_certificate": "PemCertificate 2", "object_type": "x509.Certificate", "signature_algorithm": "SignatureAlgorithm 2", "sha256_fingerprint": "Sha256Fingerprint 2", "class_id": "x509.Certificate"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenMapAccessAddressType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"EnableIpV6":true,"ClassId":"access.AddressType","ObjectType":"access.AddressType","EnableIpV4":true}`
	...
	expectedOp := map[string]interface{}{"enable_ip_v6": true, "class_id": "access.AddressType", "object_type": "access.AddressType", "enable_ip_v4": true}
...
func TestFlattenMapAccessConfigurationType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ConfigureOutOfBand":true,"ClassId":"access.ConfigurationType","ObjectType":"access.ConfigurationType","ConfigureInband":true}`
	...
	expectedOp := map[string]interface{}{"configure_out_of_band": true, "class_id": "access.ConfigurationType", "object_type": "access.ConfigurationType", "configure_inband": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeploymentDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"asset.DeploymentDeviceInformation","OldDeviceId":"OldDeviceId %d","Description":"Description %d","InstanceId":"InstanceId %d","MlbProductId":32,"ClassId":"asset.DeploymentDeviceInformation","OldDeviceStatusDescription":"OldDeviceStatusDescription %d","OldInstanceId":"OldInstanceId %d","ItemType":"ItemType %d","OldDeviceStatusId":32,"MlbProductName":"MlbProductName %d","ApplicationName":"ApplicationName %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "asset.DeploymentDeviceInformation", "old_device_id": "OldDeviceId 1", "description": "Description 1", "instance_id": "InstanceId 1", "mlb_product_id": 32, "class_id": "asset.DeploymentDeviceInformation", "old_device_status_description": "OldDeviceStatusDescription 1", "old_instance_id": "OldInstanceId 1", "item_type": "ItemType 1", "old_device_status_id": 32, "mlb_product_name": "MlbProductName 1", "application_name": "ApplicationName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ProductFamily":"ProductFamily %d","ProductType":"ProductType %d","OldDeviceStatusDescription":"OldDeviceStatusDescription %d","ObjectType":"asset.DeviceInformation","MlbProductId":32,"OldDeviceStatusId":32,"OldDeviceId":"OldDeviceId %d","ApplicationName":"ApplicationName %d","MlbOfferType":"MlbOfferType %d","ItemType":"ItemType %d","InstanceId":"InstanceId %d","ClassId":"asset.DeviceInformation","OldInstanceId":"OldInstanceId %d","UnitOfMeasure":"UnitOfMeasure %d","MlbProductName":"MlbProductName %d"}`
	...
	expectedOp := map[string]interface{}{"product_family": "ProductFamily 1", "product_type": "ProductType 1", "old_device_status_description": "OldDeviceStatusDescription 1", "object_type": "asset.DeviceInformation", "mlb_product_id": 32, "old_device_status_id": 32, "old_device_id": "OldDeviceId 1", "application_name": "ApplicationName 1", "mlb_offer_type": "MlbOfferType 1", "item_type": "ItemType 1", "instance_id": "InstanceId 1", "class_id": "asset.DeviceInformation", "old_instance_id": "OldInstanceId 1", "unit_of_measure": "UnitOfMeasure 1", "mlb_product_name": "MlbProductName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceStatistics(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClusterDeviceMoid":"ClusterDeviceMoid %d","ClusterReplicationFactor":32,"MembershipRatio":32.000000,"ClassId":"asset.DeviceStatistics","MemoryMirroringFactor":32.000000,"ClusterDeploymentType":"ClusterDeploymentType %d","ObjectType":"asset.DeviceStatistics","ClusterName":"ClusterName %d","Connected":32}`
	...
	expectedOp := map[string]interface{}{"cluster_device_moid": "ClusterDeviceMoid 1", "cluster_replication_factor": 32, "membership_ratio": 32.000000, "class_id": "asset.DeviceStatistics", "memory_mirroring_factor": 32.000000, "cluster_deployment_type": "ClusterDeploymentType 1", "object_type": "asset.DeviceStatistics", "cluster_name": "ClusterName 1", "connected": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetProductInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"asset.ProductInformation","ObjectType":"asset.ProductInformation","Group":"Group %d","Description":"Description %d","Number":"Number %d","SubType":"SubType %d","Family":"Family %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "asset.ProductInformation", "object_type": "asset.ProductInformation", "group": "Group 1", "description": "Description 1", "number": "Number 1", "sub_type": "SubType 1", "family": "Family 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetSudiInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SerialNumber":"SerialNumber %d","Signature":"Signature %d","Status":"Status %d","ClassId":"asset.SudiInfo","ObjectType":"asset.SudiInfo","Pid":"Pid %d"}`
	...
	expectedOp := map[string]interface{}{"serial_number": "SerialNumber 1", "signature": "Signature 1", "status": "Status 1", "class_id": "asset.SudiInfo", "object_type": "asset.SudiInfo", "pid": "Pid 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCapabilitySwitchNetworkLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaximumVlans":32,"ClassId":"capability.SwitchNetworkLimits","ObjectType":"capability.SwitchNetworkLimits","MaxCompressedPortVlanCount":32,"MaximumFcPortChannels":32,"MaximumPortChannelMembers":32,"MaximumEthernetUplinkPorts":32,"MaximumPrimaryVlan":32,"MaximumIgmpGroups":32,"MaximumEthernetPortChannels":32,"MaximumFcPortChannelMembers":32,"MaxUncompressedPortVlanCount":32,"MinimumActiveFans":32,"MaximumSecondaryVlanPerPrimary":32,"MaximumActiveTrafficMonitoringSessions":32,"MaximumVifs":32,"MaximumSecondaryVlan":32}`
	...
	expectedOp := map[string]interface{}{"maximum_vlans": 32, "class_id": "capability.SwitchNetworkLimits", "object_type": "capability.SwitchNetworkLimits", "max_compressed_port_vlan_count": 32, "maximum_fc_port_channels": 32, "maximum_port_channel_members": 32, "maximum_ethernet_uplink_ports": 32, "maximum_primary_vlan": 32, "maximum_igmp_groups": 32, "maximum_ethernet_port_channels": 32, "maximum_fc_port_channel_members": 32, "max_uncompressed_port_vlan_count": 32, "minimum_active_fans": 32, "maximum_secondary_vlan_per_primary": 32, "maximum_active_traffic_monitoring_sessions": 32, "maximum_vifs": 32, "maximum_secondary_vlan": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchStorageLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaximumUserZoneCount":32,"ClassId":"capability.SwitchStorageLimits","ObjectType":"capability.SwitchStorageLimits","MaximumVirtualFcInterfaces":32,"MaximumVirtualFcInterfacesPerBladeServer":32,"MaximumVsans":32,"MaximumZoneCount":32}`
	...
	expectedOp := map[string]interface{}{"maximum_user_zone_count": 32, "class_id": "capability.SwitchStorageLimits", "object_type": "capability.SwitchStorageLimits", "maximum_virtual_fc_interfaces": 32, "maximum_virtual_fc_interfaces_per_blade_server": 32, "maximum_vsans": 32, "maximum_zone_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchSystemLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaximumFexPerDomain":32,"MaximumServersPerDomain":32,"MaximumChassisCount":32,"ClassId":"capability.SwitchSystemLimits","ObjectType":"capability.SwitchSystemLimits"}`
	...
	expectedOp := map[string]interface{}{"maximum_fex_per_domain": 32, "maximum_servers_per_domain": 32, "maximum_chassis_count": 32, "class_id": "capability.SwitchSystemLimits", "object_type": "capability.SwitchSystemLimits"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudAvailabilityZone(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.AvailabilityZone","ZoneId":"ZoneId %d","Name":"Name %d","ClassId":"cloud.AvailabilityZone"}`
	...
	expectedOp := map[string]interface{}{"object_type": "cloud.AvailabilityZone", "zone_id": "ZoneId 1", "name": "Name 1", "class_id": "cloud.AvailabilityZone"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudBillingUnit(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","BillingId":"BillingId %d","ClassId":"cloud.BillingUnit","ObjectType":"cloud.BillingUnit"}`
	...
	expectedOp := map[string]interface{}{"name": "Name 1", "billing_id": "BillingId 1", "class_id": "cloud.BillingUnit", "object_type": "cloud.BillingUnit"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudInstanceType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CpuSpeed":32,"InstanceTypeId":"InstanceTypeId %d","Name":"Name %d","Architecture":"Architecture %d","Platform":"Platform %d","ClassId":"cloud.InstanceType","ObjectType":"cloud.InstanceType","Memory":32,"Cpus":32}`
	...
	expectedOp := map[string]interface{}{"cpu_speed": 32, "instance_type_id": "InstanceTypeId 1", "name": "Name 1", "architecture": "Architecture 1", "platform": "Platform 1", "class_id": "cloud.InstanceType", "object_type": "cloud.InstanceType", "memory": 32, "cpus": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCommIpV4Interface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"comm.IpV4Interface","ObjectType":"comm.IpV4Interface","IpAddress":"IpAddress %d","Netmask":"Netmask %d","Gateway":"Gateway %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "comm.IpV4Interface", "object_type": "comm.IpV4Interface", "ip_address": "IpAddress 1", "netmask": "Netmask 1", "gateway": "Gateway 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCommIpV6Interface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"comm.IpV6Interface","Prefix":"Prefix %d","Gateway":"Gateway %d","IpAddress":"IpAddress %d","ClassId":"comm.IpV6Interface"}`
	...
	expectedOp := map[string]interface{}{"object_type": "comm.IpV6Interface", "prefix": "Prefix 1", "gateway": "Gateway 1", "ip_address": "IpAddress 1", "class_id": "comm.IpV6Interface"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapComputeStorageControllerOperation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ControllerId":"ControllerId %d","ClassId":"compute.StorageControllerOperation","ObjectType":"compute.StorageControllerOperation","AdminAction":"AdminAction %d"}`
	...
	expectedOp := map[string]interface{}{"controller_id": "ControllerId 1", "class_id": "compute.StorageControllerOperation", "object_type": "compute.StorageControllerOperation", "admin_action": "AdminAction 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCondAlarmSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"cond.AlarmSummary","Critical":32,"Warning":32,"ObjectType":"cond.AlarmSummary"}`
	...
	expectedOp := map[string]interface{}{"class_id": "cond.AlarmSummary", "critical": 32, "warning": 32, "object_type": "cond.AlarmSummary"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapConvergedinfraPodSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"convergedinfra.PodSummary","StorageAvailable":32,"VmCount":32,"NodeCount":32,"StorageCapacity":32,"StorageUtilization":32.000000,"ObjectType":"convergedinfra.PodSummary","ActiveNodes":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "convergedinfra.PodSummary", "storage_available": 32, "vm_count": 32, "node_count": 32, "storage_capacity": 32, "storage_utilization": 32.000000, "object_type": "convergedinfra.PodSummary", "active_nodes": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFabricVlanSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"fabric.VlanSettings","ObjectType":"fabric.VlanSettings","NativeVlan":32,"AllowedVlans":"AllowedVlans %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "fabric.VlanSettings", "object_type": "fabric.VlanSettings", "native_vlan": 32, "allowed_vlans": "AllowedVlans 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFcpoolBlock(t *testing.T) {
	p := models.FcpoolBlock{}
	var d = &schema.ResourceData{}
	c := `{"From":"From %d","ClassId":"fcpool.Block","ObjectType":"fcpool.Block","Size":32,"To":"To %d"}`
	...
	expectedOp := map[string]interface{}{"from": "From 1", "class_id": "fcpool.Block", "object_type": "fcpool.Block", "size": 32, "to": "To 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareDirectDownload(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Upgradeoption":"Upgradeoption %d","ClassId":"firmware.DirectDownload","ObjectType":"firmware.DirectDownload","Username":"Username %d","ImageSource":"ImageSource %d","IsPasswordSet":true}`
	...
	expectedOp := map[string]interface{}{"upgradeoption": "Upgradeoption 1", "class_id": "firmware.DirectDownload", "object_type": "firmware.DirectDownload", "username": "Username 1", "image_source": "ImageSource 1", "is_password_set": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareNetworkShare(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IsPasswordSet":true,"Upgradeoption":"Upgradeoption %d","ObjectType":"firmware.NetworkShare","Username":"Username %d","MapType":"MapType %d","ClassId":"firmware.NetworkShare"}`
	...
	expectedOp := map[string]interface{}{"is_password_set": true, "upgradeoption": "Upgradeoption 1", "object_type": "firmware.NetworkShare", "username": "Username 1", "map_type": "MapType 1", "class_id": "firmware.NetworkShare"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexAlarmSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Critical":32,"Warning":32,"ClassId":"hyperflex.AlarmSummary","ObjectType":"hyperflex.AlarmSummary"}`
	...
	expectedOp := map[string]interface{}{"critical": 32, "warning": 32, "class_id": "hyperflex.AlarmSummary", "object_type": "hyperflex.AlarmSummary"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexAppSettingConstraint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"HxdpVersion":"HxdpVersion %d","HypervisorType":"HypervisorType %d","MgmtPlatform":"MgmtPlatform %d","ServerModel":"ServerModel %d","DeploymentType":"DeploymentType %d","ClassId":"hyperflex.AppSettingConstraint","ObjectType":"hyperflex.AppSettingConstraint"}`
	...
	expectedOp := map[string]interface{}{"hxdp_version": "HxdpVersion 1", "hypervisor_type": "HypervisorType 1", "mgmt_platform": "MgmtPlatform 1", "server_model": "ServerModel 1", "deployment_type": "DeploymentType 1", "class_id": "hyperflex.AppSettingConstraint", "object_type": "hyperflex.AppSettingConstraint"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexBackupPolicySettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReplicationPairNamePrefix":"ReplicationPairNamePrefix %d","SnapshotRetentionCount":32,"DataStoreEncryptionEnabled":true,"ReplicationIntervalInMinutes":32,"ObjectType":"hyperflex.BackupPolicySettings","ClassId":"hyperflex.BackupPolicySettings","BackupDataStoreSize":32,"BackupDataStoreSizeUnit":"BackupDataStoreSizeUnit %d","LocalSnapshotRetentionCount":32,"BackupDataStoreName":"BackupDataStoreName %d"}`
	...
	expectedOp := map[string]interface{}{"replication_pair_name_prefix": "ReplicationPairNamePrefix 1", "snapshot_retention_count": 32, "data_store_encryption_enabled": true, "replication_interval_in_minutes": 32, "object_type": "hyperflex.BackupPolicySettings", "class_id": "hyperflex.BackupPolicySettings", "backup_data_store_size": 32, "backup_data_store_size_unit": "BackupDataStoreSizeUnit 1", "local_snapshot_retention_count": 32, "backup_data_store_name": "BackupDataStoreName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexEntityReference(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Confignum":32,"Id":"Id %d","Idtype":"Idtype %d","Name":"Name %d","Type":"Type %d","ClassId":"hyperflex.EntityReference","ObjectType":"hyperflex.EntityReference"}`
	...
	expectedOp := map[string]interface{}{"confignum": 32, "id": "Id 1", "idtype": "Idtype 1", "name": "Name 1", "type": "Type 1", "class_id": "hyperflex.EntityReference", "object_type": "hyperflex.EntityReference"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHealthCheckScriptInfo(t *testing.T) {
	p := models.HyperflexHealthCheckScriptInfo{}
	var d = &schema.ResourceData{}
	c := `{"ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptInput":"ScriptInput %d","Version":"Version %d","ClassId":"hyperflex.HealthCheckScriptInfo","AggregateScriptName":"AggregateScriptName %d","HyperflexVersion":"HyperflexVersion %d","ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptName":"ScriptName %d"}`
	...
	expectedOp := map[string]interface{}{"script_execute_location": "ScriptExecuteLocation 1", "script_input": "ScriptInput 1", "nr_version": "Version 1", "class_id": "hyperflex.HealthCheckScriptInfo", "aggregate_script_name": "AggregateScriptName 1", "hyperflex_version": "HyperflexVersion 1", "object_type": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHxPlatformDatastoreConfigDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DataBlockSize":32,"ProvisionedCapacity":32,"NumStripesForLargeFiles":32,"SystemDatastore":true,"UsageType":"UsageType %d","ClassId":"hyperflex.HxPlatformDatastoreConfigDt","ObjectType":"hyperflex.HxPlatformDatastoreConfigDt","Name":"Name %d","NumMirrors":32}`
	...
	expectedOp := map[string]interface{}{"data_block_size": 32, "provisioned_capacity": 32, "num_stripes_for_large_files": 32, "system_datastore": true, "usage_type": "UsageType 1", "class_id": "hyperflex.HxPlatformDatastoreConfigDt", "object_type": "hyperflex.HxPlatformDatastoreConfigDt", "name": "Name 1", "num_mirrors": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxRegistrationDetailsDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SmartAccount":"SmartAccount %d","Status":"Status %d","LastRenewalAttemptDate":"LastRenewalAttemptDate %d","NextRenewalAttemptDate":"NextRenewalAttemptDate %d","VirtualAccount":"VirtualAccount %d","RegistrationFailedReason":"RegistrationFailedReason %d","ObjectType":"hyperflex.HxRegistrationDetailsDt","InitialRegistrationDate":"InitialRegistrationDate %d","RegistrationExpireDate":"RegistrationExpireDate %d","OutOfComplianceStartDate":"OutOfComplianceStartDate %d","ClassId":"hyperflex.HxRegistrationDetailsDt"}`
	...
	expectedOp := map[string]interface{}{"smart_account": "SmartAccount 1", "status": "Status 1", "last_renewal_attempt_date": "LastRenewalAttemptDate 1", "next_renewal_attempt_date": "NextRenewalAttemptDate 1", "virtual_account": "VirtualAccount 1", "registration_failed_reason": "RegistrationFailedReason 1", "object_type": "hyperflex.HxRegistrationDetailsDt", "initial_registration_date": "InitialRegistrationDate 1", "registration_expire_date": "RegistrationExpireDate 1", "out_of_compliance_start_date": "OutOfComplianceStartDate 1", "class_id": "hyperflex.HxRegistrationDetailsDt"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxResiliencyInfoDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.HxResiliencyInfoDt","ObjectType":"hyperflex.HxResiliencyInfoDt","DataReplicationFactor":"DataReplicationFactor %d","ResiliencyState":"ResiliencyState %d","HddFailuresTolerable":32,"SsdFailuresTolerable":32,"NodeFailuresTolerable":32,"PolicyCompliance":"PolicyCompliance %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.HxResiliencyInfoDt", "object_type": "hyperflex.HxResiliencyInfoDt", "data_replication_factor": "DataReplicationFactor 1", "resiliency_state": "ResiliencyState 1", "hdd_failures_tolerable": 32, "ssd_failures_tolerable": 32, "node_failures_tolerable": 32, "policy_compliance": "PolicyCompliance 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexNamedVlan(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"name": "Name 1", "vlan_id": 32, "class_id": "hyperflex.NamedVlan", "object_type": "hyperflex.NamedVlan"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexNamedVsan(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","ClassId":"hyperflex.NamedVsan","ObjectType":"hyperflex.NamedVsan","VsanId":32}`
	...
	expectedOp := map[string]interface{}{"name": "Name 1", "class_id": "hyperflex.NamedVsan", "object_type": "hyperflex.NamedVsan", "vsan_id": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexReplicationPeerInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"StatusDetails":"StatusDetails %d","Mcip":"Mcip %d","ClassId":"hyperflex.ReplicationPeerInfo","Status":"Status %d","Dcip":"Dcip %d","ObjectType":"hyperflex.ReplicationPeerInfo","ReplCip":"ReplCip %d"}`
	...
	expectedOp := map[string]interface{}{"status_details": "StatusDetails 1", "mcip": "Mcip 1", "class_id": "hyperflex.ReplicationPeerInfo", "status": "Status 1", "dcip": "Dcip 1", "object_type": "hyperflex.ReplicationPeerInfo", "repl_cip": "ReplCip 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.Summary","Downtime":"Downtime %d","SpaceStatus":"SpaceStatus %d","UsedCapacity":32,"ActiveNodes":"ActiveNodes %d","ResiliencyDetailsSize":32,"ClassId":"hyperflex.Summary","State":"State %d","FreeCapacity":32,"TotalCapacity":32,"Boottime":32,"Uptime":"Uptime %d","ClusterAccessPolicy":"ClusterAccessPolicy %d","DataReplicationCompliance":"DataReplicationCompliance %d","CompressionSavings":32.000000,"Address":"Address %d","TotalSavings":32.000000,"DataReplicationFactor":"DataReplicationFactor %d","Name":"Name %d","DeduplicationSavings":32.000000}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.Summary", "downtime": "Downtime 1", "space_status": "SpaceStatus 1", "used_capacity": 32, "active_nodes": "ActiveNodes 1", "resiliency_details_size": 32, "class_id": "hyperflex.Summary", "state": "State 1", "free_capacity": 32, "total_capacity": 32, "boottime": 32, "uptime": "Uptime 1", "cluster_access_policy": "ClusterAccessPolicy 1", "data_replication_compliance": "DataReplicationCompliance 1", "compression_savings": 32.000000, "address": "Address 1", "total_savings": 32.000000, "data_replication_factor": "DataReplicationFactor 1", "name": "Name 1", "deduplication_savings": 32.000000}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamEndPointPasswordProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PasswordHistory":32,"NotificationPeriod":32,"ForceSendPassword":true,"ClassId":"iam.EndPointPasswordProperties","GracePeriod":32,"EnablePasswordExpiry":true,"EnforceStrongPassword":true,"PasswordExpiryDuration":32,"ObjectType":"iam.EndPointPasswordProperties"}`
	...
	expectedOp := map[string]interface{}{"password_history": 32, "notification_period": 32, "force_send_password": true, "class_id": "iam.EndPointPasswordProperties", "grace_period": 32, "enable_password_expiry": true, "enforce_strong_password": true, "password_expiry_duration": 32, "object_type": "iam.EndPointPasswordProperties"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamFailureDetails(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"iam.FailureDetails","ObjectType":"iam.FailureDetails","FailureReason":"FailureReason %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "iam.FailureDetails", "object_type": "iam.FailureDetails", "failure_reason": "FailureReason 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamLdapBaseProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"BindDn":"BindDn %d","ObjectType":"iam.LdapBaseProperties","IsPasswordSet":true,"EnableEncryption":true,"NestedGroupSearchDepth":32,"Attribute":"Attribute %d","ClassId":"iam.LdapBaseProperties","Filter":"Filter %d","BindMethod":"BindMethod %d","Timeout":32,"EnableGroupAuthorization":true,"BaseDn":"BaseDn %d","GroupAttribute":"GroupAttribute %d","Domain":"Domain %d"}`
	...
	expectedOp := map[string]interface{}{"bind_dn": "BindDn 1", "object_type": "iam.LdapBaseProperties", "is_password_set": true, "enable_encryption": true, "nested_group_search_depth": 32, "attribute": "Attribute 1", "class_id": "iam.LdapBaseProperties", "filter": "Filter 1", "bind_method": "BindMethod 1", "timeout": 32, "enable_group_authorization": true, "base_dn": "BaseDn 1", "group_attribute": "GroupAttribute 1", "domain": "Domain 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIppoolIpV4Config(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PrimaryDns":"PrimaryDns %d","SecondaryDns":"SecondaryDns %d","ClassId":"ippool.IpV4Config","ObjectType":"ippool.IpV4Config","Gateway":"Gateway %d","Netmask":"Netmask %d"}`
	...
	expectedOp := map[string]interface{}{"primary_dns": "PrimaryDns 1", "secondary_dns": "SecondaryDns 1", "class_id": "ippool.IpV4Config", "object_type": "ippool.IpV4Config", "gateway": "Gateway 1", "netmask": "Netmask 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapIppoolIpV6Block(t *testing.T) {
	p := models.IppoolIpV6Block{}
	var d = &schema.ResourceData{}
	c := `{"Size":32,"From":"From %d","To":"To %d","ClassId":"ippool.IpV6Block","ObjectType":"ippool.IpV6Block"}`
	...
	expectedOp := map[string]interface{}{"size": 32, "from": "From 1", "to": "To 1", "class_id": "ippool.IpV6Block", "object_type": "ippool.IpV6Block"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIqnpoolIqnSuffixBlock(t *testing.T) {
	p := models.IqnpoolIqnSuffixBlock{}
	var d = &schema.ResourceData{}
	c := `{"From":32,"Size":32,"ClassId":"iqnpool.IqnSuffixBlock","ObjectType":"iqnpool.IqnSuffixBlock","Suffix":"Suffix %d","To":32}`
	...
	expectedOp := map[string]interface{}{"from": 32, "size": 32, "class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "suffix": "Suffix 1", "to": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesActionInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"FailureReason":"FailureReason %d","Name":"Name %d","ClassId":"kubernetes.ActionInfo","ObjectType":"kubernetes.ActionInfo","Status":"Status %d"}`
	...
	expectedOp := map[string]interface{}{"failure_reason": "FailureReason 1", "name": "Name 1", "class_id": "kubernetes.ActionInfo", "object_type": "kubernetes.ActionInfo", "status": "Status 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesAddonConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.AddonConfiguration","ReleaseName":"ReleaseName %d","UpgradeStrategy":"UpgradeStrategy %d","ClassId":"kubernetes.AddonConfiguration","ReleaseNamespace":"ReleaseNamespace %d","InstallStrategy":"InstallStrategy %d","Overrides":"Overrides %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.AddonConfiguration", "release_name": "ReleaseName 1", "upgrade_strategy": "UpgradeStrategy 1", "class_id": "kubernetes.AddonConfiguration", "release_namespace": "ReleaseNamespace 1", "install_strategy": "InstallStrategy 1", "overrides": "Overrides 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesClusterCertificateConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.ClusterCertificateConfiguration","SaPublicKey":"SaPublicKey %d","ClassId":"kubernetes.ClusterCertificateConfiguration","FrontProxyCert":"FrontProxyCert %d","CaCert":"CaCert %d","EtcdCert":"EtcdCert %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.ClusterCertificateConfiguration", "sa_public_key": "SaPublicKey 1", "class_id": "kubernetes.ClusterCertificateConfiguration", "front_proxy_cert": "FrontProxyCert 1", "ca_cert": "CaCert 1", "etcd_cert": "EtcdCert 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesClusterManagementConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SshUser":"SshUser %d","MasterVip":"MasterVip %d","ClassId":"kubernetes.ClusterManagementConfig","ObjectType":"kubernetes.ClusterManagementConfig","LoadBalancerCount":32,"IsTacPasswdSet":true}`
	...
	expectedOp := map[string]interface{}{"ssh_user": "SshUser 1", "master_vip": "MasterVip 1", "class_id": "kubernetes.ClusterManagementConfig", "object_type": "kubernetes.ClusterManagementConfig", "load_balancer_count": 32, "is_tac_passwd_set": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"KubeConfig":"KubeConfig %d","ClassId":"kubernetes.Configuration","ObjectType":"kubernetes.Configuration"}`
	...
	expectedOp := map[string]interface{}{"kube_config": "KubeConfig 1", "class_id": "kubernetes.Configuration", "object_type": "kubernetes.Configuration"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesDaemonSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NumberMisscheduled":32,"NumberReady":32,"ObservedGeneration":32,"CurrentNumberScheduled":32,"NumberAvailable":"NumberAvailable %d","ClassId":"kubernetes.DaemonSetStatus","ObjectType":"kubernetes.DaemonSetStatus","DesiredNumberScheduled":32,"UpdatedNumberScheduled":"UpdatedNumberScheduled %d"}`
	...
	expectedOp := map[string]interface{}{"number_misscheduled": 32, "number_ready": 32, "observed_generation": 32, "current_number_scheduled": 32, "number_available": "NumberAvailable 1", "class_id": "kubernetes.DaemonSetStatus", "object_type": "kubernetes.DaemonSetStatus", "desired_number_scheduled": 32, "updated_number_scheduled": "UpdatedNumberScheduled 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesDeploymentStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReadyReplicas":32,"Replicas":32,"UpdatedReplicas":32,"ClassId":"kubernetes.DeploymentStatus","ObjectType":"kubernetes.DeploymentStatus","AvailableReplicas":32,"ObservedGeneration":32}`
	...
	expectedOp := map[string]interface{}{"ready_replicas": 32, "replicas": 32, "updated_replicas": 32, "class_id": "kubernetes.DeploymentStatus", "object_type": "kubernetes.DeploymentStatus", "available_replicas": 32, "observed_generation": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesNodeInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"KubeProxyVersion":"KubeProxyVersion %d","Architecture":"Architecture %d","KernelVersion":"KernelVersion %d","ClassId":"kubernetes.NodeInfo","KubeletVersion":"KubeletVersion %d","OperatingSystem":"OperatingSystem %d","OsImage":"OsImage %d","SystemUuid":"SystemUuid %d","MachineId":"MachineId %d","BootId":"BootId %d","ContainerRuntimeVersion":"ContainerRuntimeVersion %d","ObjectType":"kubernetes.NodeInfo"}`
	...
	expectedOp := map[string]interface{}{"kube_proxy_version": "KubeProxyVersion 1", "architecture": "Architecture 1", "kernel_version": "KernelVersion 1", "class_id": "kubernetes.NodeInfo", "kubelet_version": "KubeletVersion 1", "operating_system": "OperatingSystem 1", "os_image": "OsImage 1", "system_uuid": "SystemUuid 1", "machine_id": "MachineId 1", "boot_id": "BootId 1", "container_runtime_version": "ContainerRuntimeVersion 1", "object_type": "kubernetes.NodeInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesObjectMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Uuid":"Uuid %d","CreationTimestamp":"CreationTimestamp %d","Name":"Name %d","ClassId":"kubernetes.ObjectMeta","ObjectType":"kubernetes.ObjectMeta","Namespace":"Namespace %d","ResourceVersion":"ResourceVersion %d"}`
	...
	expectedOp := map[string]interface{}{"uuid": "Uuid 1", "creation_timestamp": "CreationTimestamp 1", "name": "Name 1", "class_id": "kubernetes.ObjectMeta", "object_type": "kubernetes.ObjectMeta", "namespace": "Namespace 1", "resource_version": "ResourceVersion 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesPodStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Phase":"Phase %d","PodIp":"PodIp %d","QosClass":"QosClass %d","StartTime":"StartTime %d","HostIp":"HostIp %d","ClassId":"kubernetes.PodStatus","ObjectType":"kubernetes.PodStatus"}`
	...
	expectedOp := map[string]interface{}{"phase": "Phase 1", "pod_ip": "PodIp 1", "qos_class": "QosClass 1", "start_time": "StartTime 1", "host_ip": "HostIp 1", "class_id": "kubernetes.PodStatus", "object_type": "kubernetes.PodStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesProxyConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.ProxyConfig","Hostname":"Hostname %d","IsPasswordSet":true,"Port":32,"Username":"Username %d","Protocol":"Protocol %d","ObjectType":"kubernetes.ProxyConfig"}`
	...
	expectedOp := map[string]interface{}{"class_id": "kubernetes.ProxyConfig", "hostname": "Hostname 1", "is_password_set": true, "port": 32, "username": "Username 1", "protocol": "Protocol 1", "object_type": "kubernetes.ProxyConfig"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesStatefulSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Replicas":32,"ClassId":"kubernetes.StatefulSetStatus","UpdatedReplicas":32,"AvailableReplicas":32,"CurrentRevision":"CurrentRevision %d","ObjectType":"kubernetes.StatefulSetStatus","ObservedGeneration":32,"ReadyReplicas":32,"UpdateRevision":"UpdateRevision %d","CollisionCount":32}`
	...
	expectedOp := map[string]interface{}{"replicas": 32, "class_id": "kubernetes.StatefulSetStatus", "updated_replicas": 32, "available_replicas": 32, "current_revision": "CurrentRevision 1", "object_type": "kubernetes.StatefulSetStatus", "observed_generation": 32, "ready_replicas": 32, "update_revision": "UpdateRevision 1", "collision_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapMoBaseMo(t *testing.T) {
	p := models.MoBaseMo{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"mo.BaseMo","ObjectType":"mo.BaseMo","SharedScope":"SharedScope %d","Moid":"Moid %d","DomainGroupMoid":"DomainGroupMoid %d","AccountMoid":"AccountMoid %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "mo.BaseMo", "object_type": "mo.BaseMo", "shared_scope": "SharedScope 1", "moid": "Moid 1", "domain_group_moid": "DomainGroupMoid 1", "account_moid": "AccountMoid 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNetworkHyperFlexNetworkAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Address":"Address %d","Fqdn":"Fqdn %d","Ip":"Ip %d","ClassId":"network.HyperFlexNetworkAddress","ObjectType":"network.HyperFlexNetworkAddress"}`
	...
	expectedOp := map[string]interface{}{"address": "Address 1", "fqdn": "Fqdn 1", "ip": "Ip 1", "class_id": "network.HyperFlexNetworkAddress", "object_type": "network.HyperFlexNetworkAddress"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiaapiNewReleaseDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReleaseNoteLinkTitle":"ReleaseNoteLinkTitle %d","Version":"Version %d","ClassId":"niaapi.NewReleaseDetail","SoftwareDownloadLink":"SoftwareDownloadLink %d","ReleaseNoteLink":"ReleaseNoteLink %d","SoftwareDownloadLinkTitle":"SoftwareDownloadLinkTitle %d","Title":"Title %d","Link":"Link %d","ObjectType":"niaapi.NewReleaseDetail","Description":"Description %d"}`
	...
	expectedOp := map[string]interface{}{"release_note_link_title": "ReleaseNoteLinkTitle 1", "nr_version": "Version 1", "class_id": "niaapi.NewReleaseDetail", "software_download_link": "SoftwareDownloadLink 1", "release_note_link": "ReleaseNoteLink 1", "software_download_link_title": "SoftwareDownloadLinkTitle 1", "title": "Title 1", "link": "Link 1", "object_type": "niaapi.NewReleaseDetail", "description": "Description 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiaapiVersionRegexPlatform(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niaapi.VersionRegexPlatform","ClassId":"niaapi.VersionRegexPlatform","Anyllregex":"Anyllregex %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "niaapi.VersionRegexPlatform", "class_id": "niaapi.VersionRegexPlatform", "anyllregex": "Anyllregex 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryBootflashDetails(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.BootflashDetails","FwRev":"FwRev %d","ModelType":"ModelType %d","Serial":"Serial %d","ClassId":"niatelemetry.BootflashDetails"}`
	...
	expectedOp := map[string]interface{}{"object_type": "niatelemetry.BootflashDetails", "fw_rev": "FwRev 1", "model_type": "ModelType 1", "serial": "Serial 1", "class_id": "niatelemetry.BootflashDetails"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.Interface","ObjectType":"niatelemetry.Interface","InterfaceUpCount":32,"InterfaceDownCount":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "niatelemetry.Interface", "object_type": "niatelemetry.Interface", "interface_up_count": 32, "interface_down_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryNvePacketCounters(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UcastOutpkts":32,"ClassId":"niatelemetry.NvePacketCounters","ObjectType":"niatelemetry.NvePacketCounters","McastInpkts":32,"McastOutbytes":32,"UcastInpkts":32}`
	...
	expectedOp := map[string]interface{}{"ucast_outpkts": 32, "class_id": "niatelemetry.NvePacketCounters", "object_type": "niatelemetry.NvePacketCounters", "mcast_inpkts": 32, "mcast_outbytes": 32, "ucast_inpkts": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNveVni(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CpVniCount":32,"DpVniDown":32,"ClassId":"niatelemetry.NveVni","ObjectType":"niatelemetry.NveVni","DpVniUp":32,"CpVniDown":32,"CpVniUp":32,"DpVniCount":32}`
	...
	expectedOp := map[string]interface{}{"cp_vni_count": 32, "dp_vni_down": 32, "class_id": "niatelemetry.NveVni", "object_type": "niatelemetry.NveVni", "dp_vni_up": 32, "cp_vni_down": 32, "cp_vni_up": 32, "dp_vni_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosBgpEvpn(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"TotalNetworks":32,"TotalPaths":32,"ClassId":"niatelemetry.NxosBgpEvpn","ObjectType":"niatelemetry.NxosBgpEvpn","NxosEvpnMacCount":"NxosEvpnMacCount %d"}`
	...
	expectedOp := map[string]interface{}{"total_networks": 32, "total_paths": 32, "class_id": "niatelemetry.NxosBgpEvpn", "object_type": "niatelemetry.NxosBgpEvpn", "nxos_evpn_mac_count": "NxosEvpnMacCount 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosBgpMvpn(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NumberOfCommunities":32,"CapablePeers":32,"MemoryUsed":32,"ClassId":"niatelemetry.NxosBgpMvpn","TotalNetworks":32,"TotalPaths":32,"ConfiguredPeers":32,"NumberOfClusterLists":32,"ObjectType":"niatelemetry.NxosBgpMvpn","TableVersion":32}`
	...
	expectedOp := map[string]interface{}{"number_of_communities": 32, "capable_peers": 32, "memory_used": 32, "class_id": "niatelemetry.NxosBgpMvpn", "total_networks": 32, "total_paths": 32, "configured_peers": 32, "number_of_cluster_lists": 32, "object_type": "niatelemetry.NxosBgpMvpn", "table_version": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosVtp(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RunningVersion":"RunningVersion %d","TrapEnabled":"TrapEnabled %d","ClassId":"niatelemetry.NxosVtp","V2Mode":"V2Mode %d","Version":32,"OperMode":"OperMode %d","PruningMode":"PruningMode %d","ObjectType":"niatelemetry.NxosVtp"}`
	...
	expectedOp := map[string]interface{}{"running_version": "RunningVersion 1", "trap_enabled": "TrapEnabled 1", "class_id": "niatelemetry.NxosVtp", "v2_mode": "V2Mode 1", "nr_version": 32, "oper_mode": "OperMode 1", "pruning_mode": "PruningMode 1", "object_type": "niatelemetry.NxosVtp"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetrySmartLicense(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AuthStatus":"AuthStatus %d","LicenseUdi":"LicenseUdi %d","SmartAccount":"SmartAccount %d","ClassId":"niatelemetry.SmartLicense","ObjectType":"niatelemetry.SmartLicense","ActiveMode":"ActiveMode %d"}`
	...
	expectedOp := map[string]interface{}{"auth_status": "AuthStatus 1", "license_udi": "LicenseUdi 1", "smart_account": "SmartAccount 1", "class_id": "niatelemetry.SmartLicense", "object_type": "niatelemetry.SmartLicense", "active_mode": "ActiveMode 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOnpremSchedule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"TimeOfDay":32,"MonthOfYear":32,"DayOfMonth":32,"ClassId":"onprem.Schedule","RepeatInterval":32,"DayOfWeek":32,"ObjectType":"onprem.Schedule","WeekOfMonth":32,"TimeZone":"TimeZone %d"}`
	...
	expectedOp := map[string]interface{}{"time_of_day": 32, "month_of_year": 32, "day_of_month": 32, "class_id": "onprem.Schedule", "repeat_interval": 32, "day_of_week": 32, "object_type": "onprem.Schedule", "week_of_month": 32, "time_zone": "TimeZone 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapOnpremUpgradePhase(t *testing.T) {
	p := models.OnpremUpgradePhase{}
	var d = &schema.ResourceData{}
	c := `{"Failed":true,"Message":"Message %d","Name":"Name %d","ClassId":"onprem.UpgradePhase","ElapsedTime":32,"ObjectType":"onprem.UpgradePhase"}`
	...
	expectedOp := map[string]interface{}{"failed": true, "message": "Message 1", "name": "Name 1", "class_id": "onprem.UpgradePhase", "elapsed_time": 32, "object_type": "onprem.UpgradePhase"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsAnswers(t *testing.T) {
	p := models.OsAnswers{}
	var d = &schema.ResourceData{}
	c := `{"Hostname":"Hostname %d","IsAnswerFileSet":true,"IsRootPasswordSet":true,"IsRootPasswordCrypted":true,"NetworkDevice":"NetworkDevice %d","ClassId":"os.Answers","ProductKey":"ProductKey %d","IpConfigType":"IpConfigType %d","Source":"Source %d","ObjectType":"os.Answers","Nameserver":"Nameserver %d"}`
	...
	expectedOp := map[string]interface{}{"hostname": "Hostname 1", "is_answer_file_set": true, "is_root_password_set": true, "is_root_password_crypted": true, "network_device": "NetworkDevice 1", "class_id": "os.Answers", "product_key": "ProductKey 1", "ip_config_type": "IpConfigType 1", "nr_source": "Source 1", "object_type": "os.Answers", "nameserver": "Nameserver 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsGlobalConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"InstallMethod":"InstallMethod %d","ClassId":"os.GlobalConfig","ConfigurationSource":"ConfigurationSource %d","OsImageName":"OsImageName %d","ScuImageName":"ScuImageName %d","ConfigurationFileName":"ConfigurationFileName %d","InstallTargetType":"InstallTargetType %d","WindowsEdition":"WindowsEdition %d","ObjectType":"os.GlobalConfig"}`
	...
	expectedOp := map[string]interface{}{"install_method": "InstallMethod 1", "class_id": "os.GlobalConfig", "configuration_source": "ConfigurationSource 1", "os_image_name": "OsImageName 1", "scu_image_name": "ScuImageName 1", "configuration_file_name": "ConfigurationFileName 1", "install_target_type": "InstallTargetType 1", "windows_edition": "WindowsEdition 1", "object_type": "os.GlobalConfig"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapPolicyConfigChangeContext(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"policy.ConfigChangeContext","ConfigChangeError":"ConfigChangeError %d","ConfigChangeState":"ConfigChangeState %d","ObjectType":"policy.ConfigChangeContext"}`
	...
	expectedOp := map[string]interface{}{"class_id": "policy.ConfigChangeContext", "config_change_error": "ConfigChangeError 1", "config_change_state": "ConfigChangeState 1", "object_type": "policy.ConfigChangeContext"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapPolicyConfigContext(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"policy.ConfigContext","ConfigType":"ConfigType %d","ControlAction":"ControlAction %d","ErrorState":"ErrorState %d","OperState":"OperState %d","ConfigState":"ConfigState %d","ObjectType":"policy.ConfigContext"}`
	...
	expectedOp := map[string]interface{}{"class_id": "policy.ConfigContext", "config_type": "ConfigType 1", "control_action": "ControlAction 1", "error_state": "ErrorState 1", "oper_state": "OperState 1", "config_state": "ConfigState 1", "object_type": "policy.ConfigContext"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapPolicyConfigResultContext(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"EntityName":"EntityName %d","EntityType":"EntityType %d","ParentType":"ParentType %d","ClassId":"policy.ConfigResultContext","ObjectType":"policy.ConfigResultContext","EntityMoid":"EntityMoid %d","ParentMoid":"ParentMoid %d"}`
	...
	expectedOp := map[string]interface{}{"entity_name": "EntityName 1", "entity_type": "EntityType 1", "parent_type": "ParentType 1", "class_id": "policy.ConfigResultContext", "object_type": "policy.ConfigResultContext", "entity_moid": "EntityMoid 1", "parent_moid": "ParentMoid 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageAutomaticDriveGroup(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UseRemainingDrives":true,"DriveType":"DriveType %d","MinimumDriveSize":32,"NumDedicatedHotSpares":"NumDedicatedHotSpares %d","ClassId":"storage.AutomaticDriveGroup","DrivesPerSpan":32,"ObjectType":"storage.AutomaticDriveGroup","NumberOfSpans":32}`
	...
	expectedOp := map[string]interface{}{"use_remaining_drives": true, "drive_type": "DriveType 1", "minimum_drive_size": 32, "num_dedicated_hot_spares": "NumDedicatedHotSpares 1", "class_id": "storage.AutomaticDriveGroup", "drives_per_span": 32, "object_type": "storage.AutomaticDriveGroup", "number_of_spans": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapStorageBaseCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CapacityUtilization":32.000000,"Free":32,"Total":32,"Used":32,"ClassId":"storage.BaseCapacity","ObjectType":"storage.BaseCapacity","Available":32}`
	...
	expectedOp := map[string]interface{}{"capacity_utilization": 32.000000, "free": 32, "total": 32, "used": 32, "class_id": "storage.BaseCapacity", "object_type": "storage.BaseCapacity", "available": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppEthernetPortLag(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Mode":"Mode %d","DistributionPolicy":"DistributionPolicy %d","ClassId":"storage.NetAppEthernetPortLag","ObjectType":"storage.NetAppEthernetPortLag"}`
	...
	expectedOp := map[string]interface{}{"mode": "Mode 1", "distribution_policy": "DistributionPolicy 1", "class_id": "storage.NetAppEthernetPortLag", "object_type": "storage.NetAppEthernetPortLag"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppHighAvailability(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Enabled":true,"GivebackState":"GivebackState %d","PartnerName":"PartnerName %d","ClassId":"storage.NetAppHighAvailability","ObjectType":"storage.NetAppHighAvailability","PartnerUuid":"PartnerUuid %d","TakeoverState":"TakeoverState %d"}`
	...
	expectedOp := map[string]interface{}{"enabled": true, "giveback_state": "GivebackState 1", "partner_name": "PartnerName 1", "class_id": "storage.NetAppHighAvailability", "object_type": "storage.NetAppHighAvailability", "partner_uuid": "PartnerUuid 1", "takeover_state": "TakeoverState 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppPerformanceMetricsAverage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"storage.NetAppPerformanceMetricsAverage","Latency":32.000000,"Period":32,"Throughput":32.000000,"Iops":32.000000,"ClassId":"storage.NetAppPerformanceMetricsAverage"}`
	...
	expectedOp := map[string]interface{}{"object_type": "storage.NetAppPerformanceMetricsAverage", "latency": 32.000000, "period": 32, "throughput": 32.000000, "iops": 32.000000, "class_id": "storage.NetAppPerformanceMetricsAverage"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapStorageNetAppStorageClusterEfficiency(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Ratio":32.000000,"ClassId":"storage.NetAppStorageClusterEfficiency","ObjectType":"storage.NetAppStorageClusterEfficiency","Savings":32,"LogicalUsed":32}`
	...
	expectedOp := map[string]interface{}{"ratio": 32.000000, "class_id": "storage.NetAppStorageClusterEfficiency", "object_type": "storage.NetAppStorageClusterEfficiency", "savings": 32, "logical_used": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationActionInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Status":"Status %d","FailureReason":"FailureReason %d","ClassId":"virtualization.ActionInfo","ObjectType":"virtualization.ActionInfo","Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"status": "Status 1", "failure_reason": "FailureReason 1", "class_id": "virtualization.ActionInfo", "object_type": "virtualization.ActionInfo", "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationBaseHostConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.BaseHostConfiguration","ObjectType":"virtualization.BaseHostConfiguration"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.BaseHostConfiguration", "object_type": "virtualization.BaseHostConfiguration"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCloudInitConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UserData":"UserData %d","UserDataBase64Encoded":true,"ClassId":"virtualization.CloudInitConfig","ObjectType":"virtualization.CloudInitConfig","ConfigType":"ConfigType %d","NetworkData":"NetworkData %d","NetworkDataBase64Encoded":true}`
	...
	expectedOp := map[string]interface{}{"user_data": "UserData 1", "user_data_base64_encoded": true, "class_id": "virtualization.CloudInitConfig", "object_type": "virtualization.CloudInitConfig", "config_type": "ConfigType 1", "network_data": "NetworkData 1", "network_data_base64_encoded": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCpuAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.CpuAllocation","ObjectType":"virtualization.CpuAllocation","Total":32,"Used":32,"Free":32,"Reserved":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.CpuAllocation", "object_type": "virtualization.CpuAllocation", "total": 32, "used": 32, "free": 32, "reserved": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationCpuInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.CpuInfo","ObjectType":"virtualization.CpuInfo","Cores":32,"Description":"Description %d","Sockets":32,"Speed":32,"Vendor":"Vendor %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.CpuInfo", "object_type": "virtualization.CpuInfo", "cores": 32, "description": "Description 1", "sockets": 32, "speed": 32, "vendor": "Vendor 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationDiskStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.DiskStatus","Reason":"Reason %d","State":"State %d","VolumeName":"VolumeName %d","DownloadPercentage":"DownloadPercentage %d","ObjectType":"virtualization.DiskStatus","VolumeHandle":"VolumeHandle %d","VolumeVendor":"VolumeVendor %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.DiskStatus", "reason": "Reason 1", "state": "State 1", "volume_name": "VolumeName 1", "download_percentage": "DownloadPercentage 1", "object_type": "virtualization.DiskStatus", "volume_handle": "VolumeHandle 1", "volume_vendor": "VolumeVendor 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationGuestInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IpAddress":"IpAddress %d","Name":"Name %d","OperatingSystem":"OperatingSystem %d","ObjectType":"virtualization.GuestInfo","ClassId":"virtualization.GuestInfo","Hostname":"Hostname %d"}`
	...
	expectedOp := map[string]interface{}{"ip_address": "IpAddress 1", "name": "Name 1", "operating_system": "OperatingSystem 1", "object_type": "virtualization.GuestInfo", "class_id": "virtualization.GuestInfo", "hostname": "Hostname 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationMemoryCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Capacity":32,"Free":32,"Used":32,"ClassId":"virtualization.MemoryCapacity","ObjectType":"virtualization.MemoryCapacity"}`
	...
	expectedOp := map[string]interface{}{"capacity": 32, "free": 32, "used": 32, "class_id": "virtualization.MemoryCapacity", "object_type": "virtualization.MemoryCapacity"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationProductInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":"Version %d","ClassId":"virtualization.ProductInfo","ObjectType":"virtualization.ProductInfo","Build":"Build %d","ProductName":"ProductName %d","ProductType":"ProductType %d","ProductVendor":"ProductVendor %d"}`
	...
	expectedOp := map[string]interface{}{"nr_version": "Version 1", "class_id": "virtualization.ProductInfo", "object_type": "virtualization.ProductInfo", "build": "Build 1", "product_name": "ProductName 1", "product_type": "ProductType 1", "product_vendor": "ProductVendor 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationStorageCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.StorageCapacity","ClassId":"virtualization.StorageCapacity","Capacity":32,"Free":32,"Used":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.StorageCapacity", "class_id": "virtualization.StorageCapacity", "capacity": 32, "free": 32, "used": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareRemoteDisplayInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareRemoteDisplayInfo","ObjectType":"virtualization.VmwareRemoteDisplayInfo","RemoteDisplayPassword":"RemoteDisplayPassword %d","RemoteDisplayVncKey":"RemoteDisplayVncKey %d","RemoteDisplayVncPort":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.VmwareRemoteDisplayInfo", "object_type": "virtualization.VmwareRemoteDisplayInfo", "remote_display_password": "RemoteDisplayPassword 1", "remote_display_vnc_key": "RemoteDisplayVncKey 1", "remote_display_vnc_port": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationVmwareResourceConsumption(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareResourceConsumption","ObjectType":"virtualization.VmwareResourceConsumption","MemoryConsumed":32,"CpuConsumed":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.VmwareResourceConsumption", "object_type": "virtualization.VmwareResourceConsumption", "memory_consumed": 32, "cpu_consumed": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationVmwareSharesInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareSharesInfo","ObjectType":"virtualization.VmwareSharesInfo","Level":"Level %d","Shares":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.VmwareSharesInfo", "object_type": "virtualization.VmwareSharesInfo", "level": "Level 1", "shares": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareVmCpuShareInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareVmCpuShareInfo","ObjectType":"virtualization.VmwareVmCpuShareInfo","CpuLimit":32,"CpuOverheadLimit":32,"CpuReservation":32,"CpuShares":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.VmwareVmCpuShareInfo", "object_type": "virtualization.VmwareVmCpuShareInfo", "cpu_limit": 32, "cpu_overhead_limit": 32, "cpu_reservation": 32, "cpu_shares": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareVmDiskCommitInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UnCommittedDisk":32,"UnsharedDisk":32,"ClassId":"virtualization.VmwareVmDiskCommitInfo","ObjectType":"virtualization.VmwareVmDiskCommitInfo","CommittedDisk":32}`
	...
	expectedOp := map[string]interface{}{"un_committed_disk": 32, "unshared_disk": 32, "class_id": "virtualization.VmwareVmDiskCommitInfo", "object_type": "virtualization.VmwareVmDiskCommitInfo", "committed_disk": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationVmwareVmMemoryShareInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MemReservation":32,"MemShares":32,"ClassId":"virtualization.VmwareVmMemoryShareInfo","ObjectType":"virtualization.VmwareVmMemoryShareInfo","MemLimit":32,"MemOverheadLimit":32}`
	...
	expectedOp := map[string]interface{}{"mem_reservation": 32, "mem_shares": 32, "class_id": "virtualization.VmwareVmMemoryShareInfo", "object_type": "virtualization.VmwareVmMemoryShareInfo", "mem_limit": 32, "mem_overhead_limit": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicEthRxQueueSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RingSize":32,"Count":32,"ClassId":"vnic.EthRxQueueSettings","ObjectType":"vnic.EthRxQueueSettings"}`
	...
	expectedOp := map[string]interface{}{"ring_size": 32, "nr_count": 32, "class_id": "vnic.EthRxQueueSettings", "object_type": "vnic.EthRxQueueSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcErrorRecoverySettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.FcErrorRecoverySettings","Enabled":true,"IoRetryCount":32,"IoRetryTimeout":32,"LinkDownTimeout":32,"PortDownTimeout":32,"ClassId":"vnic.FcErrorRecoverySettings"}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.FcErrorRecoverySettings", "enabled": true, "io_retry_count": 32, "io_retry_timeout": 32, "link_down_timeout": 32, "port_down_timeout": 32, "class_id": "vnic.FcErrorRecoverySettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcQueueSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Count":32,"RingSize":32,"ClassId":"vnic.FcQueueSettings","ObjectType":"vnic.FcQueueSettings"}`
	...
	expectedOp := map[string]interface{}{"nr_count": 32, "ring_size": 32, "class_id": "vnic.FcQueueSettings", "object_type": "vnic.FcQueueSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicFlogiSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Timeout":32,"ObjectType":"vnic.FlogiSettings","ClassId":"vnic.FlogiSettings","Retries":32}`
	...
	expectedOp := map[string]interface{}{"timeout": 32, "object_type": "vnic.FlogiSettings", "class_id": "vnic.FlogiSettings", "retries": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicNvgreSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.NvgreSettings","Enabled":true,"ObjectType":"vnic.NvgreSettings"}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.NvgreSettings", "enabled": true, "object_type": "vnic.NvgreSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicPlacementSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.PlacementSettings","Uplink":32,"Id":"Id %d","PciLink":32,"SwitchId":"SwitchId %d","ObjectType":"vnic.PlacementSettings"}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.PlacementSettings", "uplink": 32, "id": "Id 1", "pci_link": 32, "switch_id": "SwitchId 1", "object_type": "vnic.PlacementSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicPtpSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.PtpSettings","ObjectType":"vnic.PtpSettings","Enabled":true}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.PtpSettings", "object_type": "vnic.PtpSettings", "enabled": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicRoceSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.RoceSettings","Enabled":true,"MemoryRegions":32,"QueuePairs":32,"ObjectType":"vnic.RoceSettings","ResourceGroups":32,"Version":32,"ClassOfService":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.RoceSettings", "enabled": true, "memory_regions": 32, "queue_pairs": 32, "object_type": "vnic.RoceSettings", "resource_groups": 32, "nr_version": 32, "class_of_service": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicRssHashSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Ipv6Hash":true,"Ipv6ExtHash":true,"Ipv4Hash":true,"TcpIpv6Hash":true,"ClassId":"vnic.RssHashSettings","ObjectType":"vnic.RssHashSettings","UdpIpv4Hash":true,"TcpIpv4Hash":true,"UdpIpv6Hash":true,"TcpIpv6ExtHash":true}`
	...
	expectedOp := map[string]interface{}{"ipv6_hash": true, "ipv6_ext_hash": true, "ipv4_hash": true, "tcp_ipv6_hash": true, "class_id": "vnic.RssHashSettings", "object_type": "vnic.RssHashSettings", "udp_ipv4_hash": true, "tcp_ipv4_hash": true, "udp_ipv6_hash": true, "tcp_ipv6_ext_hash": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicScsiQueueSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RingSize":32,"ClassId":"vnic.ScsiQueueSettings","ObjectType":"vnic.ScsiQueueSettings","Count":32}`
	...
	expectedOp := map[string]interface{}{"ring_size": 32, "class_id": "vnic.ScsiQueueSettings", "object_type": "vnic.ScsiQueueSettings", "nr_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicTcpOffloadSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"TxChecksum":true,"ClassId":"vnic.TcpOffloadSettings","ObjectType":"vnic.TcpOffloadSettings","LargeReceive":true,"LargeSend":true,"RxChecksum":true}`
	...
	expectedOp := map[string]interface{}{"tx_checksum": true, "class_id": "vnic.TcpOffloadSettings", "object_type": "vnic.TcpOffloadSettings", "large_receive": true, "large_send": true, "rx_checksum": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicUsnicSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Cos":32,"Count":32,"UsnicAdapterPolicy":"UsnicAdapterPolicy %d","ClassId":"vnic.UsnicSettings","ObjectType":"vnic.UsnicSettings"}`
	...
	expectedOp := map[string]interface{}{"cos": 32, "nr_count": 32, "usnic_adapter_policy": "UsnicAdapterPolicy 1", "class_id": "vnic.UsnicSettings", "object_type": "vnic.UsnicSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicVmqSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.VmqSettings","Enabled":true,"NumVmqs":32,"NumInterrupts":32,"NumSubVnics":32,"VmmqAdapterPolicy":"VmmqAdapterPolicy %d","ObjectType":"vnic.VmqSettings","MultiQueueSupport":true}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.VmqSettings", "enabled": true, "num_vmqs": 32, "num_interrupts": 32, "num_sub_vnics": 32, "vmmq_adapter_policy": "VmmqAdapterPolicy 1", "object_type": "vnic.VmqSettings", "multi_queue_support": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Timeout":32,"ClassId":"workflow.Properties","StartsWorkflow":true,"TimeoutPolicy":"TimeoutPolicy %d","Cloneable":true,"RetryCount":32,"ExternalMeta":true,"ObjectType":"workflow.Properties","SupportStatus":"SupportStatus %d","RetryDelay":32,"RetryPolicy":"RetryPolicy %d"}`
	...
	expectedOp := map[string]interface{}{"timeout": 32, "class_id": "workflow.Properties", "starts_workflow": true, "timeout_policy": "TimeoutPolicy 1", "cloneable": true, "retry_count": 32, "external_meta": true, "object_type": "workflow.Properties", "support_status": "SupportStatus 1", "retry_delay": 32, "retry_policy": "RetryPolicy 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowTaskLoopInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.TaskLoopInfo","Iteration":32,"LoopTaskLabel":"LoopTaskLabel %d","LoopTaskName":"LoopTaskName %d","LoopType":"LoopType %d","ObjectType":"workflow.TaskLoopInfo"}`
	...
	expectedOp := map[string]interface{}{"class_id": "workflow.TaskLoopInfo", "iteration": 32, "loop_task_label": "LoopTaskLabel 1", "loop_task_name": "LoopTaskName 1", "loop_type": "LoopType 1", "object_type": "workflow.TaskLoopInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowCtx(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"WorkflowType":"WorkflowType %d","WorkflowMetaName":"WorkflowMetaName %d","ClassId":"workflow.WorkflowCtx","ObjectType":"workflow.WorkflowCtx","WorkflowSubtype":"WorkflowSubtype %d"}`
	...
	expectedOp := map[string]interface{}{"workflow_type": "WorkflowType 1", "workflow_meta_name": "WorkflowMetaName 1", "class_id": "workflow.WorkflowCtx", "object_type": "workflow.WorkflowCtx", "workflow_subtype": "WorkflowSubtype 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowInfoProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"workflow.WorkflowInfoProperties","ClassId":"workflow.WorkflowInfoProperties","RollbackAction":"RollbackAction %d","RollbackOnCancel":true,"RollbackOnFailure":true,"Retryable":true}`
	...
	expectedOp := map[string]interface{}{"object_type": "workflow.WorkflowInfoProperties", "class_id": "workflow.WorkflowInfoProperties", "rollback_action": "RollbackAction 1", "rollback_on_cancel": true, "rollback_on_failure": true, "retryable": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.WorkflowProperties","ObjectType":"workflow.WorkflowProperties","Cloneable":true,"RollbackOnCancel":true,"RollbackOnFailure":true,"EnableDebug":true,"ExternalMeta":true,"Retryable":true,"SupportStatus":"SupportStatus %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "workflow.WorkflowProperties", "object_type": "workflow.WorkflowProperties", "cloneable": true, "rollback_on_cancel": true, "rollback_on_failure": true, "enable_debug": true, "external_meta": true, "retryable": true, "support_status": "SupportStatus 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapX509Certificate(t *testing.T) {
	p := models.X509Certificate{}
	var d = &schema.ResourceData{}
	c := `{"Sha256Fingerprint":"Sha256Fingerprint %d","ClassId":"x509.Certificate","ObjectType":"x509.Certificate","SignatureAlgorithm":"SignatureAlgorithm %d","PemCertificate":"PemCertificate %d"}`
	...
	expectedOp := map[string]interface{}{"sha256_fingerprint": "Sha256Fingerprint 1", "class_id": "x509.Certificate", "object_type": "x509.Certificate", "signature_algorithm": "SignatureAlgorithm 1", "pem_certificate": "PemCertificate 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
