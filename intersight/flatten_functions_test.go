...
func TestFlattenListApplianceKeyValuePair(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Key":"Key %d","ClassId":"appliance.KeyValuePair","ObjectType":"appliance.KeyValuePair","Value":"Value %d"}`
	...
	expectedOp := []map[string]interface{}{{"key": "Key 1", "class_id": "appliance.KeyValuePair", "object_type": "appliance.KeyValuePair", "value": "Value 1"}, {"key": "Key 2", "class_id": "appliance.KeyValuePair", "object_type": "appliance.KeyValuePair", "value": "Value 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCapabilityPortRange(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"EndSlotId":32,"StartPortId":32,"StartSlotId":32,"ObjectType":"capability.PortRange","ClassId":"capability.PortRange","EndPortId":32}`
	...
	expectedOp := []map[string]interface{}{{"end_slot_id": 32, "start_port_id": 32, "start_slot_id": 32, "object_type": "capability.PortRange", "class_id": "capability.PortRange", "end_port_id": 32}, {"end_slot_id": 32, "start_port_id": 32, "start_slot_id": 32, "object_type": "capability.PortRange", "class_id": "capability.PortRange", "end_port_id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudNetworkInterfaceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NetworkId":"NetworkId %d","ObjectType":"cloud.NetworkInterfaceAttachment","ClassId":"cloud.NetworkInterfaceAttachment","Identity":"Identity %d","NetworkName":"NetworkName %d","MacAddress":"MacAddress %d","IpForwardingEnabled":true,"SubNetworkName":"SubNetworkName %d","SubNetworkId":"SubNetworkId %d","NicIndex":32}`
	...
	expectedOp := []map[string]interface{}{{"network_id": "NetworkId 1", "object_type": "cloud.NetworkInterfaceAttachment", "class_id": "cloud.NetworkInterfaceAttachment", "identity": "Identity 1", "network_name": "NetworkName 1", "mac_address": "MacAddress 1", "ip_forwarding_enabled": true, "sub_network_name": "SubNetworkName 1", "sub_network_id": "SubNetworkId 1", "nic_index": 32}, {"network_id": "NetworkId 2", "object_type": "cloud.NetworkInterfaceAttachment", "class_id": "cloud.NetworkInterfaceAttachment", "identity": "Identity 2", "network_name": "NetworkName 2", "mac_address": "MacAddress 2", "ip_forwarding_enabled": true, "sub_network_name": "SubNetworkName 2", "sub_network_id": "SubNetworkId 2", "nic_index": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudSecurityGroupRule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Protocol":"Protocol %d","Name":"Name %d","Index":32,"ClassId":"cloud.SecurityGroupRule","SourceSecurityGroup":"SourceSecurityGroup %d","StartPort":32,"ObjectType":"cloud.SecurityGroupRule","Identity":"Identity %d","EtherType":"EtherType %d","EndPort":32,"Action":"Action %d","Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"protocol": "Protocol 1", "name": "Name 1", "index": 32, "class_id": "cloud.SecurityGroupRule", "source_security_group": "SourceSecurityGroup 1", "start_port": 32, "object_type": "cloud.SecurityGroupRule", "identity": "Identity 1", "ether_type": "EtherType 1", "end_port": 32, "action": "Action 1", "description": "Description 1"}, {"protocol": "Protocol 2", "name": "Name 2", "index": 32, "class_id": "cloud.SecurityGroupRule", "source_security_group": "SourceSecurityGroup 2", "start_port": 32, "object_type": "cloud.SecurityGroupRule", "identity": "Identity 2", "ether_type": "EtherType 2", "end_port": 32, "action": "Action 2", "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudTfcWorkspaceVariables(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Key":"Key %d","Sensitive":true,"Identity":"Identity %d","Value":"Value %d","Category":"Category %d","Hcl":true,"ObjectType":"cloud.TfcWorkspaceVariables","ClassId":"cloud.TfcWorkspaceVariables","Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"key": "Key 1", "sensitive": true, "identity": "Identity 1", "value": "Value 1", "category": "Category 1", "hcl": true, "object_type": "cloud.TfcWorkspaceVariables", "class_id": "cloud.TfcWorkspaceVariables", "description": "Description 1"}, {"key": "Key 2", "sensitive": true, "identity": "Identity 2", "value": "Value 2", "category": "Category 2", "hcl": true, "object_type": "cloud.TfcWorkspaceVariables", "class_id": "cloud.TfcWorkspaceVariables", "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.VolumeAttachment","DeviceName":"DeviceName %d","Identity":"Identity %d","Index":32,"ClassId":"cloud.VolumeAttachment","AutoDelete":true,"IsRoot":true}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "cloud.VolumeAttachment", "device_name": "DeviceName 1", "identity": "Identity 1", "index": 32, "class_id": "cloud.VolumeAttachment", "auto_delete": true, "is_root": true}, {"object_type": "cloud.VolumeAttachment", "device_name": "DeviceName 2", "identity": "Identity 2", "index": 32, "class_id": "cloud.VolumeAttachment", "auto_delete": true, "is_root": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeInstanceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AutoDelete":true,"DeviceName":"DeviceName %d","ClassId":"cloud.VolumeInstanceAttachment","ObjectType":"cloud.VolumeInstanceAttachment","InstanceId":"InstanceId %d","State":"State %d"}`
	...
	expectedOp := []map[string]interface{}{{"auto_delete": true, "device_name": "DeviceName 1", "class_id": "cloud.VolumeInstanceAttachment", "object_type": "cloud.VolumeInstanceAttachment", "instance_id": "InstanceId 1", "state": "State 1"}, {"auto_delete": true, "device_name": "DeviceName 2", "class_id": "cloud.VolumeInstanceAttachment", "object_type": "cloud.VolumeInstanceAttachment", "instance_id": "InstanceId 2", "state": "State 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeIpAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Dn":"Dn %d","Subnet":"Subnet %d","Type":"Type %d","Category":"Category %d","HttpPort":32,"KvmPort":32,"Address":"Address %d","KvmVlan":32,"DefaultGateway":"DefaultGateway %d","Name":"Name %d","HttpsPort":32,"ObjectType":"compute.IpAddress","ClassId":"compute.IpAddress"}`
	...
	expectedOp := []map[string]interface{}{{"dn": "Dn 1", "subnet": "Subnet 1", "type": "Type 1", "category": "Category 1", "http_port": 32, "kvm_port": 32, "address": "Address 1", "kvm_vlan": 32, "default_gateway": "DefaultGateway 1", "name": "Name 1", "https_port": 32, "object_type": "compute.IpAddress", "class_id": "compute.IpAddress"}, {"dn": "Dn 2", "subnet": "Subnet 2", "type": "Type 2", "category": "Category 2", "http_port": 32, "kvm_port": 32, "address": "Address 2", "kvm_vlan": 32, "default_gateway": "DefaultGateway 2", "name": "Name 2", "https_port": 32, "object_type": "compute.IpAddress", "class_id": "compute.IpAddress"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListContentParameter(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AcceptSingleValue":true,"ClassId":"content.Parameter","ComplexType":"ComplexType %d","Secure":true,"ObjectType":"content.Parameter","Path":"Path %d","ItemType":"ItemType %d","Type":"Type %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"accept_single_value": true, "class_id": "content.Parameter", "complex_type": "ComplexType 1", "secure": true, "object_type": "content.Parameter", "path": "Path 1", "item_type": "ItemType 1", "type": "Type 1", "name": "Name 1"}, {"accept_single_value": true, "class_id": "content.Parameter", "complex_type": "ComplexType 2", "secure": true, "object_type": "content.Parameter", "path": "Path 2", "item_type": "ItemType 2", "type": "Type 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricQosClass(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"BandwidthPercent":32,"ClassId":"fabric.QosClass","ObjectType":"fabric.QosClass","Weight":32,"Cos":32,"Name":"Name %d","AdminState":"AdminState %d","Mtu":32,"PacketDrop":true,"MulticastOptimize":true}`
	...
	expectedOp := []map[string]interface{}{{"bandwidth_percent": 32, "class_id": "fabric.QosClass", "object_type": "fabric.QosClass", "weight": 32, "cos": 32, "name": "Name 1", "admin_state": "AdminState 1", "mtu": 32, "packet_drop": true, "multicast_optimize": true}, {"bandwidth_percent": 32, "class_id": "fabric.QosClass", "object_type": "fabric.QosClass", "weight": 32, "cos": 32, "name": "Name 2", "admin_state": "AdminState 2", "mtu": 32, "packet_drop": true, "multicast_optimize": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareBaseImpact(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"FirmwareVersion":"FirmwareVersion %d","DomainName":"DomainName %d","VersionImpact":"VersionImpact %d","Model":"Model %d","TargetFirmwareVersion":"TargetFirmwareVersion %d","ObjectType":"firmware.BaseImpact","ComputationStatusDetail":"ComputationStatusDetail %d","EndPoint":"EndPoint %d","ImpactType":"ImpactType %d","ComputationError":"ComputationError %d","ClassId":"firmware.BaseImpact"}`
	...
	expectedOp := []map[string]interface{}{{"firmware_version": "FirmwareVersion 1", "domain_name": "DomainName 1", "version_impact": "VersionImpact 1", "model": "Model 1", "target_firmware_version": "TargetFirmwareVersion 1", "object_type": "firmware.BaseImpact", "computation_status_detail": "ComputationStatusDetail 1", "end_point": "EndPoint 1", "impact_type": "ImpactType 1", "computation_error": "ComputationError 1", "class_id": "firmware.BaseImpact"}, {"firmware_version": "FirmwareVersion 2", "domain_name": "DomainName 2", "version_impact": "VersionImpact 2", "model": "Model 2", "target_firmware_version": "TargetFirmwareVersion 2", "object_type": "firmware.BaseImpact", "computation_status_detail": "ComputationStatusDetail 2", "end_point": "EndPoint 2", "impact_type": "ImpactType 2", "computation_error": "ComputationError 2", "class_id": "firmware.BaseImpact"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareComponentMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Disruption":"Disruption %d","Description":"Description %d","ImagePath":"ImagePath %d","RedfishUrl":"RedfishUrl %d","Vendor":"Vendor %d","Model":"Model %d","PackedVersion":"PackedVersion %d","ComponentLabel":"ComponentLabel %d","IsOobSupported":true,"ComponentType":"ComponentType %d","ObjectType":"firmware.ComponentMeta","ClassId":"firmware.ComponentMeta"}`
	...
	expectedOp := []map[string]interface{}{{"disruption": "Disruption 1", "description": "Description 1", "image_path": "ImagePath 1", "redfish_url": "RedfishUrl 1", "vendor": "Vendor 1", "model": "Model 1", "packed_version": "PackedVersion 1", "component_label": "ComponentLabel 1", "is_oob_supported": true, "component_type": "ComponentType 1", "object_type": "firmware.ComponentMeta", "class_id": "firmware.ComponentMeta"}, {"disruption": "Disruption 2", "description": "Description 2", "image_path": "ImagePath 2", "redfish_url": "RedfishUrl 2", "vendor": "Vendor 2", "model": "Model 2", "packed_version": "PackedVersion 2", "component_label": "ComponentLabel 2", "is_oob_supported": true, "component_type": "ComponentType 2", "object_type": "firmware.ComponentMeta", "class_id": "firmware.ComponentMeta"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareFirmwareInventory(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Category":"Category %d","Label":"Label %d","Vendor":"Vendor %d","ClassId":"firmware.FirmwareInventory","Version":"Version %d","Model":"Model %d","UpdateUri":"UpdateUri %d","ObjectType":"firmware.FirmwareInventory"}`
	...
	expectedOp := []map[string]interface{}{{"category": "Category 1", "label": "Label 1", "vendor": "Vendor 1", "class_id": "firmware.FirmwareInventory", "nr_version": "Version 1", "model": "Model 1", "update_uri": "UpdateUri 1", "object_type": "firmware.FirmwareInventory"}, {"category": "Category 2", "label": "Label 2", "vendor": "Vendor 2", "class_id": "firmware.FirmwareInventory", "nr_version": "Version 2", "model": "Model 2", "update_uri": "UpdateUri 2", "object_type": "firmware.FirmwareInventory"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexFeatureLimitEntry(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","Value":"Value %d","ObjectType":"hyperflex.FeatureLimitEntry","ClassId":"hyperflex.FeatureLimitEntry"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "value": "Value 1", "object_type": "hyperflex.FeatureLimitEntry", "class_id": "hyperflex.FeatureLimitEntry"}, {"name": "Name 2", "value": "Value 2", "object_type": "hyperflex.FeatureLimitEntry", "class_id": "hyperflex.FeatureLimitEntry"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHealthCheckScriptInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AggregateScriptName":"AggregateScriptName %d","ClassId":"hyperflex.HealthCheckScriptInfo","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptName":"ScriptName %d","ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptInput":"ScriptInput %d","Version":"Version %d","HyperflexVersion":"HyperflexVersion %d"}`
	...
	expectedOp := []map[string]interface{}{{"aggregate_script_name": "AggregateScriptName 1", "class_id": "hyperflex.HealthCheckScriptInfo", "script_execute_location": "ScriptExecuteLocation 1", "script_name": "ScriptName 1", "object_type": "hyperflex.HealthCheckScriptInfo", "script_input": "ScriptInput 1", "nr_version": "Version 1", "hyperflex_version": "HyperflexVersion 1"}, {"aggregate_script_name": "AggregateScriptName 2", "class_id": "hyperflex.HealthCheckScriptInfo", "script_execute_location": "ScriptExecuteLocation 2", "script_name": "ScriptName 2", "object_type": "hyperflex.HealthCheckScriptInfo", "script_input": "ScriptInput 2", "nr_version": "Version 2", "hyperflex_version": "HyperflexVersion 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHxHostMountStatusDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HxHostMountStatusDt","ClassId":"hyperflex.HxHostMountStatusDt","Mounted":true,"Reason":"Reason %d","Accessibility":"Accessibility %d","HostName":"HostName %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "hyperflex.HxHostMountStatusDt", "class_id": "hyperflex.HxHostMountStatusDt", "mounted": true, "reason": "Reason 1", "accessibility": "Accessibility 1", "host_name": "HostName 1"}, {"object_type": "hyperflex.HxHostMountStatusDt", "class_id": "hyperflex.HxHostMountStatusDt", "mounted": true, "reason": "Reason 2", "accessibility": "Accessibility 2", "host_name": "HostName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexNamedVlan(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.NamedVlan","VlanId":32,"Name":"Name %d","ObjectType":"hyperflex.NamedVlan"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "hyperflex.NamedVlan", "vlan_id": 32, "name": "Name 1", "object_type": "hyperflex.NamedVlan"}, {"class_id": "hyperflex.NamedVlan", "vlan_id": 32, "name": "Name 2", "object_type": "hyperflex.NamedVlan"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexNetworkPort(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","PortType":"PortType %d","Vlans":"Vlans %d","ObjectType":"hyperflex.NetworkPort","ClassId":"hyperflex.NetworkPort"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "port_type": "PortType 1", "vlans": "Vlans 1", "object_type": "hyperflex.NetworkPort", "class_id": "hyperflex.NetworkPort"}, {"name": "Name 2", "port_type": "PortType 2", "vlans": "Vlans 2", "object_type": "hyperflex.NetworkPort", "class_id": "hyperflex.NetworkPort"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexVmDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.VmDisk","ClassId":"hyperflex.VmDisk","BootOrder":32,"VirtualDiskReference":"VirtualDiskReference %d","Bus":"Bus %d","Name":"Name %d","Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "hyperflex.VmDisk", "class_id": "hyperflex.VmDisk", "boot_order": 32, "virtual_disk_reference": "VirtualDiskReference 1", "bus": "Bus 1", "name": "Name 1", "type": "Type 1"}, {"object_type": "hyperflex.VmDisk", "class_id": "hyperflex.VmDisk", "boot_order": 32, "virtual_disk_reference": "VirtualDiskReference 2", "bus": "Bus 2", "name": "Name 2", "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseKeysInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LicenseId":"LicenseId %d","ClassId":"iaas.LicenseKeysInfo","ObjectType":"iaas.LicenseKeysInfo","Pid":"Pid %d","Count":32,"ExpirationDate":"ExpirationDate %d"}`
...
func TestFlattenListIaasWorkflowSteps(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"iaas.WorkflowSteps","Status":"Status %d","StatusMessage":"StatusMessage %d","CompletedTime":"CompletedTime %d","Name":"Name %d","ObjectType":"iaas.WorkflowSteps"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "iaas.WorkflowSteps", "status": "Status 1", "status_message": "StatusMessage 1", "completed_time": "CompletedTime 1", "name": "Name 1", "object_type": "iaas.WorkflowSteps"}, {"class_id": "iaas.WorkflowSteps", "status": "Status 2", "status_message": "StatusMessage 2", "completed_time": "CompletedTime 2", "name": "Name 2", "object_type": "iaas.WorkflowSteps"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesNodeStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Status":"Status %d","Type":"Type %d","ObjectType":"kubernetes.NodeStatus","ClassId":"kubernetes.NodeStatus"}`
	...
	expectedOp := []map[string]interface{}{{"status": "Status 1", "type": "Type 1", "object_type": "kubernetes.NodeStatus", "class_id": "kubernetes.NodeStatus"}, {"status": "Status 2", "type": "Type 2", "object_type": "kubernetes.NodeStatus", "class_id": "kubernetes.NodeStatus"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMemoryPersistentMemoryGoal(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"memory.PersistentMemoryGoal","MemoryModePercentage":32,"PersistentMemoryType":"PersistentMemoryType %d","SocketId":"SocketId %d","ObjectType":"memory.PersistentMemoryGoal"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "memory.PersistentMemoryGoal", "memory_mode_percentage": 32, "persistent_memory_type": "PersistentMemoryType 1", "socket_id": "SocketId 1", "object_type": "memory.PersistentMemoryGoal"}, {"class_id": "memory.PersistentMemoryGoal", "memory_mode_percentage": 32, "persistent_memory_type": "PersistentMemoryType 2", "socket_id": "SocketId 2", "object_type": "memory.PersistentMemoryGoal"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMemoryPersistentMemoryLogicalNamespace(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SocketMemoryId":"SocketMemoryId %d","Capacity":32,"Mode":"Mode %d","ObjectType":"memory.PersistentMemoryLogicalNamespace","ClassId":"memory.PersistentMemoryLogicalNamespace","Name":"Name %d","SocketId":32}`
	...
	expectedOp := []map[string]interface{}{{"socket_memory_id": "SocketMemoryId 1", "capacity": 32, "mode": "Mode 1", "object_type": "memory.PersistentMemoryLogicalNamespace", "class_id": "memory.PersistentMemoryLogicalNamespace", "name": "Name 1", "socket_id": 32}, {"socket_memory_id": "SocketMemoryId 2", "capacity": 32, "mode": "Mode 2", "object_type": "memory.PersistentMemoryLogicalNamespace", "class_id": "memory.PersistentMemoryLogicalNamespace", "name": "Name 2", "socket_id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaPropDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"meta.PropDefinition","ClassId":"meta.PropDefinition","ApiAccess":"ApiAccess %d","Name":"Name %d","OpSecurity":"OpSecurity %d","SearchWeight":32.000000}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "meta.PropDefinition", "class_id": "meta.PropDefinition", "api_access": "ApiAccess 1", "name": "Name 1", "op_security": "OpSecurity 1", "search_weight": 32.000000}, {"object_type": "meta.PropDefinition", "class_id": "meta.PropDefinition", "api_access": "ApiAccess 2", "name": "Name 2", "op_security": "OpSecurity 2", "search_weight": 32.000000}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaRelationshipDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Export":true,"ClassId":"meta.RelationshipDefinition","ObjectType":"meta.RelationshipDefinition","ExportWithPeer":true,"Name":"Name %d","PeerRelName":"PeerRelName %d","ApiAccess":"ApiAccess %d","PeerSync":true,"Collection":true,"Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"export": true, "class_id": "meta.RelationshipDefinition", "object_type": "meta.RelationshipDefinition", "export_with_peer": true, "name": "Name 1", "peer_rel_name": "PeerRelName 1", "api_access": "ApiAccess 1", "peer_sync": true, "collection": true, "type": "Type 1"}, {"export": true, "class_id": "meta.RelationshipDefinition", "object_type": "meta.RelationshipDefinition", "export_with_peer": true, "name": "Name 2", "peer_rel_name": "PeerRelName 2", "api_access": "ApiAccess 2", "peer_sync": true, "collection": true, "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryInterfaceElement(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"OperState":"OperState %d","ClassId":"niatelemetry.InterfaceElement","ObjectType":"niatelemetry.InterfaceElement","XcvrPresent":"XcvrPresent %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"oper_state": "OperState 1", "class_id": "niatelemetry.InterfaceElement", "object_type": "niatelemetry.InterfaceElement", "xcvr_present": "XcvrPresent 1", "name": "Name 1"}, {"oper_state": "OperState 2", "class_id": "niatelemetry.InterfaceElement", "object_type": "niatelemetry.InterfaceElement", "xcvr_present": "XcvrPresent 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryLogicalLink(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.LogicalLink","IsPresent":true,"Uptime":"Uptime %d","LinkState":"LinkState %d","DbId":32,"ObjectType":"niatelemetry.LogicalLink","LinkAddr1":"LinkAddr1 %d","LinkAddr2":"LinkAddr2 %d","LinkType":"LinkType %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "niatelemetry.LogicalLink", "is_present": true, "uptime": "Uptime 1", "link_state": "LinkState 1", "db_id": 32, "object_type": "niatelemetry.LogicalLink", "link_addr1": "LinkAddr1 1", "link_addr2": "LinkAddr2 1", "link_type": "LinkType 1"}, {"class_id": "niatelemetry.LogicalLink", "is_present": true, "uptime": "Uptime 2", "link_state": "LinkState 2", "db_id": 32, "object_type": "niatelemetry.LogicalLink", "link_addr1": "LinkAddr1 2", "link_addr2": "LinkAddr2 2", "link_type": "LinkType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOnpremImagePackage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Filename":"Filename %d","Name":"Name %d","ClassId":"onprem.ImagePackage","ObjectType":"onprem.ImagePackage","Version":"Version %d","FileSha":"FileSha %d","FilePath":"FilePath %d","FileSize":32,"PackageType":"PackageType %d"}`
	...
	expectedOp := []map[string]interface{}{{"filename": "Filename 1", "name": "Name 1", "class_id": "onprem.ImagePackage", "object_type": "onprem.ImagePackage", "nr_version": "Version 1", "file_sha": "FileSha 1", "file_path": "FilePath 1", "file_size": 32, "package_type": "PackageType 1"}, {"filename": "Filename 2", "name": "Name 2", "class_id": "onprem.ImagePackage", "object_type": "onprem.ImagePackage", "nr_version": "Version 2", "file_sha": "FileSha 2", "file_path": "FilePath 2", "file_size": 32, "package_type": "PackageType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOnpremUpgradePhase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ElapsedTime":32,"Failed":true,"Message":"Message %d","Name":"Name %d","ObjectType":"onprem.UpgradePhase","ClassId":"onprem.UpgradePhase"}`
	...
	expectedOp := []map[string]interface{}{{"elapsed_time": 32, "failed": true, "message": "Message 1", "name": "Name 1", "object_type": "onprem.UpgradePhase", "class_id": "onprem.UpgradePhase"}, {"elapsed_time": 32, "failed": true, "message": "Message 2", "name": "Name 2", "object_type": "onprem.UpgradePhase", "class_id": "onprem.UpgradePhase"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListPolicyinventoryJobInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"policyinventory.JobInfo","PolicyName":"PolicyName %d","ExecutionStatus":"ExecutionStatus %d","PolicyId":"PolicyId %d","ObjectType":"policyinventory.JobInfo"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "policyinventory.JobInfo", "policy_name": "PolicyName 1", "execution_status": "ExecutionStatus 1", "policy_id": "PolicyId 1", "object_type": "policyinventory.JobInfo"}, {"class_id": "policyinventory.JobInfo", "policy_name": "PolicyName 2", "execution_status": "ExecutionStatus 2", "policy_id": "PolicyId 2", "object_type": "policyinventory.JobInfo"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListResourcePerTypeCombinedSelector(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"resource.PerTypeCombinedSelector","SelectorObjectType":"SelectorObjectType %d","CombinedSelector":"CombinedSelector %d","EmptyFilter":true,"ObjectType":"resource.PerTypeCombinedSelector"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "resource.PerTypeCombinedSelector", "selector_object_type": "SelectorObjectType 1", "combined_selector": "CombinedSelector 1", "empty_filter": true, "object_type": "resource.PerTypeCombinedSelector"}, {"class_id": "resource.PerTypeCombinedSelector", "selector_object_type": "SelectorObjectType 2", "combined_selector": "CombinedSelector 2", "empty_filter": true, "object_type": "resource.PerTypeCombinedSelector"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSdwanNetworkConfigurationType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PortGroup":"PortGroup %d","Vlan":32,"ObjectType":"sdwan.NetworkConfigurationType","ClassId":"sdwan.NetworkConfigurationType","NetworkType":"NetworkType %d"}`
	...
	expectedOp := []map[string]interface{}{{"port_group": "PortGroup 1", "vlan": 32, "object_type": "sdwan.NetworkConfigurationType", "class_id": "sdwan.NetworkConfigurationType", "network_type": "NetworkType 1"}, {"port_group": "PortGroup 2", "vlan": 32, "object_type": "sdwan.NetworkConfigurationType", "class_id": "sdwan.NetworkConfigurationType", "network_type": "NetworkType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSdwanTemplateInputsType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"sdwan.TemplateInputsType","Value":"Value %d","Required":true,"Type":"Type %d","Editable":true,"ObjectType":"sdwan.TemplateInputsType","Key":"Key %d","Title":"Title %d","Template":"Template %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "sdwan.TemplateInputsType", "value": "Value 1", "required": true, "type": "Type 1", "editable": true, "object_type": "sdwan.TemplateInputsType", "key": "Key 1", "title": "Title 1", "template": "Template 1"}, {"class_id": "sdwan.TemplateInputsType", "value": "Value 2", "required": true, "type": "Type 2", "editable": true, "object_type": "sdwan.TemplateInputsType", "key": "Key 2", "title": "Title 2", "template": "Template 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpTrap(t *testing.T) {
	p := []models.SnmpTrap{}
	var d = &schema.ResourceData{}
	c := `{"Community":"Community %d","Port":32,"Version":"Version %d","User":"User %d","ObjectType":"snmp.Trap","ClassId":"snmp.Trap","Destination":"Destination %d","Type":"Type %d","Enabled":true}`
	...
	expectedOp := []map[string]interface{}{{"community": "Community 1", "port": 32, "nr_version": "Version 1", "user": "User 1", "object_type": "snmp.Trap", "class_id": "snmp.Trap", "destination": "Destination 1", "type": "Type 1", "enabled": true}, {"community": "Community 2", "port": 32, "nr_version": "Version 2", "user": "User 2", "object_type": "snmp.Trap", "class_id": "snmp.Trap", "destination": "Destination 2", "type": "Type 2", "enabled": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpUser(t *testing.T) {
	p := []models.SnmpUser{}
	var d = &schema.ResourceData{}
	c := `{"PrivacyType":"PrivacyType %d","IsPrivacyPasswordSet":true,"Name":"Name %d","AuthType":"AuthType %d","ClassId":"snmp.User","ObjectType":"snmp.User","SecurityLevel":"SecurityLevel %d","IsAuthPasswordSet":true}`
	...
	expectedOp := []map[string]interface{}{{"privacy_type": "PrivacyType 1", "is_privacy_password_set": true, "name": "Name 1", "auth_type": "AuthType 1", "class_id": "snmp.User", "object_type": "snmp.User", "security_level": "SecurityLevel 1", "is_auth_password_set": true}, {"privacy_type": "PrivacyType 2", "is_privacy_password_set": true, "name": "Name 2", "auth_type": "AuthType 2", "class_id": "snmp.User", "object_type": "snmp.User", "security_level": "SecurityLevel 2", "is_auth_password_set": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSyslogRemoteClientBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"syslog.RemoteClientBase","ClassId":"syslog.RemoteClientBase","Enabled":true,"Hostname":"Hostname %d","MinSeverity":"MinSeverity %d","Port":32,"Protocol":"Protocol %d"}`
...
func TestFlattenListTamAction(t *testing.T) {
	p := []models.TamAction{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"tam.Action","OperationType":"OperationType %d","Type":"Type %d","ObjectType":"tam.Action","AlertType":"AlertType %d","AffectedObjectType":"AffectedObjectType %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "tam.Action", "operation_type": "OperationType 1", "type": "Type 1", "object_type": "tam.Action", "alert_type": "AlertType 1", "affected_object_type": "AffectedObjectType 1", "name": "Name 1"}, {"class_id": "tam.Action", "operation_type": "OperationType 2", "type": "Type 2", "object_type": "tam.Action", "alert_type": "AlertType 2", "affected_object_type": "AffectedObjectType 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTamS3DataSource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"tam.S3DataSource","Name":"Name %d","Type":"Type %d","ClassId":"tam.S3DataSource","S3Path":"S3Path %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "tam.S3DataSource", "name": "Name 1", "type": "Type 1", "class_id": "tam.S3DataSource", "s3_path": "S3Path 1"}, {"object_type": "tam.S3DataSource", "name": "Name 2", "type": "Type 2", "class_id": "tam.S3DataSource", "s3_path": "S3Path 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUcsdConnectorPack(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"ucsd.ConnectorPack","Name":"Name %d","ConnectorFeature":"ConnectorFeature %d","State":"State %d","ObjectType":"ucsd.ConnectorPack","Version":"Version %d","DownloadedVersion":"DownloadedVersion %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "ucsd.ConnectorPack", "name": "Name 1", "connector_feature": "ConnectorFeature 1", "state": "State 1", "object_type": "ucsd.ConnectorPack", "nr_version": "Version 1", "downloaded_version": "DownloadedVersion 1"}, {"class_id": "ucsd.ConnectorPack", "name": "Name 2", "connector_feature": "ConnectorFeature 2", "state": "State 2", "object_type": "ucsd.ConnectorPack", "nr_version": "Version 2", "downloaded_version": "DownloadedVersion 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationNetworkInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ConnectAtPowerOn":true,"MacAddress":"MacAddress %d","Name":"Name %d","ObjectType":"virtualization.NetworkInterface","ClassId":"virtualization.NetworkInterface","DirectPathIo":true,"AdaptorType":"AdaptorType %d","Bridge":"Bridge %d"}`
	...
	expectedOp := []map[string]interface{}{{"connect_at_power_on": true, "mac_address": "MacAddress 1", "name": "Name 1", "object_type": "virtualization.NetworkInterface", "class_id": "virtualization.NetworkInterface", "direct_path_io": true, "adaptor_type": "AdaptorType 1", "bridge": "Bridge 1"}, {"connect_at_power_on": true, "mac_address": "MacAddress 2", "name": "Name 2", "object_type": "virtualization.NetworkInterface", "class_id": "virtualization.NetworkInterface", "direct_path_io": true, "adaptor_type": "AdaptorType 2", "bridge": "Bridge 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVirtualMachineDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Bus":"Bus %d","Name":"Name %d","Order":32,"Type":"Type %d","VirtualDiskReference":"VirtualDiskReference %d","ObjectType":"virtualization.VirtualMachineDisk","ClassId":"virtualization.VirtualMachineDisk"}`
	...
	expectedOp := []map[string]interface{}{{"bus": "Bus 1", "name": "Name 1", "order": 32, "type": "Type 1", "virtual_disk_reference": "VirtualDiskReference 1", "object_type": "virtualization.VirtualMachineDisk", "class_id": "virtualization.VirtualMachineDisk"}, {"bus": "Bus 2", "name": "Name 2", "order": 32, "type": "Type 2", "virtual_disk_reference": "VirtualDiskReference 2", "object_type": "virtualization.VirtualMachineDisk", "class_id": "virtualization.VirtualMachineDisk"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVmediaMapping(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IsPasswordSet":true,"MountOptions":"MountOptions %d","VolumeName":"VolumeName %d","RemoteFile":"RemoteFile %d","HostName":"HostName %d","Username":"Username %d","FileLocation":"FileLocation %d","SanitizedFileLocation":"SanitizedFileLocation %d","ObjectType":"vmedia.Mapping","AuthenticationProtocol":"AuthenticationProtocol %d","DeviceType":"DeviceType %d","ClassId":"vmedia.Mapping","RemotePath":"RemotePath %d","MountProtocol":"MountProtocol %d"}`
	...
	expectedOp := []map[string]interface{}{{"is_password_set": true, "mount_options": "MountOptions 1", "volume_name": "VolumeName 1", "remote_file": "RemoteFile 1", "host_name": "HostName 1", "username": "Username 1", "file_location": "FileLocation 1", "sanitized_file_location": "SanitizedFileLocation 1", "object_type": "vmedia.Mapping", "authentication_protocol": "AuthenticationProtocol 1", "device_type": "DeviceType 1", "class_id": "vmedia.Mapping", "remote_path": "RemotePath 1", "mount_protocol": "MountProtocol 1"}, {"is_password_set": true, "mount_options": "MountOptions 2", "volume_name": "VolumeName 2", "remote_file": "RemoteFile 2", "host_name": "HostName 2", "username": "Username 2", "file_location": "FileLocation 2", "sanitized_file_location": "SanitizedFileLocation 2", "object_type": "vmedia.Mapping", "authentication_protocol": "AuthenticationProtocol 2", "device_type": "DeviceType 2", "class_id": "vmedia.Mapping", "remote_path": "RemotePath 2", "mount_protocol": "MountProtocol 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowApi(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AssetTargetMoid":"AssetTargetMoid %d","Label":"Label %d","ErrorContentType":"ErrorContentType %d","Name":"Name %d","ContentType":"ContentType %d","ObjectType":"workflow.Api","Timeout":32,"Body":"Body %d","ClassId":"workflow.Api","SkipOnCondition":"SkipOnCondition %d","StartDelay":32,"Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"asset_target_moid": "AssetTargetMoid 1", "label": "Label 1", "error_content_type": "ErrorContentType 1", "name": "Name 1", "content_type": "ContentType 1", "object_type": "workflow.Api", "timeout": 32, "body": "Body 1", "class_id": "workflow.Api", "skip_on_condition": "SkipOnCondition 1", "start_delay": 32, "description": "Description 1"}, {"asset_target_moid": "AssetTargetMoid 2", "label": "Label 2", "error_content_type": "ErrorContentType 2", "name": "Name 2", "content_type": "ContentType 2", "object_type": "workflow.Api", "timeout": 32, "body": "Body 2", "class_id": "workflow.Api", "skip_on_condition": "SkipOnCondition 2", "start_delay": 32, "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowBaseDataType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.BaseDataType","ObjectType":"workflow.BaseDataType","Label":"Label %d","Description":"Description %d","Required":true,"Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.BaseDataType", "object_type": "workflow.BaseDataType", "label": "Label 1", "description": "Description 1", "required": true, "name": "Name 1"}, {"class_id": "workflow.BaseDataType", "object_type": "workflow.BaseDataType", "label": "Label 2", "description": "Description 2", "required": true, "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowDynamicWorkflowActionTaskList(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.DynamicWorkflowActionTaskList","Action":"Action %d","ObjectType":"workflow.DynamicWorkflowActionTaskList"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.DynamicWorkflowActionTaskList", "action": "Action 1", "object_type": "workflow.DynamicWorkflowActionTaskList"}, {"class_id": "workflow.DynamicWorkflowActionTaskList", "action": "Action 2", "object_type": "workflow.DynamicWorkflowActionTaskList"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Description":"Description %d","ClassId":"workflow.RollbackTask","Name":"Name %d","TaskMoid":"TaskMoid %d","CatalogMoid":"CatalogMoid %d","ObjectType":"workflow.RollbackTask","Version":32}`
	...
	expectedOp := []map[string]interface{}{{"description": "Description 1", "class_id": "workflow.RollbackTask", "name": "Name 1", "task_moid": "TaskMoid 1", "catalog_moid": "CatalogMoid 1", "object_type": "workflow.RollbackTask", "nr_version": 32}, {"description": "Description 2", "class_id": "workflow.RollbackTask", "name": "Name 2", "task_moid": "TaskMoid 2", "catalog_moid": "CatalogMoid 2", "object_type": "workflow.RollbackTask", "nr_version": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackWorkflowTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.RollbackWorkflowTask","RollbackTaskName":"RollbackTaskName %d","TaskPath":"TaskPath %d","Description":"Description %d","ObjectType":"workflow.RollbackWorkflowTask","TaskInfoMoid":"TaskInfoMoid %d","Name":"Name %d","RefName":"RefName %d","RollbackCompleted":true,"Status":"Status %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.RollbackWorkflowTask", "rollback_task_name": "RollbackTaskName 1", "task_path": "TaskPath 1", "description": "Description 1", "object_type": "workflow.RollbackWorkflowTask", "task_info_moid": "TaskInfoMoid 1", "name": "Name 1", "ref_name": "RefName 1", "rollback_completed": true, "status": "Status 1"}, {"class_id": "workflow.RollbackWorkflowTask", "rollback_task_name": "RollbackTaskName 2", "task_path": "TaskPath 2", "description": "Description 2", "object_type": "workflow.RollbackWorkflowTask", "task_info_moid": "TaskInfoMoid 2", "name": "Name 2", "ref_name": "RefName 2", "rollback_completed": true, "status": "Status 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListX509Certificate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"x509.Certificate","PemCertificate":"PemCertificate %d","Sha256Fingerprint":"Sha256Fingerprint %d","SignatureAlgorithm":"SignatureAlgorithm %d","ClassId":"x509.Certificate"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "x509.Certificate", "pem_certificate": "PemCertificate 1", "sha256_fingerprint": "Sha256Fingerprint 1", "signature_algorithm": "SignatureAlgorithm 1", "class_id": "x509.Certificate"}, {"object_type": "x509.Certificate", "pem_certificate": "PemCertificate 2", "sha256_fingerprint": "Sha256Fingerprint 2", "signature_algorithm": "SignatureAlgorithm 2", "class_id": "x509.Certificate"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenMapAssetCustomerInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"asset.CustomerInformation","Id":"Id %d","Name":"Name %d","ObjectType":"asset.CustomerInformation"}`
	...
	expectedOp := map[string]interface{}{"class_id": "asset.CustomerInformation", "id": "Id 1", "name": "Name 1", "object_type": "asset.CustomerInformation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeploymentDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ItemType":"ItemType %d","Description":"Description %d","MlbProductName":"MlbProductName %d","ClassId":"asset.DeploymentDeviceInformation","MlbProductId":32,"InstanceId":"InstanceId %d","ObjectType":"asset.DeploymentDeviceInformation","OldDeviceStatusId":32,"OldInstanceId":"OldInstanceId %d","OldDeviceId":"OldDeviceId %d","OldDeviceStatusDescription":"OldDeviceStatusDescription %d","ApplicationName":"ApplicationName %d"}`
	...
	expectedOp := map[string]interface{}{"item_type": "ItemType 1", "description": "Description 1", "mlb_product_name": "MlbProductName 1", "class_id": "asset.DeploymentDeviceInformation", "mlb_product_id": 32, "instance_id": "InstanceId 1", "object_type": "asset.DeploymentDeviceInformation", "old_device_status_id": 32, "old_instance_id": "OldInstanceId 1", "old_device_id": "OldDeviceId 1", "old_device_status_description": "OldDeviceStatusDescription 1", "application_name": "ApplicationName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MlbProductName":"MlbProductName %d","ItemType":"ItemType %d","ObjectType":"asset.DeviceInformation","OldDeviceId":"OldDeviceId %d","OldInstanceId":"OldInstanceId %d","UnitOfMeasure":"UnitOfMeasure %d","MlbOfferType":"MlbOfferType %d","ProductType":"ProductType %d","OldDeviceStatusId":32,"ProductFamily":"ProductFamily %d","ClassId":"asset.DeviceInformation","OldDeviceStatusDescription":"OldDeviceStatusDescription %d","ApplicationName":"ApplicationName %d","InstanceId":"InstanceId %d","MlbProductId":32}`
	...
	expectedOp := map[string]interface{}{"mlb_product_name": "MlbProductName 1", "item_type": "ItemType 1", "object_type": "asset.DeviceInformation", "old_device_id": "OldDeviceId 1", "old_instance_id": "OldInstanceId 1", "unit_of_measure": "UnitOfMeasure 1", "mlb_offer_type": "MlbOfferType 1", "product_type": "ProductType 1", "old_device_status_id": 32, "product_family": "ProductFamily 1", "class_id": "asset.DeviceInformation", "old_device_status_description": "OldDeviceStatusDescription 1", "application_name": "ApplicationName 1", "instance_id": "InstanceId 1", "mlb_product_id": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceStatistics(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Connected":32,"MembershipRatio":32.000000,"MemoryMirroringFactor":32.000000,"ClusterName":"ClusterName %d","ObjectType":"asset.DeviceStatistics","ClassId":"asset.DeviceStatistics"}`
	...
	expectedOp := map[string]interface{}{"connected": 32, "membership_ratio": 32.000000, "memory_mirroring_factor": 32.000000, "cluster_name": "ClusterName 1", "object_type": "asset.DeviceStatistics", "class_id": "asset.DeviceStatistics"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetProductInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"asset.ProductInformation","Description":"Description %d","SubType":"SubType %d","Family":"Family %d","Group":"Group %d","Number":"Number %d","ObjectType":"asset.ProductInformation"}`
	...
	expectedOp := map[string]interface{}{"class_id": "asset.ProductInformation", "description": "Description 1", "sub_type": "SubType 1", "family": "Family 1", "group": "Group 1", "number": "Number 1", "object_type": "asset.ProductInformation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCapabilitySwitchNetworkLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaximumEthernetPortChannels":32,"MaximumSecondaryVlanPerPrimary":32,"MaximumPrimaryVlan":32,"MaximumSecondaryVlan":32,"MaximumVifs":32,"MaximumFcPortChannels":32,"MaxCompressedPortVlanCount":32,"MaximumVlans":32,"MaximumActiveTrafficMonitoringSessions":32,"MaximumPortChannelMembers":32,"MinimumActiveFans":32,"ObjectType":"capability.SwitchNetworkLimits","ClassId":"capability.SwitchNetworkLimits","MaximumFcPortChannelMembers":32,"MaximumIgmpGroups":32,"MaxUncompressedPortVlanCount":32,"MaximumEthernetUplinkPorts":32}`
	...
	expectedOp := map[string]interface{}{"maximum_ethernet_port_channels": 32, "maximum_secondary_vlan_per_primary": 32, "maximum_primary_vlan": 32, "maximum_secondary_vlan": 32, "maximum_vifs": 32, "maximum_fc_port_channels": 32, "max_compressed_port_vlan_count": 32, "maximum_vlans": 32, "maximum_active_traffic_monitoring_sessions": 32, "maximum_port_channel_members": 32, "minimum_active_fans": 32, "object_type": "capability.SwitchNetworkLimits", "class_id": "capability.SwitchNetworkLimits", "maximum_fc_port_channel_members": 32, "maximum_igmp_groups": 32, "max_uncompressed_port_vlan_count": 32, "maximum_ethernet_uplink_ports": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchStorageLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"capability.SwitchStorageLimits","MaximumVirtualFcInterfaces":32,"MaximumVirtualFcInterfacesPerBladeServer":32,"MaximumVsans":32,"MaximumZoneCount":32,"MaximumUserZoneCount":32,"ObjectType":"capability.SwitchStorageLimits"}`
	...
	expectedOp := map[string]interface{}{"class_id": "capability.SwitchStorageLimits", "maximum_virtual_fc_interfaces": 32, "maximum_virtual_fc_interfaces_per_blade_server": 32, "maximum_vsans": 32, "maximum_zone_count": 32, "maximum_user_zone_count": 32, "object_type": "capability.SwitchStorageLimits"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudAvailabilityZone(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"cloud.AvailabilityZone","Name":"Name %d","ZoneId":"ZoneId %d","ObjectType":"cloud.AvailabilityZone"}`
	...
	expectedOp := map[string]interface{}{"class_id": "cloud.AvailabilityZone", "name": "Name 1", "zone_id": "ZoneId 1", "object_type": "cloud.AvailabilityZone"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudImageReference(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.ImageReference","ImageId":"ImageId %d","Name":"Name %d","ClassId":"cloud.ImageReference"}`
	...
	expectedOp := map[string]interface{}{"object_type": "cloud.ImageReference", "image_id": "ImageId 1", "name": "Name 1", "class_id": "cloud.ImageReference"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCloudInstanceType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Architecture":"Architecture %d","Cpus":32,"Platform":"Platform %d","Memory":32,"Name":"Name %d","ClassId":"cloud.InstanceType","ObjectType":"cloud.InstanceType","CpuSpeed":32,"InstanceTypeId":"InstanceTypeId %d"}`
	...
	expectedOp := map[string]interface{}{"architecture": "Architecture 1", "cpus": 32, "platform": "Platform 1", "memory": 32, "name": "Name 1", "class_id": "cloud.InstanceType", "object_type": "cloud.InstanceType", "cpu_speed": 32, "instance_type_id": "InstanceTypeId 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCloudNetworkInstanceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"cloud.NetworkInstanceAttachment","AutoDelete":true,"DeviceIndex":32,"InstanceId":"InstanceId %d","State":"State %d","ObjectType":"cloud.NetworkInstanceAttachment"}`
	...
	expectedOp := map[string]interface{}{"class_id": "cloud.NetworkInstanceAttachment", "auto_delete": true, "device_index": 32, "instance_id": "InstanceId 1", "state": "State 1", "object_type": "cloud.NetworkInstanceAttachment"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudVolumeType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","VolumeTypeId":"VolumeTypeId %d","ObjectType":"cloud.VolumeType","ClassId":"cloud.VolumeType"}`
	...
	expectedOp := map[string]interface{}{"name": "Name 1", "volume_type_id": "VolumeTypeId 1", "object_type": "cloud.VolumeType", "class_id": "cloud.VolumeType"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapComputePersistentMemoryOperation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"compute.PersistentMemoryOperation","ObjectType":"compute.PersistentMemoryOperation","AdminAction":"AdminAction %d","IsSecurePassphraseSet":true}`
	...
	expectedOp := map[string]interface{}{"class_id": "compute.PersistentMemoryOperation", "object_type": "compute.PersistentMemoryOperation", "admin_action": "AdminAction 1", "is_secure_passphrase_set": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFabricMacAgingSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"fabric.MacAgingSettings","MacAgingOption":"MacAgingOption %d","MacAgingTime":32,"ClassId":"fabric.MacAgingSettings"}`
	...
	expectedOp := map[string]interface{}{"object_type": "fabric.MacAgingSettings", "mac_aging_option": "MacAgingOption 1", "mac_aging_time": 32, "class_id": "fabric.MacAgingSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareDirectDownload(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"firmware.DirectDownload","ClassId":"firmware.DirectDownload","ImageSource":"ImageSource %d","Upgradeoption":"Upgradeoption %d","Username":"Username %d","IsPasswordSet":true}`
	...
	expectedOp := map[string]interface{}{"object_type": "firmware.DirectDownload", "class_id": "firmware.DirectDownload", "image_source": "ImageSource 1", "upgradeoption": "Upgradeoption 1", "username": "Username 1", "is_password_set": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareNetworkShare(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Upgradeoption":"Upgradeoption %d","MapType":"MapType %d","Username":"Username %d","ObjectType":"firmware.NetworkShare","ClassId":"firmware.NetworkShare","IsPasswordSet":true}`
	...
	expectedOp := map[string]interface{}{"upgradeoption": "Upgradeoption 1", "map_type": "MapType 1", "username": "Username 1", "object_type": "firmware.NetworkShare", "class_id": "firmware.NetworkShare", "is_password_set": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexDiskStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"VolumeName":"VolumeName %d","ClassId":"hyperflex.DiskStatus","ObjectType":"hyperflex.DiskStatus","DownloadPercentage":"DownloadPercentage %d","State":"State %d","VolumeHandle":"VolumeHandle %d"}`
	...
	expectedOp := map[string]interface{}{"volume_name": "VolumeName 1", "class_id": "hyperflex.DiskStatus", "object_type": "hyperflex.DiskStatus", "download_percentage": "DownloadPercentage 1", "state": "State 1", "volume_handle": "VolumeHandle 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHealthCheckScriptInfo(t *testing.T) {
	p := models.HyperflexHealthCheckScriptInfo{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptInput":"ScriptInput %d","Version":"Version %d","HyperflexVersion":"HyperflexVersion %d","ClassId":"hyperflex.HealthCheckScriptInfo","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptName":"ScriptName %d","AggregateScriptName":"AggregateScriptName %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.HealthCheckScriptInfo", "script_input": "ScriptInput 1", "nr_version": "Version 1", "hyperflex_version": "HyperflexVersion 1", "class_id": "hyperflex.HealthCheckScriptInfo", "script_execute_location": "ScriptExecuteLocation 1", "script_name": "ScriptName 1", "aggregate_script_name": "AggregateScriptName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHxPlatformDatastoreConfigDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.HxPlatformDatastoreConfigDt","NumStripesForLargeFiles":32,"SystemDatastore":true,"ObjectType":"hyperflex.HxPlatformDatastoreConfigDt","DataBlockSize":32,"Name":"Name %d","NumMirrors":32,"ProvisionedCapacity":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.HxPlatformDatastoreConfigDt", "num_stripes_for_large_files": 32, "system_datastore": true, "object_type": "hyperflex.HxPlatformDatastoreConfigDt", "data_block_size": 32, "name": "Name 1", "num_mirrors": 32, "provisioned_capacity": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxRegistrationDetailsDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.HxRegistrationDetailsDt","RegistrationFailedReason":"RegistrationFailedReason %d","SmartAccount":"SmartAccount %d","Status":"Status %d","OutOfComplianceStartDate":"OutOfComplianceStartDate %d","ObjectType":"hyperflex.HxRegistrationDetailsDt","LastRenewalAttemptDate":"LastRenewalAttemptDate %d","RegistrationExpireDate":"RegistrationExpireDate %d","NextRenewalAttemptDate":"NextRenewalAttemptDate %d","VirtualAccount":"VirtualAccount %d","InitialRegistrationDate":"InitialRegistrationDate %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.HxRegistrationDetailsDt", "registration_failed_reason": "RegistrationFailedReason 1", "smart_account": "SmartAccount 1", "status": "Status 1", "out_of_compliance_start_date": "OutOfComplianceStartDate 1", "object_type": "hyperflex.HxRegistrationDetailsDt", "last_renewal_attempt_date": "LastRenewalAttemptDate 1", "registration_expire_date": "RegistrationExpireDate 1", "next_renewal_attempt_date": "NextRenewalAttemptDate 1", "virtual_account": "VirtualAccount 1", "initial_registration_date": "InitialRegistrationDate 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxResiliencyInfoDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SsdFailuresTolerable":32,"NodeFailuresTolerable":32,"PolicyCompliance":"PolicyCompliance %d","ResiliencyState":"ResiliencyState %d","ClassId":"hyperflex.HxResiliencyInfoDt","ObjectType":"hyperflex.HxResiliencyInfoDt","HddFailuresTolerable":32,"DataReplicationFactor":"DataReplicationFactor %d"}`
	...
	expectedOp := map[string]interface{}{"ssd_failures_tolerable": 32, "node_failures_tolerable": 32, "policy_compliance": "PolicyCompliance 1", "resiliency_state": "ResiliencyState 1", "class_id": "hyperflex.HxResiliencyInfoDt", "object_type": "hyperflex.HxResiliencyInfoDt", "hdd_failures_tolerable": 32, "data_replication_factor": "DataReplicationFactor 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexNamedVlan(t *testing.T) {
	p := models.HyperflexNamedVlan{}
	var d = &schema.ResourceData{}
	c := `{"VlanId":32,"Name":"Name %d","ObjectType":"hyperflex.NamedVlan","ClassId":"hyperflex.NamedVlan"}`
	...
	expectedOp := map[string]interface{}{"vlan_id": 32, "name": "Name 1", "object_type": "hyperflex.NamedVlan", "class_id": "hyperflex.NamedVlan"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexReplicationPeerInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.ReplicationPeerInfo","Status":"Status %d","Dcip":"Dcip %d","ClassId":"hyperflex.ReplicationPeerInfo","StatusDetails":"StatusDetails %d","Mcip":"Mcip %d","ReplCip":"ReplCip %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.ReplicationPeerInfo", "status": "Status 1", "dcip": "Dcip 1", "class_id": "hyperflex.ReplicationPeerInfo", "status_details": "StatusDetails 1", "mcip": "Mcip 1", "repl_cip": "ReplCip 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Downtime":"Downtime %d","DataReplicationFactor":"DataReplicationFactor %d","DeduplicationSavings":32.000000,"State":"State %d","FreeCapacity":32,"Name":"Name %d","CompressionSavings":32.000000,"ObjectType":"hyperflex.Summary","ClassId":"hyperflex.Summary","Address":"Address %d","ResiliencyDetailsSize":32,"ClusterAccessPolicy":"ClusterAccessPolicy %d","TotalCapacity":32,"UsedCapacity":32,"DataReplicationCompliance":"DataReplicationCompliance %d","ActiveNodes":"ActiveNodes %d","SpaceStatus":"SpaceStatus %d","TotalSavings":32.000000,"Uptime":"Uptime %d","Boottime":32}`
	...
	expectedOp := map[string]interface{}{"downtime": "Downtime 1", "data_replication_factor": "DataReplicationFactor 1", "deduplication_savings": 32.000000, "state": "State 1", "free_capacity": 32, "name": "Name 1", "compression_savings": 32.000000, "object_type": "hyperflex.Summary", "class_id": "hyperflex.Summary", "address": "Address 1", "resiliency_details_size": 32, "cluster_access_policy": "ClusterAccessPolicy 1", "total_capacity": 32, "used_capacity": 32, "data_replication_compliance": "DataReplicationCompliance 1", "active_nodes": "ActiveNodes 1", "space_status": "SpaceStatus 1", "total_savings": 32.000000, "uptime": "Uptime 1", "boottime": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamEndPointPasswordProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NotificationPeriod":32,"GracePeriod":32,"ObjectType":"iam.EndPointPasswordProperties","ClassId":"iam.EndPointPasswordProperties","EnforceStrongPassword":true,"EnablePasswordExpiry":true,"ForceSendPassword":true,"PasswordHistory":32,"PasswordExpiryDuration":32}`
	...
	expectedOp := map[string]interface{}{"notification_period": 32, "grace_period": 32, "object_type": "iam.EndPointPasswordProperties", "class_id": "iam.EndPointPasswordProperties", "enforce_strong_password": true, "enable_password_expiry": true, "force_send_password": true, "password_history": 32, "password_expiry_duration": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamLdapBaseProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Filter":"Filter %d","ObjectType":"iam.LdapBaseProperties","Domain":"Domain %d","Timeout":32,"EnableEncryption":true,"ClassId":"iam.LdapBaseProperties","BindMethod":"BindMethod %d","GroupAttribute":"GroupAttribute %d","Attribute":"Attribute %d","BaseDn":"BaseDn %d","BindDn":"BindDn %d","NestedGroupSearchDepth":32,"IsPasswordSet":true,"EnableGroupAuthorization":true}`
	...
	expectedOp := map[string]interface{}{"filter": "Filter 1", "object_type": "iam.LdapBaseProperties", "domain": "Domain 1", "timeout": 32, "enable_encryption": true, "class_id": "iam.LdapBaseProperties", "bind_method": "BindMethod 1", "group_attribute": "GroupAttribute 1", "attribute": "Attribute 1", "base_dn": "BaseDn 1", "bind_dn": "BindDn 1", "nested_group_search_depth": 32, "is_password_set": true, "enable_group_authorization": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIppoolIpV4Block(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"from": "From 1", "to": "To 1", "class_id": "ippool.IpV4Block", "object_type": "ippool.IpV4Block", "size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIqnpoolIqnSuffixBlock(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"from": 32, "suffix": "Suffix 1", "to": 32, "object_type": "iqnpool.IqnSuffixBlock", "class_id": "iqnpool.IqnSuffixBlock", "size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesAddonConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"InstallStrategy":"InstallStrategy %d","ReleaseName":"ReleaseName %d","UpgradeStrategy":"UpgradeStrategy %d","Overrides":"Overrides %d","ReleaseNamespace":"ReleaseNamespace %d","ClassId":"kubernetes.AddonConfiguration","ObjectType":"kubernetes.AddonConfiguration"}`
	...
	expectedOp := map[string]interface{}{"install_strategy": "InstallStrategy 1", "release_name": "ReleaseName 1", "upgrade_strategy": "UpgradeStrategy 1", "overrides": "Overrides 1", "release_namespace": "ReleaseNamespace 1", "class_id": "kubernetes.AddonConfiguration", "object_type": "kubernetes.AddonConfiguration"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesClusterCertificateConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.ClusterCertificateConfiguration","FrontProxyCert":"FrontProxyCert %d","CaCert":"CaCert %d","ClassId":"kubernetes.ClusterCertificateConfiguration","SaPublicKey":"SaPublicKey %d","EtcdCert":"EtcdCert %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.ClusterCertificateConfiguration", "front_proxy_cert": "FrontProxyCert 1", "ca_cert": "CaCert 1", "class_id": "kubernetes.ClusterCertificateConfiguration", "sa_public_key": "SaPublicKey 1", "etcd_cert": "EtcdCert 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesClusterManagementConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LoadBalancerCount":32,"ObjectType":"kubernetes.ClusterManagementConfig","ClassId":"kubernetes.ClusterManagementConfig","MasterVip":"MasterVip %d","SshUser":"SshUser %d"}`
	...
	expectedOp := map[string]interface{}{"load_balancer_count": 32, "object_type": "kubernetes.ClusterManagementConfig", "class_id": "kubernetes.ClusterManagementConfig", "master_vip": "MasterVip 1", "ssh_user": "SshUser 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesCniConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Registry":"Registry %d","Version":"Version %d","ObjectType":"kubernetes.CniConfig","ClassId":"kubernetes.CniConfig"}`
	...
	expectedOp := map[string]interface{}{"registry": "Registry 1", "nr_version": "Version 1", "object_type": "kubernetes.CniConfig", "class_id": "kubernetes.CniConfig"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesDaemonSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.DaemonSetStatus","ObservedGeneration":32,"CurrentNumberScheduled":32,"NumberAvailable":"NumberAvailable %d","NumberReady":32,"UpdatedNumberScheduled":"UpdatedNumberScheduled %d","ClassId":"kubernetes.DaemonSetStatus","NumberMisscheduled":32,"DesiredNumberScheduled":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.DaemonSetStatus", "observed_generation": 32, "current_number_scheduled": 32, "number_available": "NumberAvailable 1", "number_ready": 32, "updated_number_scheduled": "UpdatedNumberScheduled 1", "class_id": "kubernetes.DaemonSetStatus", "number_misscheduled": 32, "desired_number_scheduled": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesDeploymentStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AvailableReplicas":32,"ObservedGeneration":32,"ReadyReplicas":32,"Replicas":32,"UpdatedReplicas":32,"ObjectType":"kubernetes.DeploymentStatus","ClassId":"kubernetes.DeploymentStatus"}`
	...
	expectedOp := map[string]interface{}{"available_replicas": 32, "observed_generation": 32, "ready_replicas": 32, "replicas": 32, "updated_replicas": 32, "object_type": "kubernetes.DeploymentStatus", "class_id": "kubernetes.DeploymentStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesNodeInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"KubeProxyVersion":"KubeProxyVersion %d","KubeletVersion":"KubeletVersion %d","OperatingSystem":"OperatingSystem %d","Architecture":"Architecture %d","BootId":"BootId %d","ObjectType":"kubernetes.NodeInfo","SystemUuid":"SystemUuid %d","KernelVersion":"KernelVersion %d","MachineId":"MachineId %d","OsImage":"OsImage %d","ContainerRuntimeVersion":"ContainerRuntimeVersion %d","ClassId":"kubernetes.NodeInfo"}`
	...
	expectedOp := map[string]interface{}{"kube_proxy_version": "KubeProxyVersion 1", "kubelet_version": "KubeletVersion 1", "operating_system": "OperatingSystem 1", "architecture": "Architecture 1", "boot_id": "BootId 1", "object_type": "kubernetes.NodeInfo", "system_uuid": "SystemUuid 1", "kernel_version": "KernelVersion 1", "machine_id": "MachineId 1", "os_image": "OsImage 1", "container_runtime_version": "ContainerRuntimeVersion 1", "class_id": "kubernetes.NodeInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesPodStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Phase":"Phase %d","PodIp":"PodIp %d","QosClass":"QosClass %d","StartTime":"StartTime %d","HostIp":"HostIp %d","ObjectType":"kubernetes.PodStatus","ClassId":"kubernetes.PodStatus"}`
	...
	expectedOp := map[string]interface{}{"phase": "Phase 1", "pod_ip": "PodIp 1", "qos_class": "QosClass 1", "start_time": "StartTime 1", "host_ip": "HostIp 1", "object_type": "kubernetes.PodStatus", "class_id": "kubernetes.PodStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesProxyConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.ProxyConfig","Port":32,"ObjectType":"kubernetes.ProxyConfig","Username":"Username %d","Hostname":"Hostname %d","IsPasswordSet":true,"Protocol":"Protocol %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "kubernetes.ProxyConfig", "port": 32, "object_type": "kubernetes.ProxyConfig", "username": "Username 1", "hostname": "Hostname 1", "is_password_set": true, "protocol": "Protocol 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesStatefulSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UpdateRevision":"UpdateRevision %d","ReadyReplicas":32,"CurrentRevision":"CurrentRevision %d","ClassId":"kubernetes.StatefulSetStatus","ObjectType":"kubernetes.StatefulSetStatus","UpdatedReplicas":32,"CollisionCount":32,"Replicas":32,"AvailableReplicas":32,"ObservedGeneration":32}`
	...
	expectedOp := map[string]interface{}{"update_revision": "UpdateRevision 1", "ready_replicas": 32, "current_revision": "CurrentRevision 1", "class_id": "kubernetes.StatefulSetStatus", "object_type": "kubernetes.StatefulSetStatus", "updated_replicas": 32, "collision_count": 32, "replicas": 32, "available_replicas": 32, "observed_generation": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapMoVersionContext(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":"Version %d","VersionType":"VersionType %d","ObjectType":"mo.VersionContext","ClassId":"mo.VersionContext"}`
	...
	expectedOp := map[string]interface{}{"nr_version": "Version 1", "version_type": "VersionType 1", "object_type": "mo.VersionContext", "class_id": "mo.VersionContext"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiaapiNewReleaseDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReleaseNoteLink":"ReleaseNoteLink %d","SoftwareDownloadLinkTitle":"SoftwareDownloadLinkTitle %d","ClassId":"niaapi.NewReleaseDetail","Link":"Link %d","ReleaseNoteLinkTitle":"ReleaseNoteLinkTitle %d","Version":"Version %d","Description":"Description %d","SoftwareDownloadLink":"SoftwareDownloadLink %d","Title":"Title %d","ObjectType":"niaapi.NewReleaseDetail"}`
	...
	expectedOp := map[string]interface{}{"release_note_link": "ReleaseNoteLink 1", "software_download_link_title": "SoftwareDownloadLinkTitle 1", "class_id": "niaapi.NewReleaseDetail", "link": "Link 1", "release_note_link_title": "ReleaseNoteLinkTitle 1", "nr_version": "Version 1", "description": "Description 1", "software_download_link": "SoftwareDownloadLink 1", "title": "Title 1", "object_type": "niaapi.NewReleaseDetail"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryNvePacketCounters(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.NvePacketCounters","ClassId":"niatelemetry.NvePacketCounters","McastOutbytes":32,"UcastInpkts":32,"UcastOutpkts":32,"McastInpkts":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "niatelemetry.NvePacketCounters", "class_id": "niatelemetry.NvePacketCounters", "mcast_outbytes": 32, "ucast_inpkts": 32, "ucast_outpkts": 32, "mcast_inpkts": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNveVni(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DpVniDown":32,"CpVniCount":32,"DpVniCount":32,"DpVniUp":32,"CpVniDown":32,"CpVniUp":32,"ClassId":"niatelemetry.NveVni","ObjectType":"niatelemetry.NveVni"}`
	...
	expectedOp := map[string]interface{}{"dp_vni_down": 32, "cp_vni_count": 32, "dp_vni_count": 32, "dp_vni_up": 32, "cp_vni_down": 32, "cp_vni_up": 32, "class_id": "niatelemetry.NveVni", "object_type": "niatelemetry.NveVni"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosBgpMvpn(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"TotalNetworks":32,"ObjectType":"niatelemetry.NxosBgpMvpn","NumberOfClusterLists":32,"NumberOfCommunities":32,"CapablePeers":32,"ConfiguredPeers":32,"MemoryUsed":32,"ClassId":"niatelemetry.NxosBgpMvpn","TotalPaths":32,"TableVersion":32}`
	...
	expectedOp := map[string]interface{}{"total_networks": 32, "object_type": "niatelemetry.NxosBgpMvpn", "number_of_cluster_lists": 32, "number_of_communities": 32, "capable_peers": 32, "configured_peers": 32, "memory_used": 32, "class_id": "niatelemetry.NxosBgpMvpn", "total_paths": 32, "table_version": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosVtp(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":32,"ClassId":"niatelemetry.NxosVtp","V2Mode":"V2Mode %d","RunningVersion":"RunningVersion %d","TrapEnabled":"TrapEnabled %d","OperMode":"OperMode %d","PruningMode":"PruningMode %d","ObjectType":"niatelemetry.NxosVtp"}`
	...
	expectedOp := map[string]interface{}{"nr_version": 32, "class_id": "niatelemetry.NxosVtp", "v2_mode": "V2Mode 1", "running_version": "RunningVersion 1", "trap_enabled": "TrapEnabled 1", "oper_mode": "OperMode 1", "pruning_mode": "PruningMode 1", "object_type": "niatelemetry.NxosVtp"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOnpremSchedule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"onprem.Schedule","TimeOfDay":32,"RepeatInterval":32,"MonthOfYear":32,"DayOfMonth":32,"ObjectType":"onprem.Schedule","TimeZone":"TimeZone %d","WeekOfMonth":32,"DayOfWeek":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "onprem.Schedule", "time_of_day": 32, "repeat_interval": 32, "month_of_year": 32, "day_of_month": 32, "object_type": "onprem.Schedule", "time_zone": "TimeZone 1", "week_of_month": 32, "day_of_week": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapOnpremUpgradePhase(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"elapsed_time": 32, "failed": true, "message": "Message 1", "name": "Name 1", "object_type": "onprem.UpgradePhase", "class_id": "onprem.UpgradePhase"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsAnswers(t *testing.T) {
	p := models.OsAnswers{}
	var d = &schema.ResourceData{}
	c := `{"IsAnswerFileSet":true,"ProductKey":"ProductKey %d","ObjectType":"os.Answers","Source":"Source %d","IpConfigType":"IpConfigType %d","Nameserver":"Nameserver %d","IsRootPasswordSet":true,"ClassId":"os.Answers","Hostname":"Hostname %d","IsRootPasswordCrypted":true}`
	...
	expectedOp := map[string]interface{}{"is_answer_file_set": true, "product_key": "ProductKey 1", "object_type": "os.Answers", "nr_source": "Source 1", "ip_config_type": "IpConfigType 1", "nameserver": "Nameserver 1", "is_root_password_set": true, "class_id": "os.Answers", "hostname": "Hostname 1", "is_root_password_crypted": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsGlobalConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"os.GlobalConfig","ConfigurationFileName":"ConfigurationFileName %d","InstallMethod":"InstallMethod %d","ScuImageName":"ScuImageName %d","OsImageName":"OsImageName %d","WindowsEdition":"WindowsEdition %d","ObjectType":"os.GlobalConfig","ConfigurationSource":"ConfigurationSource %d","InstallTargetType":"InstallTargetType %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "os.GlobalConfig", "configuration_file_name": "ConfigurationFileName 1", "install_method": "InstallMethod 1", "scu_image_name": "ScuImageName 1", "os_image_name": "OsImageName 1", "windows_edition": "WindowsEdition 1", "object_type": "os.GlobalConfig", "configuration_source": "ConfigurationSource 1", "install_target_type": "InstallTargetType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapPkixKeyGenerationSpec(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"pkix.KeyGenerationSpec","Name":"Name %d","ClassId":"pkix.KeyGenerationSpec"}`
	...
	expectedOp := map[string]interface{}{"object_type": "pkix.KeyGenerationSpec", "name": "Name 1", "class_id": "pkix.KeyGenerationSpec"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageAutomaticDriveGroup(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NumberOfSpans":32,"UseRemainingDrives":true,"DriveType":"DriveType %d","ClassId":"storage.AutomaticDriveGroup","NumDedicatedHotSpares":"NumDedicatedHotSpares %d","DrivesPerSpan":32,"ObjectType":"storage.AutomaticDriveGroup","MinimumDriveSize":32}`
	...
	expectedOp := map[string]interface{}{"number_of_spans": 32, "use_remaining_drives": true, "drive_type": "DriveType 1", "class_id": "storage.AutomaticDriveGroup", "num_dedicated_hot_spares": "NumDedicatedHotSpares 1", "drives_per_span": 32, "object_type": "storage.AutomaticDriveGroup", "minimum_drive_size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapStorageBaseCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CapacityUtilization":32.000000,"Free":32,"ClassId":"storage.BaseCapacity","ObjectType":"storage.BaseCapacity","Total":32,"Used":32,"Available":32}`
	...
	expectedOp := map[string]interface{}{"capacity_utilization": 32.000000, "free": 32, "class_id": "storage.BaseCapacity", "object_type": "storage.BaseCapacity", "total": 32, "used": 32, "available": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageM2VirtualDriveConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"storage.M2VirtualDriveConfig","Enable":true,"ControllerSlot":"ControllerSlot %d","ClassId":"storage.M2VirtualDriveConfig"}`
	...
	expectedOp := map[string]interface{}{"object_type": "storage.M2VirtualDriveConfig", "enable": true, "controller_slot": "ControllerSlot 1", "class_id": "storage.M2VirtualDriveConfig"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageR0Drive(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"storage.R0Drive","DriveSlots":"DriveSlots %d","DriveSlotsList":"DriveSlotsList %d","Enable":true,"ClassId":"storage.R0Drive"}`
	...
	expectedOp := map[string]interface{}{"object_type": "storage.R0Drive", "drive_slots": "DriveSlots 1", "drive_slots_list": "DriveSlotsList 1", "enable": true, "class_id": "storage.R0Drive"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCloudInitConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.CloudInitConfig","NetworkData":"NetworkData %d","NetworkDataBase64Encoded":true,"UserData":"UserData %d","UserDataBase64Encoded":true,"ConfigType":"ConfigType %d","ClassId":"virtualization.CloudInitConfig"}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.CloudInitConfig", "network_data": "NetworkData 1", "network_data_base64_encoded": true, "user_data": "UserData 1", "user_data_base64_encoded": true, "config_type": "ConfigType 1", "class_id": "virtualization.CloudInitConfig"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationComputeCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.ComputeCapacity","ClassId":"virtualization.ComputeCapacity","Used":32,"Capacity":32,"Free":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.ComputeCapacity", "class_id": "virtualization.ComputeCapacity", "used": 32, "capacity": 32, "free": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationCpuAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.CpuAllocation","Free":32,"Reserved":32,"Total":32,"Used":32,"ObjectType":"virtualization.CpuAllocation"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.CpuAllocation", "free": 32, "reserved": 32, "total": 32, "used": 32, "object_type": "virtualization.CpuAllocation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationCpuInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Sockets":32,"Speed":32,"Vendor":"Vendor %d","Cores":32,"ObjectType":"virtualization.CpuInfo","ClassId":"virtualization.CpuInfo","Description":"Description %d"}`
	...
	expectedOp := map[string]interface{}{"sockets": 32, "speed": 32, "vendor": "Vendor 1", "cores": 32, "object_type": "virtualization.CpuInfo", "class_id": "virtualization.CpuInfo", "description": "Description 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationMemoryAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.MemoryAllocation","ClassId":"virtualization.MemoryAllocation","Reserved":32,"Total":32,"Used":32,"Free":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.MemoryAllocation", "class_id": "virtualization.MemoryAllocation", "reserved": 32, "total": 32, "used": 32, "free": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationMemoryCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Free":32,"Used":32,"ObjectType":"virtualization.MemoryCapacity","ClassId":"virtualization.MemoryCapacity","Capacity":32}`
	...
	expectedOp := map[string]interface{}{"free": 32, "used": 32, "object_type": "virtualization.MemoryCapacity", "class_id": "virtualization.MemoryCapacity", "capacity": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationProductInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ProductType":"ProductType %d","ProductVendor":"ProductVendor %d","Version":"Version %d","ObjectType":"virtualization.ProductInfo","ClassId":"virtualization.ProductInfo","Build":"Build %d","ProductName":"ProductName %d"}`
	...
	expectedOp := map[string]interface{}{"product_type": "ProductType 1", "product_vendor": "ProductVendor 1", "nr_version": "Version 1", "object_type": "virtualization.ProductInfo", "class_id": "virtualization.ProductInfo", "build": "Build 1", "product_name": "ProductName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationStorageCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.StorageCapacity","ObjectType":"virtualization.StorageCapacity","Free":32,"Used":32,"Capacity":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.StorageCapacity", "object_type": "virtualization.StorageCapacity", "free": 32, "used": 32, "capacity": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareRemoteDisplayInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareRemoteDisplayInfo","RemoteDisplayVncPort":32,"RemoteDisplayPassword":"RemoteDisplayPassword %d","RemoteDisplayVncKey":"RemoteDisplayVncKey %d","ObjectType":"virtualization.VmwareRemoteDisplayInfo"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.VmwareRemoteDisplayInfo", "remote_display_vnc_port": 32, "remote_display_password": "RemoteDisplayPassword 1", "remote_display_vnc_key": "RemoteDisplayVncKey 1", "object_type": "virtualization.VmwareRemoteDisplayInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareTeamingAndFailover(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","NotifySwitches":true,"ClassId":"virtualization.VmwareTeamingAndFailover","Failback":true,"NetworkFailureDetection":"NetworkFailureDetection %d","ObjectType":"virtualization.VmwareTeamingAndFailover","LoadBalancing":"LoadBalancing %d"}`
	...
	expectedOp := map[string]interface{}{"name": "Name 1", "notify_switches": true, "class_id": "virtualization.VmwareTeamingAndFailover", "failback": true, "network_failure_detection": "NetworkFailureDetection 1", "object_type": "virtualization.VmwareTeamingAndFailover", "load_balancing": "LoadBalancing 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareVmCpuShareInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.VmwareVmCpuShareInfo","ClassId":"virtualization.VmwareVmCpuShareInfo","CpuOverheadLimit":32,"CpuReservation":32,"CpuShares":32,"CpuLimit":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.VmwareVmCpuShareInfo", "class_id": "virtualization.VmwareVmCpuShareInfo", "cpu_overhead_limit": 32, "cpu_reservation": 32, "cpu_shares": 32, "cpu_limit": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicIscsiAuthProfile(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UserId":"UserId %d","IsPasswordSet":true,"ObjectType":"vnic.IscsiAuthProfile","ClassId":"vnic.IscsiAuthProfile"}`
	...
	expectedOp := map[string]interface{}{"user_id": "UserId 1", "is_password_set": true, "object_type": "vnic.IscsiAuthProfile", "class_id": "vnic.IscsiAuthProfile"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicRoceSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.RoceSettings","QueuePairs":32,"ResourceGroups":32,"ClassOfService":32,"ClassId":"vnic.RoceSettings","Enabled":true,"MemoryRegions":32,"Version":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.RoceSettings", "queue_pairs": 32, "resource_groups": 32, "class_of_service": 32, "class_id": "vnic.RoceSettings", "enabled": true, "memory_regions": 32, "nr_version": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicRssHashSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UdpIpv6Hash":true,"Ipv6Hash":true,"ObjectType":"vnic.RssHashSettings","ClassId":"vnic.RssHashSettings","Ipv6ExtHash":true,"TcpIpv6Hash":true,"TcpIpv6ExtHash":true,"UdpIpv4Hash":true,"Ipv4Hash":true,"TcpIpv4Hash":true}`
	...
	expectedOp := map[string]interface{}{"udp_ipv6_hash": true, "ipv6_hash": true, "object_type": "vnic.RssHashSettings", "class_id": "vnic.RssHashSettings", "ipv6_ext_hash": true, "tcp_ipv6_hash": true, "tcp_ipv6_ext_hash": true, "udp_ipv4_hash": true, "ipv4_hash": true, "tcp_ipv4_hash": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicVmqSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"VmmqAdapterPolicy":"VmmqAdapterPolicy %d","ClassId":"vnic.VmqSettings","Enabled":true,"MultiQueueSupport":true,"NumInterrupts":32,"ObjectType":"vnic.VmqSettings","NumSubVnics":32,"NumVmqs":32}`
	...
	expectedOp := map[string]interface{}{"vmmq_adapter_policy": "VmmqAdapterPolicy 1", "class_id": "vnic.VmqSettings", "enabled": true, "multi_queue_support": true, "num_interrupts": 32, "object_type": "vnic.VmqSettings", "num_sub_vnics": 32, "num_vmqs": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicVsanSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Id":32,"ObjectType":"vnic.VsanSettings","ClassId":"vnic.VsanSettings","DefaultVlanId":32}`
	...
	expectedOp := map[string]interface{}{"id": 32, "object_type": "vnic.VsanSettings", "class_id": "vnic.VsanSettings", "default_vlan_id": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowCustomDataTypeProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.CustomDataTypeProperties","Cloneable":true,"ExternalMeta":true,"ObjectType":"workflow.CustomDataTypeProperties"}`
	...
	expectedOp := map[string]interface{}{"class_id": "workflow.CustomDataTypeProperties", "cloneable": true, "external_meta": true, "object_type": "workflow.CustomDataTypeProperties"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowInternalProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Internal":true,"Owner":"Owner %d","BaseTaskType":"BaseTaskType %d","ObjectType":"workflow.InternalProperties","ClassId":"workflow.InternalProperties"}`
	...
	expectedOp := map[string]interface{}{"internal": true, "owner": "Owner 1", "base_task_type": "BaseTaskType 1", "object_type": "workflow.InternalProperties", "class_id": "workflow.InternalProperties"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RetryCount":32,"ObjectType":"workflow.Properties","ClassId":"workflow.Properties","RetryDelay":32,"RetryPolicy":"RetryPolicy %d","TimeoutPolicy":"TimeoutPolicy %d","ExternalMeta":true,"Timeout":32,"SupportStatus":"SupportStatus %d"}`
	...
	expectedOp := map[string]interface{}{"retry_count": 32, "object_type": "workflow.Properties", "class_id": "workflow.Properties", "retry_delay": 32, "retry_policy": "RetryPolicy 1", "timeout_policy": "TimeoutPolicy 1", "external_meta": true, "timeout": 32, "support_status": "SupportStatus 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowCtx(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"WorkflowSubtype":"WorkflowSubtype %d","WorkflowType":"WorkflowType %d","ClassId":"workflow.WorkflowCtx","ObjectType":"workflow.WorkflowCtx","WorkflowMetaName":"WorkflowMetaName %d"}`
	...
	expectedOp := map[string]interface{}{"workflow_subtype": "WorkflowSubtype 1", "workflow_type": "WorkflowType 1", "class_id": "workflow.WorkflowCtx", "object_type": "workflow.WorkflowCtx", "workflow_meta_name": "WorkflowMetaName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowInfoProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"workflow.WorkflowInfoProperties","ClassId":"workflow.WorkflowInfoProperties","Retryable":true,"RollbackAction":"RollbackAction %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "workflow.WorkflowInfoProperties", "class_id": "workflow.WorkflowInfoProperties", "retryable": true, "rollback_action": "RollbackAction 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Cloneable":true,"EnableDebug":true,"ObjectType":"workflow.WorkflowProperties","ClassId":"workflow.WorkflowProperties","ExternalMeta":true,"Retryable":true,"SupportStatus":"SupportStatus %d"}`
	...
	expectedOp := map[string]interface{}{"cloneable": true, "enable_debug": true, "object_type": "workflow.WorkflowProperties", "class_id": "workflow.WorkflowProperties", "external_meta": true, "retryable": true, "support_status": "SupportStatus 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapX509Certificate(t *testing.T) {
	p := models.X509Certificate{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"x509.Certificate","SignatureAlgorithm":"SignatureAlgorithm %d","ObjectType":"x509.Certificate","PemCertificate":"PemCertificate %d","Sha256Fingerprint":"Sha256Fingerprint %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "x509.Certificate", "signature_algorithm": "SignatureAlgorithm 1", "object_type": "x509.Certificate", "pem_certificate": "PemCertificate 1", "sha256_fingerprint": "Sha256Fingerprint 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
