...
func TestFlattenListApplianceApiStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Reason":"Reason %d","ObjectType":"appliance.ApiStatus","ClassId":"appliance.ApiStatus","Response":32,"ElapsedTime":32.000000,"ObjectName":"ObjectName %d"}`
	...
	expectedOp := []map[string]interface{}{{"reason": "Reason 1", "object_type": "appliance.ApiStatus", "class_id": "appliance.ApiStatus", "response": 32, "elapsed_time": 32.000000, "object_name": "ObjectName 1"}, {"reason": "Reason 2", "object_type": "appliance.ApiStatus", "class_id": "appliance.ApiStatus", "response": 32, "elapsed_time": 32.000000, "object_name": "ObjectName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListApplianceCertRenewalPhase(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"message": "Message 1", "name": "Name 1", "failed": true, "class_id": "appliance.CertRenewalPhase", "object_type": "appliance.CertRenewalPhase"}, {"message": "Message 2", "name": "Name 2", "failed": true, "class_id": "appliance.CertRenewalPhase", "object_type": "appliance.CertRenewalPhase"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListBootDeviceBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"boot.DeviceBase","ObjectType":"boot.DeviceBase","Name":"Name %d","Enabled":true}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "boot.DeviceBase", "object_type": "boot.DeviceBase", "name": "Name 1", "enabled": true}, {"class_id": "boot.DeviceBase", "object_type": "boot.DeviceBase", "name": "Name 2", "enabled": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCertificatemanagementCertificateBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Enabled":true,"ClassId":"certificatemanagement.CertificateBase","ObjectType":"certificatemanagement.CertificateBase","IsPrivatekeySet":true}`
	...
	expectedOp := []map[string]interface{}{{"enabled": true, "class_id": "certificatemanagement.CertificateBase", "object_type": "certificatemanagement.CertificateBase", "is_privatekey_set": true}, {"enabled": true, "class_id": "certificatemanagement.CertificateBase", "object_type": "certificatemanagement.CertificateBase", "is_privatekey_set": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudCustomAttributes(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AttributeType":"AttributeType %d","AttributeValue":"AttributeValue %d","ClassId":"cloud.CustomAttributes","ObjectType":"cloud.CustomAttributes","AttributeName":"AttributeName %d"}`
...
func TestFlattenListCloudNetworkInterfaceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NetworkName":"NetworkName %d","MacAddress":"MacAddress %d","SubNetworkName":"SubNetworkName %d","Identity":"Identity %d","IpForwardingEnabled":true,"SubNetworkId":"SubNetworkId %d","ObjectType":"cloud.NetworkInterfaceAttachment","NetworkId":"NetworkId %d","ClassId":"cloud.NetworkInterfaceAttachment","NicIndex":32}`
	...
	expectedOp := []map[string]interface{}{{"network_name": "NetworkName 1", "mac_address": "MacAddress 1", "sub_network_name": "SubNetworkName 1", "identity": "Identity 1", "ip_forwarding_enabled": true, "sub_network_id": "SubNetworkId 1", "object_type": "cloud.NetworkInterfaceAttachment", "network_id": "NetworkId 1", "class_id": "cloud.NetworkInterfaceAttachment", "nic_index": 32}, {"network_name": "NetworkName 2", "mac_address": "MacAddress 2", "sub_network_name": "SubNetworkName 2", "identity": "Identity 2", "ip_forwarding_enabled": true, "sub_network_id": "SubNetworkId 2", "object_type": "cloud.NetworkInterfaceAttachment", "network_id": "NetworkId 2", "class_id": "cloud.NetworkInterfaceAttachment", "nic_index": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudSecurityGroupRule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.SecurityGroupRule","StartPort":32,"Protocol":"Protocol %d","EndPort":32,"Index":32,"Name":"Name %d","Description":"Description %d","EtherType":"EtherType %d","SourceSecurityGroup":"SourceSecurityGroup %d","ClassId":"cloud.SecurityGroupRule","Identity":"Identity %d","Action":"Action %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "cloud.SecurityGroupRule", "start_port": 32, "protocol": "Protocol 1", "end_port": 32, "index": 32, "name": "Name 1", "description": "Description 1", "ether_type": "EtherType 1", "source_security_group": "SourceSecurityGroup 1", "class_id": "cloud.SecurityGroupRule", "identity": "Identity 1", "action": "Action 1"}, {"object_type": "cloud.SecurityGroupRule", "start_port": 32, "protocol": "Protocol 2", "end_port": 32, "index": 32, "name": "Name 2", "description": "Description 2", "ether_type": "EtherType 2", "source_security_group": "SourceSecurityGroup 2", "class_id": "cloud.SecurityGroupRule", "identity": "Identity 2", "action": "Action 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudTfcWorkspaceVariables(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Hcl":true,"Sensitive":true,"Identity":"Identity %d","Value":"Value %d","ClassId":"cloud.TfcWorkspaceVariables","ObjectType":"cloud.TfcWorkspaceVariables","Key":"Key %d","Description":"Description %d","Category":"Category %d"}`
	...
	expectedOp := []map[string]interface{}{{"hcl": true, "sensitive": true, "identity": "Identity 1", "value": "Value 1", "class_id": "cloud.TfcWorkspaceVariables", "object_type": "cloud.TfcWorkspaceVariables", "key": "Key 1", "description": "Description 1", "category": "Category 1"}, {"hcl": true, "sensitive": true, "identity": "Identity 2", "value": "Value 2", "class_id": "cloud.TfcWorkspaceVariables", "object_type": "cloud.TfcWorkspaceVariables", "key": "Key 2", "description": "Description 2", "category": "Category 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IsRoot":true,"ClassId":"cloud.VolumeAttachment","ObjectType":"cloud.VolumeAttachment","AutoDelete":true,"DeviceName":"DeviceName %d","Identity":"Identity %d","Index":32}`
	...
	expectedOp := []map[string]interface{}{{"is_root": true, "class_id": "cloud.VolumeAttachment", "object_type": "cloud.VolumeAttachment", "auto_delete": true, "device_name": "DeviceName 1", "identity": "Identity 1", "index": 32}, {"is_root": true, "class_id": "cloud.VolumeAttachment", "object_type": "cloud.VolumeAttachment", "auto_delete": true, "device_name": "DeviceName 2", "identity": "Identity 2", "index": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeInstanceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DeviceName":"DeviceName %d","ClassId":"cloud.VolumeInstanceAttachment","ObjectType":"cloud.VolumeInstanceAttachment","InstanceId":"InstanceId %d","State":"State %d","AutoDelete":true}`
	...
	expectedOp := []map[string]interface{}{{"device_name": "DeviceName 1", "class_id": "cloud.VolumeInstanceAttachment", "object_type": "cloud.VolumeInstanceAttachment", "instance_id": "InstanceId 1", "state": "State 1", "auto_delete": true}, {"device_name": "DeviceName 2", "class_id": "cloud.VolumeInstanceAttachment", "object_type": "cloud.VolumeInstanceAttachment", "instance_id": "InstanceId 2", "state": "State 2", "auto_delete": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeIpAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"HttpPort":32,"ClassId":"compute.IpAddress","KvmPort":32,"KvmVlan":32,"Dn":"Dn %d","DefaultGateway":"DefaultGateway %d","HttpsPort":32,"ObjectType":"compute.IpAddress","Subnet":"Subnet %d","Category":"Category %d","Address":"Address %d","Name":"Name %d","Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"http_port": 32, "class_id": "compute.IpAddress", "kvm_port": 32, "kvm_vlan": 32, "dn": "Dn 1", "default_gateway": "DefaultGateway 1", "https_port": 32, "object_type": "compute.IpAddress", "subnet": "Subnet 1", "category": "Category 1", "address": "Address 1", "name": "Name 1", "type": "Type 1"}, {"http_port": 32, "class_id": "compute.IpAddress", "kvm_port": 32, "kvm_vlan": 32, "dn": "Dn 2", "default_gateway": "DefaultGateway 2", "https_port": 32, "object_type": "compute.IpAddress", "subnet": "Subnet 2", "category": "Category 2", "address": "Address 2", "name": "Name 2", "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListConnectorpackConnectorPackUpdate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"connectorpack.ConnectorPackUpdate","ObjectType":"connectorpack.ConnectorPackUpdate","Name":"Name %d","NewVersion":"NewVersion %d","CurrentVersion":"CurrentVersion %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "connectorpack.ConnectorPackUpdate", "object_type": "connectorpack.ConnectorPackUpdate", "name": "Name 1", "new_version": "NewVersion 1", "current_version": "CurrentVersion 1"}, {"class_id": "connectorpack.ConnectorPackUpdate", "object_type": "connectorpack.ConnectorPackUpdate", "name": "Name 2", "new_version": "NewVersion 2", "current_version": "CurrentVersion 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListContentParameter(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AcceptSingleValue":true,"ComplexType":"ComplexType %d","ClassId":"content.Parameter","Path":"Path %d","Type":"Type %d","Name":"Name %d","ObjectType":"content.Parameter","ItemType":"ItemType %d","Secure":true}`
	...
	expectedOp := []map[string]interface{}{{"accept_single_value": true, "complex_type": "ComplexType 1", "class_id": "content.Parameter", "path": "Path 1", "type": "Type 1", "name": "Name 1", "object_type": "content.Parameter", "item_type": "ItemType 1", "secure": true}, {"accept_single_value": true, "complex_type": "ComplexType 2", "class_id": "content.Parameter", "path": "Path 2", "type": "Type 2", "name": "Name 2", "object_type": "content.Parameter", "item_type": "ItemType 2", "secure": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListEquipmentIoCardIdentity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"equipment.IoCardIdentity","ObjectType":"equipment.IoCardIdentity","ModuleId":32,"NetworkElementMoid":"NetworkElementMoid %d","SwitchId":32,"IoCardMoid":"IoCardMoid %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "equipment.IoCardIdentity", "object_type": "equipment.IoCardIdentity", "module_id": 32, "network_element_moid": "NetworkElementMoid 1", "switch_id": 32, "io_card_moid": "IoCardMoid 1"}, {"class_id": "equipment.IoCardIdentity", "object_type": "equipment.IoCardIdentity", "module_id": 32, "network_element_moid": "NetworkElementMoid 2", "switch_id": 32, "io_card_moid": "IoCardMoid 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricQosClass(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PacketDrop":true,"Mtu":32,"AdminState":"AdminState %d","MulticastOptimize":true,"ObjectType":"fabric.QosClass","BandwidthPercent":32,"Weight":32,"Cos":32,"Name":"Name %d","ClassId":"fabric.QosClass"}`
	...
	expectedOp := []map[string]interface{}{{"packet_drop": true, "mtu": 32, "admin_state": "AdminState 1", "multicast_optimize": true, "object_type": "fabric.QosClass", "bandwidth_percent": 32, "weight": 32, "cos": 32, "name": "Name 1", "class_id": "fabric.QosClass"}, {"packet_drop": true, "mtu": 32, "admin_state": "AdminState 2", "multicast_optimize": true, "object_type": "fabric.QosClass", "bandwidth_percent": 32, "weight": 32, "cos": 32, "name": "Name 2", "class_id": "fabric.QosClass"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFcpoolBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"fcpool.Block","Size":32,"From":"From %d","To":"To %d","ClassId":"fcpool.Block"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "fcpool.Block", "size": 32, "from": "From 1", "to": "To 1", "class_id": "fcpool.Block"}, {"object_type": "fcpool.Block", "size": 32, "from": "From 2", "to": "To 2", "class_id": "fcpool.Block"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareBaseImpact(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ImpactType":"ImpactType %d","Model":"Model %d","ComputationStatusDetail":"ComputationStatusDetail %d","ClassId":"firmware.BaseImpact","ObjectType":"firmware.BaseImpact","VersionImpact":"VersionImpact %d","EndPoint":"EndPoint %d","ComputationError":"ComputationError %d","DomainName":"DomainName %d","FirmwareVersion":"FirmwareVersion %d","TargetFirmwareVersion":"TargetFirmwareVersion %d"}`
	...
	expectedOp := []map[string]interface{}{{"impact_type": "ImpactType 1", "model": "Model 1", "computation_status_detail": "ComputationStatusDetail 1", "class_id": "firmware.BaseImpact", "object_type": "firmware.BaseImpact", "version_impact": "VersionImpact 1", "end_point": "EndPoint 1", "computation_error": "ComputationError 1", "domain_name": "DomainName 1", "firmware_version": "FirmwareVersion 1", "target_firmware_version": "TargetFirmwareVersion 1"}, {"impact_type": "ImpactType 2", "model": "Model 2", "computation_status_detail": "ComputationStatusDetail 2", "class_id": "firmware.BaseImpact", "object_type": "firmware.BaseImpact", "version_impact": "VersionImpact 2", "end_point": "EndPoint 2", "computation_error": "ComputationError 2", "domain_name": "DomainName 2", "firmware_version": "FirmwareVersion 2", "target_firmware_version": "TargetFirmwareVersion 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareComponentMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Vendor":"Vendor %d","RedfishUrl":"RedfishUrl %d","ComponentLabel":"ComponentLabel %d","ComponentType":"ComponentType %d","ClassId":"firmware.ComponentMeta","IsOobSupported":true,"Model":"Model %d","ImagePath":"ImagePath %d","Description":"Description %d","Disruption":"Disruption %d","ObjectType":"firmware.ComponentMeta","PackedVersion":"PackedVersion %d"}`
	...
	expectedOp := []map[string]interface{}{{"vendor": "Vendor 1", "redfish_url": "RedfishUrl 1", "component_label": "ComponentLabel 1", "component_type": "ComponentType 1", "class_id": "firmware.ComponentMeta", "is_oob_supported": true, "model": "Model 1", "image_path": "ImagePath 1", "description": "Description 1", "disruption": "Disruption 1", "object_type": "firmware.ComponentMeta", "packed_version": "PackedVersion 1"}, {"vendor": "Vendor 2", "redfish_url": "RedfishUrl 2", "component_label": "ComponentLabel 2", "component_type": "ComponentType 2", "class_id": "firmware.ComponentMeta", "is_oob_supported": true, "model": "Model 2", "image_path": "ImagePath 2", "description": "Description 2", "disruption": "Disruption 2", "object_type": "firmware.ComponentMeta", "packed_version": "PackedVersion 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareFirmwareInventory(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"firmware.FirmwareInventory","Category":"Category %d","ObjectType":"firmware.FirmwareInventory","Label":"Label %d","Model":"Model %d","UpdateUri":"UpdateUri %d","Vendor":"Vendor %d","Version":"Version %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "firmware.FirmwareInventory", "category": "Category 1", "object_type": "firmware.FirmwareInventory", "label": "Label 1", "model": "Model 1", "update_uri": "UpdateUri 1", "vendor": "Vendor 1", "nr_version": "Version 1"}, {"class_id": "firmware.FirmwareInventory", "category": "Category 2", "object_type": "firmware.FirmwareInventory", "label": "Label 2", "model": "Model 2", "update_uri": "UpdateUri 2", "vendor": "Vendor 2", "nr_version": "Version 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHealthCheckScriptInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptName":"ScriptName %d","HyperflexVersion":"HyperflexVersion %d","ClassId":"hyperflex.HealthCheckScriptInfo","ScriptInput":"ScriptInput %d","Version":"Version %d","AggregateScriptName":"AggregateScriptName %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "hyperflex.HealthCheckScriptInfo", "script_execute_location": "ScriptExecuteLocation 1", "script_name": "ScriptName 1", "hyperflex_version": "HyperflexVersion 1", "class_id": "hyperflex.HealthCheckScriptInfo", "script_input": "ScriptInput 1", "nr_version": "Version 1", "aggregate_script_name": "AggregateScriptName 1"}, {"object_type": "hyperflex.HealthCheckScriptInfo", "script_execute_location": "ScriptExecuteLocation 2", "script_name": "ScriptName 2", "hyperflex_version": "HyperflexVersion 2", "class_id": "hyperflex.HealthCheckScriptInfo", "script_input": "ScriptInput 2", "nr_version": "Version 2", "aggregate_script_name": "AggregateScriptName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHxHostMountStatusDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Reason":"Reason %d","Accessibility":"Accessibility %d","ClassId":"hyperflex.HxHostMountStatusDt","ObjectType":"hyperflex.HxHostMountStatusDt","HostName":"HostName %d","Mounted":true}`
...
func TestFlattenListHyperflexIpAddrRange(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.IpAddrRange","ObjectType":"hyperflex.IpAddrRange","Netmask":"Netmask %d","StartAddr":"StartAddr %d","EndAddr":"EndAddr %d","Gateway":"Gateway %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "hyperflex.IpAddrRange", "object_type": "hyperflex.IpAddrRange", "netmask": "Netmask 1", "start_addr": "StartAddr 1", "end_addr": "EndAddr 1", "gateway": "Gateway 1"}, {"class_id": "hyperflex.IpAddrRange", "object_type": "hyperflex.IpAddrRange", "netmask": "Netmask 2", "start_addr": "StartAddr 2", "end_addr": "EndAddr 2", "gateway": "Gateway 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexNetworkPort(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","ClassId":"hyperflex.NetworkPort","ObjectType":"hyperflex.NetworkPort","PortType":"PortType %d","Vlans":"Vlans %d"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "class_id": "hyperflex.NetworkPort", "object_type": "hyperflex.NetworkPort", "port_type": "PortType 1", "vlans": "Vlans 1"}, {"name": "Name 2", "class_id": "hyperflex.NetworkPort", "object_type": "hyperflex.NetworkPort", "port_type": "PortType 2", "vlans": "Vlans 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexVmDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"BootOrder":32,"ClassId":"hyperflex.VmDisk","ObjectType":"hyperflex.VmDisk","Name":"Name %d","Type":"Type %d","VirtualDiskReference":"VirtualDiskReference %d","Bus":"Bus %d"}`
	...
	expectedOp := []map[string]interface{}{{"boot_order": 32, "class_id": "hyperflex.VmDisk", "object_type": "hyperflex.VmDisk", "name": "Name 1", "type": "Type 1", "virtual_disk_reference": "VirtualDiskReference 1", "bus": "Bus 1"}, {"boot_order": 32, "class_id": "hyperflex.VmDisk", "object_type": "hyperflex.VmDisk", "name": "Name 2", "type": "Type 2", "virtual_disk_reference": "VirtualDiskReference 2", "bus": "Bus 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexVmInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Bridge":"Bridge %d","ClassId":"hyperflex.VmInterface","ObjectType":"hyperflex.VmInterface","MacAddress":"MacAddress %d","Model":"Model %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"bridge": "Bridge 1", "class_id": "hyperflex.VmInterface", "object_type": "hyperflex.VmInterface", "mac_address": "MacAddress 1", "model": "Model 1", "name": "Name 1"}, {"bridge": "Bridge 2", "class_id": "hyperflex.VmInterface", "object_type": "hyperflex.VmInterface", "mac_address": "MacAddress 2", "model": "Model 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseUtilizationInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Label":"Label %d","LicensedLimit":"LicensedLimit %d","Sku":"Sku %d","ClassId":"iaas.LicenseUtilizationInfo","ObjectType":"iaas.LicenseUtilizationInfo","ActualUsed":32}`
	...
	expectedOp := []map[string]interface{}{{"label": "Label 1", "licensed_limit": "LicensedLimit 1", "sku": "Sku 1", "class_id": "iaas.LicenseUtilizationInfo", "object_type": "iaas.LicenseUtilizationInfo", "actual_used": 32}, {"label": "Label 2", "licensed_limit": "LicensedLimit 2", "sku": "Sku 2", "class_id": "iaas.LicenseUtilizationInfo", "object_type": "iaas.LicenseUtilizationInfo", "actual_used": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMacpoolBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"To":"To %d","Size":32,"ClassId":"macpool.Block","ObjectType":"macpool.Block","From":"From %d"}`
	...
	expectedOp := []map[string]interface{}{{"to": "To 1", "size": 32, "class_id": "macpool.Block", "object_type": "macpool.Block", "from": "From 1"}, {"to": "To 2", "size": 32, "class_id": "macpool.Block", "object_type": "macpool.Block", "from": "From 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMemoryPersistentMemoryLogicalNamespace(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SocketId":32,"SocketMemoryId":"SocketMemoryId %d","ClassId":"memory.PersistentMemoryLogicalNamespace","ObjectType":"memory.PersistentMemoryLogicalNamespace","Capacity":32,"Mode":"Mode %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"socket_id": 32, "socket_memory_id": "SocketMemoryId 1", "class_id": "memory.PersistentMemoryLogicalNamespace", "object_type": "memory.PersistentMemoryLogicalNamespace", "capacity": 32, "mode": "Mode 1", "name": "Name 1"}, {"socket_id": 32, "socket_memory_id": "SocketMemoryId 2", "class_id": "memory.PersistentMemoryLogicalNamespace", "object_type": "memory.PersistentMemoryLogicalNamespace", "capacity": 32, "mode": "Mode 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaPropDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SearchWeight":32.000000,"ClassId":"meta.PropDefinition","ObjectType":"meta.PropDefinition","ApiAccess":"ApiAccess %d","Name":"Name %d","OpSecurity":"OpSecurity %d"}`
	...
	expectedOp := []map[string]interface{}{{"search_weight": 32.000000, "class_id": "meta.PropDefinition", "object_type": "meta.PropDefinition", "api_access": "ApiAccess 1", "name": "Name 1", "op_security": "OpSecurity 1"}, {"search_weight": 32.000000, "class_id": "meta.PropDefinition", "object_type": "meta.PropDefinition", "api_access": "ApiAccess 2", "name": "Name 2", "op_security": "OpSecurity 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaRelationshipDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","ExportWithPeer":true,"ClassId":"meta.RelationshipDefinition","PeerRelName":"PeerRelName %d","PeerSync":true,"ApiAccess":"ApiAccess %d","Collection":true,"Export":true,"Name":"Name %d","ObjectType":"meta.RelationshipDefinition"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "export_with_peer": true, "class_id": "meta.RelationshipDefinition", "peer_rel_name": "PeerRelName 1", "peer_sync": true, "api_access": "ApiAccess 1", "collection": true, "export": true, "name": "Name 1", "object_type": "meta.RelationshipDefinition"}, {"type": "Type 2", "export_with_peer": true, "class_id": "meta.RelationshipDefinition", "peer_rel_name": "PeerRelName 2", "peer_sync": true, "api_access": "ApiAccess 2", "collection": true, "export": true, "name": "Name 2", "object_type": "meta.RelationshipDefinition"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiaapiDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niaapi.Detail","ObjectType":"niaapi.Detail","Name":"Name %d","Chksum":"Chksum %d","Filename":"Filename %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "niaapi.Detail", "object_type": "niaapi.Detail", "name": "Name 1", "chksum": "Chksum 1", "filename": "Filename 1"}, {"class_id": "niaapi.Detail", "object_type": "niaapi.Detail", "name": "Name 2", "chksum": "Chksum 2", "filename": "Filename 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryLogicalLink(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LinkAddr1":"LinkAddr1 %d","LinkType":"LinkType %d","IsPresent":true,"ClassId":"niatelemetry.LogicalLink","LinkState":"LinkState %d","DbId":32,"Uptime":"Uptime %d","LinkAddr2":"LinkAddr2 %d","ObjectType":"niatelemetry.LogicalLink"}`
	...
	expectedOp := []map[string]interface{}{{"link_addr1": "LinkAddr1 1", "link_type": "LinkType 1", "is_present": true, "class_id": "niatelemetry.LogicalLink", "link_state": "LinkState 1", "db_id": 32, "uptime": "Uptime 1", "link_addr2": "LinkAddr2 1", "object_type": "niatelemetry.LogicalLink"}, {"link_addr1": "LinkAddr1 2", "link_type": "LinkType 2", "is_present": true, "class_id": "niatelemetry.LogicalLink", "link_state": "LinkState 2", "db_id": 32, "uptime": "Uptime 2", "link_addr2": "LinkAddr2 2", "object_type": "niatelemetry.LogicalLink"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNtpAuthNtpServer(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"ntp.AuthNtpServer","ServerName":"ServerName %d","SymKeyId":32,"SymKeyValue":"SymKeyValue %d","KeyType":"KeyType %d","ClassId":"ntp.AuthNtpServer"}`
...
func TestFlattenListOnpremImagePackage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":"Version %d","ObjectType":"onprem.ImagePackage","FileSize":32,"Filename":"Filename %d","Name":"Name %d","PackageType":"PackageType %d","FileSha":"FileSha %d","FilePath":"FilePath %d","ClassId":"onprem.ImagePackage"}`
	...
	expectedOp := []map[string]interface{}{{"nr_version": "Version 1", "object_type": "onprem.ImagePackage", "file_size": 32, "filename": "Filename 1", "name": "Name 1", "package_type": "PackageType 1", "file_sha": "FileSha 1", "file_path": "FilePath 1", "class_id": "onprem.ImagePackage"}, {"nr_version": "Version 2", "object_type": "onprem.ImagePackage", "file_size": 32, "filename": "Filename 2", "name": "Name 2", "package_type": "PackageType 2", "file_sha": "FileSha 2", "file_path": "FilePath 2", "class_id": "onprem.ImagePackage"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOnpremUpgradePhase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"onprem.UpgradePhase","ElapsedTime":32,"Failed":true,"ClassId":"onprem.UpgradePhase","Message":"Message %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "onprem.UpgradePhase", "elapsed_time": 32, "failed": true, "class_id": "onprem.UpgradePhase", "message": "Message 1", "name": "Name 1"}, {"object_type": "onprem.UpgradePhase", "elapsed_time": 32, "failed": true, "class_id": "onprem.UpgradePhase", "message": "Message 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOsServerConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"InstallTarget":"InstallTarget %d","SerialNumber":"SerialNumber %d","ClassId":"os.ServerConfig","ObjectType":"os.ServerConfig"}`
	...
	expectedOp := []map[string]interface{}{{"install_target": "InstallTarget 1", "serial_number": "SerialNumber 1", "class_id": "os.ServerConfig", "object_type": "os.ServerConfig"}, {"install_target": "InstallTarget 2", "serial_number": "SerialNumber 2", "class_id": "os.ServerConfig", "object_type": "os.ServerConfig"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOsValidationInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"os.ValidationInformation","ClassId":"os.ValidationInformation","ErrorMsg":"ErrorMsg %d","Status":"Status %d","StepName":"StepName %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "os.ValidationInformation", "class_id": "os.ValidationInformation", "error_msg": "ErrorMsg 1", "status": "Status 1", "step_name": "StepName 1"}, {"object_type": "os.ValidationInformation", "class_id": "os.ValidationInformation", "error_msg": "ErrorMsg 2", "status": "Status 2", "step_name": "StepName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListPolicyinventoryJobInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"policyinventory.JobInfo","PolicyId":"PolicyId %d","PolicyName":"PolicyName %d","ExecutionStatus":"ExecutionStatus %d","ClassId":"policyinventory.JobInfo"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "policyinventory.JobInfo", "policy_id": "PolicyId 1", "policy_name": "PolicyName 1", "execution_status": "ExecutionStatus 1", "class_id": "policyinventory.JobInfo"}, {"object_type": "policyinventory.JobInfo", "policy_id": "PolicyId 2", "policy_name": "PolicyName 2", "execution_status": "ExecutionStatus 2", "class_id": "policyinventory.JobInfo"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSdwanTemplateInputsType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Title":"Title %d","Required":true,"Key":"Key %d","ClassId":"sdwan.TemplateInputsType","ObjectType":"sdwan.TemplateInputsType","Template":"Template %d","Value":"Value %d","Type":"Type %d","Editable":true}`
	...
	expectedOp := []map[string]interface{}{{"title": "Title 1", "required": true, "key": "Key 1", "class_id": "sdwan.TemplateInputsType", "object_type": "sdwan.TemplateInputsType", "template": "Template 1", "value": "Value 1", "type": "Type 1", "editable": true}, {"title": "Title 2", "required": true, "key": "Key 2", "class_id": "sdwan.TemplateInputsType", "object_type": "sdwan.TemplateInputsType", "template": "Template 2", "value": "Value 2", "type": "Type 2", "editable": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpTrap(t *testing.T) {
	p := []models.SnmpTrap{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"snmp.Trap","Destination":"Destination %d","Version":"Version %d","User":"User %d","Community":"Community %d","ClassId":"snmp.Trap","Type":"Type %d","Enabled":true,"Port":32}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "snmp.Trap", "destination": "Destination 1", "nr_version": "Version 1", "user": "User 1", "community": "Community 1", "class_id": "snmp.Trap", "type": "Type 1", "enabled": true, "port": 32}, {"object_type": "snmp.Trap", "destination": "Destination 2", "nr_version": "Version 2", "user": "User 2", "community": "Community 2", "class_id": "snmp.Trap", "type": "Type 2", "enabled": true, "port": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpUser(t *testing.T) {
	p := []models.SnmpUser{}
	var d = &schema.ResourceData{}
	c := `{"SecurityLevel":"SecurityLevel %d","AuthType":"AuthType %d","Name":"Name %d","PrivacyType":"PrivacyType %d","IsPrivacyPasswordSet":true,"ClassId":"snmp.User","ObjectType":"snmp.User","IsAuthPasswordSet":true}`
	...
	expectedOp := []map[string]interface{}{{"security_level": "SecurityLevel 1", "auth_type": "AuthType 1", "name": "Name 1", "privacy_type": "PrivacyType 1", "is_privacy_password_set": true, "class_id": "snmp.User", "object_type": "snmp.User", "is_auth_password_set": true}, {"security_level": "SecurityLevel 2", "auth_type": "AuthType 2", "name": "Name 2", "privacy_type": "PrivacyType 2", "is_privacy_password_set": true, "class_id": "snmp.User", "object_type": "snmp.User", "is_auth_password_set": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStorageBaseInitiator(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.BaseInitiator","Wwn":"Wwn %d","Iqn":"Iqn %d","Name":"Name %d","Type":"Type %d","ObjectType":"storage.BaseInitiator"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "storage.BaseInitiator", "wwn": "Wwn 1", "iqn": "Iqn 1", "name": "Name 1", "type": "Type 1", "object_type": "storage.BaseInitiator"}, {"class_id": "storage.BaseInitiator", "wwn": "Wwn 2", "iqn": "Iqn 2", "name": "Name 2", "type": "Type 2", "object_type": "storage.BaseInitiator"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStorageVirtualDriveConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ExpandToAvailable":true,"Name":"Name %d","ClassId":"storage.VirtualDriveConfiguration","ObjectType":"storage.VirtualDriveConfiguration","Size":32,"BootDrive":true}`
...
func TestFlattenListTamAction(t *testing.T) {
	p := []models.TamAction{}
	var d = &schema.ResourceData{}
	c := `{"AlertType":"AlertType %d","OperationType":"OperationType %d","AffectedObjectType":"AffectedObjectType %d","Name":"Name %d","Type":"Type %d","ClassId":"tam.Action","ObjectType":"tam.Action"}`
	...
	expectedOp := []map[string]interface{}{{"alert_type": "AlertType 1", "operation_type": "OperationType 1", "affected_object_type": "AffectedObjectType 1", "name": "Name 1", "type": "Type 1", "class_id": "tam.Action", "object_type": "tam.Action"}, {"alert_type": "AlertType 2", "operation_type": "OperationType 2", "affected_object_type": "AffectedObjectType 2", "name": "Name 2", "type": "Type 2", "class_id": "tam.Action", "object_type": "tam.Action"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTamApiDataSource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","MoType":"MoType %d","Type":"Type %d","ClassId":"tam.ApiDataSource","ObjectType":"tam.ApiDataSource"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "mo_type": "MoType 1", "type": "Type 1", "class_id": "tam.ApiDataSource", "object_type": "tam.ApiDataSource"}, {"name": "Name 2", "mo_type": "MoType 2", "type": "Type 2", "class_id": "tam.ApiDataSource", "object_type": "tam.ApiDataSource"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUcsdConnectorPack(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"ucsd.ConnectorPack","Name":"Name %d","Version":"Version %d","DownloadedVersion":"DownloadedVersion %d","ClassId":"ucsd.ConnectorPack","ConnectorFeature":"ConnectorFeature %d","State":"State %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "ucsd.ConnectorPack", "name": "Name 1", "nr_version": "Version 1", "downloaded_version": "DownloadedVersion 1", "class_id": "ucsd.ConnectorPack", "connector_feature": "ConnectorFeature 1", "state": "State 1"}, {"object_type": "ucsd.ConnectorPack", "name": "Name 2", "nr_version": "Version 2", "downloaded_version": "DownloadedVersion 2", "class_id": "ucsd.ConnectorPack", "connector_feature": "ConnectorFeature 2", "state": "State 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationNetworkInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.NetworkInterface","AdaptorType":"AdaptorType %d","MacAddress":"MacAddress %d","ClassId":"virtualization.NetworkInterface","Name":"Name %d","Bridge":"Bridge %d","ConnectAtPowerOn":true,"DirectPathIo":true}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "virtualization.NetworkInterface", "adaptor_type": "AdaptorType 1", "mac_address": "MacAddress 1", "class_id": "virtualization.NetworkInterface", "name": "Name 1", "bridge": "Bridge 1", "connect_at_power_on": true, "direct_path_io": true}, {"object_type": "virtualization.NetworkInterface", "adaptor_type": "AdaptorType 2", "mac_address": "MacAddress 2", "class_id": "virtualization.NetworkInterface", "name": "Name 2", "bridge": "Bridge 2", "connect_at_power_on": true, "direct_path_io": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVmediaMapping(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vmedia.Mapping","AuthenticationProtocol":"AuthenticationProtocol %d","Username":"Username %d","FileLocation":"FileLocation %d","DeviceType":"DeviceType %d","ObjectType":"vmedia.Mapping","IsPasswordSet":true,"HostName":"HostName %d","RemoteFile":"RemoteFile %d","SanitizedFileLocation":"SanitizedFileLocation %d","MountOptions":"MountOptions %d","MountProtocol":"MountProtocol %d","VolumeName":"VolumeName %d","RemotePath":"RemotePath %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "vmedia.Mapping", "authentication_protocol": "AuthenticationProtocol 1", "username": "Username 1", "file_location": "FileLocation 1", "device_type": "DeviceType 1", "object_type": "vmedia.Mapping", "is_password_set": true, "host_name": "HostName 1", "remote_file": "RemoteFile 1", "sanitized_file_location": "SanitizedFileLocation 1", "mount_options": "MountOptions 1", "mount_protocol": "MountProtocol 1", "volume_name": "VolumeName 1", "remote_path": "RemotePath 1"}, {"class_id": "vmedia.Mapping", "authentication_protocol": "AuthenticationProtocol 2", "username": "Username 2", "file_location": "FileLocation 2", "device_type": "DeviceType 2", "object_type": "vmedia.Mapping", "is_password_set": true, "host_name": "HostName 2", "remote_file": "RemoteFile 2", "sanitized_file_location": "SanitizedFileLocation 2", "mount_options": "MountOptions 2", "mount_protocol": "MountProtocol 2", "volume_name": "VolumeName 2", "remote_path": "RemotePath 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowApi(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ErrorContentType":"ErrorContentType %d","Timeout":32,"Name":"Name %d","Body":"Body %d","ContentType":"ContentType %d","StartDelay":32,"ObjectType":"workflow.Api","Label":"Label %d","SkipOnCondition":"SkipOnCondition %d","AssetTargetMoid":"AssetTargetMoid %d","Description":"Description %d","ClassId":"workflow.Api"}`
	...
	expectedOp := []map[string]interface{}{{"error_content_type": "ErrorContentType 1", "timeout": 32, "name": "Name 1", "body": "Body 1", "content_type": "ContentType 1", "start_delay": 32, "object_type": "workflow.Api", "label": "Label 1", "skip_on_condition": "SkipOnCondition 1", "asset_target_moid": "AssetTargetMoid 1", "description": "Description 1", "class_id": "workflow.Api"}, {"error_content_type": "ErrorContentType 2", "timeout": 32, "name": "Name 2", "body": "Body 2", "content_type": "ContentType 2", "start_delay": 32, "object_type": "workflow.Api", "label": "Label 2", "skip_on_condition": "SkipOnCondition 2", "asset_target_moid": "AssetTargetMoid 2", "description": "Description 2", "class_id": "workflow.Api"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowBaseDataType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"workflow.BaseDataType","Name":"Name %d","Label":"Label %d","ClassId":"workflow.BaseDataType","Required":true,"Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "workflow.BaseDataType", "name": "Name 1", "label": "Label 1", "class_id": "workflow.BaseDataType", "required": true, "description": "Description 1"}, {"object_type": "workflow.BaseDataType", "name": "Name 2", "label": "Label 2", "class_id": "workflow.BaseDataType", "required": true, "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowParameterSet(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.ParameterSet","ObjectType":"workflow.ParameterSet","Condition":"Condition %d","ControlParameter":"ControlParameter %d","Name":"Name %d","Value":"Value %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.ParameterSet", "object_type": "workflow.ParameterSet", "condition": "Condition 1", "control_parameter": "ControlParameter 1", "name": "Name 1", "value": "Value 1"}, {"class_id": "workflow.ParameterSet", "object_type": "workflow.ParameterSet", "condition": "Condition 2", "control_parameter": "ControlParameter 2", "name": "Name 2", "value": "Value 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"workflow.RollbackTask","TaskMoid":"TaskMoid %d","Version":32,"ClassId":"workflow.RollbackTask","CatalogMoid":"CatalogMoid %d","Description":"Description %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "workflow.RollbackTask", "task_moid": "TaskMoid 1", "nr_version": 32, "class_id": "workflow.RollbackTask", "catalog_moid": "CatalogMoid 1", "description": "Description 1", "name": "Name 1"}, {"object_type": "workflow.RollbackTask", "task_moid": "TaskMoid 2", "nr_version": 32, "class_id": "workflow.RollbackTask", "catalog_moid": "CatalogMoid 2", "description": "Description 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackWorkflowTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RollbackCompleted":true,"Description":"Description %d","RefName":"RefName %d","TaskInfoMoid":"TaskInfoMoid %d","Status":"Status %d","ObjectType":"workflow.RollbackWorkflowTask","TaskPath":"TaskPath %d","RollbackTaskName":"RollbackTaskName %d","Name":"Name %d","ClassId":"workflow.RollbackWorkflowTask"}`
	...
	expectedOp := []map[string]interface{}{{"rollback_completed": true, "description": "Description 1", "ref_name": "RefName 1", "task_info_moid": "TaskInfoMoid 1", "status": "Status 1", "object_type": "workflow.RollbackWorkflowTask", "task_path": "TaskPath 1", "rollback_task_name": "RollbackTaskName 1", "name": "Name 1", "class_id": "workflow.RollbackWorkflowTask"}, {"rollback_completed": true, "description": "Description 2", "ref_name": "RefName 2", "task_info_moid": "TaskInfoMoid 2", "status": "Status 2", "object_type": "workflow.RollbackWorkflowTask", "task_path": "TaskPath 2", "rollback_task_name": "RollbackTaskName 2", "name": "Name 2", "class_id": "workflow.RollbackWorkflowTask"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListX509Certificate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"x509.Certificate","Sha256Fingerprint":"Sha256Fingerprint %d","ClassId":"x509.Certificate","PemCertificate":"PemCertificate %d","SignatureAlgorithm":"SignatureAlgorithm %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "x509.Certificate", "sha256_fingerprint": "Sha256Fingerprint 1", "class_id": "x509.Certificate", "pem_certificate": "PemCertificate 1", "signature_algorithm": "SignatureAlgorithm 1"}, {"object_type": "x509.Certificate", "sha256_fingerprint": "Sha256Fingerprint 2", "class_id": "x509.Certificate", "pem_certificate": "PemCertificate 2", "signature_algorithm": "SignatureAlgorithm 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenMapAssetDeploymentDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"OldDeviceStatusDescription":"OldDeviceStatusDescription %d","MlbProductId":32,"OldDeviceId":"OldDeviceId %d","Description":"Description %d","ClassId":"asset.DeploymentDeviceInformation","ObjectType":"asset.DeploymentDeviceInformation","ApplicationName":"ApplicationName %d","OldDeviceStatusId":32,"OldInstanceId":"OldInstanceId %d","InstanceId":"InstanceId %d","MlbProductName":"MlbProductName %d","ItemType":"ItemType %d"}`
	...
	expectedOp := map[string]interface{}{"old_device_status_description": "OldDeviceStatusDescription 1", "mlb_product_id": 32, "old_device_id": "OldDeviceId 1", "description": "Description 1", "class_id": "asset.DeploymentDeviceInformation", "object_type": "asset.DeploymentDeviceInformation", "application_name": "ApplicationName 1", "old_device_status_id": 32, "old_instance_id": "OldInstanceId 1", "instance_id": "InstanceId 1", "mlb_product_name": "MlbProductName 1", "item_type": "ItemType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"asset.DeviceInformation","ApplicationName":"ApplicationName %d","ProductFamily":"ProductFamily %d","OldDeviceStatusId":32,"InstanceId":"InstanceId %d","OldInstanceId":"OldInstanceId %d","OldDeviceStatusDescription":"OldDeviceStatusDescription %d","ProductType":"ProductType %d","MlbProductId":32,"OldDeviceId":"OldDeviceId %d","ItemType":"ItemType %d","MlbOfferType":"MlbOfferType %d","ObjectType":"asset.DeviceInformation","UnitOfMeasure":"UnitOfMeasure %d","MlbProductName":"MlbProductName %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "asset.DeviceInformation", "application_name": "ApplicationName 1", "product_family": "ProductFamily 1", "old_device_status_id": 32, "instance_id": "InstanceId 1", "old_instance_id": "OldInstanceId 1", "old_device_status_description": "OldDeviceStatusDescription 1", "product_type": "ProductType 1", "mlb_product_id": 32, "old_device_id": "OldDeviceId 1", "item_type": "ItemType 1", "mlb_offer_type": "MlbOfferType 1", "object_type": "asset.DeviceInformation", "unit_of_measure": "UnitOfMeasure 1", "mlb_product_name": "MlbProductName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceStatistics(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"asset.DeviceStatistics","MembershipRatio":32.000000,"MemoryMirroringFactor":32.000000,"ClusterName":"ClusterName %d","Connected":32,"ClassId":"asset.DeviceStatistics"}`
	...
	expectedOp := map[string]interface{}{"object_type": "asset.DeviceStatistics", "membership_ratio": 32.000000, "memory_mirroring_factor": 32.000000, "cluster_name": "ClusterName 1", "connected": 32, "class_id": "asset.DeviceStatistics"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetProductInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SubType":"SubType %d","ClassId":"asset.ProductInformation","ObjectType":"asset.ProductInformation","Family":"Family %d","Number":"Number %d","Description":"Description %d","Group":"Group %d"}`
	...
	expectedOp := map[string]interface{}{"sub_type": "SubType 1", "class_id": "asset.ProductInformation", "object_type": "asset.ProductInformation", "family": "Family 1", "number": "Number 1", "description": "Description 1", "group": "Group 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetSudiInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SerialNumber":"SerialNumber %d","ClassId":"asset.SudiInfo","ObjectType":"asset.SudiInfo","Signature":"Signature %d","Status":"Status %d","Pid":"Pid %d"}`
	...
	expectedOp := map[string]interface{}{"serial_number": "SerialNumber 1", "class_id": "asset.SudiInfo", "object_type": "asset.SudiInfo", "signature": "Signature 1", "status": "Status 1", "pid": "Pid 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCapabilitySwitchNetworkLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaxCompressedPortVlanCount":32,"MaximumActiveTrafficMonitoringSessions":32,"MaximumFcPortChannels":32,"MaximumIgmpGroups":32,"MaximumSecondaryVlanPerPrimary":32,"ObjectType":"capability.SwitchNetworkLimits","MaxUncompressedPortVlanCount":32,"MaximumEthernetUplinkPorts":32,"ClassId":"capability.SwitchNetworkLimits","MaximumVlans":32,"MaximumPortChannelMembers":32,"MaximumSecondaryVlan":32,"MaximumVifs":32,"MaximumFcPortChannelMembers":32,"MinimumActiveFans":32,"MaximumEthernetPortChannels":32,"MaximumPrimaryVlan":32}`
	...
	expectedOp := map[string]interface{}{"max_compressed_port_vlan_count": 32, "maximum_active_traffic_monitoring_sessions": 32, "maximum_fc_port_channels": 32, "maximum_igmp_groups": 32, "maximum_secondary_vlan_per_primary": 32, "object_type": "capability.SwitchNetworkLimits", "max_uncompressed_port_vlan_count": 32, "maximum_ethernet_uplink_ports": 32, "class_id": "capability.SwitchNetworkLimits", "maximum_vlans": 32, "maximum_port_channel_members": 32, "maximum_secondary_vlan": 32, "maximum_vifs": 32, "maximum_fc_port_channel_members": 32, "minimum_active_fans": 32, "maximum_ethernet_port_channels": 32, "maximum_primary_vlan": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchStorageLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaximumVirtualFcInterfacesPerBladeServer":32,"MaximumVsans":32,"MaximumZoneCount":32,"ClassId":"capability.SwitchStorageLimits","ObjectType":"capability.SwitchStorageLimits","MaximumUserZoneCount":32,"MaximumVirtualFcInterfaces":32}`
	...
	expectedOp := map[string]interface{}{"maximum_virtual_fc_interfaces_per_blade_server": 32, "maximum_vsans": 32, "maximum_zone_count": 32, "class_id": "capability.SwitchStorageLimits", "object_type": "capability.SwitchStorageLimits", "maximum_user_zone_count": 32, "maximum_virtual_fc_interfaces": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCertificatemanagementCertificateBase(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"enabled": true, "class_id": "certificatemanagement.CertificateBase", "object_type": "certificatemanagement.CertificateBase", "is_privatekey_set": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudInstanceType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"cloud.InstanceType","ObjectType":"cloud.InstanceType","Architecture":"Architecture %d","InstanceTypeId":"InstanceTypeId %d","Memory":32,"Name":"Name %d","CpuSpeed":32,"Platform":"Platform %d","Cpus":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "cloud.InstanceType", "object_type": "cloud.InstanceType", "architecture": "Architecture 1", "instance_type_id": "InstanceTypeId 1", "memory": 32, "name": "Name 1", "cpu_speed": 32, "platform": "Platform 1", "cpus": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudVolumeIopsInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.VolumeIopsInfo","ThroughputWriteLimit":32,"IopsReadLimit":32,"IopsWriteLimit":32,"ThroughputReadLimit":32,"ClassId":"cloud.VolumeIopsInfo"}`
	...
	expectedOp := map[string]interface{}{"object_type": "cloud.VolumeIopsInfo", "throughput_write_limit": 32, "iops_read_limit": 32, "iops_write_limit": 32, "throughput_read_limit": 32, "class_id": "cloud.VolumeIopsInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapComputePersistentMemoryOperation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"compute.PersistentMemoryOperation","AdminAction":"AdminAction %d","IsSecurePassphraseSet":true,"ClassId":"compute.PersistentMemoryOperation"}`
	...
	expectedOp := map[string]interface{}{"object_type": "compute.PersistentMemoryOperation", "admin_action": "AdminAction 1", "is_secure_passphrase_set": true, "class_id": "compute.PersistentMemoryOperation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFabricMacAgingSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MacAgingTime":32,"MacAgingOption":"MacAgingOption %d","ObjectType":"fabric.MacAgingSettings","ClassId":"fabric.MacAgingSettings"}`
	...
	expectedOp := map[string]interface{}{"mac_aging_time": 32, "mac_aging_option": "MacAgingOption 1", "object_type": "fabric.MacAgingSettings", "class_id": "fabric.MacAgingSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFcpoolBlock(t *testing.T) {
	p := models.FcpoolBlock{}
	var d = &schema.ResourceData{}
	c := `{"To":"To %d","ClassId":"fcpool.Block","ObjectType":"fcpool.Block","Size":32,"From":"From %d"}`
	...
	expectedOp := map[string]interface{}{"to": "To 1", "class_id": "fcpool.Block", "object_type": "fcpool.Block", "size": 32, "from": "From 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareDirectDownload(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Username":"Username %d","ObjectType":"firmware.DirectDownload","ClassId":"firmware.DirectDownload","ImageSource":"ImageSource %d","IsPasswordSet":true,"Upgradeoption":"Upgradeoption %d"}`
	...
	expectedOp := map[string]interface{}{"username": "Username 1", "object_type": "firmware.DirectDownload", "class_id": "firmware.DirectDownload", "image_source": "ImageSource 1", "is_password_set": true, "upgradeoption": "Upgradeoption 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareNetworkShare(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"firmware.NetworkShare","Username":"Username %d","ClassId":"firmware.NetworkShare","IsPasswordSet":true,"MapType":"MapType %d","Upgradeoption":"Upgradeoption %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "firmware.NetworkShare", "username": "Username 1", "class_id": "firmware.NetworkShare", "is_password_set": true, "map_type": "MapType 1", "upgradeoption": "Upgradeoption 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexAppSettingConstraint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.AppSettingConstraint","HypervisorType":"HypervisorType %d","MgmtPlatform":"MgmtPlatform %d","ServerModel":"ServerModel %d","DeploymentType":"DeploymentType %d","HxdpVersion":"HxdpVersion %d","ClassId":"hyperflex.AppSettingConstraint"}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.AppSettingConstraint", "hypervisor_type": "HypervisorType 1", "mgmt_platform": "MgmtPlatform 1", "server_model": "ServerModel 1", "deployment_type": "DeploymentType 1", "hxdp_version": "HxdpVersion 1", "class_id": "hyperflex.AppSettingConstraint"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexDiskStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"State":"State %d","VolumeHandle":"VolumeHandle %d","ObjectType":"hyperflex.DiskStatus","ClassId":"hyperflex.DiskStatus","VolumeName":"VolumeName %d","DownloadPercentage":"DownloadPercentage %d"}`
	...
	expectedOp := map[string]interface{}{"state": "State 1", "volume_handle": "VolumeHandle 1", "object_type": "hyperflex.DiskStatus", "class_id": "hyperflex.DiskStatus", "volume_name": "VolumeName 1", "download_percentage": "DownloadPercentage 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexErrorStack(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MessageId":32,"ClassId":"hyperflex.ErrorStack","ObjectType":"hyperflex.ErrorStack","Message":"Message %d"}`
	...
	expectedOp := map[string]interface{}{"message_id": 32, "class_id": "hyperflex.ErrorStack", "object_type": "hyperflex.ErrorStack", "message": "Message 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHealthCheckScriptInfo(t *testing.T) {
	p := models.HyperflexHealthCheckScriptInfo{}
	var d = &schema.ResourceData{}
	c := `{"ScriptInput":"ScriptInput %d","Version":"Version %d","AggregateScriptName":"AggregateScriptName %d","ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptName":"ScriptName %d","HyperflexVersion":"HyperflexVersion %d","ClassId":"hyperflex.HealthCheckScriptInfo"}`
	...
	expectedOp := map[string]interface{}{"script_input": "ScriptInput 1", "nr_version": "Version 1", "aggregate_script_name": "AggregateScriptName 1", "object_type": "hyperflex.HealthCheckScriptInfo", "script_execute_location": "ScriptExecuteLocation 1", "script_name": "ScriptName 1", "hyperflex_version": "HyperflexVersion 1", "class_id": "hyperflex.HealthCheckScriptInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxLicenseAuthorizationDetailsDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LastCommunicationAttemptDate":"LastCommunicationAttemptDate %d","CommunicationDeadlineDate":"CommunicationDeadlineDate %d","ObjectType":"hyperflex.HxLicenseAuthorizationDetailsDt","EvaluationPeriodRemaining":"EvaluationPeriodRemaining %d","EvaluationPeriodExpiredAt":"EvaluationPeriodExpiredAt %d","ClassId":"hyperflex.HxLicenseAuthorizationDetailsDt","NextCommunicationAttemptDate":"NextCommunicationAttemptDate %d","Status":"Status %d"}`
	...
	expectedOp := map[string]interface{}{"last_communication_attempt_date": "LastCommunicationAttemptDate 1", "communication_deadline_date": "CommunicationDeadlineDate 1", "object_type": "hyperflex.HxLicenseAuthorizationDetailsDt", "evaluation_period_remaining": "EvaluationPeriodRemaining 1", "evaluation_period_expired_at": "EvaluationPeriodExpiredAt 1", "class_id": "hyperflex.HxLicenseAuthorizationDetailsDt", "next_communication_attempt_date": "NextCommunicationAttemptDate 1", "status": "Status 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHxPlatformDatastoreConfigDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ProvisionedCapacity":32,"DataBlockSize":32,"ObjectType":"hyperflex.HxPlatformDatastoreConfigDt","NumStripesForLargeFiles":32,"SystemDatastore":true,"Name":"Name %d","NumMirrors":32,"ClassId":"hyperflex.HxPlatformDatastoreConfigDt"}`
	...
	expectedOp := map[string]interface{}{"provisioned_capacity": 32, "data_block_size": 32, "object_type": "hyperflex.HxPlatformDatastoreConfigDt", "num_stripes_for_large_files": 32, "system_datastore": true, "name": "Name 1", "num_mirrors": 32, "class_id": "hyperflex.HxPlatformDatastoreConfigDt"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxRegistrationDetailsDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HxRegistrationDetailsDt","RegistrationExpireDate":"RegistrationExpireDate %d","LastRenewalAttemptDate":"LastRenewalAttemptDate %d","NextRenewalAttemptDate":"NextRenewalAttemptDate %d","VirtualAccount":"VirtualAccount %d","RegistrationFailedReason":"RegistrationFailedReason %d","InitialRegistrationDate":"InitialRegistrationDate %d","ClassId":"hyperflex.HxRegistrationDetailsDt","Status":"Status %d","OutOfComplianceStartDate":"OutOfComplianceStartDate %d","SmartAccount":"SmartAccount %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.HxRegistrationDetailsDt", "registration_expire_date": "RegistrationExpireDate 1", "last_renewal_attempt_date": "LastRenewalAttemptDate 1", "next_renewal_attempt_date": "NextRenewalAttemptDate 1", "virtual_account": "VirtualAccount 1", "registration_failed_reason": "RegistrationFailedReason 1", "initial_registration_date": "InitialRegistrationDate 1", "class_id": "hyperflex.HxRegistrationDetailsDt", "status": "Status 1", "out_of_compliance_start_date": "OutOfComplianceStartDate 1", "smart_account": "SmartAccount 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxResiliencyInfoDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PolicyCompliance":"PolicyCompliance %d","ResiliencyState":"ResiliencyState %d","HddFailuresTolerable":32,"SsdFailuresTolerable":32,"DataReplicationFactor":"DataReplicationFactor %d","ClassId":"hyperflex.HxResiliencyInfoDt","ObjectType":"hyperflex.HxResiliencyInfoDt","NodeFailuresTolerable":32}`
	...
	expectedOp := map[string]interface{}{"policy_compliance": "PolicyCompliance 1", "resiliency_state": "ResiliencyState 1", "hdd_failures_tolerable": 32, "ssd_failures_tolerable": 32, "data_replication_factor": "DataReplicationFactor 1", "class_id": "hyperflex.HxResiliencyInfoDt", "object_type": "hyperflex.HxResiliencyInfoDt", "node_failures_tolerable": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxSiteDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","ClassId":"hyperflex.HxSiteDt","ObjectType":"hyperflex.HxSiteDt"}`
	...
	expectedOp := map[string]interface{}{"name": "Name 1", "class_id": "hyperflex.HxSiteDt", "object_type": "hyperflex.HxSiteDt"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexIpAddrRange(t *testing.T) {
	p := models.HyperflexIpAddrRange{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.IpAddrRange","Netmask":"Netmask %d","StartAddr":"StartAddr %d","EndAddr":"EndAddr %d","Gateway":"Gateway %d","ClassId":"hyperflex.IpAddrRange"}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.IpAddrRange", "netmask": "Netmask 1", "start_addr": "StartAddr 1", "end_addr": "EndAddr 1", "gateway": "Gateway 1", "class_id": "hyperflex.IpAddrRange"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexReplicationPeerInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReplCip":"ReplCip %d","Status":"Status %d","Mcip":"Mcip %d","StatusDetails":"StatusDetails %d","Dcip":"Dcip %d","ClassId":"hyperflex.ReplicationPeerInfo","ObjectType":"hyperflex.ReplicationPeerInfo"}`
	...
	expectedOp := map[string]interface{}{"repl_cip": "ReplCip 1", "status": "Status 1", "mcip": "Mcip 1", "status_details": "StatusDetails 1", "dcip": "Dcip 1", "class_id": "hyperflex.ReplicationPeerInfo", "object_type": "hyperflex.ReplicationPeerInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ActiveNodes":"ActiveNodes %d","CompressionSavings":32.000000,"DataReplicationFactor":"DataReplicationFactor %d","TotalCapacity":32,"Name":"Name %d","Address":"Address %d","ClassId":"hyperflex.Summary","SpaceStatus":"SpaceStatus %d","TotalSavings":32.000000,"DataReplicationCompliance":"DataReplicationCompliance %d","Boottime":32,"DeduplicationSavings":32.000000,"Downtime":"Downtime %d","Uptime":"Uptime %d","ClusterAccessPolicy":"ClusterAccessPolicy %d","FreeCapacity":32,"UsedCapacity":32,"State":"State %d","ResiliencyDetailsSize":32,"ObjectType":"hyperflex.Summary"}`
	...
	expectedOp := map[string]interface{}{"active_nodes": "ActiveNodes 1", "compression_savings": 32.000000, "data_replication_factor": "DataReplicationFactor 1", "total_capacity": 32, "name": "Name 1", "address": "Address 1", "class_id": "hyperflex.Summary", "space_status": "SpaceStatus 1", "total_savings": 32.000000, "data_replication_compliance": "DataReplicationCompliance 1", "boottime": 32, "deduplication_savings": 32.000000, "downtime": "Downtime 1", "uptime": "Uptime 1", "cluster_access_policy": "ClusterAccessPolicy 1", "free_capacity": 32, "used_capacity": 32, "state": "State 1", "resiliency_details_size": 32, "object_type": "hyperflex.Summary"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexWwxnPrefixRange(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"EndAddr":"EndAddr %d","StartAddr":"StartAddr %d","ClassId":"hyperflex.WwxnPrefixRange","ObjectType":"hyperflex.WwxnPrefixRange"}`
	...
	expectedOp := map[string]interface{}{"end_addr": "EndAddr 1", "start_addr": "StartAddr 1", "class_id": "hyperflex.WwxnPrefixRange", "object_type": "hyperflex.WwxnPrefixRange"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamEndPointPasswordProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"GracePeriod":32,"PasswordExpiryDuration":32,"ForceSendPassword":true,"NotificationPeriod":32,"EnablePasswordExpiry":true,"PasswordHistory":32,"ClassId":"iam.EndPointPasswordProperties","ObjectType":"iam.EndPointPasswordProperties","EnforceStrongPassword":true}`
	...
	expectedOp := map[string]interface{}{"grace_period": 32, "password_expiry_duration": 32, "force_send_password": true, "notification_period": 32, "enable_password_expiry": true, "password_history": 32, "class_id": "iam.EndPointPasswordProperties", "object_type": "iam.EndPointPasswordProperties", "enforce_strong_password": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamLdapBaseProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"BindMethod":"BindMethod %d","ClassId":"iam.LdapBaseProperties","IsPasswordSet":true,"ObjectType":"iam.LdapBaseProperties","EnableGroupAuthorization":true,"Attribute":"Attribute %d","BindDn":"BindDn %d","Timeout":32,"EnableEncryption":true,"Filter":"Filter %d","BaseDn":"BaseDn %d","NestedGroupSearchDepth":32,"GroupAttribute":"GroupAttribute %d","Domain":"Domain %d"}`
	...
	expectedOp := map[string]interface{}{"bind_method": "BindMethod 1", "class_id": "iam.LdapBaseProperties", "is_password_set": true, "object_type": "iam.LdapBaseProperties", "enable_group_authorization": true, "attribute": "Attribute 1", "bind_dn": "BindDn 1", "timeout": 32, "enable_encryption": true, "filter": "Filter 1", "base_dn": "BaseDn 1", "nested_group_search_depth": 32, "group_attribute": "GroupAttribute 1", "domain": "Domain 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIppoolIpV4Config(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"ippool.IpV4Config","Gateway":"Gateway %d","Netmask":"Netmask %d","PrimaryDns":"PrimaryDns %d","SecondaryDns":"SecondaryDns %d","ClassId":"ippool.IpV4Config"}`
	...
	expectedOp := map[string]interface{}{"object_type": "ippool.IpV4Config", "gateway": "Gateway 1", "netmask": "Netmask 1", "primary_dns": "PrimaryDns 1", "secondary_dns": "SecondaryDns 1", "class_id": "ippool.IpV4Config"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIqnpoolIqnSuffixBlock(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"suffix": "Suffix 1", "to": 32, "class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "size": 32, "from": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesAddonConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.AddonConfiguration","Overrides":"Overrides %d","ReleaseName":"ReleaseName %d","ReleaseNamespace":"ReleaseNamespace %d","ObjectType":"kubernetes.AddonConfiguration","InstallStrategy":"InstallStrategy %d","UpgradeStrategy":"UpgradeStrategy %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "kubernetes.AddonConfiguration", "overrides": "Overrides 1", "release_name": "ReleaseName 1", "release_namespace": "ReleaseNamespace 1", "object_type": "kubernetes.AddonConfiguration", "install_strategy": "InstallStrategy 1", "upgrade_strategy": "UpgradeStrategy 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesClusterCertificateConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"EtcdCert":"EtcdCert %d","CaCert":"CaCert %d","FrontProxyCert":"FrontProxyCert %d","ClassId":"kubernetes.ClusterCertificateConfiguration","SaPublicKey":"SaPublicKey %d","ObjectType":"kubernetes.ClusterCertificateConfiguration"}`
	...
	expectedOp := map[string]interface{}{"etcd_cert": "EtcdCert 1", "ca_cert": "CaCert 1", "front_proxy_cert": "FrontProxyCert 1", "class_id": "kubernetes.ClusterCertificateConfiguration", "sa_public_key": "SaPublicKey 1", "object_type": "kubernetes.ClusterCertificateConfiguration"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesClusterManagementConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.ClusterManagementConfig","ObjectType":"kubernetes.ClusterManagementConfig","SshUser":"SshUser %d","LoadBalancerCount":32,"MasterVip":"MasterVip %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "kubernetes.ClusterManagementConfig", "object_type": "kubernetes.ClusterManagementConfig", "ssh_user": "SshUser 1", "load_balancer_count": 32, "master_vip": "MasterVip 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.Configuration","KubeConfig":"KubeConfig %d","ClassId":"kubernetes.Configuration"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.Configuration", "kube_config": "KubeConfig 1", "class_id": "kubernetes.Configuration"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesDaemonSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DesiredNumberScheduled":32,"ObservedGeneration":32,"CurrentNumberScheduled":32,"ObjectType":"kubernetes.DaemonSetStatus","UpdatedNumberScheduled":"UpdatedNumberScheduled %d","NumberAvailable":"NumberAvailable %d","NumberMisscheduled":32,"NumberReady":32,"ClassId":"kubernetes.DaemonSetStatus"}`
	...
	expectedOp := map[string]interface{}{"desired_number_scheduled": 32, "observed_generation": 32, "current_number_scheduled": 32, "object_type": "kubernetes.DaemonSetStatus", "updated_number_scheduled": "UpdatedNumberScheduled 1", "number_available": "NumberAvailable 1", "number_misscheduled": 32, "number_ready": 32, "class_id": "kubernetes.DaemonSetStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesDeploymentStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObservedGeneration":32,"ReadyReplicas":32,"Replicas":32,"ClassId":"kubernetes.DeploymentStatus","ObjectType":"kubernetes.DeploymentStatus","UpdatedReplicas":32,"AvailableReplicas":32}`
	...
	expectedOp := map[string]interface{}{"observed_generation": 32, "ready_replicas": 32, "replicas": 32, "class_id": "kubernetes.DeploymentStatus", "object_type": "kubernetes.DeploymentStatus", "updated_replicas": 32, "available_replicas": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesNodeInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"KernelVersion":"KernelVersion %d","KubeProxyVersion":"KubeProxyVersion %d","KubeletVersion":"KubeletVersion %d","OsImage":"OsImage %d","Architecture":"Architecture %d","SystemUuid":"SystemUuid %d","ObjectType":"kubernetes.NodeInfo","MachineId":"MachineId %d","OperatingSystem":"OperatingSystem %d","BootId":"BootId %d","ClassId":"kubernetes.NodeInfo","ContainerRuntimeVersion":"ContainerRuntimeVersion %d"}`
	...
	expectedOp := map[string]interface{}{"kernel_version": "KernelVersion 1", "kube_proxy_version": "KubeProxyVersion 1", "kubelet_version": "KubeletVersion 1", "os_image": "OsImage 1", "architecture": "Architecture 1", "system_uuid": "SystemUuid 1", "object_type": "kubernetes.NodeInfo", "machine_id": "MachineId 1", "operating_system": "OperatingSystem 1", "boot_id": "BootId 1", "class_id": "kubernetes.NodeInfo", "container_runtime_version": "ContainerRuntimeVersion 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesObjectMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Uuid":"Uuid %d","ClassId":"kubernetes.ObjectMeta","ObjectType":"kubernetes.ObjectMeta","CreationTimestamp":"CreationTimestamp %d","Name":"Name %d","Namespace":"Namespace %d","ResourceVersion":"ResourceVersion %d"}`
	...
	expectedOp := map[string]interface{}{"uuid": "Uuid 1", "class_id": "kubernetes.ObjectMeta", "object_type": "kubernetes.ObjectMeta", "creation_timestamp": "CreationTimestamp 1", "name": "Name 1", "namespace": "Namespace 1", "resource_version": "ResourceVersion 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesPodStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"StartTime":"StartTime %d","HostIp":"HostIp %d","Phase":"Phase %d","ClassId":"kubernetes.PodStatus","ObjectType":"kubernetes.PodStatus","PodIp":"PodIp %d","QosClass":"QosClass %d"}`
	...
	expectedOp := map[string]interface{}{"start_time": "StartTime 1", "host_ip": "HostIp 1", "phase": "Phase 1", "class_id": "kubernetes.PodStatus", "object_type": "kubernetes.PodStatus", "pod_ip": "PodIp 1", "qos_class": "QosClass 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesProxyConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Protocol":"Protocol %d","ObjectType":"kubernetes.ProxyConfig","Hostname":"Hostname %d","IsPasswordSet":true,"ClassId":"kubernetes.ProxyConfig","Port":32,"Username":"Username %d"}`
	...
	expectedOp := map[string]interface{}{"protocol": "Protocol 1", "object_type": "kubernetes.ProxyConfig", "hostname": "Hostname 1", "is_password_set": true, "class_id": "kubernetes.ProxyConfig", "port": 32, "username": "Username 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesStatefulSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.StatefulSetStatus","CurrentRevision":"CurrentRevision %d","UpdatedReplicas":32,"AvailableReplicas":32,"Replicas":32,"UpdateRevision":"UpdateRevision %d","ClassId":"kubernetes.StatefulSetStatus","ReadyReplicas":32,"ObservedGeneration":32,"CollisionCount":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.StatefulSetStatus", "current_revision": "CurrentRevision 1", "updated_replicas": 32, "available_replicas": 32, "replicas": 32, "update_revision": "UpdateRevision 1", "class_id": "kubernetes.StatefulSetStatus", "ready_replicas": 32, "observed_generation": 32, "collision_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiaapiNewReleaseDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReleaseNoteLinkTitle":"ReleaseNoteLinkTitle %d","SoftwareDownloadLinkTitle":"SoftwareDownloadLinkTitle %d","ObjectType":"niaapi.NewReleaseDetail","Description":"Description %d","ReleaseNoteLink":"ReleaseNoteLink %d","SoftwareDownloadLink":"SoftwareDownloadLink %d","Title":"Title %d","Link":"Link %d","Version":"Version %d","ClassId":"niaapi.NewReleaseDetail"}`
	...
	expectedOp := map[string]interface{}{"release_note_link_title": "ReleaseNoteLinkTitle 1", "software_download_link_title": "SoftwareDownloadLinkTitle 1", "object_type": "niaapi.NewReleaseDetail", "description": "Description 1", "release_note_link": "ReleaseNoteLink 1", "software_download_link": "SoftwareDownloadLink 1", "title": "Title 1", "link": "Link 1", "nr_version": "Version 1", "class_id": "niaapi.NewReleaseDetail"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryBootflashDetails(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.BootflashDetails","ClassId":"niatelemetry.BootflashDetails","ModelType":"ModelType %d","Serial":"Serial %d","FwRev":"FwRev %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "niatelemetry.BootflashDetails", "class_id": "niatelemetry.BootflashDetails", "model_type": "ModelType 1", "serial": "Serial 1", "fw_rev": "FwRev 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryNveVni(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.NveVni","DpVniDown":32,"CpVniDown":32,"CpVniUp":32,"DpVniCount":32,"ObjectType":"niatelemetry.NveVni","DpVniUp":32,"CpVniCount":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "niatelemetry.NveVni", "dp_vni_down": 32, "cp_vni_down": 32, "cp_vni_up": 32, "dp_vni_count": 32, "object_type": "niatelemetry.NveVni", "dp_vni_up": 32, "cp_vni_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosBgpMvpn(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MemoryUsed":32,"NumberOfClusterLists":32,"TotalPaths":32,"TableVersion":32,"CapablePeers":32,"ConfiguredPeers":32,"ClassId":"niatelemetry.NxosBgpMvpn","ObjectType":"niatelemetry.NxosBgpMvpn","TotalNetworks":32,"NumberOfCommunities":32}`
	...
	expectedOp := map[string]interface{}{"memory_used": 32, "number_of_cluster_lists": 32, "total_paths": 32, "table_version": 32, "capable_peers": 32, "configured_peers": 32, "class_id": "niatelemetry.NxosBgpMvpn", "object_type": "niatelemetry.NxosBgpMvpn", "total_networks": 32, "number_of_communities": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosVtp(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"V2Mode":"V2Mode %d","Version":32,"OperMode":"OperMode %d","PruningMode":"PruningMode %d","RunningVersion":"RunningVersion %d","ObjectType":"niatelemetry.NxosVtp","TrapEnabled":"TrapEnabled %d","ClassId":"niatelemetry.NxosVtp"}`
	...
	expectedOp := map[string]interface{}{"v2_mode": "V2Mode 1", "nr_version": 32, "oper_mode": "OperMode 1", "pruning_mode": "PruningMode 1", "running_version": "RunningVersion 1", "object_type": "niatelemetry.NxosVtp", "trap_enabled": "TrapEnabled 1", "class_id": "niatelemetry.NxosVtp"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetrySmartLicense(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LicenseUdi":"LicenseUdi %d","SmartAccount":"SmartAccount %d","ClassId":"niatelemetry.SmartLicense","ObjectType":"niatelemetry.SmartLicense","ActiveMode":"ActiveMode %d","AuthStatus":"AuthStatus %d"}`
	...
	expectedOp := map[string]interface{}{"license_udi": "LicenseUdi 1", "smart_account": "SmartAccount 1", "class_id": "niatelemetry.SmartLicense", "object_type": "niatelemetry.SmartLicense", "active_mode": "ActiveMode 1", "auth_status": "AuthStatus 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapOnpremSchedule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DayOfMonth":32,"ObjectType":"onprem.Schedule","TimeZone":"TimeZone %d","WeekOfMonth":32,"MonthOfYear":32,"ClassId":"onprem.Schedule","DayOfWeek":32,"TimeOfDay":32,"RepeatInterval":32}`
	...
	expectedOp := map[string]interface{}{"day_of_month": 32, "object_type": "onprem.Schedule", "time_zone": "TimeZone 1", "week_of_month": 32, "month_of_year": 32, "class_id": "onprem.Schedule", "day_of_week": 32, "time_of_day": 32, "repeat_interval": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapOnpremUpgradePhase(t *testing.T) {
	p := models.OnpremUpgradePhase{}
	var d = &schema.ResourceData{}
	c := `{"ElapsedTime":32,"Failed":true,"ObjectType":"onprem.UpgradePhase","Name":"Name %d","ClassId":"onprem.UpgradePhase","Message":"Message %d"}`
	...
	expectedOp := map[string]interface{}{"elapsed_time": 32, "failed": true, "object_type": "onprem.UpgradePhase", "name": "Name 1", "class_id": "onprem.UpgradePhase", "message": "Message 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsAnswers(t *testing.T) {
	p := models.OsAnswers{}
	var d = &schema.ResourceData{}
	c := `{"ProductKey":"ProductKey %d","IsAnswerFileSet":true,"IsRootPasswordCrypted":true,"IpConfigType":"IpConfigType %d","ObjectType":"os.Answers","IsRootPasswordSet":true,"Nameserver":"Nameserver %d","Hostname":"Hostname %d","Source":"Source %d","ClassId":"os.Answers"}`
	...
	expectedOp := map[string]interface{}{"product_key": "ProductKey 1", "is_answer_file_set": true, "is_root_password_crypted": true, "ip_config_type": "IpConfigType 1", "object_type": "os.Answers", "is_root_password_set": true, "nameserver": "Nameserver 1", "hostname": "Hostname 1", "nr_source": "Source 1", "class_id": "os.Answers"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsGlobalConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ScuImageName":"ScuImageName %d","ClassId":"os.GlobalConfig","OsImageName":"OsImageName %d","ConfigurationFileName":"ConfigurationFileName %d","InstallMethod":"InstallMethod %d","ObjectType":"os.GlobalConfig","WindowsEdition":"WindowsEdition %d","ConfigurationSource":"ConfigurationSource %d","InstallTargetType":"InstallTargetType %d"}`
	...
	expectedOp := map[string]interface{}{"scu_image_name": "ScuImageName 1", "class_id": "os.GlobalConfig", "os_image_name": "OsImageName 1", "configuration_file_name": "ConfigurationFileName 1", "install_method": "InstallMethod 1", "object_type": "os.GlobalConfig", "windows_edition": "WindowsEdition 1", "configuration_source": "ConfigurationSource 1", "install_target_type": "InstallTargetType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapPolicyConfigContext(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ErrorState":"ErrorState %d","ObjectType":"policy.ConfigContext","ClassId":"policy.ConfigContext","OperState":"OperState %d","ConfigState":"ConfigState %d","ConfigType":"ConfigType %d","ControlAction":"ControlAction %d"}`
	...
	expectedOp := map[string]interface{}{"error_state": "ErrorState 1", "object_type": "policy.ConfigContext", "class_id": "policy.ConfigContext", "oper_state": "OperState 1", "config_state": "ConfigState 1", "config_type": "ConfigType 1", "control_action": "ControlAction 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageAutomaticDriveGroup(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"storage.AutomaticDriveGroup","NumberOfSpans":32,"DriveType":"DriveType %d","ClassId":"storage.AutomaticDriveGroup","MinimumDriveSize":32,"NumDedicatedHotSpares":"NumDedicatedHotSpares %d","UseRemainingDrives":true,"DrivesPerSpan":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "storage.AutomaticDriveGroup", "number_of_spans": 32, "drive_type": "DriveType 1", "class_id": "storage.AutomaticDriveGroup", "minimum_drive_size": 32, "num_dedicated_hot_spares": "NumDedicatedHotSpares 1", "use_remaining_drives": true, "drives_per_span": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageR0Drive(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Enable":true,"DriveSlots":"DriveSlots %d","DriveSlotsList":"DriveSlotsList %d","ClassId":"storage.R0Drive","ObjectType":"storage.R0Drive"}`
	...
	expectedOp := map[string]interface{}{"enable": true, "drive_slots": "DriveSlots 1", "drive_slots_list": "DriveSlotsList 1", "class_id": "storage.R0Drive", "object_type": "storage.R0Drive"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCloudInitConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ConfigType":"ConfigType %d","NetworkData":"NetworkData %d","NetworkDataBase64Encoded":true,"UserData":"UserData %d","UserDataBase64Encoded":true,"ClassId":"virtualization.CloudInitConfig","ObjectType":"virtualization.CloudInitConfig"}`
	...
	expectedOp := map[string]interface{}{"config_type": "ConfigType 1", "network_data": "NetworkData 1", "network_data_base64_encoded": true, "user_data": "UserData 1", "user_data_base64_encoded": true, "class_id": "virtualization.CloudInitConfig", "object_type": "virtualization.CloudInitConfig"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCpuInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Cores":32,"Description":"Description %d","Sockets":32,"Speed":32,"Vendor":"Vendor %d","ClassId":"virtualization.CpuInfo","ObjectType":"virtualization.CpuInfo"}`
	...
	expectedOp := map[string]interface{}{"cores": 32, "description": "Description 1", "sockets": 32, "speed": 32, "vendor": "Vendor 1", "class_id": "virtualization.CpuInfo", "object_type": "virtualization.CpuInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationGuestInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.GuestInfo","ObjectType":"virtualization.GuestInfo","Name":"Name %d","OperatingSystem":"OperatingSystem %d","Hostname":"Hostname %d","IpAddress":"IpAddress %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.GuestInfo", "object_type": "virtualization.GuestInfo", "name": "Name 1", "operating_system": "OperatingSystem 1", "hostname": "Hostname 1", "ip_address": "IpAddress 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationMemoryAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Used":32,"Free":32,"Reserved":32,"Total":32,"ClassId":"virtualization.MemoryAllocation","ObjectType":"virtualization.MemoryAllocation"}`
	...
	expectedOp := map[string]interface{}{"used": 32, "free": 32, "reserved": 32, "total": 32, "class_id": "virtualization.MemoryAllocation", "object_type": "virtualization.MemoryAllocation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationMemoryCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Used":32,"ClassId":"virtualization.MemoryCapacity","ObjectType":"virtualization.MemoryCapacity","Capacity":32,"Free":32}`
	...
	expectedOp := map[string]interface{}{"used": 32, "class_id": "virtualization.MemoryCapacity", "object_type": "virtualization.MemoryCapacity", "capacity": 32, "free": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationProductInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ProductType":"ProductType %d","ProductVendor":"ProductVendor %d","Version":"Version %d","Build":"Build %d","ProductName":"ProductName %d","ClassId":"virtualization.ProductInfo","ObjectType":"virtualization.ProductInfo"}`
	...
	expectedOp := map[string]interface{}{"product_type": "ProductType 1", "product_vendor": "ProductVendor 1", "nr_version": "Version 1", "build": "Build 1", "product_name": "ProductName 1", "class_id": "virtualization.ProductInfo", "object_type": "virtualization.ProductInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationStorageCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Free":32,"ClassId":"virtualization.StorageCapacity","ObjectType":"virtualization.StorageCapacity","Used":32,"Capacity":32}`
	...
	expectedOp := map[string]interface{}{"free": 32, "class_id": "virtualization.StorageCapacity", "object_type": "virtualization.StorageCapacity", "used": 32, "capacity": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareTeamingAndFailover(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.VmwareTeamingAndFailover","NotifySwitches":true,"NetworkFailureDetection":"NetworkFailureDetection %d","Failback":true,"ClassId":"virtualization.VmwareTeamingAndFailover","LoadBalancing":"LoadBalancing %d","Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.VmwareTeamingAndFailover", "notify_switches": true, "network_failure_detection": "NetworkFailureDetection 1", "failback": true, "class_id": "virtualization.VmwareTeamingAndFailover", "load_balancing": "LoadBalancing 1", "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareVmDiskCommitInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CommittedDisk":32,"UnCommittedDisk":32,"UnsharedDisk":32,"ClassId":"virtualization.VmwareVmDiskCommitInfo","ObjectType":"virtualization.VmwareVmDiskCommitInfo"}`
	...
	expectedOp := map[string]interface{}{"committed_disk": 32, "un_committed_disk": 32, "unshared_disk": 32, "class_id": "virtualization.VmwareVmDiskCommitInfo", "object_type": "virtualization.VmwareVmDiskCommitInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationVmwareVmMemoryShareInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MemShares":32,"MemLimit":32,"MemOverheadLimit":32,"MemReservation":32,"ClassId":"virtualization.VmwareVmMemoryShareInfo","ObjectType":"virtualization.VmwareVmMemoryShareInfo"}`
	...
	expectedOp := map[string]interface{}{"mem_shares": 32, "mem_limit": 32, "mem_overhead_limit": 32, "mem_reservation": 32, "class_id": "virtualization.VmwareVmMemoryShareInfo", "object_type": "virtualization.VmwareVmMemoryShareInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicArfsSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.ArfsSettings","Enabled":true,"ClassId":"vnic.ArfsSettings"}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.ArfsSettings", "enabled": true, "class_id": "vnic.ArfsSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicCompletionQueueSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.CompletionQueueSettings","RingSize":32,"Count":32,"ObjectType":"vnic.CompletionQueueSettings"}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.CompletionQueueSettings", "ring_size": 32, "nr_count": 32, "object_type": "vnic.CompletionQueueSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicEthInterruptSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CoalescingType":"CoalescingType %d","Count":32,"ClassId":"vnic.EthInterruptSettings","ObjectType":"vnic.EthInterruptSettings","Mode":"Mode %d","CoalescingTime":32}`
	...
	expectedOp := map[string]interface{}{"coalescing_type": "CoalescingType 1", "nr_count": 32, "class_id": "vnic.EthInterruptSettings", "object_type": "vnic.EthInterruptSettings", "mode": "Mode 1", "coalescing_time": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcErrorRecoverySettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PortDownTimeout":32,"Enabled":true,"IoRetryCount":32,"ClassId":"vnic.FcErrorRecoverySettings","ObjectType":"vnic.FcErrorRecoverySettings","IoRetryTimeout":32,"LinkDownTimeout":32}`
	...
	expectedOp := map[string]interface{}{"port_down_timeout": 32, "enabled": true, "io_retry_count": 32, "class_id": "vnic.FcErrorRecoverySettings", "object_type": "vnic.FcErrorRecoverySettings", "io_retry_timeout": 32, "link_down_timeout": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicPlacementSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SwitchId":"SwitchId %d","Uplink":32,"ClassId":"vnic.PlacementSettings","ObjectType":"vnic.PlacementSettings","Id":"Id %d","PciLink":32}`
	...
	expectedOp := map[string]interface{}{"switch_id": "SwitchId 1", "uplink": 32, "class_id": "vnic.PlacementSettings", "object_type": "vnic.PlacementSettings", "id": "Id 1", "pci_link": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicRoceSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.RoceSettings","Enabled":true,"MemoryRegions":32,"ResourceGroups":32,"Version":32,"ObjectType":"vnic.RoceSettings","ClassOfService":32,"QueuePairs":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.RoceSettings", "enabled": true, "memory_regions": 32, "resource_groups": 32, "nr_version": 32, "object_type": "vnic.RoceSettings", "class_of_service": 32, "queue_pairs": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicRssHashSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.RssHashSettings","TcpIpv4Hash":true,"UdpIpv6Hash":true,"TcpIpv6Hash":true,"Ipv6Hash":true,"ClassId":"vnic.RssHashSettings","Ipv6ExtHash":true,"UdpIpv4Hash":true,"TcpIpv6ExtHash":true,"Ipv4Hash":true}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.RssHashSettings", "tcp_ipv4_hash": true, "udp_ipv6_hash": true, "tcp_ipv6_hash": true, "ipv6_hash": true, "class_id": "vnic.RssHashSettings", "ipv6_ext_hash": true, "udp_ipv4_hash": true, "tcp_ipv6_ext_hash": true, "ipv4_hash": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicUsnicSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Count":32,"ClassId":"vnic.UsnicSettings","ObjectType":"vnic.UsnicSettings","UsnicAdapterPolicy":"UsnicAdapterPolicy %d","Cos":32}`
	...
	expectedOp := map[string]interface{}{"nr_count": 32, "class_id": "vnic.UsnicSettings", "object_type": "vnic.UsnicSettings", "usnic_adapter_policy": "UsnicAdapterPolicy 1", "cos": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicVmqSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MultiQueueSupport":true,"NumInterrupts":32,"NumSubVnics":32,"VmmqAdapterPolicy":"VmmqAdapterPolicy %d","ClassId":"vnic.VmqSettings","Enabled":true,"NumVmqs":32,"ObjectType":"vnic.VmqSettings"}`
	...
	expectedOp := map[string]interface{}{"multi_queue_support": true, "num_interrupts": 32, "num_sub_vnics": 32, "vmmq_adapter_policy": "VmmqAdapterPolicy 1", "class_id": "vnic.VmqSettings", "enabled": true, "num_vmqs": 32, "object_type": "vnic.VmqSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ExternalMeta":true,"SupportStatus":"SupportStatus %d","Timeout":32,"RetryDelay":32,"ClassId":"workflow.Properties","TimeoutPolicy":"TimeoutPolicy %d","RetryCount":32,"RetryPolicy":"RetryPolicy %d","ObjectType":"workflow.Properties"}`
	...
	expectedOp := map[string]interface{}{"external_meta": true, "support_status": "SupportStatus 1", "timeout": 32, "retry_delay": 32, "class_id": "workflow.Properties", "timeout_policy": "TimeoutPolicy 1", "retry_count": 32, "retry_policy": "RetryPolicy 1", "object_type": "workflow.Properties"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Retryable":true,"SupportStatus":"SupportStatus %d","Cloneable":true,"EnableDebug":true,"ExternalMeta":true,"ClassId":"workflow.WorkflowProperties","ObjectType":"workflow.WorkflowProperties"}`
	...
	expectedOp := map[string]interface{}{"retryable": true, "support_status": "SupportStatus 1", "cloneable": true, "enable_debug": true, "external_meta": true, "class_id": "workflow.WorkflowProperties", "object_type": "workflow.WorkflowProperties"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapX509Certificate(t *testing.T) {
	p := models.X509Certificate{}
	var d = &schema.ResourceData{}
	c := `{"Sha256Fingerprint":"Sha256Fingerprint %d","ObjectType":"x509.Certificate","PemCertificate":"PemCertificate %d","SignatureAlgorithm":"SignatureAlgorithm %d","ClassId":"x509.Certificate"}`
	...
	expectedOp := map[string]interface{}{"sha256_fingerprint": "Sha256Fingerprint 1", "object_type": "x509.Certificate", "pem_certificate": "PemCertificate 1", "signature_algorithm": "SignatureAlgorithm 1", "class_id": "x509.Certificate"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
