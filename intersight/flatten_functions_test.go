...
func TestFlattenListApplianceApiStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectName":"ObjectName %d","Reason":"Reason %d","Response":32,"ElapsedTime":32.000000,"ClassId":"appliance.ApiStatus","ObjectType":"appliance.ApiStatus"}`
...
func TestFlattenListBootDeviceBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Enabled":true,"Name":"Name %d","ClassId":"boot.DeviceBase","ObjectType":"boot.DeviceBase"}`
	...
	expectedOp := []map[string]interface{}{{"enabled": true, "name": "Name 1", "class_id": "boot.DeviceBase", "object_type": "boot.DeviceBase"}, {"enabled": true, "name": "Name 2", "class_id": "boot.DeviceBase", "object_type": "boot.DeviceBase"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListBulkHttpHeader(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"bulk.HttpHeader","Name":"Name %d","Value":"Value %d","ObjectType":"bulk.HttpHeader"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "bulk.HttpHeader", "name": "Name 1", "value": "Value 1", "object_type": "bulk.HttpHeader"}, {"class_id": "bulk.HttpHeader", "name": "Name 2", "value": "Value 2", "object_type": "bulk.HttpHeader"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListBulkSubRequest(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Verb":"Verb %d","ClassId":"bulk.SubRequest","ObjectType":"bulk.SubRequest","Uri":"Uri %d"}`
	...
	expectedOp := []map[string]interface{}{{"verb": "Verb 1", "class_id": "bulk.SubRequest", "object_type": "bulk.SubRequest", "uri": "Uri 1"}, {"verb": "Verb 2", "class_id": "bulk.SubRequest", "object_type": "bulk.SubRequest", "uri": "Uri 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCapabilitySwitchingModeCapability(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"capability.SwitchingModeCapability","SwitchingMode":"SwitchingMode %d","VpCompressionSupported":true,"ClassId":"capability.SwitchingModeCapability"}`
...
func TestFlattenListCloudCloudTag(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Key":"Key %d","ClassId":"cloud.CloudTag","ObjectType":"cloud.CloudTag","Value":"Value %d"}`
	...
	expectedOp := []map[string]interface{}{{"key": "Key 1", "class_id": "cloud.CloudTag", "object_type": "cloud.CloudTag", "value": "Value 1"}, {"key": "Key 2", "class_id": "cloud.CloudTag", "object_type": "cloud.CloudTag", "value": "Value 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudCustomAttributes(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AttributeType":"AttributeType %d","AttributeValue":"AttributeValue %d","AttributeName":"AttributeName %d","ClassId":"cloud.CustomAttributes","ObjectType":"cloud.CustomAttributes"}`
...
func TestFlattenListCloudNetworkInterfaceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MacAddress":"MacAddress %d","NetworkId":"NetworkId %d","NetworkName":"NetworkName %d","IpForwardingEnabled":true,"SubNetworkId":"SubNetworkId %d","Identity":"Identity %d","ObjectType":"cloud.NetworkInterfaceAttachment","SubNetworkName":"SubNetworkName %d","NicIndex":32,"ClassId":"cloud.NetworkInterfaceAttachment"}`
	...
	expectedOp := []map[string]interface{}{{"mac_address": "MacAddress 1", "network_id": "NetworkId 1", "network_name": "NetworkName 1", "ip_forwarding_enabled": true, "sub_network_id": "SubNetworkId 1", "identity": "Identity 1", "object_type": "cloud.NetworkInterfaceAttachment", "sub_network_name": "SubNetworkName 1", "nic_index": 32, "class_id": "cloud.NetworkInterfaceAttachment"}, {"mac_address": "MacAddress 2", "network_id": "NetworkId 2", "network_name": "NetworkName 2", "ip_forwarding_enabled": true, "sub_network_id": "SubNetworkId 2", "identity": "Identity 2", "object_type": "cloud.NetworkInterfaceAttachment", "sub_network_name": "SubNetworkName 2", "nic_index": 32, "class_id": "cloud.NetworkInterfaceAttachment"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudSecurityGroupRule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Description":"Description %d","Identity":"Identity %d","ClassId":"cloud.SecurityGroupRule","StartPort":32,"Name":"Name %d","ObjectType":"cloud.SecurityGroupRule","EtherType":"EtherType %d","Action":"Action %d","SourceSecurityGroup":"SourceSecurityGroup %d","EndPort":32,"Protocol":"Protocol %d","Index":32}`
	...
	expectedOp := []map[string]interface{}{{"description": "Description 1", "identity": "Identity 1", "class_id": "cloud.SecurityGroupRule", "start_port": 32, "name": "Name 1", "object_type": "cloud.SecurityGroupRule", "ether_type": "EtherType 1", "action": "Action 1", "source_security_group": "SourceSecurityGroup 1", "end_port": 32, "protocol": "Protocol 1", "index": 32}, {"description": "Description 2", "identity": "Identity 2", "class_id": "cloud.SecurityGroupRule", "start_port": 32, "name": "Name 2", "object_type": "cloud.SecurityGroupRule", "ether_type": "EtherType 2", "action": "Action 2", "source_security_group": "SourceSecurityGroup 2", "end_port": 32, "protocol": "Protocol 2", "index": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudTfcWorkspaceVariables(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.TfcWorkspaceVariables","Identity":"Identity %d","Hcl":true,"Description":"Description %d","Key":"Key %d","ClassId":"cloud.TfcWorkspaceVariables","Sensitive":true,"Value":"Value %d","Category":"Category %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "cloud.TfcWorkspaceVariables", "identity": "Identity 1", "hcl": true, "description": "Description 1", "key": "Key 1", "class_id": "cloud.TfcWorkspaceVariables", "sensitive": true, "value": "Value 1", "category": "Category 1"}, {"object_type": "cloud.TfcWorkspaceVariables", "identity": "Identity 2", "hcl": true, "description": "Description 2", "key": "Key 2", "class_id": "cloud.TfcWorkspaceVariables", "sensitive": true, "value": "Value 2", "category": "Category 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AutoDelete":true,"Identity":"Identity %d","IsRoot":true,"Index":32,"DeviceName":"DeviceName %d","ClassId":"cloud.VolumeAttachment","ObjectType":"cloud.VolumeAttachment"}`
	...
	expectedOp := []map[string]interface{}{{"auto_delete": true, "identity": "Identity 1", "is_root": true, "index": 32, "device_name": "DeviceName 1", "class_id": "cloud.VolumeAttachment", "object_type": "cloud.VolumeAttachment"}, {"auto_delete": true, "identity": "Identity 2", "is_root": true, "index": 32, "device_name": "DeviceName 2", "class_id": "cloud.VolumeAttachment", "object_type": "cloud.VolumeAttachment"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeInstanceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"InstanceId":"InstanceId %d","State":"State %d","AutoDelete":true,"DeviceName":"DeviceName %d","ClassId":"cloud.VolumeInstanceAttachment","ObjectType":"cloud.VolumeInstanceAttachment"}`
	...
	expectedOp := []map[string]interface{}{{"instance_id": "InstanceId 1", "state": "State 1", "auto_delete": true, "device_name": "DeviceName 1", "class_id": "cloud.VolumeInstanceAttachment", "object_type": "cloud.VolumeInstanceAttachment"}, {"instance_id": "InstanceId 2", "state": "State 2", "auto_delete": true, "device_name": "DeviceName 2", "class_id": "cloud.VolumeInstanceAttachment", "object_type": "cloud.VolumeInstanceAttachment"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeIpAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Dn":"Dn %d","KvmVlan":32,"ObjectType":"compute.IpAddress","Address":"Address %d","HttpsPort":32,"Type":"Type %d","ClassId":"compute.IpAddress","Subnet":"Subnet %d","Category":"Category %d","KvmPort":32,"DefaultGateway":"DefaultGateway %d","Name":"Name %d","HttpPort":32}`
	...
	expectedOp := []map[string]interface{}{{"dn": "Dn 1", "kvm_vlan": 32, "object_type": "compute.IpAddress", "address": "Address 1", "https_port": 32, "type": "Type 1", "class_id": "compute.IpAddress", "subnet": "Subnet 1", "category": "Category 1", "kvm_port": 32, "default_gateway": "DefaultGateway 1", "name": "Name 1", "http_port": 32}, {"dn": "Dn 2", "kvm_vlan": 32, "object_type": "compute.IpAddress", "address": "Address 2", "https_port": 32, "type": "Type 2", "class_id": "compute.IpAddress", "subnet": "Subnet 2", "category": "Category 2", "kvm_port": 32, "default_gateway": "DefaultGateway 2", "name": "Name 2", "http_port": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeServerOpStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"compute.ServerOpStatus","ObjectType":"compute.ServerOpStatus","ConfigState":"ConfigState %d","WorkflowType":"WorkflowType %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "compute.ServerOpStatus", "object_type": "compute.ServerOpStatus", "config_state": "ConfigState 1", "workflow_type": "WorkflowType 1"}, {"class_id": "compute.ServerOpStatus", "object_type": "compute.ServerOpStatus", "config_state": "ConfigState 2", "workflow_type": "WorkflowType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListConnectorpackConnectorPackUpdate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"connectorpack.ConnectorPackUpdate","CurrentVersion":"CurrentVersion %d","Name":"Name %d","NewVersion":"NewVersion %d","ObjectType":"connectorpack.ConnectorPackUpdate"}`
...
func TestFlattenListContentParameter(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"content.Parameter","Path":"Path %d","Type":"Type %d","Name":"Name %d","ItemType":"ItemType %d","Secure":true,"ObjectType":"content.Parameter","AcceptSingleValue":true,"ComplexType":"ComplexType %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "content.Parameter", "path": "Path 1", "type": "Type 1", "name": "Name 1", "item_type": "ItemType 1", "secure": true, "object_type": "content.Parameter", "accept_single_value": true, "complex_type": "ComplexType 1"}, {"class_id": "content.Parameter", "path": "Path 2", "type": "Type 2", "name": "Name 2", "item_type": "ItemType 2", "secure": true, "object_type": "content.Parameter", "accept_single_value": true, "complex_type": "ComplexType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCrdCustomResourceConfigProperty(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Value":"Value %d","ClassId":"crd.CustomResourceConfigProperty","ObjectType":"crd.CustomResourceConfigProperty","Key":"Key %d"}`
	...
	expectedOp := []map[string]interface{}{{"value": "Value 1", "class_id": "crd.CustomResourceConfigProperty", "object_type": "crd.CustomResourceConfigProperty", "key": "Key 1"}, {"value": "Value 2", "class_id": "crd.CustomResourceConfigProperty", "object_type": "crd.CustomResourceConfigProperty", "key": "Key 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricPortIdentifier(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"fabric.PortIdentifier","ClassId":"fabric.PortIdentifier","SlotId":32,"AggregatePortId":32,"PortId":32}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "fabric.PortIdentifier", "class_id": "fabric.PortIdentifier", "slot_id": 32, "aggregate_port_id": 32, "port_id": 32}, {"object_type": "fabric.PortIdentifier", "class_id": "fabric.PortIdentifier", "slot_id": 32, "aggregate_port_id": 32, "port_id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricQosClass(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Mtu":32,"Weight":32,"ClassId":"fabric.QosClass","ObjectType":"fabric.QosClass","BandwidthPercent":32,"MulticastOptimize":true,"PacketDrop":true,"Name":"Name %d","AdminState":"AdminState %d","Cos":32}`
	...
	expectedOp := []map[string]interface{}{{"mtu": 32, "weight": 32, "class_id": "fabric.QosClass", "object_type": "fabric.QosClass", "bandwidth_percent": 32, "multicast_optimize": true, "packet_drop": true, "name": "Name 1", "admin_state": "AdminState 1", "cos": 32}, {"mtu": 32, "weight": 32, "class_id": "fabric.QosClass", "object_type": "fabric.QosClass", "bandwidth_percent": 32, "multicast_optimize": true, "packet_drop": true, "name": "Name 2", "admin_state": "AdminState 2", "cos": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFcpoolBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Size":32,"ClassId":"fcpool.Block","From":"From %d","To":"To %d","ObjectType":"fcpool.Block"}`
	...
	expectedOp := []map[string]interface{}{{"size": 32, "class_id": "fcpool.Block", "from": "From 1", "to": "To 1", "object_type": "fcpool.Block"}, {"size": 32, "class_id": "fcpool.Block", "from": "From 2", "to": "To 2", "object_type": "fcpool.Block"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareBaseImpact(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ComputationStatusDetail":"ComputationStatusDetail %d","TargetFirmwareVersion":"TargetFirmwareVersion %d","Model":"Model %d","ImpactType":"ImpactType %d","ComputationError":"ComputationError %d","VersionImpact":"VersionImpact %d","ObjectType":"firmware.BaseImpact","ClassId":"firmware.BaseImpact","FirmwareVersion":"FirmwareVersion %d","DomainName":"DomainName %d","EndPoint":"EndPoint %d"}`
	...
	expectedOp := []map[string]interface{}{{"computation_status_detail": "ComputationStatusDetail 1", "target_firmware_version": "TargetFirmwareVersion 1", "model": "Model 1", "impact_type": "ImpactType 1", "computation_error": "ComputationError 1", "version_impact": "VersionImpact 1", "object_type": "firmware.BaseImpact", "class_id": "firmware.BaseImpact", "firmware_version": "FirmwareVersion 1", "domain_name": "DomainName 1", "end_point": "EndPoint 1"}, {"computation_status_detail": "ComputationStatusDetail 2", "target_firmware_version": "TargetFirmwareVersion 2", "model": "Model 2", "impact_type": "ImpactType 2", "computation_error": "ComputationError 2", "version_impact": "VersionImpact 2", "object_type": "firmware.BaseImpact", "class_id": "firmware.BaseImpact", "firmware_version": "FirmwareVersion 2", "domain_name": "DomainName 2", "end_point": "EndPoint 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareComponentMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ComponentType":"ComponentType %d","ComponentLabel":"ComponentLabel %d","Model":"Model %d","IsOobSupported":true,"Description":"Description %d","PackedVersion":"PackedVersion %d","ObjectType":"firmware.ComponentMeta","ClassId":"firmware.ComponentMeta","ImagePath":"ImagePath %d","Vendor":"Vendor %d","RedfishUrl":"RedfishUrl %d","Disruption":"Disruption %d"}`
	...
	expectedOp := []map[string]interface{}{{"component_type": "ComponentType 1", "component_label": "ComponentLabel 1", "model": "Model 1", "is_oob_supported": true, "description": "Description 1", "packed_version": "PackedVersion 1", "object_type": "firmware.ComponentMeta", "class_id": "firmware.ComponentMeta", "image_path": "ImagePath 1", "vendor": "Vendor 1", "redfish_url": "RedfishUrl 1", "disruption": "Disruption 1"}, {"component_type": "ComponentType 2", "component_label": "ComponentLabel 2", "model": "Model 2", "is_oob_supported": true, "description": "Description 2", "packed_version": "PackedVersion 2", "object_type": "firmware.ComponentMeta", "class_id": "firmware.ComponentMeta", "image_path": "ImagePath 2", "vendor": "Vendor 2", "redfish_url": "RedfishUrl 2", "disruption": "Disruption 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareFirmwareInventory(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"firmware.FirmwareInventory","Category":"Category %d","Label":"Label %d","ClassId":"firmware.FirmwareInventory","UpdateUri":"UpdateUri %d","Vendor":"Vendor %d","Version":"Version %d","Model":"Model %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "firmware.FirmwareInventory", "category": "Category 1", "label": "Label 1", "class_id": "firmware.FirmwareInventory", "update_uri": "UpdateUri 1", "vendor": "Vendor 1", "nr_version": "Version 1", "model": "Model 1"}, {"object_type": "firmware.FirmwareInventory", "category": "Category 2", "label": "Label 2", "class_id": "firmware.FirmwareInventory", "update_uri": "UpdateUri 2", "vendor": "Vendor 2", "nr_version": "Version 2", "model": "Model 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHealthCheckScriptInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptName":"ScriptName %d","Version":"Version %d","AggregateScriptName":"AggregateScriptName %d","HyperflexVersion":"HyperflexVersion %d","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptInput":"ScriptInput %d","ClassId":"hyperflex.HealthCheckScriptInfo"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 1", "nr_version": "Version 1", "aggregate_script_name": "AggregateScriptName 1", "hyperflex_version": "HyperflexVersion 1", "script_execute_location": "ScriptExecuteLocation 1", "script_input": "ScriptInput 1", "class_id": "hyperflex.HealthCheckScriptInfo"}, {"object_type": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 2", "nr_version": "Version 2", "aggregate_script_name": "AggregateScriptName 2", "hyperflex_version": "HyperflexVersion 2", "script_execute_location": "ScriptExecuteLocation 2", "script_input": "ScriptInput 2", "class_id": "hyperflex.HealthCheckScriptInfo"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexNamedVlan(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"object_type": "hyperflex.NamedVlan", "class_id": "hyperflex.NamedVlan", "name": "Name 1", "vlan_id": 32}, {"object_type": "hyperflex.NamedVlan", "class_id": "hyperflex.NamedVlan", "name": "Name 2", "vlan_id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexServerFirmwareVersionInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.ServerFirmwareVersionInfo","ObjectType":"hyperflex.ServerFirmwareVersionInfo","Version":"Version %d","ServerPlatform":"ServerPlatform %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "hyperflex.ServerFirmwareVersionInfo", "object_type": "hyperflex.ServerFirmwareVersionInfo", "nr_version": "Version 1", "server_platform": "ServerPlatform 1"}, {"class_id": "hyperflex.ServerFirmwareVersionInfo", "object_type": "hyperflex.ServerFirmwareVersionInfo", "nr_version": "Version 2", "server_platform": "ServerPlatform 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseKeysInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LicenseId":"LicenseId %d","Pid":"Pid %d","ClassId":"iaas.LicenseKeysInfo","ObjectType":"iaas.LicenseKeysInfo","Count":32,"ExpirationDate":"ExpirationDate %d"}`
	...
	expectedOp := []map[string]interface{}{{"license_id": "LicenseId 1", "pid": "Pid 1", "class_id": "iaas.LicenseKeysInfo", "object_type": "iaas.LicenseKeysInfo", "nr_count": 32, "expiration_date": "ExpirationDate 1"}, {"license_id": "LicenseId 2", "pid": "Pid 2", "class_id": "iaas.LicenseKeysInfo", "object_type": "iaas.LicenseKeysInfo", "nr_count": 32, "expiration_date": "ExpirationDate 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasWorkflowSteps(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"StatusMessage":"StatusMessage %d","CompletedTime":"CompletedTime %d","ObjectType":"iaas.WorkflowSteps","ClassId":"iaas.WorkflowSteps","Name":"Name %d","Status":"Status %d"}`
	...
	expectedOp := []map[string]interface{}{{"status_message": "StatusMessage 1", "completed_time": "CompletedTime 1", "object_type": "iaas.WorkflowSteps", "class_id": "iaas.WorkflowSteps", "name": "Name 1", "status": "Status 1"}, {"status_message": "StatusMessage 2", "completed_time": "CompletedTime 2", "object_type": "iaas.WorkflowSteps", "class_id": "iaas.WorkflowSteps", "name": "Name 2", "status": "Status 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIamAccountPermissions(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"iam.AccountPermissions","ClassId":"iam.AccountPermissions","AccountName":"AccountName %d","AccountStatus":"AccountStatus %d","AccountIdentifier":"AccountIdentifier %d"}`
...
func TestFlattenListIppoolIpV4Block(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"From":"From %d","To":"To %d","ClassId":"ippool.IpV4Block","ObjectType":"ippool.IpV4Block","Size":32}`
	...
	expectedOp := []map[string]interface{}{{"from": "From 1", "to": "To 1", "class_id": "ippool.IpV4Block", "object_type": "ippool.IpV4Block", "size": 32}, {"from": "From 2", "to": "To 2", "class_id": "ippool.IpV4Block", "object_type": "ippool.IpV4Block", "size": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIppoolIpV6Block(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"ippool.IpV6Block","Size":32,"ClassId":"ippool.IpV6Block","To":"To %d","From":"From %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "ippool.IpV6Block", "size": 32, "class_id": "ippool.IpV6Block", "to": "To 1", "from": "From 1"}, {"object_type": "ippool.IpV6Block", "size": 32, "class_id": "ippool.IpV6Block", "to": "To 2", "from": "From 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIqnpoolIqnSuffixBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"From":32,"Suffix":"Suffix %d","ClassId":"iqnpool.IqnSuffixBlock","ObjectType":"iqnpool.IqnSuffixBlock","Size":32,"To":32}`
	...
	expectedOp := []map[string]interface{}{{"from": 32, "suffix": "Suffix 1", "class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "size": 32, "to": 32}, {"from": 32, "suffix": "Suffix 2", "class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "size": 32, "to": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesEthernet(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.Ethernet","Name":"Name %d","ClassId":"kubernetes.Ethernet","Gateway":"Gateway %d","Mtu":32,"ProviderName":"ProviderName %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "kubernetes.Ethernet", "name": "Name 1", "class_id": "kubernetes.Ethernet", "gateway": "Gateway 1", "mtu": 32, "provider_name": "ProviderName 1"}, {"object_type": "kubernetes.Ethernet", "name": "Name 2", "class_id": "kubernetes.Ethernet", "gateway": "Gateway 2", "mtu": 32, "provider_name": "ProviderName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesNodeAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","ClassId":"kubernetes.NodeAddress","ObjectType":"kubernetes.NodeAddress","Address":"Address %d"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "class_id": "kubernetes.NodeAddress", "object_type": "kubernetes.NodeAddress", "address": "Address 1"}, {"type": "Type 2", "class_id": "kubernetes.NodeAddress", "object_type": "kubernetes.NodeAddress", "address": "Address 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesNodeGroupLabel(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.NodeGroupLabel","ObjectType":"kubernetes.NodeGroupLabel","Key":"Key %d","Value":"Value %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "kubernetes.NodeGroupLabel", "object_type": "kubernetes.NodeGroupLabel", "key": "Key 1", "value": "Value 1"}, {"class_id": "kubernetes.NodeGroupLabel", "object_type": "kubernetes.NodeGroupLabel", "key": "Key 2", "value": "Value 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesTaint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Effect":"Effect %d","Key":"Key %d","Value":"Value %d","ClassId":"kubernetes.Taint","ObjectType":"kubernetes.Taint"}`
	...
	expectedOp := []map[string]interface{}{{"effect": "Effect 1", "key": "Key 1", "value": "Value 1", "class_id": "kubernetes.Taint", "object_type": "kubernetes.Taint"}, {"effect": "Effect 2", "key": "Key 2", "value": "Value 2", "class_id": "kubernetes.Taint", "object_type": "kubernetes.Taint"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMemoryPersistentMemoryGoal(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"memory.PersistentMemoryGoal","MemoryModePercentage":32,"PersistentMemoryType":"PersistentMemoryType %d","SocketId":"SocketId %d","ClassId":"memory.PersistentMemoryGoal"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "memory.PersistentMemoryGoal", "memory_mode_percentage": 32, "persistent_memory_type": "PersistentMemoryType 1", "socket_id": "SocketId 1", "class_id": "memory.PersistentMemoryGoal"}, {"object_type": "memory.PersistentMemoryGoal", "memory_mode_percentage": 32, "persistent_memory_type": "PersistentMemoryType 2", "socket_id": "SocketId 2", "class_id": "memory.PersistentMemoryGoal"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaPropDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"meta.PropDefinition","ObjectType":"meta.PropDefinition","Name":"Name %d","OpSecurity":"OpSecurity %d","Type":"Type %d","IsCollection":true,"Kind":"Kind %d","SearchWeight":32.000000,"ApiAccess":"ApiAccess %d","IsComplexType":true}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "meta.PropDefinition", "object_type": "meta.PropDefinition", "name": "Name 1", "op_security": "OpSecurity 1", "type": "Type 1", "is_collection": true, "kind": "Kind 1", "search_weight": 32.000000, "api_access": "ApiAccess 1", "is_complex_type": true}, {"class_id": "meta.PropDefinition", "object_type": "meta.PropDefinition", "name": "Name 2", "op_security": "OpSecurity 2", "type": "Type 2", "is_collection": true, "kind": "Kind 2", "search_weight": 32.000000, "api_access": "ApiAccess 2", "is_complex_type": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaRelationshipDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","ObjectType":"meta.RelationshipDefinition","Collection":true,"PeerSync":true,"ApiAccess":"ApiAccess %d","ClassId":"meta.RelationshipDefinition","PeerRelName":"PeerRelName %d","Type":"Type %d","ExportWithPeer":true,"Export":true}`
...
func TestFlattenListNiaapiRevisionInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RevisionNo":"RevisionNo %d","RevisionComment":"RevisionComment %d","ClassId":"niaapi.RevisionInfo","ObjectType":"niaapi.RevisionInfo"}`
	...
	expectedOp := []map[string]interface{}{{"revision_no": "RevisionNo 1", "revision_comment": "RevisionComment 1", "class_id": "niaapi.RevisionInfo", "object_type": "niaapi.RevisionInfo"}, {"revision_no": "RevisionNo 2", "revision_comment": "RevisionComment 2", "class_id": "niaapi.RevisionInfo", "object_type": "niaapi.RevisionInfo"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryDeploymentStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.DeploymentStatus","ObjectType":"niatelemetry.DeploymentStatus","Name":"Name %d","Status":"Status %d","Id":32}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "niatelemetry.DeploymentStatus", "object_type": "niatelemetry.DeploymentStatus", "name": "Name 1", "status": "Status 1", "id": 32}, {"class_id": "niatelemetry.DeploymentStatus", "object_type": "niatelemetry.DeploymentStatus", "name": "Name 2", "status": "Status 2", "id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryLogicalLink(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LinkAddr2":"LinkAddr2 %d","LinkType":"LinkType %d","IsPresent":true,"LinkState":"LinkState %d","Uptime":"Uptime %d","DbId":32,"ClassId":"niatelemetry.LogicalLink","ObjectType":"niatelemetry.LogicalLink","LinkAddr1":"LinkAddr1 %d"}`
	...
	expectedOp := []map[string]interface{}{{"link_addr2": "LinkAddr2 1", "link_type": "LinkType 1", "is_present": true, "link_state": "LinkState 1", "uptime": "Uptime 1", "db_id": 32, "class_id": "niatelemetry.LogicalLink", "object_type": "niatelemetry.LogicalLink", "link_addr1": "LinkAddr1 1"}, {"link_addr2": "LinkAddr2 2", "link_type": "LinkType 2", "is_present": true, "link_state": "LinkState 2", "uptime": "Uptime 2", "db_id": 32, "class_id": "niatelemetry.LogicalLink", "object_type": "niatelemetry.LogicalLink", "link_addr1": "LinkAddr1 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryVniStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Vni":"Vni %d","VniState":"VniState %d","ClassId":"niatelemetry.VniStatus","ObjectType":"niatelemetry.VniStatus","VniType":"VniType %d"}`
	...
	expectedOp := []map[string]interface{}{{"vni": "Vni 1", "vni_state": "VniState 1", "class_id": "niatelemetry.VniStatus", "object_type": "niatelemetry.VniStatus", "vni_type": "VniType 1"}, {"vni": "Vni 2", "vni_state": "VniState 2", "class_id": "niatelemetry.VniStatus", "object_type": "niatelemetry.VniStatus", "vni_type": "VniType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNtpAuthNtpServer(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ServerName":"ServerName %d","SymKeyId":32,"SymKeyValue":"SymKeyValue %d","ClassId":"ntp.AuthNtpServer","ObjectType":"ntp.AuthNtpServer","KeyType":"KeyType %d"}`
...
func TestFlattenListOnpremImagePackage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"onprem.ImagePackage","FileSize":32,"FilePath":"FilePath %d","PackageType":"PackageType %d","FileSha":"FileSha %d","Name":"Name %d","ObjectType":"onprem.ImagePackage","Version":"Version %d","Filename":"Filename %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "onprem.ImagePackage", "file_size": 32, "file_path": "FilePath 1", "package_type": "PackageType 1", "file_sha": "FileSha 1", "name": "Name 1", "object_type": "onprem.ImagePackage", "nr_version": "Version 1", "filename": "Filename 1"}, {"class_id": "onprem.ImagePackage", "file_size": 32, "file_path": "FilePath 2", "package_type": "PackageType 2", "file_sha": "FileSha 2", "name": "Name 2", "object_type": "onprem.ImagePackage", "nr_version": "Version 2", "filename": "Filename 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOnpremUpgradePhase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Failed":true,"Message":"Message %d","ClassId":"onprem.UpgradePhase","ObjectType":"onprem.UpgradePhase","ElapsedTime":32,"Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"failed": true, "message": "Message 1", "class_id": "onprem.UpgradePhase", "object_type": "onprem.UpgradePhase", "elapsed_time": 32, "name": "Name 1"}, {"failed": true, "message": "Message 2", "class_id": "onprem.UpgradePhase", "object_type": "onprem.UpgradePhase", "elapsed_time": 32, "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOprsKvpair(t *testing.T) {
	p := []models.OprsKvpair{}
	var d = &schema.ResourceData{}
	c := `{"Value":"Value %d","ClassId":"oprs.Kvpair","ObjectType":"oprs.Kvpair","Key":"Key %d"}`
	...
	expectedOp := []map[string]interface{}{{"value": "Value 1", "class_id": "oprs.Kvpair", "object_type": "oprs.Kvpair", "key": "Key 1"}, {"value": "Value 2", "class_id": "oprs.Kvpair", "object_type": "oprs.Kvpair", "key": "Key 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOsPlaceHolder(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"os.PlaceHolder","IsValueSet":true,"ClassId":"os.PlaceHolder"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "os.PlaceHolder", "is_value_set": true, "class_id": "os.PlaceHolder"}, {"object_type": "os.PlaceHolder", "is_value_set": true, "class_id": "os.PlaceHolder"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSdwanTemplateInputsType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Editable":true,"Required":true,"Key":"Key %d","Title":"Title %d","ClassId":"sdwan.TemplateInputsType","ObjectType":"sdwan.TemplateInputsType","Type":"Type %d","Template":"Template %d","Value":"Value %d"}`
	...
	expectedOp := []map[string]interface{}{{"editable": true, "required": true, "key": "Key 1", "title": "Title 1", "class_id": "sdwan.TemplateInputsType", "object_type": "sdwan.TemplateInputsType", "type": "Type 1", "template": "Template 1", "value": "Value 1"}, {"editable": true, "required": true, "key": "Key 2", "title": "Title 2", "class_id": "sdwan.TemplateInputsType", "object_type": "sdwan.TemplateInputsType", "type": "Type 2", "template": "Template 2", "value": "Value 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpTrap(t *testing.T) {
	p := []models.SnmpTrap{}
	var d = &schema.ResourceData{}
	c := `{"Version":"Version %d","Community":"Community %d","Type":"Type %d","Destination":"Destination %d","Port":32,"ClassId":"snmp.Trap","ObjectType":"snmp.Trap","Enabled":true,"User":"User %d"}`
	...
	expectedOp := []map[string]interface{}{{"nr_version": "Version 1", "community": "Community 1", "type": "Type 1", "destination": "Destination 1", "port": 32, "class_id": "snmp.Trap", "object_type": "snmp.Trap", "enabled": true, "user": "User 1"}, {"nr_version": "Version 2", "community": "Community 2", "type": "Type 2", "destination": "Destination 2", "port": 32, "class_id": "snmp.Trap", "object_type": "snmp.Trap", "enabled": true, "user": "User 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpUser(t *testing.T) {
	p := []models.SnmpUser{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"snmp.User","IsAuthPasswordSet":true,"SecurityLevel":"SecurityLevel %d","Name":"Name %d","AuthType":"AuthType %d","ClassId":"snmp.User","IsPrivacyPasswordSet":true,"PrivacyType":"PrivacyType %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "snmp.User", "is_auth_password_set": true, "security_level": "SecurityLevel 1", "name": "Name 1", "auth_type": "AuthType 1", "class_id": "snmp.User", "is_privacy_password_set": true, "privacy_type": "PrivacyType 1"}, {"object_type": "snmp.User", "is_auth_password_set": true, "security_level": "SecurityLevel 2", "name": "Name 2", "auth_type": "AuthType 2", "class_id": "snmp.User", "is_privacy_password_set": true, "privacy_type": "PrivacyType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStorageNetAppExportPolicyRule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"storage.NetAppExportPolicyRule","ClassId":"storage.NetAppExportPolicyRule","Index":32,"User":"User %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "storage.NetAppExportPolicyRule", "class_id": "storage.NetAppExportPolicyRule", "index": 32, "user": "User 1"}, {"object_type": "storage.NetAppExportPolicyRule", "class_id": "storage.NetAppExportPolicyRule", "index": 32, "user": "User 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStoragePureReplicationBlackout(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.PureReplicationBlackout","ObjectType":"storage.PureReplicationBlackout","Start":"Start %d","End":"End %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "storage.PureReplicationBlackout", "object_type": "storage.PureReplicationBlackout", "start": "Start 1", "end": "End 1"}, {"class_id": "storage.PureReplicationBlackout", "object_type": "storage.PureReplicationBlackout", "start": "Start 2", "end": "End 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStorageStorageContainerHostMountStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Reason":"Reason %d","Accessibility":"Accessibility %d","HostName":"HostName %d","Mounted":true,"ClassId":"storage.StorageContainerHostMountStatus","ObjectType":"storage.StorageContainerHostMountStatus"}`
...
func TestFlattenListSyslogRemoteClientBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Hostname":"Hostname %d","MinSeverity":"MinSeverity %d","Port":32,"Protocol":"Protocol %d","ClassId":"syslog.RemoteClientBase","ObjectType":"syslog.RemoteClientBase","Enabled":true}`
...
func TestFlattenListTamAction(t *testing.T) {
	p := []models.TamAction{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"tam.Action","AlertType":"AlertType %d","AffectedObjectType":"AffectedObjectType %d","Name":"Name %d","OperationType":"OperationType %d","ClassId":"tam.Action","Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "tam.Action", "alert_type": "AlertType 1", "affected_object_type": "AffectedObjectType 1", "name": "Name 1", "operation_type": "OperationType 1", "class_id": "tam.Action", "type": "Type 1"}, {"object_type": "tam.Action", "alert_type": "AlertType 2", "affected_object_type": "AffectedObjectType 2", "name": "Name 2", "operation_type": "OperationType 2", "class_id": "tam.Action", "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTamS3DataSource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","Type":"Type %d","S3Path":"S3Path %d","ClassId":"tam.S3DataSource","ObjectType":"tam.S3DataSource"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "type": "Type 1", "s3_path": "S3Path 1", "class_id": "tam.S3DataSource", "object_type": "tam.S3DataSource"}, {"name": "Name 2", "type": "Type 2", "s3_path": "S3Path 2", "class_id": "tam.S3DataSource", "object_type": "tam.S3DataSource"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTerraformCloudResource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ResourceId":"ResourceId %d","CurrentStatus":"CurrentStatus %d","DesiredStatus":"DesiredStatus %d","ClassId":"terraform.CloudResource","ObjectType":"terraform.CloudResource"}`
	...
	expectedOp := []map[string]interface{}{{"resource_id": "ResourceId 1", "current_status": "CurrentStatus 1", "desired_status": "DesiredStatus 1", "class_id": "terraform.CloudResource", "object_type": "terraform.CloudResource"}, {"resource_id": "ResourceId 2", "current_status": "CurrentStatus 2", "desired_status": "DesiredStatus 2", "class_id": "terraform.CloudResource", "object_type": "terraform.CloudResource"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUcsdConnectorPack(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"ucsd.ConnectorPack","State":"State %d","ClassId":"ucsd.ConnectorPack","ConnectorFeature":"ConnectorFeature %d","Name":"Name %d","Version":"Version %d","DownloadedVersion":"DownloadedVersion %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "ucsd.ConnectorPack", "state": "State 1", "class_id": "ucsd.ConnectorPack", "connector_feature": "ConnectorFeature 1", "name": "Name 1", "nr_version": "Version 1", "downloaded_version": "DownloadedVersion 1"}, {"object_type": "ucsd.ConnectorPack", "state": "State 2", "class_id": "ucsd.ConnectorPack", "connector_feature": "ConnectorFeature 2", "name": "Name 2", "nr_version": "Version 2", "downloaded_version": "DownloadedVersion 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUuidpoolUuidBlock(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"to": "To 1", "class_id": "uuidpool.UuidBlock", "object_type": "uuidpool.UuidBlock", "size": 32, "from": "From 1"}, {"to": "To 2", "class_id": "uuidpool.UuidBlock", "object_type": "uuidpool.UuidBlock", "size": 32, "from": "From 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationNetworkInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ConnectAtPowerOn":true,"Bridge":"Bridge %d","NicId":"NicId %d","Name":"Name %d","PrivateIpAllocationMode":"PrivateIpAllocationMode %d","Ipv6Address":true,"ClassId":"virtualization.NetworkInterface","SubnetId":"SubnetId %d","DirectPathIo":true,"Order":32,"NetworkId":"NetworkId %d","ObjectType":"virtualization.NetworkInterface","IpForwardingEnabled":true,"MacAddress":"MacAddress %d","AdaptorType":"AdaptorType %d","PublicIpAllocate":true}`
	...
	expectedOp := []map[string]interface{}{{"connect_at_power_on": true, "bridge": "Bridge 1", "nic_id": "NicId 1", "name": "Name 1", "private_ip_allocation_mode": "PrivateIpAllocationMode 1", "ipv6_address": true, "class_id": "virtualization.NetworkInterface", "subnet_id": "SubnetId 1", "direct_path_io": true, "order": 32, "network_id": "NetworkId 1", "object_type": "virtualization.NetworkInterface", "ip_forwarding_enabled": true, "mac_address": "MacAddress 1", "adaptor_type": "AdaptorType 1", "public_ip_allocate": true}, {"connect_at_power_on": true, "bridge": "Bridge 2", "nic_id": "NicId 2", "name": "Name 2", "private_ip_allocation_mode": "PrivateIpAllocationMode 2", "ipv6_address": true, "class_id": "virtualization.NetworkInterface", "subnet_id": "SubnetId 2", "direct_path_io": true, "order": 32, "network_id": "NetworkId 2", "object_type": "virtualization.NetworkInterface", "ip_forwarding_enabled": true, "mac_address": "MacAddress 2", "adaptor_type": "AdaptorType 2", "public_ip_allocate": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVirtualMachineDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.VirtualMachineDisk","ClassId":"virtualization.VirtualMachineDisk","Bus":"Bus %d","Name":"Name %d","Order":32,"Type":"Type %d","VirtualDiskReference":"VirtualDiskReference %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "virtualization.VirtualMachineDisk", "class_id": "virtualization.VirtualMachineDisk", "bus": "Bus 1", "name": "Name 1", "order": 32, "type": "Type 1", "virtual_disk_reference": "VirtualDiskReference 1"}, {"object_type": "virtualization.VirtualMachineDisk", "class_id": "virtualization.VirtualMachineDisk", "bus": "Bus 2", "name": "Name 2", "order": 32, "type": "Type 2", "virtual_disk_reference": "VirtualDiskReference 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"BootOrder":32,"Bus":"Bus %d","Name":"Name %d","Type":"Type %d","ClassId":"virtualization.VmDisk","ObjectType":"virtualization.VmDisk","VirtualDiskReference":"VirtualDiskReference %d"}`
	...
	expectedOp := []map[string]interface{}{{"boot_order": 32, "bus": "Bus 1", "name": "Name 1", "type": "Type 1", "class_id": "virtualization.VmDisk", "object_type": "virtualization.VmDisk", "virtual_disk_reference": "VirtualDiskReference 1"}, {"boot_order": 32, "bus": "Bus 2", "name": "Name 2", "type": "Type 2", "class_id": "virtualization.VmDisk", "object_type": "virtualization.VmDisk", "virtual_disk_reference": "VirtualDiskReference 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmInterface","Bridge":"Bridge %d","MacAddress":"MacAddress %d","Model":"Model %d","Name":"Name %d","ObjectType":"virtualization.VmInterface"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "virtualization.VmInterface", "bridge": "Bridge 1", "mac_address": "MacAddress 1", "model": "Model 1", "name": "Name 1", "object_type": "virtualization.VmInterface"}, {"class_id": "virtualization.VmInterface", "bridge": "Bridge 2", "mac_address": "MacAddress 2", "model": "Model 2", "name": "Name 2", "object_type": "virtualization.VmInterface"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVmediaMapping(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RemotePath":"RemotePath %d","AuthenticationProtocol":"AuthenticationProtocol %d","FileLocation":"FileLocation %d","MountProtocol":"MountProtocol %d","VolumeName":"VolumeName %d","IsPasswordSet":true,"SanitizedFileLocation":"SanitizedFileLocation %d","RemoteFile":"RemoteFile %d","ObjectType":"vmedia.Mapping","MountOptions":"MountOptions %d","HostName":"HostName %d","DeviceType":"DeviceType %d","ClassId":"vmedia.Mapping","Username":"Username %d"}`
	...
	expectedOp := []map[string]interface{}{{"remote_path": "RemotePath 1", "authentication_protocol": "AuthenticationProtocol 1", "file_location": "FileLocation 1", "mount_protocol": "MountProtocol 1", "volume_name": "VolumeName 1", "is_password_set": true, "sanitized_file_location": "SanitizedFileLocation 1", "remote_file": "RemoteFile 1", "object_type": "vmedia.Mapping", "mount_options": "MountOptions 1", "host_name": "HostName 1", "device_type": "DeviceType 1", "class_id": "vmedia.Mapping", "username": "Username 1"}, {"remote_path": "RemotePath 2", "authentication_protocol": "AuthenticationProtocol 2", "file_location": "FileLocation 2", "mount_protocol": "MountProtocol 2", "volume_name": "VolumeName 2", "is_password_set": true, "sanitized_file_location": "SanitizedFileLocation 2", "remote_file": "RemoteFile 2", "object_type": "vmedia.Mapping", "mount_options": "MountOptions 2", "host_name": "HostName 2", "device_type": "DeviceType 2", "class_id": "vmedia.Mapping", "username": "Username 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowActionWorkflowDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","WorkflowDefinitionName":"WorkflowDefinitionName %d","Label":"Label %d","ClassId":"workflow.ActionWorkflowDefinition","CatalogMoid":"CatalogMoid %d","Description":"Description %d","Version":32,"ObjectType":"workflow.ActionWorkflowDefinition"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "workflow_definition_name": "WorkflowDefinitionName 1", "label": "Label 1", "class_id": "workflow.ActionWorkflowDefinition", "catalog_moid": "CatalogMoid 1", "description": "Description 1", "nr_version": 32, "object_type": "workflow.ActionWorkflowDefinition"}, {"name": "Name 2", "workflow_definition_name": "WorkflowDefinitionName 2", "label": "Label 2", "class_id": "workflow.ActionWorkflowDefinition", "catalog_moid": "CatalogMoid 2", "description": "Description 2", "nr_version": 32, "object_type": "workflow.ActionWorkflowDefinition"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowApi(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Label":"Label %d","ObjectType":"workflow.Api","AssetTargetMoid":"AssetTargetMoid %d","Description":"Description %d","ErrorContentType":"ErrorContentType %d","ClassId":"workflow.Api","Body":"Body %d","StartDelay":32,"Name":"Name %d","ContentType":"ContentType %d","Timeout":32,"SkipOnCondition":"SkipOnCondition %d"}`
	...
	expectedOp := []map[string]interface{}{{"label": "Label 1", "object_type": "workflow.Api", "asset_target_moid": "AssetTargetMoid 1", "description": "Description 1", "error_content_type": "ErrorContentType 1", "class_id": "workflow.Api", "body": "Body 1", "start_delay": 32, "name": "Name 1", "content_type": "ContentType 1", "timeout": 32, "skip_on_condition": "SkipOnCondition 1"}, {"label": "Label 2", "object_type": "workflow.Api", "asset_target_moid": "AssetTargetMoid 2", "description": "Description 2", "error_content_type": "ErrorContentType 2", "class_id": "workflow.Api", "body": "Body 2", "start_delay": 32, "name": "Name 2", "content_type": "ContentType 2", "timeout": 32, "skip_on_condition": "SkipOnCondition 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowBaseDataType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","Required":true,"ClassId":"workflow.BaseDataType","ObjectType":"workflow.BaseDataType","Description":"Description %d","Label":"Label %d"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "required": true, "class_id": "workflow.BaseDataType", "object_type": "workflow.BaseDataType", "description": "Description 1", "label": "Label 1"}, {"name": "Name 2", "required": true, "class_id": "workflow.BaseDataType", "object_type": "workflow.BaseDataType", "description": "Description 2", "label": "Label 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SkipCondition":"SkipCondition %d","TaskMoid":"TaskMoid %d","CatalogMoid":"CatalogMoid %d","ObjectType":"workflow.RollbackTask","Version":32,"Description":"Description %d","Name":"Name %d","ClassId":"workflow.RollbackTask"}`
	...
	expectedOp := []map[string]interface{}{{"skip_condition": "SkipCondition 1", "task_moid": "TaskMoid 1", "catalog_moid": "CatalogMoid 1", "object_type": "workflow.RollbackTask", "nr_version": 32, "description": "Description 1", "name": "Name 1", "class_id": "workflow.RollbackTask"}, {"skip_condition": "SkipCondition 2", "task_moid": "TaskMoid 2", "catalog_moid": "CatalogMoid 2", "object_type": "workflow.RollbackTask", "nr_version": 32, "description": "Description 2", "name": "Name 2", "class_id": "workflow.RollbackTask"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackWorkflowTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RollbackCompleted":true,"ObjectType":"workflow.RollbackWorkflowTask","ClassId":"workflow.RollbackWorkflowTask","TaskInfoMoid":"TaskInfoMoid %d","Name":"Name %d","TaskPath":"TaskPath %d","RefName":"RefName %d","Description":"Description %d","RollbackTaskName":"RollbackTaskName %d","Status":"Status %d"}`
	...
	expectedOp := []map[string]interface{}{{"rollback_completed": true, "object_type": "workflow.RollbackWorkflowTask", "class_id": "workflow.RollbackWorkflowTask", "task_info_moid": "TaskInfoMoid 1", "name": "Name 1", "task_path": "TaskPath 1", "ref_name": "RefName 1", "description": "Description 1", "rollback_task_name": "RollbackTaskName 1", "status": "Status 1"}, {"rollback_completed": true, "object_type": "workflow.RollbackWorkflowTask", "class_id": "workflow.RollbackWorkflowTask", "task_info_moid": "TaskInfoMoid 2", "name": "Name 2", "task_path": "TaskPath 2", "ref_name": "RefName 2", "description": "Description 2", "rollback_task_name": "RollbackTaskName 2", "status": "Status 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListX509Certificate(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"object_type": "x509.Certificate", "class_id": "x509.Certificate", "sha256_fingerprint": "Sha256Fingerprint 1", "pem_certificate": "PemCertificate 1", "signature_algorithm": "SignatureAlgorithm 1"}, {"object_type": "x509.Certificate", "class_id": "x509.Certificate", "sha256_fingerprint": "Sha256Fingerprint 2", "pem_certificate": "PemCertificate 2", "signature_algorithm": "SignatureAlgorithm 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenMapAssetDeploymentDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"OldInstanceId":"OldInstanceId %d","ApplicationName":"ApplicationName %d","ItemType":"ItemType %d","ClassId":"asset.DeploymentDeviceInformation","OldDeviceStatusId":32,"OldDeviceId":"OldDeviceId %d","MlbProductName":"MlbProductName %d","ObjectType":"asset.DeploymentDeviceInformation","InstanceId":"InstanceId %d","Description":"Description %d","MlbProductId":32,"OldDeviceStatusDescription":"OldDeviceStatusDescription %d"}`
	...
	expectedOp := map[string]interface{}{"old_instance_id": "OldInstanceId 1", "application_name": "ApplicationName 1", "item_type": "ItemType 1", "class_id": "asset.DeploymentDeviceInformation", "old_device_status_id": 32, "old_device_id": "OldDeviceId 1", "mlb_product_name": "MlbProductName 1", "object_type": "asset.DeploymentDeviceInformation", "instance_id": "InstanceId 1", "description": "Description 1", "mlb_product_id": 32, "old_device_status_description": "OldDeviceStatusDescription 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"asset.DeviceInformation","InstanceId":"InstanceId %d","MlbProductId":32,"MlbOfferType":"MlbOfferType %d","OldDeviceId":"OldDeviceId %d","OldDeviceStatusId":32,"ObjectType":"asset.DeviceInformation","ProductFamily":"ProductFamily %d","ItemType":"ItemType %d","UnitOfMeasure":"UnitOfMeasure %d","MlbProductName":"MlbProductName %d","OldInstanceId":"OldInstanceId %d","ApplicationName":"ApplicationName %d","OldDeviceStatusDescription":"OldDeviceStatusDescription %d","ProductType":"ProductType %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "asset.DeviceInformation", "instance_id": "InstanceId 1", "mlb_product_id": 32, "mlb_offer_type": "MlbOfferType 1", "old_device_id": "OldDeviceId 1", "old_device_status_id": 32, "object_type": "asset.DeviceInformation", "product_family": "ProductFamily 1", "item_type": "ItemType 1", "unit_of_measure": "UnitOfMeasure 1", "mlb_product_name": "MlbProductName 1", "old_instance_id": "OldInstanceId 1", "application_name": "ApplicationName 1", "old_device_status_description": "OldDeviceStatusDescription 1", "product_type": "ProductType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceStatistics(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MembershipRatio":32.000000,"ClusterReplicationFactor":32,"ObjectType":"asset.DeviceStatistics","ClassId":"asset.DeviceStatistics","ClusterName":"ClusterName %d","Connected":32,"ClusterDeploymentType":"ClusterDeploymentType %d","MemoryMirroringFactor":32.000000,"ClusterDeviceMoid":"ClusterDeviceMoid %d"}`
	...
	expectedOp := map[string]interface{}{"membership_ratio": 32.000000, "cluster_replication_factor": 32, "object_type": "asset.DeviceStatistics", "class_id": "asset.DeviceStatistics", "cluster_name": "ClusterName 1", "connected": 32, "cluster_deployment_type": "ClusterDeploymentType 1", "memory_mirroring_factor": 32.000000, "cluster_device_moid": "ClusterDeviceMoid 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetProductInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"asset.ProductInformation","ObjectType":"asset.ProductInformation","Description":"Description %d","Family":"Family %d","Group":"Group %d","Number":"Number %d","SubType":"SubType %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "asset.ProductInformation", "object_type": "asset.ProductInformation", "description": "Description 1", "family": "Family 1", "group": "Group 1", "number": "Number 1", "sub_type": "SubType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCapabilitySwitchNetworkLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"capability.SwitchNetworkLimits","MaximumIgmpGroups":32,"MaximumEthernetPortChannels":32,"MaxCompressedPortVlanCount":32,"MaxUncompressedPortVlanCount":32,"MaximumVlans":32,"MaximumVifs":32,"MaximumFcPortChannelMembers":32,"MaximumActiveTrafficMonitoringSessions":32,"MaximumSecondaryVlan":32,"ObjectType":"capability.SwitchNetworkLimits","MinimumActiveFans":32,"MaximumFcPortChannels":32,"MaximumEthernetUplinkPorts":32,"MaximumPortChannelMembers":32,"MaximumSecondaryVlanPerPrimary":32,"MaximumPrimaryVlan":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "capability.SwitchNetworkLimits", "maximum_igmp_groups": 32, "maximum_ethernet_port_channels": 32, "max_compressed_port_vlan_count": 32, "max_uncompressed_port_vlan_count": 32, "maximum_vlans": 32, "maximum_vifs": 32, "maximum_fc_port_channel_members": 32, "maximum_active_traffic_monitoring_sessions": 32, "maximum_secondary_vlan": 32, "object_type": "capability.SwitchNetworkLimits", "minimum_active_fans": 32, "maximum_fc_port_channels": 32, "maximum_ethernet_uplink_ports": 32, "maximum_port_channel_members": 32, "maximum_secondary_vlan_per_primary": 32, "maximum_primary_vlan": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchStorageLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaximumZoneCount":32,"MaximumUserZoneCount":32,"MaximumVirtualFcInterfaces":32,"ClassId":"capability.SwitchStorageLimits","ObjectType":"capability.SwitchStorageLimits","MaximumVirtualFcInterfacesPerBladeServer":32,"MaximumVsans":32}`
	...
	expectedOp := map[string]interface{}{"maximum_zone_count": 32, "maximum_user_zone_count": 32, "maximum_virtual_fc_interfaces": 32, "class_id": "capability.SwitchStorageLimits", "object_type": "capability.SwitchStorageLimits", "maximum_virtual_fc_interfaces_per_blade_server": 32, "maximum_vsans": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchSystemLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"capability.SwitchSystemLimits","MaximumChassisCount":32,"MaximumFexPerDomain":32,"MaximumServersPerDomain":32,"ObjectType":"capability.SwitchSystemLimits"}`
	...
	expectedOp := map[string]interface{}{"class_id": "capability.SwitchSystemLimits", "maximum_chassis_count": 32, "maximum_fex_per_domain": 32, "maximum_servers_per_domain": 32, "object_type": "capability.SwitchSystemLimits"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudBillingUnit(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"cloud.BillingUnit","ObjectType":"cloud.BillingUnit","BillingId":"BillingId %d","Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "cloud.BillingUnit", "object_type": "cloud.BillingUnit", "billing_id": "BillingId 1", "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCloudCloudRegion(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RegionId":"RegionId %d","ClassId":"cloud.CloudRegion","ObjectType":"cloud.CloudRegion","Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"region_id": "RegionId 1", "class_id": "cloud.CloudRegion", "object_type": "cloud.CloudRegion", "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudInstanceType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.InstanceType","Name":"Name %d","Memory":32,"Cpus":32,"ClassId":"cloud.InstanceType","InstanceTypeId":"InstanceTypeId %d","Platform":"Platform %d","CpuSpeed":32,"Architecture":"Architecture %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "cloud.InstanceType", "name": "Name 1", "memory": 32, "cpus": 32, "class_id": "cloud.InstanceType", "instance_type_id": "InstanceTypeId 1", "platform": "Platform 1", "cpu_speed": 32, "architecture": "Architecture 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudVolumeIopsInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ThroughputWriteLimit":32,"IopsReadLimit":32,"ClassId":"cloud.VolumeIopsInfo","ObjectType":"cloud.VolumeIopsInfo","IopsWriteLimit":32,"ThroughputReadLimit":32}`
	...
	expectedOp := map[string]interface{}{"throughput_write_limit": 32, "iops_read_limit": 32, "class_id": "cloud.VolumeIopsInfo", "object_type": "cloud.VolumeIopsInfo", "iops_write_limit": 32, "throughput_read_limit": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCommIpV4Interface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Gateway":"Gateway %d","IpAddress":"IpAddress %d","ClassId":"comm.IpV4Interface","ObjectType":"comm.IpV4Interface","Netmask":"Netmask %d"}`
	...
	expectedOp := map[string]interface{}{"gateway": "Gateway 1", "ip_address": "IpAddress 1", "class_id": "comm.IpV4Interface", "object_type": "comm.IpV4Interface", "netmask": "Netmask 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCommIpV6Interface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"comm.IpV6Interface","ObjectType":"comm.IpV6Interface","Gateway":"Gateway %d","IpAddress":"IpAddress %d","Prefix":"Prefix %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "comm.IpV6Interface", "object_type": "comm.IpV6Interface", "gateway": "Gateway 1", "ip_address": "IpAddress 1", "prefix": "Prefix 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapComputeAlarmSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"compute.AlarmSummary","Warning":32,"Critical":32,"ClassId":"compute.AlarmSummary"}`
	...
	expectedOp := map[string]interface{}{"object_type": "compute.AlarmSummary", "warning": 32, "critical": 32, "class_id": "compute.AlarmSummary"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapComputeServerConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UserLabel":"UserLabel %d","ClassId":"compute.ServerConfig","ObjectType":"compute.ServerConfig","AssetTag":"AssetTag %d"}`
	...
	expectedOp := map[string]interface{}{"user_label": "UserLabel 1", "class_id": "compute.ServerConfig", "object_type": "compute.ServerConfig", "asset_tag": "AssetTag 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapComputeStoragePhysicalDriveOperation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AdminAction":"AdminAction %d","ControllerId":"ControllerId %d","ObjectType":"compute.StoragePhysicalDriveOperation","ClassId":"compute.StoragePhysicalDriveOperation"}`
	...
	expectedOp := map[string]interface{}{"admin_action": "AdminAction 1", "controller_id": "ControllerId 1", "object_type": "compute.StoragePhysicalDriveOperation", "class_id": "compute.StoragePhysicalDriveOperation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCondAlarmSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"cond.AlarmSummary","ObjectType":"cond.AlarmSummary","Warning":32,"Critical":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "cond.AlarmSummary", "object_type": "cond.AlarmSummary", "warning": 32, "critical": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapConvergedinfraPodSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"convergedinfra.PodSummary","ClassId":"convergedinfra.PodSummary","StorageCapacity":32,"StorageUtilization":32.000000,"NodeCount":32,"StorageAvailable":32,"ActiveNodes":32,"VmCount":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "convergedinfra.PodSummary", "class_id": "convergedinfra.PodSummary", "storage_capacity": 32, "storage_utilization": 32.000000, "node_count": 32, "storage_available": 32, "active_nodes": 32, "vm_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFabricUdldSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AdminState":"AdminState %d","Mode":"Mode %d","ClassId":"fabric.UdldSettings","ObjectType":"fabric.UdldSettings"}`
	...
	expectedOp := map[string]interface{}{"admin_state": "AdminState 1", "mode": "Mode 1", "class_id": "fabric.UdldSettings", "object_type": "fabric.UdldSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFcpoolBlock(t *testing.T) {
	p := models.FcpoolBlock{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"fcpool.Block","Size":32,"ClassId":"fcpool.Block","From":"From %d","To":"To %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "fcpool.Block", "size": 32, "class_id": "fcpool.Block", "from": "From 1", "to": "To 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareDirectDownload(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"firmware.DirectDownload","ClassId":"firmware.DirectDownload","Username":"Username %d","ImageSource":"ImageSource %d","IsPasswordSet":true,"Upgradeoption":"Upgradeoption %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "firmware.DirectDownload", "class_id": "firmware.DirectDownload", "username": "Username 1", "image_source": "ImageSource 1", "is_password_set": true, "upgradeoption": "Upgradeoption 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareNetworkShare(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Upgradeoption":"Upgradeoption %d","MapType":"MapType %d","Username":"Username %d","IsPasswordSet":true,"ClassId":"firmware.NetworkShare","ObjectType":"firmware.NetworkShare"}`
	...
	expectedOp := map[string]interface{}{"upgradeoption": "Upgradeoption 1", "map_type": "MapType 1", "username": "Username 1", "is_password_set": true, "class_id": "firmware.NetworkShare", "object_type": "firmware.NetworkShare"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexAlarmSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.AlarmSummary","ObjectType":"hyperflex.AlarmSummary","Warning":32,"Critical":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.AlarmSummary", "object_type": "hyperflex.AlarmSummary", "warning": 32, "critical": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexAppSettingConstraint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MgmtPlatform":"MgmtPlatform %d","ServerModel":"ServerModel %d","DeploymentType":"DeploymentType %d","HxdpVersion":"HxdpVersion %d","HypervisorType":"HypervisorType %d","ClassId":"hyperflex.AppSettingConstraint","ObjectType":"hyperflex.AppSettingConstraint"}`
	...
	expectedOp := map[string]interface{}{"mgmt_platform": "MgmtPlatform 1", "server_model": "ServerModel 1", "deployment_type": "DeploymentType 1", "hxdp_version": "HxdpVersion 1", "hypervisor_type": "HypervisorType 1", "class_id": "hyperflex.AppSettingConstraint", "object_type": "hyperflex.AppSettingConstraint"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexBackupPolicySettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.BackupPolicySettings","SnapshotRetentionCount":32,"BackupDataStoreSizeUnit":"BackupDataStoreSizeUnit %d","ReplicationPairNamePrefix":"ReplicationPairNamePrefix %d","DataStoreEncryptionEnabled":true,"ClassId":"hyperflex.BackupPolicySettings","ReplicationIntervalInMinutes":32,"BackupDataStoreSize":32,"LocalSnapshotRetentionCount":32,"BackupDataStoreName":"BackupDataStoreName %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.BackupPolicySettings", "snapshot_retention_count": 32, "backup_data_store_size_unit": "BackupDataStoreSizeUnit 1", "replication_pair_name_prefix": "ReplicationPairNamePrefix 1", "data_store_encryption_enabled": true, "class_id": "hyperflex.BackupPolicySettings", "replication_interval_in_minutes": 32, "backup_data_store_size": 32, "local_snapshot_retention_count": 32, "backup_data_store_name": "BackupDataStoreName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexErrorStack(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MessageId":32,"Message":"Message %d","ObjectType":"hyperflex.ErrorStack","ClassId":"hyperflex.ErrorStack"}`
	...
	expectedOp := map[string]interface{}{"message_id": 32, "message": "Message 1", "object_type": "hyperflex.ErrorStack", "class_id": "hyperflex.ErrorStack"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHealthCheckScriptInfo(t *testing.T) {
	p := models.HyperflexHealthCheckScriptInfo{}
	var d = &schema.ResourceData{}
	c := `{"ScriptInput":"ScriptInput %d","ClassId":"hyperflex.HealthCheckScriptInfo","ScriptName":"ScriptName %d","Version":"Version %d","AggregateScriptName":"AggregateScriptName %d","HyperflexVersion":"HyperflexVersion %d","ScriptExecuteLocation":"ScriptExecuteLocation %d","ObjectType":"hyperflex.HealthCheckScriptInfo"}`
	...
	expectedOp := map[string]interface{}{"script_input": "ScriptInput 1", "class_id": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 1", "nr_version": "Version 1", "aggregate_script_name": "AggregateScriptName 1", "hyperflex_version": "HyperflexVersion 1", "script_execute_location": "ScriptExecuteLocation 1", "object_type": "hyperflex.HealthCheckScriptInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxLicenseAuthorizationDetailsDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HxLicenseAuthorizationDetailsDt","Status":"Status %d","CommunicationDeadlineDate":"CommunicationDeadlineDate %d","LastCommunicationAttemptDate":"LastCommunicationAttemptDate %d","ClassId":"hyperflex.HxLicenseAuthorizationDetailsDt","EvaluationPeriodExpiredAt":"EvaluationPeriodExpiredAt %d","EvaluationPeriodRemaining":"EvaluationPeriodRemaining %d","NextCommunicationAttemptDate":"NextCommunicationAttemptDate %d"}`
...
func TestFlattenMapHyperflexHxPlatformDatastoreConfigDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.HxPlatformDatastoreConfigDt","UsageType":"UsageType %d","NumStripesForLargeFiles":32,"NumMirrors":32,"ProvisionedCapacity":32,"ObjectType":"hyperflex.HxPlatformDatastoreConfigDt","SystemDatastore":true,"Name":"Name %d","DataBlockSize":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.HxPlatformDatastoreConfigDt", "usage_type": "UsageType 1", "num_stripes_for_large_files": 32, "num_mirrors": 32, "provisioned_capacity": 32, "object_type": "hyperflex.HxPlatformDatastoreConfigDt", "system_datastore": true, "name": "Name 1", "data_block_size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxRegistrationDetailsDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NextRenewalAttemptDate":"NextRenewalAttemptDate %d","LastRenewalAttemptDate":"LastRenewalAttemptDate %d","Status":"Status %d","RegistrationExpireDate":"RegistrationExpireDate %d","SmartAccount":"SmartAccount %d","OutOfComplianceStartDate":"OutOfComplianceStartDate %d","ObjectType":"hyperflex.HxRegistrationDetailsDt","ClassId":"hyperflex.HxRegistrationDetailsDt","RegistrationFailedReason":"RegistrationFailedReason %d","InitialRegistrationDate":"InitialRegistrationDate %d","VirtualAccount":"VirtualAccount %d"}`
	...
	expectedOp := map[string]interface{}{"next_renewal_attempt_date": "NextRenewalAttemptDate 1", "last_renewal_attempt_date": "LastRenewalAttemptDate 1", "status": "Status 1", "registration_expire_date": "RegistrationExpireDate 1", "smart_account": "SmartAccount 1", "out_of_compliance_start_date": "OutOfComplianceStartDate 1", "object_type": "hyperflex.HxRegistrationDetailsDt", "class_id": "hyperflex.HxRegistrationDetailsDt", "registration_failed_reason": "RegistrationFailedReason 1", "initial_registration_date": "InitialRegistrationDate 1", "virtual_account": "VirtualAccount 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxResiliencyInfoDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DataReplicationFactor":"DataReplicationFactor %d","PolicyCompliance":"PolicyCompliance %d","ClassId":"hyperflex.HxResiliencyInfoDt","ObjectType":"hyperflex.HxResiliencyInfoDt","HddFailuresTolerable":32,"ResiliencyState":"ResiliencyState %d","SsdFailuresTolerable":32,"NodeFailuresTolerable":32}`
	...
	expectedOp := map[string]interface{}{"data_replication_factor": "DataReplicationFactor 1", "policy_compliance": "PolicyCompliance 1", "class_id": "hyperflex.HxResiliencyInfoDt", "object_type": "hyperflex.HxResiliencyInfoDt", "hdd_failures_tolerable": 32, "resiliency_state": "ResiliencyState 1", "ssd_failures_tolerable": 32, "node_failures_tolerable": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexIpAddrRange(t *testing.T) {
	p := models.HyperflexIpAddrRange{}
	var d = &schema.ResourceData{}
	c := `{"Gateway":"Gateway %d","Netmask":"Netmask %d","StartAddr":"StartAddr %d","ClassId":"hyperflex.IpAddrRange","ObjectType":"hyperflex.IpAddrRange","EndAddr":"EndAddr %d"}`
	...
	expectedOp := map[string]interface{}{"gateway": "Gateway 1", "netmask": "Netmask 1", "start_addr": "StartAddr 1", "class_id": "hyperflex.IpAddrRange", "object_type": "hyperflex.IpAddrRange", "end_addr": "EndAddr 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexReplicationPeerInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Dcip":"Dcip %d","ClassId":"hyperflex.ReplicationPeerInfo","Mcip":"Mcip %d","Status":"Status %d","ReplCip":"ReplCip %d","ObjectType":"hyperflex.ReplicationPeerInfo","StatusDetails":"StatusDetails %d"}`
	...
	expectedOp := map[string]interface{}{"dcip": "Dcip 1", "class_id": "hyperflex.ReplicationPeerInfo", "mcip": "Mcip 1", "status": "Status 1", "repl_cip": "ReplCip 1", "object_type": "hyperflex.ReplicationPeerInfo", "status_details": "StatusDetails 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DataReplicationCompliance":"DataReplicationCompliance %d","Name":"Name %d","ActiveNodes":"ActiveNodes %d","Uptime":"Uptime %d","ClusterAccessPolicy":"ClusterAccessPolicy %d","Downtime":"Downtime %d","DataReplicationFactor":"DataReplicationFactor %d","ObjectType":"hyperflex.Summary","ClassId":"hyperflex.Summary","TotalCapacity":32,"CompressionSavings":32.000000,"Boottime":32,"TotalSavings":32.000000,"Address":"Address %d","DeduplicationSavings":32.000000,"FreeCapacity":32,"SpaceStatus":"SpaceStatus %d","UsedCapacity":32,"State":"State %d","ResiliencyDetailsSize":32}`
	...
	expectedOp := map[string]interface{}{"data_replication_compliance": "DataReplicationCompliance 1", "name": "Name 1", "active_nodes": "ActiveNodes 1", "uptime": "Uptime 1", "cluster_access_policy": "ClusterAccessPolicy 1", "downtime": "Downtime 1", "data_replication_factor": "DataReplicationFactor 1", "object_type": "hyperflex.Summary", "class_id": "hyperflex.Summary", "total_capacity": 32, "compression_savings": 32.000000, "boottime": 32, "total_savings": 32.000000, "address": "Address 1", "deduplication_savings": 32.000000, "free_capacity": 32, "space_status": "SpaceStatus 1", "used_capacity": 32, "state": "State 1", "resiliency_details_size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamEndPointPasswordProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"GracePeriod":32,"PasswordHistory":32,"ForceSendPassword":true,"PasswordExpiryDuration":32,"NotificationPeriod":32,"ClassId":"iam.EndPointPasswordProperties","ObjectType":"iam.EndPointPasswordProperties","EnablePasswordExpiry":true,"EnforceStrongPassword":true}`
	...
	expectedOp := map[string]interface{}{"grace_period": 32, "password_history": 32, "force_send_password": true, "password_expiry_duration": 32, "notification_period": 32, "class_id": "iam.EndPointPasswordProperties", "object_type": "iam.EndPointPasswordProperties", "enable_password_expiry": true, "enforce_strong_password": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamLdapBaseProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"BindDn":"BindDn %d","Timeout":32,"EnableEncryption":true,"Domain":"Domain %d","Attribute":"Attribute %d","Filter":"Filter %d","EnableGroupAuthorization":true,"GroupAttribute":"GroupAttribute %d","ObjectType":"iam.LdapBaseProperties","ClassId":"iam.LdapBaseProperties","BaseDn":"BaseDn %d","NestedGroupSearchDepth":32,"IsPasswordSet":true,"BindMethod":"BindMethod %d"}`
	...
	expectedOp := map[string]interface{}{"bind_dn": "BindDn 1", "timeout": 32, "enable_encryption": true, "domain": "Domain 1", "attribute": "Attribute 1", "filter": "Filter 1", "enable_group_authorization": true, "group_attribute": "GroupAttribute 1", "object_type": "iam.LdapBaseProperties", "class_id": "iam.LdapBaseProperties", "base_dn": "BaseDn 1", "nested_group_search_depth": 32, "is_password_set": true, "bind_method": "BindMethod 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapIamLdapDnsParameters(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SearchForest":"SearchForest %d","Source":"Source %d","ClassId":"iam.LdapDnsParameters","ObjectType":"iam.LdapDnsParameters","SearchDomain":"SearchDomain %d"}`
	...
	expectedOp := map[string]interface{}{"search_forest": "SearchForest 1", "nr_source": "Source 1", "class_id": "iam.LdapDnsParameters", "object_type": "iam.LdapDnsParameters", "search_domain": "SearchDomain 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIppoolIpV4Block(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"to": "To 1", "class_id": "ippool.IpV4Block", "object_type": "ippool.IpV4Block", "size": 32, "from": "From 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIppoolIpV6Block(t *testing.T) {
	p := models.IppoolIpV6Block{}
	var d = &schema.ResourceData{}
	c := `{"From":"From %d","ObjectType":"ippool.IpV6Block","Size":32,"ClassId":"ippool.IpV6Block","To":"To %d"}`
	...
	expectedOp := map[string]interface{}{"from": "From 1", "object_type": "ippool.IpV6Block", "size": 32, "class_id": "ippool.IpV6Block", "to": "To 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIqnpoolIqnSuffixBlock(t *testing.T) {
	p := models.IqnpoolIqnSuffixBlock{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"iqnpool.IqnSuffixBlock","ObjectType":"iqnpool.IqnSuffixBlock","Size":32,"To":32,"From":32,"Suffix":"Suffix %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "size": 32, "to": 32, "from": 32, "suffix": "Suffix 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesActionInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.ActionInfo","ClassId":"kubernetes.ActionInfo","Name":"Name %d","Status":"Status %d","FailureReason":"FailureReason %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.ActionInfo", "class_id": "kubernetes.ActionInfo", "name": "Name 1", "status": "Status 1", "failure_reason": "FailureReason 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesAddonConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReleaseNamespace":"ReleaseNamespace %d","InstallStrategy":"InstallStrategy %d","ClassId":"kubernetes.AddonConfiguration","ObjectType":"kubernetes.AddonConfiguration","Overrides":"Overrides %d","ReleaseName":"ReleaseName %d","UpgradeStrategy":"UpgradeStrategy %d"}`
	...
	expectedOp := map[string]interface{}{"release_namespace": "ReleaseNamespace 1", "install_strategy": "InstallStrategy 1", "class_id": "kubernetes.AddonConfiguration", "object_type": "kubernetes.AddonConfiguration", "overrides": "Overrides 1", "release_name": "ReleaseName 1", "upgrade_strategy": "UpgradeStrategy 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesClusterCertificateConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SaPublicKey":"SaPublicKey %d","EtcdCert":"EtcdCert %d","ObjectType":"kubernetes.ClusterCertificateConfiguration","FrontProxyCert":"FrontProxyCert %d","CaCert":"CaCert %d","ClassId":"kubernetes.ClusterCertificateConfiguration"}`
	...
	expectedOp := map[string]interface{}{"sa_public_key": "SaPublicKey 1", "etcd_cert": "EtcdCert 1", "object_type": "kubernetes.ClusterCertificateConfiguration", "front_proxy_cert": "FrontProxyCert 1", "ca_cert": "CaCert 1", "class_id": "kubernetes.ClusterCertificateConfiguration"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesClusterManagementConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LoadBalancerCount":32,"ClassId":"kubernetes.ClusterManagementConfig","MasterVip":"MasterVip %d","IsTacPasswdSet":true,"ObjectType":"kubernetes.ClusterManagementConfig","SshUser":"SshUser %d"}`
	...
	expectedOp := map[string]interface{}{"load_balancer_count": 32, "class_id": "kubernetes.ClusterManagementConfig", "master_vip": "MasterVip 1", "is_tac_passwd_set": true, "object_type": "kubernetes.ClusterManagementConfig", "ssh_user": "SshUser 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesDaemonSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DesiredNumberScheduled":32,"NumberMisscheduled":32,"NumberAvailable":"NumberAvailable %d","NumberReady":32,"UpdatedNumberScheduled":"UpdatedNumberScheduled %d","ClassId":"kubernetes.DaemonSetStatus","ObjectType":"kubernetes.DaemonSetStatus","ObservedGeneration":32,"CurrentNumberScheduled":32}`
	...
	expectedOp := map[string]interface{}{"desired_number_scheduled": 32, "number_misscheduled": 32, "number_available": "NumberAvailable 1", "number_ready": 32, "updated_number_scheduled": "UpdatedNumberScheduled 1", "class_id": "kubernetes.DaemonSetStatus", "object_type": "kubernetes.DaemonSetStatus", "observed_generation": 32, "current_number_scheduled": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesDeploymentStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AvailableReplicas":32,"ClassId":"kubernetes.DeploymentStatus","ObjectType":"kubernetes.DeploymentStatus","ObservedGeneration":32,"ReadyReplicas":32,"Replicas":32,"UpdatedReplicas":32}`
	...
	expectedOp := map[string]interface{}{"available_replicas": 32, "class_id": "kubernetes.DeploymentStatus", "object_type": "kubernetes.DeploymentStatus", "observed_generation": 32, "ready_replicas": 32, "replicas": 32, "updated_replicas": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesNodeInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.NodeInfo","KubeletVersion":"KubeletVersion %d","Architecture":"Architecture %d","BootId":"BootId %d","KernelVersion":"KernelVersion %d","SystemUuid":"SystemUuid %d","KubeProxyVersion":"KubeProxyVersion %d","ClassId":"kubernetes.NodeInfo","OperatingSystem":"OperatingSystem %d","OsImage":"OsImage %d","ContainerRuntimeVersion":"ContainerRuntimeVersion %d","MachineId":"MachineId %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.NodeInfo", "kubelet_version": "KubeletVersion 1", "architecture": "Architecture 1", "boot_id": "BootId 1", "kernel_version": "KernelVersion 1", "system_uuid": "SystemUuid 1", "kube_proxy_version": "KubeProxyVersion 1", "class_id": "kubernetes.NodeInfo", "operating_system": "OperatingSystem 1", "os_image": "OsImage 1", "container_runtime_version": "ContainerRuntimeVersion 1", "machine_id": "MachineId 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesPodStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"StartTime":"StartTime %d","HostIp":"HostIp %d","Phase":"Phase %d","PodIp":"PodIp %d","QosClass":"QosClass %d","ClassId":"kubernetes.PodStatus","ObjectType":"kubernetes.PodStatus"}`
	...
	expectedOp := map[string]interface{}{"start_time": "StartTime 1", "host_ip": "HostIp 1", "phase": "Phase 1", "pod_ip": "PodIp 1", "qos_class": "QosClass 1", "class_id": "kubernetes.PodStatus", "object_type": "kubernetes.PodStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesProxyConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.ProxyConfig","Username":"Username %d","Hostname":"Hostname %d","IsPasswordSet":true,"Port":32,"Protocol":"Protocol %d","ClassId":"kubernetes.ProxyConfig"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.ProxyConfig", "username": "Username 1", "hostname": "Hostname 1", "is_password_set": true, "port": 32, "protocol": "Protocol 1", "class_id": "kubernetes.ProxyConfig"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesStatefulSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AvailableReplicas":32,"ClassId":"kubernetes.StatefulSetStatus","ObjectType":"kubernetes.StatefulSetStatus","UpdateRevision":"UpdateRevision %d","ReadyReplicas":32,"ObservedGeneration":32,"Replicas":32,"CurrentRevision":"CurrentRevision %d","UpdatedReplicas":32,"CollisionCount":32}`
	...
	expectedOp := map[string]interface{}{"available_replicas": 32, "class_id": "kubernetes.StatefulSetStatus", "object_type": "kubernetes.StatefulSetStatus", "update_revision": "UpdateRevision 1", "ready_replicas": 32, "observed_generation": 32, "replicas": 32, "current_revision": "CurrentRevision 1", "updated_replicas": 32, "collision_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapMoBaseMo(t *testing.T) {
	p := models.MoBaseMo{}
	var d = &schema.ResourceData{}
	c := `{"AccountMoid":"AccountMoid %d","Moid":"Moid %d","ClassId":"mo.BaseMo","SharedScope":"SharedScope %d","DomainGroupMoid":"DomainGroupMoid %d","ObjectType":"mo.BaseMo"}`
	...
	expectedOp := map[string]interface{}{"account_moid": "AccountMoid 1", "moid": "Moid 1", "class_id": "mo.BaseMo", "shared_scope": "SharedScope 1", "domain_group_moid": "DomainGroupMoid 1", "object_type": "mo.BaseMo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiaapiNewReleaseDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Link":"Link %d","ClassId":"niaapi.NewReleaseDetail","ObjectType":"niaapi.NewReleaseDetail","ReleaseNoteLinkTitle":"ReleaseNoteLinkTitle %d","Title":"Title %d","ReleaseNoteLink":"ReleaseNoteLink %d","SoftwareDownloadLink":"SoftwareDownloadLink %d","SoftwareDownloadLinkTitle":"SoftwareDownloadLinkTitle %d","Description":"Description %d","Version":"Version %d"}`
	...
	expectedOp := map[string]interface{}{"link": "Link 1", "class_id": "niaapi.NewReleaseDetail", "object_type": "niaapi.NewReleaseDetail", "release_note_link_title": "ReleaseNoteLinkTitle 1", "title": "Title 1", "release_note_link": "ReleaseNoteLink 1", "software_download_link": "SoftwareDownloadLink 1", "software_download_link_title": "SoftwareDownloadLinkTitle 1", "description": "Description 1", "nr_version": "Version 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryDiskinfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Free":32,"Name":"Name %d","Total":32,"ClassId":"niatelemetry.Diskinfo","ObjectType":"niatelemetry.Diskinfo","Used":32}`
	...
	expectedOp := map[string]interface{}{"free": 32, "name": "Name 1", "total": 32, "class_id": "niatelemetry.Diskinfo", "object_type": "niatelemetry.Diskinfo", "used": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryNveVni(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CpVniDown":32,"DpVniCount":32,"DpVniDown":32,"ClassId":"niatelemetry.NveVni","ObjectType":"niatelemetry.NveVni","DpVniUp":32,"CpVniCount":32,"CpVniUp":32}`
	...
	expectedOp := map[string]interface{}{"cp_vni_down": 32, "dp_vni_count": 32, "dp_vni_down": 32, "class_id": "niatelemetry.NveVni", "object_type": "niatelemetry.NveVni", "dp_vni_up": 32, "cp_vni_count": 32, "cp_vni_up": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosBgpMvpn(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CapablePeers":32,"NumberOfClusterLists":32,"TotalPaths":32,"NumberOfCommunities":32,"TableVersion":32,"TotalNetworks":32,"ConfiguredPeers":32,"ClassId":"niatelemetry.NxosBgpMvpn","ObjectType":"niatelemetry.NxosBgpMvpn","MemoryUsed":32}`
	...
	expectedOp := map[string]interface{}{"capable_peers": 32, "number_of_cluster_lists": 32, "total_paths": 32, "number_of_communities": 32, "table_version": 32, "total_networks": 32, "configured_peers": 32, "class_id": "niatelemetry.NxosBgpMvpn", "object_type": "niatelemetry.NxosBgpMvpn", "memory_used": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosVtp(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"OperMode":"OperMode %d","TrapEnabled":"TrapEnabled %d","V2Mode":"V2Mode %d","ClassId":"niatelemetry.NxosVtp","ObjectType":"niatelemetry.NxosVtp","Version":32,"PruningMode":"PruningMode %d","RunningVersion":"RunningVersion %d"}`
	...
	expectedOp := map[string]interface{}{"oper_mode": "OperMode 1", "trap_enabled": "TrapEnabled 1", "v2_mode": "V2Mode 1", "class_id": "niatelemetry.NxosVtp", "object_type": "niatelemetry.NxosVtp", "nr_version": 32, "pruning_mode": "PruningMode 1", "running_version": "RunningVersion 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOnpremSchedule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"onprem.Schedule","TimeZone":"TimeZone %d","DayOfWeek":32,"TimeOfDay":32,"MonthOfYear":32,"WeekOfMonth":32,"RepeatInterval":32,"ObjectType":"onprem.Schedule","DayOfMonth":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "onprem.Schedule", "time_zone": "TimeZone 1", "day_of_week": 32, "time_of_day": 32, "month_of_year": 32, "week_of_month": 32, "repeat_interval": 32, "object_type": "onprem.Schedule", "day_of_month": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapOnpremUpgradePhase(t *testing.T) {
	p := models.OnpremUpgradePhase{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"onprem.UpgradePhase","ObjectType":"onprem.UpgradePhase","ElapsedTime":32,"Failed":true,"Message":"Message %d","Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "onprem.UpgradePhase", "object_type": "onprem.UpgradePhase", "elapsed_time": 32, "failed": true, "message": "Message 1", "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsAnswers(t *testing.T) {
	p := models.OsAnswers{}
	var d = &schema.ResourceData{}
	c := `{"IsAnswerFileSet":true,"ProductKey":"ProductKey %d","IpConfigType":"IpConfigType %d","IsRootPasswordSet":true,"Source":"Source %d","ObjectType":"os.Answers","Hostname":"Hostname %d","Nameserver":"Nameserver %d","IsRootPasswordCrypted":true,"NetworkDevice":"NetworkDevice %d","ClassId":"os.Answers"}`
	...
	expectedOp := map[string]interface{}{"is_answer_file_set": true, "product_key": "ProductKey 1", "ip_config_type": "IpConfigType 1", "is_root_password_set": true, "nr_source": "Source 1", "object_type": "os.Answers", "hostname": "Hostname 1", "nameserver": "Nameserver 1", "is_root_password_crypted": true, "network_device": "NetworkDevice 1", "class_id": "os.Answers"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsGlobalConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"WindowsEdition":"WindowsEdition %d","OsImageName":"OsImageName %d","ScuImageName":"ScuImageName %d","ClassId":"os.GlobalConfig","ObjectType":"os.GlobalConfig","InstallMethod":"InstallMethod %d","ConfigurationSource":"ConfigurationSource %d","InstallTargetType":"InstallTargetType %d","ConfigurationFileName":"ConfigurationFileName %d"}`
	...
	expectedOp := map[string]interface{}{"windows_edition": "WindowsEdition 1", "os_image_name": "OsImageName 1", "scu_image_name": "ScuImageName 1", "class_id": "os.GlobalConfig", "object_type": "os.GlobalConfig", "install_method": "InstallMethod 1", "configuration_source": "ConfigurationSource 1", "install_target_type": "InstallTargetType 1", "configuration_file_name": "ConfigurationFileName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapPolicyConfigContext(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"OperState":"OperState %d","ConfigState":"ConfigState %d","ConfigType":"ConfigType %d","ControlAction":"ControlAction %d","ClassId":"policy.ConfigContext","ObjectType":"policy.ConfigContext","ErrorState":"ErrorState %d"}`
	...
	expectedOp := map[string]interface{}{"oper_state": "OperState 1", "config_state": "ConfigState 1", "config_type": "ConfigType 1", "control_action": "ControlAction 1", "class_id": "policy.ConfigContext", "object_type": "policy.ConfigContext", "error_state": "ErrorState 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapRecoveryBackupSchedule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Hours":32,"FrequencyUnit":"FrequencyUnit %d","ClassId":"recovery.BackupSchedule","ObjectType":"recovery.BackupSchedule"}`
	...
	expectedOp := map[string]interface{}{"hours": 32, "frequency_unit": "FrequencyUnit 1", "class_id": "recovery.BackupSchedule", "object_type": "recovery.BackupSchedule"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageAutomaticDriveGroup(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DrivesPerSpan":32,"MinimumDriveSize":32,"UseRemainingDrives":true,"ClassId":"storage.AutomaticDriveGroup","ObjectType":"storage.AutomaticDriveGroup","DriveType":"DriveType %d","NumDedicatedHotSpares":"NumDedicatedHotSpares %d","NumberOfSpans":32}`
	...
	expectedOp := map[string]interface{}{"drives_per_span": 32, "minimum_drive_size": 32, "use_remaining_drives": true, "class_id": "storage.AutomaticDriveGroup", "object_type": "storage.AutomaticDriveGroup", "drive_type": "DriveType 1", "num_dedicated_hot_spares": "NumDedicatedHotSpares 1", "number_of_spans": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapStorageBaseCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Free":32,"Total":32,"Used":32,"Available":32,"ClassId":"storage.BaseCapacity","ObjectType":"storage.BaseCapacity","CapacityUtilization":32.000000}`
	...
	expectedOp := map[string]interface{}{"free": 32, "total": 32, "used": 32, "available": 32, "class_id": "storage.BaseCapacity", "object_type": "storage.BaseCapacity", "capacity_utilization": 32.000000}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppEthernetPortLag(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DistributionPolicy":"DistributionPolicy %d","ClassId":"storage.NetAppEthernetPortLag","ObjectType":"storage.NetAppEthernetPortLag","Mode":"Mode %d"}`
	...
	expectedOp := map[string]interface{}{"distribution_policy": "DistributionPolicy 1", "class_id": "storage.NetAppEthernetPortLag", "object_type": "storage.NetAppEthernetPortLag", "mode": "Mode 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppHighAvailability(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"GivebackState":"GivebackState %d","PartnerName":"PartnerName %d","ClassId":"storage.NetAppHighAvailability","ObjectType":"storage.NetAppHighAvailability","PartnerUuid":"PartnerUuid %d","TakeoverState":"TakeoverState %d","Enabled":true}`
	...
	expectedOp := map[string]interface{}{"giveback_state": "GivebackState 1", "partner_name": "PartnerName 1", "class_id": "storage.NetAppHighAvailability", "object_type": "storage.NetAppHighAvailability", "partner_uuid": "PartnerUuid 1", "takeover_state": "TakeoverState 1", "enabled": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppPerformanceMetricsAverage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Iops":32.000000,"Latency":32.000000,"ClassId":"storage.NetAppPerformanceMetricsAverage","ObjectType":"storage.NetAppPerformanceMetricsAverage","Period":32,"Throughput":32.000000}`
	...
	expectedOp := map[string]interface{}{"iops": 32.000000, "latency": 32.000000, "class_id": "storage.NetAppPerformanceMetricsAverage", "object_type": "storage.NetAppPerformanceMetricsAverage", "period": 32, "throughput": 32.000000}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapStorageNetAppStorageClusterEfficiency(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.NetAppStorageClusterEfficiency","ObjectType":"storage.NetAppStorageClusterEfficiency","Savings":32,"LogicalUsed":32,"Ratio":32.000000}`
	...
	expectedOp := map[string]interface{}{"class_id": "storage.NetAppStorageClusterEfficiency", "object_type": "storage.NetAppStorageClusterEfficiency", "savings": 32, "logical_used": 32, "ratio": 32.000000}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageR0Drive(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"storage.R0Drive","Enable":true,"DriveSlots":"DriveSlots %d","DriveSlotsList":"DriveSlotsList %d","ClassId":"storage.R0Drive"}`
	...
	expectedOp := map[string]interface{}{"object_type": "storage.R0Drive", "enable": true, "drive_slots": "DriveSlots 1", "drive_slots_list": "DriveSlotsList 1", "class_id": "storage.R0Drive"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapUuidpoolUuidBlock(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"size": 32, "from": "From 1", "to": "To 1", "class_id": "uuidpool.UuidBlock", "object_type": "uuidpool.UuidBlock"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationActionInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.ActionInfo","ObjectType":"virtualization.ActionInfo","Status":"Status %d","FailureReason":"FailureReason %d","Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.ActionInfo", "object_type": "virtualization.ActionInfo", "status": "Status 1", "failure_reason": "FailureReason 1", "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationBondState(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Mode":"Mode %d","ClassId":"virtualization.BondState","ObjectType":"virtualization.BondState","ActiveSlave":"ActiveSlave %d"}`
	...
	expectedOp := map[string]interface{}{"mode": "Mode 1", "class_id": "virtualization.BondState", "object_type": "virtualization.BondState", "active_slave": "ActiveSlave 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCloudInitConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NetworkData":"NetworkData %d","NetworkDataBase64Encoded":true,"ClassId":"virtualization.CloudInitConfig","ObjectType":"virtualization.CloudInitConfig","UserData":"UserData %d","UserDataBase64Encoded":true,"ConfigType":"ConfigType %d"}`
	...
	expectedOp := map[string]interface{}{"network_data": "NetworkData 1", "network_data_base64_encoded": true, "class_id": "virtualization.CloudInitConfig", "object_type": "virtualization.CloudInitConfig", "user_data": "UserData 1", "user_data_base64_encoded": true, "config_type": "ConfigType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCpuAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Total":32,"ClassId":"virtualization.CpuAllocation","ObjectType":"virtualization.CpuAllocation","Used":32,"Free":32,"Reserved":32}`
	...
	expectedOp := map[string]interface{}{"total": 32, "class_id": "virtualization.CpuAllocation", "object_type": "virtualization.CpuAllocation", "used": 32, "free": 32, "reserved": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationDiskStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"VolumeName":"VolumeName %d","DownloadPercentage":"DownloadPercentage %d","VolumeHandle":"VolumeHandle %d","VolumeVendor":"VolumeVendor %d","Reason":"Reason %d","ClassId":"virtualization.DiskStatus","ObjectType":"virtualization.DiskStatus","State":"State %d"}`
	...
	expectedOp := map[string]interface{}{"volume_name": "VolumeName 1", "download_percentage": "DownloadPercentage 1", "volume_handle": "VolumeHandle 1", "volume_vendor": "VolumeVendor 1", "reason": "Reason 1", "class_id": "virtualization.DiskStatus", "object_type": "virtualization.DiskStatus", "state": "State 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationGuestInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IpAddress":"IpAddress %d","Name":"Name %d","ClassId":"virtualization.GuestInfo","ObjectType":"virtualization.GuestInfo","OperatingSystem":"OperatingSystem %d","Hostname":"Hostname %d"}`
	...
	expectedOp := map[string]interface{}{"ip_address": "IpAddress 1", "name": "Name 1", "class_id": "virtualization.GuestInfo", "object_type": "virtualization.GuestInfo", "operating_system": "OperatingSystem 1", "hostname": "Hostname 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationMemoryAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Used":32,"ClassId":"virtualization.MemoryAllocation","ObjectType":"virtualization.MemoryAllocation","Free":32,"Reserved":32,"Total":32}`
	...
	expectedOp := map[string]interface{}{"used": 32, "class_id": "virtualization.MemoryAllocation", "object_type": "virtualization.MemoryAllocation", "free": 32, "reserved": 32, "total": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationProductInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ProductName":"ProductName %d","ProductType":"ProductType %d","ProductVendor":"ProductVendor %d","Version":"Version %d","ClassId":"virtualization.ProductInfo","ObjectType":"virtualization.ProductInfo","Build":"Build %d"}`
	...
	expectedOp := map[string]interface{}{"product_name": "ProductName 1", "product_type": "ProductType 1", "product_vendor": "ProductVendor 1", "nr_version": "Version 1", "class_id": "virtualization.ProductInfo", "object_type": "virtualization.ProductInfo", "build": "Build 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationStorageCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Free":32,"Used":32,"Capacity":32,"ObjectType":"virtualization.StorageCapacity","ClassId":"virtualization.StorageCapacity"}`
	...
	expectedOp := map[string]interface{}{"free": 32, "used": 32, "capacity": 32, "object_type": "virtualization.StorageCapacity", "class_id": "virtualization.StorageCapacity"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareTeamingAndFailover(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NotifySwitches":true,"NetworkFailureDetection":"NetworkFailureDetection %d","Name":"Name %d","LoadBalancing":"LoadBalancing %d","ClassId":"virtualization.VmwareTeamingAndFailover","ObjectType":"virtualization.VmwareTeamingAndFailover","Failback":true}`
	...
	expectedOp := map[string]interface{}{"notify_switches": true, "network_failure_detection": "NetworkFailureDetection 1", "name": "Name 1", "load_balancing": "LoadBalancing 1", "class_id": "virtualization.VmwareTeamingAndFailover", "object_type": "virtualization.VmwareTeamingAndFailover", "failback": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareVmCpuSocketInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareVmCpuSocketInfo","CoresPerSocket":32,"NumCpus":32,"NumSockets":32,"ObjectType":"virtualization.VmwareVmCpuSocketInfo"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.VmwareVmCpuSocketInfo", "cores_per_socket": 32, "num_cpus": 32, "num_sockets": 32, "object_type": "virtualization.VmwareVmCpuSocketInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareVmMemoryShareInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MemShares":32,"ClassId":"virtualization.VmwareVmMemoryShareInfo","ObjectType":"virtualization.VmwareVmMemoryShareInfo","MemLimit":32,"MemOverheadLimit":32,"MemReservation":32}`
	...
	expectedOp := map[string]interface{}{"mem_shares": 32, "class_id": "virtualization.VmwareVmMemoryShareInfo", "object_type": "virtualization.VmwareVmMemoryShareInfo", "mem_limit": 32, "mem_overhead_limit": 32, "mem_reservation": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicCdn(t *testing.T) {
	p := models.VnicCdn{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.Cdn","Source":"Source %d","Value":"Value %d","ClassId":"vnic.Cdn"}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.Cdn", "nr_source": "Source 1", "value": "Value 1", "class_id": "vnic.Cdn"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicCompletionQueueSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.CompletionQueueSettings","Count":32,"RingSize":32,"ClassId":"vnic.CompletionQueueSettings"}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.CompletionQueueSettings", "nr_count": 32, "ring_size": 32, "class_id": "vnic.CompletionQueueSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcErrorRecoverySettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PortDownTimeout":32,"Enabled":true,"IoRetryCount":32,"IoRetryTimeout":32,"ClassId":"vnic.FcErrorRecoverySettings","ObjectType":"vnic.FcErrorRecoverySettings","LinkDownTimeout":32}`
	...
	expectedOp := map[string]interface{}{"port_down_timeout": 32, "enabled": true, "io_retry_count": 32, "io_retry_timeout": 32, "class_id": "vnic.FcErrorRecoverySettings", "object_type": "vnic.FcErrorRecoverySettings", "link_down_timeout": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcQueueSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RingSize":32,"ClassId":"vnic.FcQueueSettings","ObjectType":"vnic.FcQueueSettings","Count":32}`
	...
	expectedOp := map[string]interface{}{"ring_size": 32, "class_id": "vnic.FcQueueSettings", "object_type": "vnic.FcQueueSettings", "nr_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicPlacementSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Id":"Id %d","ClassId":"vnic.PlacementSettings","ObjectType":"vnic.PlacementSettings","PciLink":32,"SwitchId":"SwitchId %d","Uplink":32}`
	...
	expectedOp := map[string]interface{}{"id": "Id 1", "class_id": "vnic.PlacementSettings", "object_type": "vnic.PlacementSettings", "pci_link": 32, "switch_id": "SwitchId 1", "uplink": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicRoceSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":32,"ClassOfService":32,"Enabled":true,"MemoryRegions":32,"ObjectType":"vnic.RoceSettings","ResourceGroups":32,"QueuePairs":32,"ClassId":"vnic.RoceSettings"}`
	...
	expectedOp := map[string]interface{}{"nr_version": 32, "class_of_service": 32, "enabled": true, "memory_regions": 32, "object_type": "vnic.RoceSettings", "resource_groups": 32, "queue_pairs": 32, "class_id": "vnic.RoceSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicRssHashSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Ipv6Hash":true,"Ipv6ExtHash":true,"TcpIpv6ExtHash":true,"Ipv4Hash":true,"UdpIpv6Hash":true,"TcpIpv6Hash":true,"ClassId":"vnic.RssHashSettings","ObjectType":"vnic.RssHashSettings","TcpIpv4Hash":true,"UdpIpv4Hash":true}`
	...
	expectedOp := map[string]interface{}{"ipv6_hash": true, "ipv6_ext_hash": true, "tcp_ipv6_ext_hash": true, "ipv4_hash": true, "udp_ipv6_hash": true, "tcp_ipv6_hash": true, "class_id": "vnic.RssHashSettings", "object_type": "vnic.RssHashSettings", "tcp_ipv4_hash": true, "udp_ipv4_hash": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicTcpOffloadSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.TcpOffloadSettings","ObjectType":"vnic.TcpOffloadSettings","LargeSend":true,"RxChecksum":true,"TxChecksum":true,"LargeReceive":true}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.TcpOffloadSettings", "object_type": "vnic.TcpOffloadSettings", "large_send": true, "rx_checksum": true, "tx_checksum": true, "large_receive": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicVmqSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NumSubVnics":32,"NumVmqs":32,"VmmqAdapterPolicy":"VmmqAdapterPolicy %d","ClassId":"vnic.VmqSettings","MultiQueueSupport":true,"NumInterrupts":32,"ObjectType":"vnic.VmqSettings","Enabled":true}`
	...
	expectedOp := map[string]interface{}{"num_sub_vnics": 32, "num_vmqs": 32, "vmmq_adapter_policy": "VmmqAdapterPolicy 1", "class_id": "vnic.VmqSettings", "multi_queue_support": true, "num_interrupts": 32, "object_type": "vnic.VmqSettings", "enabled": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicVsanSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.VsanSettings","Id":32,"DefaultVlanId":32,"ClassId":"vnic.VsanSettings"}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.VsanSettings", "id": 32, "default_vlan_id": 32, "class_id": "vnic.VsanSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RetryCount":32,"RetryDelay":32,"TimeoutPolicy":"TimeoutPolicy %d","ExternalMeta":true,"Cloneable":true,"Timeout":32,"ClassId":"workflow.Properties","RetryPolicy":"RetryPolicy %d","StartsWorkflow":true,"ObjectType":"workflow.Properties","SupportStatus":"SupportStatus %d"}`
	...
	expectedOp := map[string]interface{}{"retry_count": 32, "retry_delay": 32, "timeout_policy": "TimeoutPolicy 1", "external_meta": true, "cloneable": true, "timeout": 32, "class_id": "workflow.Properties", "retry_policy": "RetryPolicy 1", "starts_workflow": true, "object_type": "workflow.Properties", "support_status": "SupportStatus 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"EnableDebug":true,"ExternalMeta":true,"Retryable":true,"SupportStatus":"SupportStatus %d","ObjectType":"workflow.WorkflowProperties","ClassId":"workflow.WorkflowProperties","Cloneable":true}`
	...
	expectedOp := map[string]interface{}{"enable_debug": true, "external_meta": true, "retryable": true, "support_status": "SupportStatus 1", "object_type": "workflow.WorkflowProperties", "class_id": "workflow.WorkflowProperties", "cloneable": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapX509Certificate(t *testing.T) {
	p := models.X509Certificate{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"x509.Certificate","Sha256Fingerprint":"Sha256Fingerprint %d","PemCertificate":"PemCertificate %d","SignatureAlgorithm":"SignatureAlgorithm %d","ObjectType":"x509.Certificate"}`
	...
	expectedOp := map[string]interface{}{"class_id": "x509.Certificate", "sha256_fingerprint": "Sha256Fingerprint 1", "pem_certificate": "PemCertificate 1", "signature_algorithm": "SignatureAlgorithm 1", "object_type": "x509.Certificate"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
