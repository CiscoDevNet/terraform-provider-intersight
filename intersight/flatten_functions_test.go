...
func TestFlattenListAdapterAdapterConfig(t *testing.T) {
	...
	c := `{"ClassId":"adapter.AdapterConfig","SlotId":"SlotId %d","ObjectType":"adapter.AdapterConfig"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "adapter.AdapterConfig", "slot_id": "SlotId 1", "object_type": "adapter.AdapterConfig"}, {"class_id": "adapter.AdapterConfig", "slot_id": "SlotId 2", "object_type": "adapter.AdapterConfig"}}
...
func TestFlattenListApplianceApiStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"appliance.ApiStatus","ClassId":"appliance.ApiStatus","ElapsedTime":32.000000,"ObjectName":"ObjectName %d","Reason":"Reason %d","Response":32}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "appliance.ApiStatus", "class_id": "appliance.ApiStatus", "elapsed_time": 32.000000, "object_name": "ObjectName 1", "reason": "Reason 1", "response": 32}, {"object_type": "appliance.ApiStatus", "class_id": "appliance.ApiStatus", "elapsed_time": 32.000000, "object_name": "ObjectName 2", "reason": "Reason 2", "response": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListApplianceCertRenewalPhase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Failed":true,"Message":"Message %d","ObjectType":"appliance.CertRenewalPhase","ClassId":"appliance.CertRenewalPhase","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"failed": true, "message": "Message 1", "object_type": "appliance.CertRenewalPhase", "class_id": "appliance.CertRenewalPhase", "name": "Name 1"}, {"failed": true, "message": "Message 2", "object_type": "appliance.CertRenewalPhase", "class_id": "appliance.CertRenewalPhase", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListBootDeviceBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","ObjectType":"boot.DeviceBase","ClassId":"boot.DeviceBase","Enabled":true}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "object_type": "boot.DeviceBase", "class_id": "boot.DeviceBase", "enabled": true}, {"name": "Name 2", "object_type": "boot.DeviceBase", "class_id": "boot.DeviceBase", "enabled": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListBulkApiResult(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"bulk.ApiResult","Status":32,"ObjectType":"bulk.ApiResult"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "bulk.ApiResult", "status": 32, "object_type": "bulk.ApiResult"}, {"class_id": "bulk.ApiResult", "status": 32, "object_type": "bulk.ApiResult"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCapabilityPortRange(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"capability.PortRange","ClassId":"capability.PortRange","EndSlotId":32,"StartPortId":32,"StartSlotId":32,"EndPortId":32}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "capability.PortRange", "class_id": "capability.PortRange", "end_slot_id": 32, "start_port_id": 32, "start_slot_id": 32, "end_port_id": 32}, {"object_type": "capability.PortRange", "class_id": "capability.PortRange", "end_slot_id": 32, "start_port_id": 32, "start_slot_id": 32, "end_port_id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCapabilitySwitchingModeCapability(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"capability.SwitchingModeCapability","ObjectType":"capability.SwitchingModeCapability","VpCompressionSupported":true,"SwitchingMode":"SwitchingMode %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "capability.SwitchingModeCapability", "object_type": "capability.SwitchingModeCapability", "vp_compression_supported": true, "switching_mode": "SwitchingMode 1"}, {"class_id": "capability.SwitchingModeCapability", "object_type": "capability.SwitchingModeCapability", "vp_compression_supported": true, "switching_mode": "SwitchingMode 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudCloudTag(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Value":"Value %d","ObjectType":"cloud.CloudTag","ClassId":"cloud.CloudTag","Key":"Key %d"}`
	...
	expectedOp := []map[string]interface{}{{"value": "Value 1", "object_type": "cloud.CloudTag", "class_id": "cloud.CloudTag", "key": "Key 1"}, {"value": "Value 2", "object_type": "cloud.CloudTag", "class_id": "cloud.CloudTag", "key": "Key 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudNetworkInterfaceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.NetworkInterfaceAttachment","MacAddress":"MacAddress %d","NicIndex":32,"Identity":"Identity %d","IpForwardingEnabled":true,"SubNetworkId":"SubNetworkId %d","ClassId":"cloud.NetworkInterfaceAttachment","NetworkName":"NetworkName %d","SubNetworkName":"SubNetworkName %d","NetworkId":"NetworkId %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "cloud.NetworkInterfaceAttachment", "mac_address": "MacAddress 1", "nic_index": 32, "identity": "Identity 1", "ip_forwarding_enabled": true, "sub_network_id": "SubNetworkId 1", "class_id": "cloud.NetworkInterfaceAttachment", "network_name": "NetworkName 1", "sub_network_name": "SubNetworkName 1", "network_id": "NetworkId 1"}, {"object_type": "cloud.NetworkInterfaceAttachment", "mac_address": "MacAddress 2", "nic_index": 32, "identity": "Identity 2", "ip_forwarding_enabled": true, "sub_network_id": "SubNetworkId 2", "class_id": "cloud.NetworkInterfaceAttachment", "network_name": "NetworkName 2", "sub_network_name": "SubNetworkName 2", "network_id": "NetworkId 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudSecurityGroupRule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Description":"Description %d","ClassId":"cloud.SecurityGroupRule","EndPort":32,"Action":"Action %d","StartPort":32,"SourceSecurityGroup":"SourceSecurityGroup %d","Identity":"Identity %d","Name":"Name %d","Index":32,"EtherType":"EtherType %d","Protocol":"Protocol %d","ObjectType":"cloud.SecurityGroupRule"}`
	...
	expectedOp := []map[string]interface{}{{"description": "Description 1", "class_id": "cloud.SecurityGroupRule", "end_port": 32, "action": "Action 1", "start_port": 32, "source_security_group": "SourceSecurityGroup 1", "identity": "Identity 1", "name": "Name 1", "index": 32, "ether_type": "EtherType 1", "protocol": "Protocol 1", "object_type": "cloud.SecurityGroupRule"}, {"description": "Description 2", "class_id": "cloud.SecurityGroupRule", "end_port": 32, "action": "Action 2", "start_port": 32, "source_security_group": "SourceSecurityGroup 2", "identity": "Identity 2", "name": "Name 2", "index": 32, "ether_type": "EtherType 2", "protocol": "Protocol 2", "object_type": "cloud.SecurityGroupRule"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudTfcWorkspaceVariables(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"cloud.TfcWorkspaceVariables","Key":"Key %d","Value":"Value %d","Description":"Description %d","Identity":"Identity %d","ObjectType":"cloud.TfcWorkspaceVariables","Sensitive":true,"Hcl":true,"Category":"Category %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "cloud.TfcWorkspaceVariables", "key": "Key 1", "value": "Value 1", "description": "Description 1", "identity": "Identity 1", "object_type": "cloud.TfcWorkspaceVariables", "sensitive": true, "hcl": true, "category": "Category 1"}, {"class_id": "cloud.TfcWorkspaceVariables", "key": "Key 2", "value": "Value 2", "description": "Description 2", "identity": "Identity 2", "object_type": "cloud.TfcWorkspaceVariables", "sensitive": true, "hcl": true, "category": "Category 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Identity":"Identity %d","AutoDelete":true,"Index":32,"ObjectType":"cloud.VolumeAttachment","ClassId":"cloud.VolumeAttachment","IsRoot":true,"DeviceName":"DeviceName %d"}`
	...
	expectedOp := []map[string]interface{}{{"identity": "Identity 1", "auto_delete": true, "index": 32, "object_type": "cloud.VolumeAttachment", "class_id": "cloud.VolumeAttachment", "is_root": true, "device_name": "DeviceName 1"}, {"identity": "Identity 2", "auto_delete": true, "index": 32, "object_type": "cloud.VolumeAttachment", "class_id": "cloud.VolumeAttachment", "is_root": true, "device_name": "DeviceName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeIpAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","Dn":"Dn %d","HttpsPort":32,"Name":"Name %d","ClassId":"compute.IpAddress","Subnet":"Subnet %d","HttpPort":32,"KvmVlan":32,"Address":"Address %d","DefaultGateway":"DefaultGateway %d","KvmPort":32,"ObjectType":"compute.IpAddress","Category":"Category %d"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "dn": "Dn 1", "https_port": 32, "name": "Name 1", "class_id": "compute.IpAddress", "subnet": "Subnet 1", "http_port": 32, "kvm_vlan": 32, "address": "Address 1", "default_gateway": "DefaultGateway 1", "kvm_port": 32, "object_type": "compute.IpAddress", "category": "Category 1"}, {"type": "Type 2", "dn": "Dn 2", "https_port": 32, "name": "Name 2", "class_id": "compute.IpAddress", "subnet": "Subnet 2", "http_port": 32, "kvm_vlan": 32, "address": "Address 2", "default_gateway": "DefaultGateway 2", "kvm_port": 32, "object_type": "compute.IpAddress", "category": "Category 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeServerOpStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"WorkflowType":"WorkflowType %d","ConfigState":"ConfigState %d","ClassId":"compute.ServerOpStatus","ObjectType":"compute.ServerOpStatus"}`
	...
	expectedOp := []map[string]interface{}{{"workflow_type": "WorkflowType 1", "config_state": "ConfigState 1", "class_id": "compute.ServerOpStatus", "object_type": "compute.ServerOpStatus"}, {"workflow_type": "WorkflowType 2", "config_state": "ConfigState 2", "class_id": "compute.ServerOpStatus", "object_type": "compute.ServerOpStatus"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListContentParameter(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ItemType":"ItemType %d","Secure":true,"ComplexType":"ComplexType %d","Name":"Name %d","ObjectType":"content.Parameter","ClassId":"content.Parameter","Type":"Type %d","Path":"Path %d","AcceptSingleValue":true}`
	...
	expectedOp := []map[string]interface{}{{"item_type": "ItemType 1", "secure": true, "complex_type": "ComplexType 1", "name": "Name 1", "object_type": "content.Parameter", "class_id": "content.Parameter", "type": "Type 1", "path": "Path 1", "accept_single_value": true}, {"item_type": "ItemType 2", "secure": true, "complex_type": "ComplexType 2", "name": "Name 2", "object_type": "content.Parameter", "class_id": "content.Parameter", "type": "Type 2", "path": "Path 2", "accept_single_value": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListEquipmentIoCardIdentity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"equipment.IoCardIdentity","ModuleId":32,"NetworkElementMoid":"NetworkElementMoid %d","SwitchId":32,"IoCardMoid":"IoCardMoid %d","ObjectType":"equipment.IoCardIdentity"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "equipment.IoCardIdentity", "module_id": 32, "network_element_moid": "NetworkElementMoid 1", "switch_id": 32, "io_card_moid": "IoCardMoid 1", "object_type": "equipment.IoCardIdentity"}, {"class_id": "equipment.IoCardIdentity", "module_id": 32, "network_element_moid": "NetworkElementMoid 2", "switch_id": 32, "io_card_moid": "IoCardMoid 2", "object_type": "equipment.IoCardIdentity"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricQosClass(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"fabric.QosClass","PacketDrop":true,"Weight":32,"BandwidthPercent":32,"Name":"Name %d","ObjectType":"fabric.QosClass","MulticastOptimize":true,"Cos":32,"AdminState":"AdminState %d","Mtu":32}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "fabric.QosClass", "packet_drop": true, "weight": 32, "bandwidth_percent": 32, "name": "Name 1", "object_type": "fabric.QosClass", "multicast_optimize": true, "cos": 32, "admin_state": "AdminState 1", "mtu": 32}, {"class_id": "fabric.QosClass", "packet_drop": true, "weight": 32, "bandwidth_percent": 32, "name": "Name 2", "object_type": "fabric.QosClass", "multicast_optimize": true, "cos": 32, "admin_state": "AdminState 2", "mtu": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFcpoolBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"From":"From %d","Size":32,"ClassId":"fcpool.Block","ObjectType":"fcpool.Block","To":"To %d"}`
	...
	expectedOp := []map[string]interface{}{{"from": "From 1", "size": 32, "class_id": "fcpool.Block", "object_type": "fcpool.Block", "to": "To 1"}, {"from": "From 2", "size": 32, "class_id": "fcpool.Block", "object_type": "fcpool.Block", "to": "To 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareBaseImpact(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"VersionImpact":"VersionImpact %d","ImpactType":"ImpactType %d","ComputationStatusDetail":"ComputationStatusDetail %d","TargetFirmwareVersion":"TargetFirmwareVersion %d","ObjectType":"firmware.BaseImpact","DomainName":"DomainName %d","FirmwareVersion":"FirmwareVersion %d","EndPoint":"EndPoint %d","ComputationError":"ComputationError %d","Model":"Model %d","ClassId":"firmware.BaseImpact"}`
	...
	expectedOp := []map[string]interface{}{{"version_impact": "VersionImpact 1", "impact_type": "ImpactType 1", "computation_status_detail": "ComputationStatusDetail 1", "target_firmware_version": "TargetFirmwareVersion 1", "object_type": "firmware.BaseImpact", "domain_name": "DomainName 1", "firmware_version": "FirmwareVersion 1", "end_point": "EndPoint 1", "computation_error": "ComputationError 1", "model": "Model 1", "class_id": "firmware.BaseImpact"}, {"version_impact": "VersionImpact 2", "impact_type": "ImpactType 2", "computation_status_detail": "ComputationStatusDetail 2", "target_firmware_version": "TargetFirmwareVersion 2", "object_type": "firmware.BaseImpact", "domain_name": "DomainName 2", "firmware_version": "FirmwareVersion 2", "end_point": "EndPoint 2", "computation_error": "ComputationError 2", "model": "Model 2", "class_id": "firmware.BaseImpact"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareComponentMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IsOobSupported":true,"RedfishUrl":"RedfishUrl %d","PackedVersion":"PackedVersion %d","ComponentType":"ComponentType %d","Vendor":"Vendor %d","Description":"Description %d","Model":"Model %d","ComponentLabel":"ComponentLabel %d","ImagePath":"ImagePath %d","Disruption":"Disruption %d","ObjectType":"firmware.ComponentMeta","ClassId":"firmware.ComponentMeta"}`
	...
	expectedOp := []map[string]interface{}{{"is_oob_supported": true, "redfish_url": "RedfishUrl 1", "packed_version": "PackedVersion 1", "component_type": "ComponentType 1", "vendor": "Vendor 1", "description": "Description 1", "model": "Model 1", "component_label": "ComponentLabel 1", "image_path": "ImagePath 1", "disruption": "Disruption 1", "object_type": "firmware.ComponentMeta", "class_id": "firmware.ComponentMeta"}, {"is_oob_supported": true, "redfish_url": "RedfishUrl 2", "packed_version": "PackedVersion 2", "component_type": "ComponentType 2", "vendor": "Vendor 2", "description": "Description 2", "model": "Model 2", "component_label": "ComponentLabel 2", "image_path": "ImagePath 2", "disruption": "Disruption 2", "object_type": "firmware.ComponentMeta", "class_id": "firmware.ComponentMeta"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFirmwareFirmwareInventory(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Category":"Category %d","Label":"Label %d","ObjectType":"firmware.FirmwareInventory","Model":"Model %d","UpdateUri":"UpdateUri %d","ClassId":"firmware.FirmwareInventory","Vendor":"Vendor %d","Version":"Version %d"}`
	...
	expectedOp := []map[string]interface{}{{"category": "Category 1", "label": "Label 1", "object_type": "firmware.FirmwareInventory", "model": "Model 1", "update_uri": "UpdateUri 1", "class_id": "firmware.FirmwareInventory", "vendor": "Vendor 1", "nr_version": "Version 1"}, {"category": "Category 2", "label": "Label 2", "object_type": "firmware.FirmwareInventory", "model": "Model 2", "update_uri": "UpdateUri 2", "class_id": "firmware.FirmwareInventory", "vendor": "Vendor 2", "nr_version": "Version 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHclConstraint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hcl.Constraint","ClassId":"hcl.Constraint","ConstraintName":"ConstraintName %d","ConstraintValue":"ConstraintValue %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "hcl.Constraint", "class_id": "hcl.Constraint", "constraint_name": "ConstraintName 1", "constraint_value": "ConstraintValue 1"}, {"object_type": "hcl.Constraint", "class_id": "hcl.Constraint", "constraint_name": "ConstraintName 2", "constraint_value": "ConstraintValue 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexFeatureLimitEntry(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","Value":"Value %d","ClassId":"hyperflex.FeatureLimitEntry","ObjectType":"hyperflex.FeatureLimitEntry"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "value": "Value 1", "class_id": "hyperflex.FeatureLimitEntry", "object_type": "hyperflex.FeatureLimitEntry"}, {"name": "Name 2", "value": "Value 2", "class_id": "hyperflex.FeatureLimitEntry", "object_type": "hyperflex.FeatureLimitEntry"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHealthCheckScriptInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AggregateScriptName":"AggregateScriptName %d","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptInput":"ScriptInput %d","ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptName":"ScriptName %d","Version":"Version %d","HyperflexVersion":"HyperflexVersion %d","ClassId":"hyperflex.HealthCheckScriptInfo"}`
	...
	expectedOp := []map[string]interface{}{{"aggregate_script_name": "AggregateScriptName 1", "script_execute_location": "ScriptExecuteLocation 1", "script_input": "ScriptInput 1", "object_type": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 1", "nr_version": "Version 1", "hyperflex_version": "HyperflexVersion 1", "class_id": "hyperflex.HealthCheckScriptInfo"}, {"aggregate_script_name": "AggregateScriptName 2", "script_execute_location": "ScriptExecuteLocation 2", "script_input": "ScriptInput 2", "object_type": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 2", "nr_version": "Version 2", "hyperflex_version": "HyperflexVersion 2", "class_id": "hyperflex.HealthCheckScriptInfo"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexMapClusterIdToStSnapshotPoint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.MapClusterIdToStSnapshotPoint","ClusterId":"ClusterId %d","ObjectType":"hyperflex.MapClusterIdToStSnapshotPoint"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "hyperflex.MapClusterIdToStSnapshotPoint", "cluster_id": "ClusterId 1", "object_type": "hyperflex.MapClusterIdToStSnapshotPoint"}, {"class_id": "hyperflex.MapClusterIdToStSnapshotPoint", "cluster_id": "ClusterId 2", "object_type": "hyperflex.MapClusterIdToStSnapshotPoint"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexNamedVlan(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "vlan_id": 32, "object_type": "hyperflex.NamedVlan", "class_id": "hyperflex.NamedVlan"}, {"name": "Name 2", "vlan_id": 32, "object_type": "hyperflex.NamedVlan", "class_id": "hyperflex.NamedVlan"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexServerFirmwareVersionInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":"Version %d","ObjectType":"hyperflex.ServerFirmwareVersionInfo","ClassId":"hyperflex.ServerFirmwareVersionInfo","ServerPlatform":"ServerPlatform %d"}`
	...
	expectedOp := []map[string]interface{}{{"nr_version": "Version 1", "object_type": "hyperflex.ServerFirmwareVersionInfo", "class_id": "hyperflex.ServerFirmwareVersionInfo", "server_platform": "ServerPlatform 1"}, {"nr_version": "Version 2", "object_type": "hyperflex.ServerFirmwareVersionInfo", "class_id": "hyperflex.ServerFirmwareVersionInfo", "server_platform": "ServerPlatform 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseKeysInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Count":32,"ExpirationDate":"ExpirationDate %d","LicenseId":"LicenseId %d","ObjectType":"iaas.LicenseKeysInfo","ClassId":"iaas.LicenseKeysInfo","Pid":"Pid %d"}`
	...
	expectedOp := []map[string]interface{}{{"nr_count": 32, "expiration_date": "ExpirationDate 1", "license_id": "LicenseId 1", "object_type": "iaas.LicenseKeysInfo", "class_id": "iaas.LicenseKeysInfo", "pid": "Pid 1"}, {"nr_count": 32, "expiration_date": "ExpirationDate 2", "license_id": "LicenseId 2", "object_type": "iaas.LicenseKeysInfo", "class_id": "iaas.LicenseKeysInfo", "pid": "Pid 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseUtilizationInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"iaas.LicenseUtilizationInfo","Sku":"Sku %d","ActualUsed":32,"Label":"Label %d","LicensedLimit":"LicensedLimit %d","ClassId":"iaas.LicenseUtilizationInfo"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "iaas.LicenseUtilizationInfo", "sku": "Sku 1", "actual_used": 32, "label": "Label 1", "licensed_limit": "LicensedLimit 1", "class_id": "iaas.LicenseUtilizationInfo"}, {"object_type": "iaas.LicenseUtilizationInfo", "sku": "Sku 2", "actual_used": 32, "label": "Label 2", "licensed_limit": "LicensedLimit 2", "class_id": "iaas.LicenseUtilizationInfo"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIamAccountPermissions(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"iam.AccountPermissions","ClassId":"iam.AccountPermissions","AccountIdentifier":"AccountIdentifier %d","AccountName":"AccountName %d","AccountStatus":"AccountStatus %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "iam.AccountPermissions", "class_id": "iam.AccountPermissions", "account_identifier": "AccountIdentifier 1", "account_name": "AccountName 1", "account_status": "AccountStatus 1"}, {"object_type": "iam.AccountPermissions", "class_id": "iam.AccountPermissions", "account_identifier": "AccountIdentifier 2", "account_name": "AccountName 2", "account_status": "AccountStatus 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIppoolIpV4Block(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"ippool.IpV4Block","ObjectType":"ippool.IpV4Block","Size":32,"To":"To %d","From":"From %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "ippool.IpV4Block", "object_type": "ippool.IpV4Block", "size": 32, "to": "To 1", "from": "From 1"}, {"class_id": "ippool.IpV4Block", "object_type": "ippool.IpV4Block", "size": 32, "to": "To 2", "from": "From 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIqnpoolIqnSuffixBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"iqnpool.IqnSuffixBlock","ObjectType":"iqnpool.IqnSuffixBlock","Size":32,"From":32,"Suffix":"Suffix %d","To":32}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "size": 32, "from": 32, "suffix": "Suffix 1", "to": 32}, {"class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "size": 32, "from": 32, "suffix": "Suffix 2", "to": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesEthernet(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ProviderName":"ProviderName %d","ObjectType":"kubernetes.Ethernet","Name":"Name %d","ClassId":"kubernetes.Ethernet","Gateway":"Gateway %d","Mtu":32}`
	...
	expectedOp := []map[string]interface{}{{"provider_name": "ProviderName 1", "object_type": "kubernetes.Ethernet", "name": "Name 1", "class_id": "kubernetes.Ethernet", "gateway": "Gateway 1", "mtu": 32}, {"provider_name": "ProviderName 2", "object_type": "kubernetes.Ethernet", "name": "Name 2", "class_id": "kubernetes.Ethernet", "gateway": "Gateway 2", "mtu": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesNodeGroupTaint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Effect":"Effect %d","Key":"Key %d","ObjectType":"kubernetes.NodeGroupTaint","ClassId":"kubernetes.NodeGroupTaint","Value":"Value %d"}`
	...
	expectedOp := []map[string]interface{}{{"effect": "Effect 1", "key": "Key 1", "object_type": "kubernetes.NodeGroupTaint", "class_id": "kubernetes.NodeGroupTaint", "value": "Value 1"}, {"effect": "Effect 2", "key": "Key 2", "object_type": "kubernetes.NodeGroupTaint", "class_id": "kubernetes.NodeGroupTaint", "value": "Value 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesTaint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.Taint","ObjectType":"kubernetes.Taint","Value":"Value %d","Effect":"Effect %d","Key":"Key %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "kubernetes.Taint", "object_type": "kubernetes.Taint", "value": "Value 1", "effect": "Effect 1", "key": "Key 1"}, {"class_id": "kubernetes.Taint", "object_type": "kubernetes.Taint", "value": "Value 2", "effect": "Effect 2", "key": "Key 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMacpoolBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"From":"From %d","To":"To %d","ObjectType":"macpool.Block","Size":32,"ClassId":"macpool.Block"}`
	...
	expectedOp := []map[string]interface{}{{"from": "From 1", "to": "To 1", "object_type": "macpool.Block", "size": 32, "class_id": "macpool.Block"}, {"from": "From 2", "to": "To 2", "object_type": "macpool.Block", "size": 32, "class_id": "macpool.Block"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMemoryPersistentMemoryLogicalNamespace(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"memory.PersistentMemoryLogicalNamespace","Capacity":32,"Mode":"Mode %d","Name":"Name %d","SocketId":32,"SocketMemoryId":"SocketMemoryId %d","ClassId":"memory.PersistentMemoryLogicalNamespace"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "memory.PersistentMemoryLogicalNamespace", "capacity": 32, "mode": "Mode 1", "name": "Name 1", "socket_id": 32, "socket_memory_id": "SocketMemoryId 1", "class_id": "memory.PersistentMemoryLogicalNamespace"}, {"object_type": "memory.PersistentMemoryLogicalNamespace", "capacity": 32, "mode": "Mode 2", "name": "Name 2", "socket_id": 32, "socket_memory_id": "SocketMemoryId 2", "class_id": "memory.PersistentMemoryLogicalNamespace"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaAccessPrivilege(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Privilege":"Privilege %d","ObjectType":"meta.AccessPrivilege","ClassId":"meta.AccessPrivilege","Method":"Method %d"}`
	...
	expectedOp := []map[string]interface{}{{"privilege": "Privilege 1", "object_type": "meta.AccessPrivilege", "class_id": "meta.AccessPrivilege", "method": "Method 1"}, {"privilege": "Privilege 2", "object_type": "meta.AccessPrivilege", "class_id": "meta.AccessPrivilege", "method": "Method 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaPropDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"meta.PropDefinition","SearchWeight":32.000000,"Type":"Type %d","Name":"Name %d","ObjectType":"meta.PropDefinition","ApiAccess":"ApiAccess %d","IsComplexType":true,"IsCollection":true,"Kind":"Kind %d","OpSecurity":"OpSecurity %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "meta.PropDefinition", "search_weight": 32.000000, "type": "Type 1", "name": "Name 1", "object_type": "meta.PropDefinition", "api_access": "ApiAccess 1", "is_complex_type": true, "is_collection": true, "kind": "Kind 1", "op_security": "OpSecurity 1"}, {"class_id": "meta.PropDefinition", "search_weight": 32.000000, "type": "Type 2", "name": "Name 2", "object_type": "meta.PropDefinition", "api_access": "ApiAccess 2", "is_complex_type": true, "is_collection": true, "kind": "Kind 2", "op_security": "OpSecurity 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaRelationshipDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","Collection":true,"ObjectType":"meta.RelationshipDefinition","ExportWithPeer":true,"PeerRelName":"PeerRelName %d","PeerSync":true,"ApiAccess":"ApiAccess %d","ClassId":"meta.RelationshipDefinition","Name":"Name %d","Export":true}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "collection": true, "object_type": "meta.RelationshipDefinition", "export_with_peer": true, "peer_rel_name": "PeerRelName 1", "peer_sync": true, "api_access": "ApiAccess 1", "class_id": "meta.RelationshipDefinition", "name": "Name 1", "export": true}, {"type": "Type 2", "collection": true, "object_type": "meta.RelationshipDefinition", "export_with_peer": true, "peer_rel_name": "PeerRelName 2", "peer_sync": true, "api_access": "ApiAccess 2", "class_id": "meta.RelationshipDefinition", "name": "Name 2", "export": true}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiaapiDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","Chksum":"Chksum %d","Filename":"Filename %d","ObjectType":"niaapi.Detail","ClassId":"niaapi.Detail"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "chksum": "Chksum 1", "filename": "Filename 1", "object_type": "niaapi.Detail", "class_id": "niaapi.Detail"}, {"name": "Name 2", "chksum": "Chksum 2", "filename": "Filename 2", "object_type": "niaapi.Detail", "class_id": "niaapi.Detail"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryDeploymentStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.DeploymentStatus","ClassId":"niatelemetry.DeploymentStatus","Id":32,"Name":"Name %d","Status":"Status %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "niatelemetry.DeploymentStatus", "class_id": "niatelemetry.DeploymentStatus", "id": 32, "name": "Name 1", "status": "Status 1"}, {"object_type": "niatelemetry.DeploymentStatus", "class_id": "niatelemetry.DeploymentStatus", "id": 32, "name": "Name 2", "status": "Status 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryJobDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.JobDetail","JobId":32,"UpgStatus":"UpgStatus %d","ObjectType":"niatelemetry.JobDetail"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "niatelemetry.JobDetail", "job_id": 32, "upg_status": "UpgStatus 1", "object_type": "niatelemetry.JobDetail"}, {"class_id": "niatelemetry.JobDetail", "job_id": 32, "upg_status": "UpgStatus 2", "object_type": "niatelemetry.JobDetail"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryLogicalLink(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.LogicalLink","ObjectType":"niatelemetry.LogicalLink","IsPresent":true,"DbId":32,"Uptime":"Uptime %d","LinkAddr2":"LinkAddr2 %d","LinkType":"LinkType %d","LinkAddr1":"LinkAddr1 %d","LinkState":"LinkState %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "niatelemetry.LogicalLink", "object_type": "niatelemetry.LogicalLink", "is_present": true, "db_id": 32, "uptime": "Uptime 1", "link_addr2": "LinkAddr2 1", "link_type": "LinkType 1", "link_addr1": "LinkAddr1 1", "link_state": "LinkState 1"}, {"class_id": "niatelemetry.LogicalLink", "object_type": "niatelemetry.LogicalLink", "is_present": true, "db_id": 32, "uptime": "Uptime 2", "link_addr2": "LinkAddr2 2", "link_type": "LinkType 2", "link_addr1": "LinkAddr1 2", "link_state": "LinkState 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetrySites(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","SiteType":"SiteType %d","ClassId":"niatelemetry.Sites","ObjectType":"niatelemetry.Sites","Uuid":"Uuid %d"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "site_type": "SiteType 1", "class_id": "niatelemetry.Sites", "object_type": "niatelemetry.Sites", "uuid": "Uuid 1"}, {"name": "Name 2", "site_type": "SiteType 2", "class_id": "niatelemetry.Sites", "object_type": "niatelemetry.Sites", "uuid": "Uuid 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryVniStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"VniState":"VniState %d","ObjectType":"niatelemetry.VniStatus","ClassId":"niatelemetry.VniStatus","VniType":"VniType %d","Vni":"Vni %d"}`
	...
	expectedOp := []map[string]interface{}{{"vni_state": "VniState 1", "object_type": "niatelemetry.VniStatus", "class_id": "niatelemetry.VniStatus", "vni_type": "VniType 1", "vni": "Vni 1"}, {"vni_state": "VniState 2", "object_type": "niatelemetry.VniStatus", "class_id": "niatelemetry.VniStatus", "vni_type": "VniType 2", "vni": "Vni 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryVpcDetails(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.VpcDetails","ClassId":"niatelemetry.VpcDetails","IsVpcConfigured":true,"PeerSwitchDbId":32,"SwitchDbId":32}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "niatelemetry.VpcDetails", "class_id": "niatelemetry.VpcDetails", "is_vpc_configured": true, "peer_switch_db_id": 32, "switch_db_id": 32}, {"object_type": "niatelemetry.VpcDetails", "class_id": "niatelemetry.VpcDetails", "is_vpc_configured": true, "peer_switch_db_id": 32, "switch_db_id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOnpremImagePackage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"onprem.ImagePackage","FilePath":"FilePath %d","FileSha":"FileSha %d","Name":"Name %d","PackageType":"PackageType %d","FileSize":32,"ClassId":"onprem.ImagePackage","Filename":"Filename %d","Version":"Version %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "onprem.ImagePackage", "file_path": "FilePath 1", "file_sha": "FileSha 1", "name": "Name 1", "package_type": "PackageType 1", "file_size": 32, "class_id": "onprem.ImagePackage", "filename": "Filename 1", "nr_version": "Version 1"}, {"object_type": "onprem.ImagePackage", "file_path": "FilePath 2", "file_sha": "FileSha 2", "name": "Name 2", "package_type": "PackageType 2", "file_size": 32, "class_id": "onprem.ImagePackage", "filename": "Filename 2", "nr_version": "Version 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOprsKvpair(t *testing.T) {
	p := []models.OprsKvpair{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"oprs.Kvpair","ObjectType":"oprs.Kvpair","Value":"Value %d","Key":"Key %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "oprs.Kvpair", "object_type": "oprs.Kvpair", "value": "Value 1", "key": "Key 1"}, {"class_id": "oprs.Kvpair", "object_type": "oprs.Kvpair", "value": "Value 2", "key": "Key 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOsValidationInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"StepName":"StepName %d","ErrorMsg":"ErrorMsg %d","ClassId":"os.ValidationInformation","ObjectType":"os.ValidationInformation","Status":"Status %d"}`
	...
	expectedOp := []map[string]interface{}{{"step_name": "StepName 1", "error_msg": "ErrorMsg 1", "class_id": "os.ValidationInformation", "object_type": "os.ValidationInformation", "status": "Status 1"}, {"step_name": "StepName 2", "error_msg": "ErrorMsg 2", "class_id": "os.ValidationInformation", "object_type": "os.ValidationInformation", "status": "Status 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListPolicyinventoryJobInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"policyinventory.JobInfo","ExecutionStatus":"ExecutionStatus %d","PolicyId":"PolicyId %d","PolicyName":"PolicyName %d","ObjectType":"policyinventory.JobInfo"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "policyinventory.JobInfo", "execution_status": "ExecutionStatus 1", "policy_id": "PolicyId 1", "policy_name": "PolicyName 1", "object_type": "policyinventory.JobInfo"}, {"class_id": "policyinventory.JobInfo", "execution_status": "ExecutionStatus 2", "policy_id": "PolicyId 2", "policy_name": "PolicyName 2", "object_type": "policyinventory.JobInfo"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListResourcePerTypeCombinedSelector(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"resource.PerTypeCombinedSelector","ClassId":"resource.PerTypeCombinedSelector","EmptyFilter":true,"SelectorObjectType":"SelectorObjectType %d","CombinedSelector":"CombinedSelector %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "resource.PerTypeCombinedSelector", "class_id": "resource.PerTypeCombinedSelector", "empty_filter": true, "selector_object_type": "SelectorObjectType 1", "combined_selector": "CombinedSelector 1"}, {"object_type": "resource.PerTypeCombinedSelector", "class_id": "resource.PerTypeCombinedSelector", "empty_filter": true, "selector_object_type": "SelectorObjectType 2", "combined_selector": "CombinedSelector 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpTrap(t *testing.T) {
	p := []models.SnmpTrap{}
	var d = &schema.ResourceData{}
	c := `{"Port":32,"ObjectType":"snmp.Trap","ClassId":"snmp.Trap","Type":"Type %d","Version":"Version %d","Community":"Community %d","Enabled":true,"Destination":"Destination %d","User":"User %d"}`
	...
	expectedOp := []map[string]interface{}{{"port": 32, "object_type": "snmp.Trap", "class_id": "snmp.Trap", "type": "Type 1", "nr_version": "Version 1", "community": "Community 1", "enabled": true, "destination": "Destination 1", "user": "User 1"}, {"port": 32, "object_type": "snmp.Trap", "class_id": "snmp.Trap", "type": "Type 2", "nr_version": "Version 2", "community": "Community 2", "enabled": true, "destination": "Destination 2", "user": "User 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpUser(t *testing.T) {
	p := []models.SnmpUser{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"snmp.User","Name":"Name %d","PrivacyType":"PrivacyType %d","ObjectType":"snmp.User","IsAuthPasswordSet":true,"IsPrivacyPasswordSet":true,"AuthType":"AuthType %d","SecurityLevel":"SecurityLevel %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "snmp.User", "name": "Name 1", "privacy_type": "PrivacyType 1", "object_type": "snmp.User", "is_auth_password_set": true, "is_privacy_password_set": true, "auth_type": "AuthType 1", "security_level": "SecurityLevel 1"}, {"class_id": "snmp.User", "name": "Name 2", "privacy_type": "PrivacyType 2", "object_type": "snmp.User", "is_auth_password_set": true, "is_privacy_password_set": true, "auth_type": "AuthType 2", "security_level": "SecurityLevel 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStorageBaseInitiator(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.BaseInitiator","Iqn":"Iqn %d","Name":"Name %d","Type":"Type %d","Wwn":"Wwn %d","ObjectType":"storage.BaseInitiator"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "storage.BaseInitiator", "iqn": "Iqn 1", "name": "Name 1", "type": "Type 1", "wwn": "Wwn 1", "object_type": "storage.BaseInitiator"}, {"class_id": "storage.BaseInitiator", "iqn": "Iqn 2", "name": "Name 2", "type": "Type 2", "wwn": "Wwn 2", "object_type": "storage.BaseInitiator"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStoragePureReplicationBlackout(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Start":"Start %d","End":"End %d","ObjectType":"storage.PureReplicationBlackout","ClassId":"storage.PureReplicationBlackout"}`
	...
	expectedOp := []map[string]interface{}{{"start": "Start 1", "end": "End 1", "object_type": "storage.PureReplicationBlackout", "class_id": "storage.PureReplicationBlackout"}, {"start": "Start 2", "end": "End 2", "object_type": "storage.PureReplicationBlackout", "class_id": "storage.PureReplicationBlackout"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListStorageStorageContainerHostMountStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.StorageContainerHostMountStatus","Accessibility":"Accessibility %d","HostName":"HostName %d","Mounted":true,"Reason":"Reason %d","ObjectType":"storage.StorageContainerHostMountStatus"}`
...
func TestFlattenListStorageVirtualDriveConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.VirtualDriveConfiguration","ObjectType":"storage.VirtualDriveConfiguration","BootDrive":true,"ExpandToAvailable":true,"Name":"Name %d","Size":32}`
...
func TestFlattenListSyslogRemoteClientBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Protocol":"Protocol %d","Enabled":true,"ClassId":"syslog.RemoteClientBase","ObjectType":"syslog.RemoteClientBase","Hostname":"Hostname %d","MinSeverity":"MinSeverity %d","Port":32}`
	...
	expectedOp := []map[string]interface{}{{"protocol": "Protocol 1", "enabled": true, "class_id": "syslog.RemoteClientBase", "object_type": "syslog.RemoteClientBase", "hostname": "Hostname 1", "min_severity": "MinSeverity 1", "port": 32}, {"protocol": "Protocol 2", "enabled": true, "class_id": "syslog.RemoteClientBase", "object_type": "syslog.RemoteClientBase", "hostname": "Hostname 2", "min_severity": "MinSeverity 2", "port": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTamAction(t *testing.T) {
	p := []models.TamAction{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"tam.Action","Type":"Type %d","AlertType":"AlertType %d","AffectedObjectType":"AffectedObjectType %d","ObjectType":"tam.Action","Name":"Name %d","OperationType":"OperationType %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "tam.Action", "type": "Type 1", "alert_type": "AlertType 1", "affected_object_type": "AffectedObjectType 1", "object_type": "tam.Action", "name": "Name 1", "operation_type": "OperationType 1"}, {"class_id": "tam.Action", "type": "Type 2", "alert_type": "AlertType 2", "affected_object_type": "AffectedObjectType 2", "object_type": "tam.Action", "name": "Name 2", "operation_type": "OperationType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTerraformCloudResource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"terraform.CloudResource","ClassId":"terraform.CloudResource","ResourceId":"ResourceId %d","CurrentStatus":"CurrentStatus %d","DesiredStatus":"DesiredStatus %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "terraform.CloudResource", "class_id": "terraform.CloudResource", "resource_id": "ResourceId 1", "current_status": "CurrentStatus 1", "desired_status": "DesiredStatus 1"}, {"object_type": "terraform.CloudResource", "class_id": "terraform.CloudResource", "resource_id": "ResourceId 2", "current_status": "CurrentStatus 2", "desired_status": "DesiredStatus 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListTerraformRunstate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"terraform.Runstate","ClassId":"terraform.Runstate","RunId":"RunId %d","StateFile":"StateFile %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "terraform.Runstate", "class_id": "terraform.Runstate", "run_id": "RunId 1", "state_file": "StateFile 1"}, {"object_type": "terraform.Runstate", "class_id": "terraform.Runstate", "run_id": "RunId 2", "state_file": "StateFile 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUcsdConnectorPack(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"ucsd.ConnectorPack","DownloadedVersion":"DownloadedVersion %d","State":"State %d","Version":"Version %d","ConnectorFeature":"ConnectorFeature %d","ObjectType":"ucsd.ConnectorPack","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "ucsd.ConnectorPack", "downloaded_version": "DownloadedVersion 1", "state": "State 1", "nr_version": "Version 1", "connector_feature": "ConnectorFeature 1", "object_type": "ucsd.ConnectorPack", "name": "Name 1"}, {"class_id": "ucsd.ConnectorPack", "downloaded_version": "DownloadedVersion 2", "state": "State 2", "nr_version": "Version 2", "connector_feature": "ConnectorFeature 2", "object_type": "ucsd.ConnectorPack", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUuidpoolUuidBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"To":"To %d","Size":32,"ClassId":"uuidpool.UuidBlock","ObjectType":"uuidpool.UuidBlock","From":"From %d"}`
	...
	expectedOp := []map[string]interface{}{{"to": "To 1", "size": 32, "class_id": "uuidpool.UuidBlock", "object_type": "uuidpool.UuidBlock", "from": "From 1"}, {"to": "To 2", "size": 32, "class_id": "uuidpool.UuidBlock", "object_type": "uuidpool.UuidBlock", "from": "From 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationNetworkInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IpForwardingEnabled":true,"ClassId":"virtualization.NetworkInterface","ConnectAtPowerOn":true,"AdaptorType":"AdaptorType %d","MacAddress":"MacAddress %d","PrivateIpAllocationMode":"PrivateIpAllocationMode %d","DirectPathIo":true,"ObjectType":"virtualization.NetworkInterface","Ipv6Address":true,"Order":32,"PublicIpAllocate":true,"NicId":"NicId %d","Name":"Name %d","Bridge":"Bridge %d","NetworkId":"NetworkId %d","SubnetId":"SubnetId %d"}`
	...
	expectedOp := []map[string]interface{}{{"ip_forwarding_enabled": true, "class_id": "virtualization.NetworkInterface", "connect_at_power_on": true, "adaptor_type": "AdaptorType 1", "mac_address": "MacAddress 1", "private_ip_allocation_mode": "PrivateIpAllocationMode 1", "direct_path_io": true, "object_type": "virtualization.NetworkInterface", "ipv6_address": true, "order": 32, "public_ip_allocate": true, "nic_id": "NicId 1", "name": "Name 1", "bridge": "Bridge 1", "network_id": "NetworkId 1", "subnet_id": "SubnetId 1"}, {"ip_forwarding_enabled": true, "class_id": "virtualization.NetworkInterface", "connect_at_power_on": true, "adaptor_type": "AdaptorType 2", "mac_address": "MacAddress 2", "private_ip_allocation_mode": "PrivateIpAllocationMode 2", "direct_path_io": true, "object_type": "virtualization.NetworkInterface", "ipv6_address": true, "order": 32, "public_ip_allocate": true, "nic_id": "NicId 2", "name": "Name 2", "bridge": "Bridge 2", "network_id": "NetworkId 2", "subnet_id": "SubnetId 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVirtualMachineDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"VirtualDiskReference":"VirtualDiskReference %d","Bus":"Bus %d","Name":"Name %d","Type":"Type %d","ObjectType":"virtualization.VirtualMachineDisk","ClassId":"virtualization.VirtualMachineDisk","Order":32}`
	...
	expectedOp := []map[string]interface{}{{"virtual_disk_reference": "VirtualDiskReference 1", "bus": "Bus 1", "name": "Name 1", "type": "Type 1", "object_type": "virtualization.VirtualMachineDisk", "class_id": "virtualization.VirtualMachineDisk", "order": 32}, {"virtual_disk_reference": "VirtualDiskReference 2", "bus": "Bus 2", "name": "Name 2", "type": "Type 2", "object_type": "virtualization.VirtualMachineDisk", "class_id": "virtualization.VirtualMachineDisk", "order": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.VmDisk","BootOrder":32,"VirtualDiskReference":"VirtualDiskReference %d","ClassId":"virtualization.VmDisk","Bus":"Bus %d","Name":"Name %d","Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "virtualization.VmDisk", "boot_order": 32, "virtual_disk_reference": "VirtualDiskReference 1", "class_id": "virtualization.VmDisk", "bus": "Bus 1", "name": "Name 1", "type": "Type 1"}, {"object_type": "virtualization.VmDisk", "boot_order": 32, "virtual_disk_reference": "VirtualDiskReference 2", "class_id": "virtualization.VmDisk", "bus": "Bus 2", "name": "Name 2", "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.VmInterface","Bridge":"Bridge %d","MacAddress":"MacAddress %d","Model":"Model %d","Name":"Name %d","ClassId":"virtualization.VmInterface"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "virtualization.VmInterface", "bridge": "Bridge 1", "mac_address": "MacAddress 1", "model": "Model 1", "name": "Name 1", "class_id": "virtualization.VmInterface"}, {"object_type": "virtualization.VmInterface", "bridge": "Bridge 2", "mac_address": "MacAddress 2", "model": "Model 2", "name": "Name 2", "class_id": "virtualization.VmInterface"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVmwareResourceAllocationSystemTrafficTypes(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Limit":32,"Reservation":32,"Shares":"Shares %d","SharesValue":32,"TrafficType":"TrafficType %d","ClassId":"virtualization.VmwareResourceAllocationSystemTrafficTypes","ObjectType":"virtualization.VmwareResourceAllocationSystemTrafficTypes"}`
	...
	expectedOp := []map[string]interface{}{{"limit": 32, "reservation": 32, "shares": "Shares 1", "shares_value": 32, "traffic_type": "TrafficType 1", "class_id": "virtualization.VmwareResourceAllocationSystemTrafficTypes", "object_type": "virtualization.VmwareResourceAllocationSystemTrafficTypes"}, {"limit": 32, "reservation": 32, "shares": "Shares 2", "shares_value": 32, "traffic_type": "TrafficType 2", "class_id": "virtualization.VmwareResourceAllocationSystemTrafficTypes", "object_type": "virtualization.VmwareResourceAllocationSystemTrafficTypes"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVmediaMapping(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"HostName":"HostName %d","SanitizedFileLocation":"SanitizedFileLocation %d","IsPasswordSet":true,"DeviceType":"DeviceType %d","RemotePath":"RemotePath %d","MountOptions":"MountOptions %d","RemoteFile":"RemoteFile %d","AuthenticationProtocol":"AuthenticationProtocol %d","FileLocation":"FileLocation %d","ObjectType":"vmedia.Mapping","ClassId":"vmedia.Mapping","Username":"Username %d","MountProtocol":"MountProtocol %d","VolumeName":"VolumeName %d"}`
	...
	expectedOp := []map[string]interface{}{{"host_name": "HostName 1", "sanitized_file_location": "SanitizedFileLocation 1", "is_password_set": true, "device_type": "DeviceType 1", "remote_path": "RemotePath 1", "mount_options": "MountOptions 1", "remote_file": "RemoteFile 1", "authentication_protocol": "AuthenticationProtocol 1", "file_location": "FileLocation 1", "object_type": "vmedia.Mapping", "class_id": "vmedia.Mapping", "username": "Username 1", "mount_protocol": "MountProtocol 1", "volume_name": "VolumeName 1"}, {"host_name": "HostName 2", "sanitized_file_location": "SanitizedFileLocation 2", "is_password_set": true, "device_type": "DeviceType 2", "remote_path": "RemotePath 2", "mount_options": "MountOptions 2", "remote_file": "RemoteFile 2", "authentication_protocol": "AuthenticationProtocol 2", "file_location": "FileLocation 2", "object_type": "vmedia.Mapping", "class_id": "vmedia.Mapping", "username": "Username 2", "mount_protocol": "MountProtocol 2", "volume_name": "VolumeName 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowActionWorkflowDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Description":"Description %d","WorkflowDefinitionName":"WorkflowDefinitionName %d","CatalogMoid":"CatalogMoid %d","Name":"Name %d","Version":32,"Label":"Label %d","ObjectType":"workflow.ActionWorkflowDefinition","ClassId":"workflow.ActionWorkflowDefinition"}`
	...
	expectedOp := []map[string]interface{}{{"description": "Description 1", "workflow_definition_name": "WorkflowDefinitionName 1", "catalog_moid": "CatalogMoid 1", "name": "Name 1", "nr_version": 32, "label": "Label 1", "object_type": "workflow.ActionWorkflowDefinition", "class_id": "workflow.ActionWorkflowDefinition"}, {"description": "Description 2", "workflow_definition_name": "WorkflowDefinitionName 2", "catalog_moid": "CatalogMoid 2", "name": "Name 2", "nr_version": 32, "label": "Label 2", "object_type": "workflow.ActionWorkflowDefinition", "class_id": "workflow.ActionWorkflowDefinition"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowApi(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.Api","Body":"Body %d","ObjectType":"workflow.Api","Name":"Name %d","StartDelay":32,"Description":"Description %d","AssetTargetMoid":"AssetTargetMoid %d","Label":"Label %d","ErrorContentType":"ErrorContentType %d","ContentType":"ContentType %d","SkipOnCondition":"SkipOnCondition %d","Timeout":32}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.Api", "body": "Body 1", "object_type": "workflow.Api", "name": "Name 1", "start_delay": 32, "description": "Description 1", "asset_target_moid": "AssetTargetMoid 1", "label": "Label 1", "error_content_type": "ErrorContentType 1", "content_type": "ContentType 1", "skip_on_condition": "SkipOnCondition 1", "timeout": 32}, {"class_id": "workflow.Api", "body": "Body 2", "object_type": "workflow.Api", "name": "Name 2", "start_delay": 32, "description": "Description 2", "asset_target_moid": "AssetTargetMoid 2", "label": "Label 2", "error_content_type": "ErrorContentType 2", "content_type": "ContentType 2", "skip_on_condition": "SkipOnCondition 2", "timeout": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowBaseDataType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.BaseDataType","Description":"Description %d","Label":"Label %d","ObjectType":"workflow.BaseDataType","Required":true,"Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.BaseDataType", "description": "Description 1", "label": "Label 1", "object_type": "workflow.BaseDataType", "required": true, "name": "Name 1"}, {"class_id": "workflow.BaseDataType", "description": "Description 2", "label": "Label 2", "object_type": "workflow.BaseDataType", "required": true, "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"TaskMoid":"TaskMoid %d","Description":"Description %d","ObjectType":"workflow.RollbackTask","ClassId":"workflow.RollbackTask","SkipCondition":"SkipCondition %d","Version":32,"CatalogMoid":"CatalogMoid %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"task_moid": "TaskMoid 1", "description": "Description 1", "object_type": "workflow.RollbackTask", "class_id": "workflow.RollbackTask", "skip_condition": "SkipCondition 1", "nr_version": 32, "catalog_moid": "CatalogMoid 1", "name": "Name 1"}, {"task_moid": "TaskMoid 2", "description": "Description 2", "object_type": "workflow.RollbackTask", "class_id": "workflow.RollbackTask", "skip_condition": "SkipCondition 2", "nr_version": 32, "catalog_moid": "CatalogMoid 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowRollbackWorkflowTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Description":"Description %d","RefName":"RefName %d","RollbackCompleted":true,"ObjectType":"workflow.RollbackWorkflowTask","ClassId":"workflow.RollbackWorkflowTask","RollbackTaskName":"RollbackTaskName %d","TaskPath":"TaskPath %d","Name":"Name %d","Status":"Status %d","TaskInfoMoid":"TaskInfoMoid %d"}`
	...
	expectedOp := []map[string]interface{}{{"description": "Description 1", "ref_name": "RefName 1", "rollback_completed": true, "object_type": "workflow.RollbackWorkflowTask", "class_id": "workflow.RollbackWorkflowTask", "rollback_task_name": "RollbackTaskName 1", "task_path": "TaskPath 1", "name": "Name 1", "status": "Status 1", "task_info_moid": "TaskInfoMoid 1"}, {"description": "Description 2", "ref_name": "RefName 2", "rollback_completed": true, "object_type": "workflow.RollbackWorkflowTask", "class_id": "workflow.RollbackWorkflowTask", "rollback_task_name": "RollbackTaskName 2", "task_path": "TaskPath 2", "name": "Name 2", "status": "Status 2", "task_info_moid": "TaskInfoMoid 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowServiceItemActionWorkflowDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.ServiceItemActionWorkflowDefinition","Version":32,"WorkflowDefinitionName":"WorkflowDefinitionName %d","Label":"Label %d","CatalogMoid":"CatalogMoid %d","ObjectType":"workflow.ServiceItemActionWorkflowDefinition","Name":"Name %d","Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.ServiceItemActionWorkflowDefinition", "nr_version": 32, "workflow_definition_name": "WorkflowDefinitionName 1", "label": "Label 1", "catalog_moid": "CatalogMoid 1", "object_type": "workflow.ServiceItemActionWorkflowDefinition", "name": "Name 1", "description": "Description 1"}, {"class_id": "workflow.ServiceItemActionWorkflowDefinition", "nr_version": 32, "workflow_definition_name": "WorkflowDefinitionName 2", "label": "Label 2", "catalog_moid": "CatalogMoid 2", "object_type": "workflow.ServiceItemActionWorkflowDefinition", "name": "Name 2", "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowWorkflowTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","Description":"Description %d","Label":"Label %d","ObjectType":"workflow.WorkflowTask","ClassId":"workflow.WorkflowTask"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "description": "Description 1", "label": "Label 1", "object_type": "workflow.WorkflowTask", "class_id": "workflow.WorkflowTask"}, {"name": "Name 2", "description": "Description 2", "label": "Label 2", "object_type": "workflow.WorkflowTask", "class_id": "workflow.WorkflowTask"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenMapAccessConfigurationType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"access.ConfigurationType","ConfigureInband":true,"ConfigureOutOfBand":true,"ClassId":"access.ConfigurationType"}`
	...
	expectedOp := map[string]interface{}{"object_type": "access.ConfigurationType", "configure_inband": true, "configure_out_of_band": true, "class_id": "access.ConfigurationType"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapApplianceCertRenewalPhase(t *testing.T) {
	p := models.ApplianceCertRenewalPhase{}
	var d = &schema.ResourceData{}
	c := `{"Message":"Message %d","ObjectType":"appliance.CertRenewalPhase","ClassId":"appliance.CertRenewalPhase","Name":"Name %d","Failed":true}`
	...
	expectedOp := map[string]interface{}{"message": "Message 1", "object_type": "appliance.CertRenewalPhase", "class_id": "appliance.CertRenewalPhase", "name": "Name 1", "failed": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeploymentDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"asset.DeploymentDeviceInformation","OldDeviceStatusId":32,"OldDeviceId":"OldDeviceId %d","OldDeviceStatusDescription":"OldDeviceStatusDescription %d","MlbProductName":"MlbProductName %d","MlbProductId":32,"ItemType":"ItemType %d","ClassId":"asset.DeploymentDeviceInformation","ApplicationName":"ApplicationName %d","Description":"Description %d","InstanceId":"InstanceId %d","OldInstanceId":"OldInstanceId %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "asset.DeploymentDeviceInformation", "old_device_status_id": 32, "old_device_id": "OldDeviceId 1", "old_device_status_description": "OldDeviceStatusDescription 1", "mlb_product_name": "MlbProductName 1", "mlb_product_id": 32, "item_type": "ItemType 1", "class_id": "asset.DeploymentDeviceInformation", "application_name": "ApplicationName 1", "description": "Description 1", "instance_id": "InstanceId 1", "old_instance_id": "OldInstanceId 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"OldDeviceStatusId":32,"OldDeviceId":"OldDeviceId %d","ClassId":"asset.DeviceInformation","ApplicationName":"ApplicationName %d","InstanceId":"InstanceId %d","ProductType":"ProductType %d","ProductFamily":"ProductFamily %d","MlbOfferType":"MlbOfferType %d","MlbProductId":32,"ObjectType":"asset.DeviceInformation","OldInstanceId":"OldInstanceId %d","UnitOfMeasure":"UnitOfMeasure %d","OldDeviceStatusDescription":"OldDeviceStatusDescription %d","MlbProductName":"MlbProductName %d","ItemType":"ItemType %d"}`
	...
	expectedOp := map[string]interface{}{"old_device_status_id": 32, "old_device_id": "OldDeviceId 1", "class_id": "asset.DeviceInformation", "application_name": "ApplicationName 1", "instance_id": "InstanceId 1", "product_type": "ProductType 1", "product_family": "ProductFamily 1", "mlb_offer_type": "MlbOfferType 1", "mlb_product_id": 32, "object_type": "asset.DeviceInformation", "old_instance_id": "OldInstanceId 1", "unit_of_measure": "UnitOfMeasure 1", "old_device_status_description": "OldDeviceStatusDescription 1", "mlb_product_name": "MlbProductName 1", "item_type": "ItemType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceStatistics(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"asset.DeviceStatistics","MembershipRatio":32.000000,"ClusterDeviceMoid":"ClusterDeviceMoid %d","MemoryMirroringFactor":32.000000,"ClusterName":"ClusterName %d","ObjectType":"asset.DeviceStatistics","ClusterDeploymentType":"ClusterDeploymentType %d","ClusterReplicationFactor":32,"Connected":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "asset.DeviceStatistics", "membership_ratio": 32.000000, "cluster_device_moid": "ClusterDeviceMoid 1", "memory_mirroring_factor": 32.000000, "cluster_name": "ClusterName 1", "object_type": "asset.DeviceStatistics", "cluster_deployment_type": "ClusterDeploymentType 1", "cluster_replication_factor": 32, "connected": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapAssetGlobalUltimate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","Id":"Id %d","ClassId":"asset.GlobalUltimate","ObjectType":"asset.GlobalUltimate"}`
	...
	expectedOp := map[string]interface{}{"name": "Name 1", "id": "Id 1", "class_id": "asset.GlobalUltimate", "object_type": "asset.GlobalUltimate"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapAssetProductInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Number":"Number %d","SubType":"SubType %d","Group":"Group %d","ClassId":"asset.ProductInformation","ObjectType":"asset.ProductInformation","Description":"Description %d","Family":"Family %d"}`
	...
	expectedOp := map[string]interface{}{"number": "Number 1", "sub_type": "SubType 1", "group": "Group 1", "class_id": "asset.ProductInformation", "object_type": "asset.ProductInformation", "description": "Description 1", "family": "Family 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetSudiInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"asset.SudiInfo","ClassId":"asset.SudiInfo","Pid":"Pid %d","SerialNumber":"SerialNumber %d","Signature":"Signature %d","Status":"Status %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "asset.SudiInfo", "class_id": "asset.SudiInfo", "pid": "Pid 1", "serial_number": "SerialNumber 1", "signature": "Signature 1", "status": "Status 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapBulkApiResult(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"class_id": "bulk.ApiResult", "status": 32, "object_type": "bulk.ApiResult"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCapabilitySwitchNetworkLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"capability.SwitchNetworkLimits","MaximumEthernetPortChannels":32,"MaximumIgmpGroups":32,"MaximumVlans":32,"MaximumFcPortChannelMembers":32,"MaximumFcPortChannels":32,"ClassId":"capability.SwitchNetworkLimits","MaximumSecondaryVlanPerPrimary":32,"MaximumActiveTrafficMonitoringSessions":32,"MaximumVifs":32,"MaximumPrimaryVlan":32,"MaxCompressedPortVlanCount":32,"MaximumEthernetUplinkPorts":32,"MaximumSecondaryVlan":32,"MaxUncompressedPortVlanCount":32,"MinimumActiveFans":32,"MaximumPortChannelMembers":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "capability.SwitchNetworkLimits", "maximum_ethernet_port_channels": 32, "maximum_igmp_groups": 32, "maximum_vlans": 32, "maximum_fc_port_channel_members": 32, "maximum_fc_port_channels": 32, "class_id": "capability.SwitchNetworkLimits", "maximum_secondary_vlan_per_primary": 32, "maximum_active_traffic_monitoring_sessions": 32, "maximum_vifs": 32, "maximum_primary_vlan": 32, "max_compressed_port_vlan_count": 32, "maximum_ethernet_uplink_ports": 32, "maximum_secondary_vlan": 32, "max_uncompressed_port_vlan_count": 32, "minimum_active_fans": 32, "maximum_port_channel_members": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchStorageLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaximumVirtualFcInterfacesPerBladeServer":32,"ObjectType":"capability.SwitchStorageLimits","ClassId":"capability.SwitchStorageLimits","MaximumVsans":32,"MaximumZoneCount":32,"MaximumUserZoneCount":32,"MaximumVirtualFcInterfaces":32}`
	...
	expectedOp := map[string]interface{}{"maximum_virtual_fc_interfaces_per_blade_server": 32, "object_type": "capability.SwitchStorageLimits", "class_id": "capability.SwitchStorageLimits", "maximum_vsans": 32, "maximum_zone_count": 32, "maximum_user_zone_count": 32, "maximum_virtual_fc_interfaces": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchSystemLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"capability.SwitchSystemLimits","ObjectType":"capability.SwitchSystemLimits","MaximumChassisCount":32,"MaximumFexPerDomain":32,"MaximumServersPerDomain":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "capability.SwitchSystemLimits", "object_type": "capability.SwitchSystemLimits", "maximum_chassis_count": 32, "maximum_fex_per_domain": 32, "maximum_servers_per_domain": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCertificatemanagementCertificateBase(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"is_privatekey_set": true, "class_id": "certificatemanagement.CertificateBase", "object_type": "certificatemanagement.CertificateBase", "enabled": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudBillingUnit(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"BillingId":"BillingId %d","Name":"Name %d","ObjectType":"cloud.BillingUnit","ClassId":"cloud.BillingUnit"}`
	...
	expectedOp := map[string]interface{}{"billing_id": "BillingId 1", "name": "Name 1", "object_type": "cloud.BillingUnit", "class_id": "cloud.BillingUnit"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudInstanceType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.InstanceType","CpuSpeed":32,"Architecture":"Architecture %d","ClassId":"cloud.InstanceType","Name":"Name %d","Platform":"Platform %d","Memory":32,"Cpus":32,"InstanceTypeId":"InstanceTypeId %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "cloud.InstanceType", "cpu_speed": 32, "architecture": "Architecture 1", "class_id": "cloud.InstanceType", "name": "Name 1", "platform": "Platform 1", "memory": 32, "cpus": 32, "instance_type_id": "InstanceTypeId 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCloudNetworkInstanceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"State":"State %d","AutoDelete":true,"DeviceIndex":32,"InstanceId":"InstanceId %d","ClassId":"cloud.NetworkInstanceAttachment","ObjectType":"cloud.NetworkInstanceAttachment"}`
	...
	expectedOp := map[string]interface{}{"state": "State 1", "auto_delete": true, "device_index": 32, "instance_id": "InstanceId 1", "class_id": "cloud.NetworkInstanceAttachment", "object_type": "cloud.NetworkInstanceAttachment"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudVolumeType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.VolumeType","ClassId":"cloud.VolumeType","VolumeTypeId":"VolumeTypeId %d","Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "cloud.VolumeType", "class_id": "cloud.VolumeType", "volume_type_id": "VolumeTypeId 1", "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCommIpV4Interface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"comm.IpV4Interface","ClassId":"comm.IpV4Interface","Netmask":"Netmask %d","Gateway":"Gateway %d","IpAddress":"IpAddress %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "comm.IpV4Interface", "class_id": "comm.IpV4Interface", "netmask": "Netmask 1", "gateway": "Gateway 1", "ip_address": "IpAddress 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapComputePersistentMemoryOperation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IsSecurePassphraseSet":true,"ClassId":"compute.PersistentMemoryOperation","ObjectType":"compute.PersistentMemoryOperation","AdminAction":"AdminAction %d"}`
	...
	expectedOp := map[string]interface{}{"is_secure_passphrase_set": true, "class_id": "compute.PersistentMemoryOperation", "object_type": "compute.PersistentMemoryOperation", "admin_action": "AdminAction 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapComputeStorageVirtualDriveOperation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ControllerId":"ControllerId %d","AdminAction":"AdminAction %d","ObjectType":"compute.StorageVirtualDriveOperation","ClassId":"compute.StorageVirtualDriveOperation"}`
	...
	expectedOp := map[string]interface{}{"controller_id": "ControllerId 1", "admin_action": "AdminAction 1", "object_type": "compute.StorageVirtualDriveOperation", "class_id": "compute.StorageVirtualDriveOperation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCondAlarmSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Critical":32,"Warning":32,"ClassId":"cond.AlarmSummary","ObjectType":"cond.AlarmSummary"}`
	...
	expectedOp := map[string]interface{}{"critical": 32, "warning": 32, "class_id": "cond.AlarmSummary", "object_type": "cond.AlarmSummary"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapConvergedinfraPodSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"convergedinfra.PodSummary","NodeCount":32,"StorageUtilization":32.000000,"ClassId":"convergedinfra.PodSummary","VmCount":32,"ActiveNodes":32,"StorageAvailable":32,"StorageCapacity":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "convergedinfra.PodSummary", "node_count": 32, "storage_utilization": 32.000000, "class_id": "convergedinfra.PodSummary", "vm_count": 32, "active_nodes": 32, "storage_available": 32, "storage_capacity": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFabricMacAgingSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"fabric.MacAgingSettings","ClassId":"fabric.MacAgingSettings","MacAgingTime":32,"MacAgingOption":"MacAgingOption %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "fabric.MacAgingSettings", "class_id": "fabric.MacAgingSettings", "mac_aging_time": 32, "mac_aging_option": "MacAgingOption 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFabricUdldGlobalSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MessageInterval":32,"RecoveryAction":"RecoveryAction %d","ClassId":"fabric.UdldGlobalSettings","ObjectType":"fabric.UdldGlobalSettings"}`
	...
	expectedOp := map[string]interface{}{"message_interval": 32, "recovery_action": "RecoveryAction 1", "class_id": "fabric.UdldGlobalSettings", "object_type": "fabric.UdldGlobalSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapFabricUdldSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Mode":"Mode %d","AdminState":"AdminState %d","ObjectType":"fabric.UdldSettings","ClassId":"fabric.UdldSettings"}`
	...
	expectedOp := map[string]interface{}{"mode": "Mode 1", "admin_state": "AdminState 1", "object_type": "fabric.UdldSettings", "class_id": "fabric.UdldSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapFabricVlanSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AllowedVlans":"AllowedVlans %d","NativeVlan":32,"ObjectType":"fabric.VlanSettings","ClassId":"fabric.VlanSettings"}`
	...
	expectedOp := map[string]interface{}{"allowed_vlans": "AllowedVlans 1", "native_vlan": 32, "object_type": "fabric.VlanSettings", "class_id": "fabric.VlanSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFcpoolBlock(t *testing.T) {
	p := models.FcpoolBlock{}
	var d = &schema.ResourceData{}
	c := `{"Size":32,"ClassId":"fcpool.Block","ObjectType":"fcpool.Block","To":"To %d","From":"From %d"}`
	...
	expectedOp := map[string]interface{}{"size": 32, "class_id": "fcpool.Block", "object_type": "fcpool.Block", "to": "To 1", "from": "From 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareDirectDownload(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ImageSource":"ImageSource %d","Username":"Username %d","ObjectType":"firmware.DirectDownload","ClassId":"firmware.DirectDownload","IsPasswordSet":true,"Upgradeoption":"Upgradeoption %d"}`
	...
	expectedOp := map[string]interface{}{"image_source": "ImageSource 1", "username": "Username 1", "object_type": "firmware.DirectDownload", "class_id": "firmware.DirectDownload", "is_password_set": true, "upgradeoption": "Upgradeoption 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareNetworkShare(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MapType":"MapType %d","Username":"Username %d","ObjectType":"firmware.NetworkShare","IsPasswordSet":true,"Upgradeoption":"Upgradeoption %d","ClassId":"firmware.NetworkShare"}`
	...
	expectedOp := map[string]interface{}{"map_type": "MapType 1", "username": "Username 1", "object_type": "firmware.NetworkShare", "is_password_set": true, "upgradeoption": "Upgradeoption 1", "class_id": "firmware.NetworkShare"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexAppSettingConstraint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.AppSettingConstraint","DeploymentType":"DeploymentType %d","HxdpVersion":"HxdpVersion %d","HypervisorType":"HypervisorType %d","MgmtPlatform":"MgmtPlatform %d","ServerModel":"ServerModel %d","ObjectType":"hyperflex.AppSettingConstraint"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.AppSettingConstraint", "deployment_type": "DeploymentType 1", "hxdp_version": "HxdpVersion 1", "hypervisor_type": "HypervisorType 1", "mgmt_platform": "MgmtPlatform 1", "server_model": "ServerModel 1", "object_type": "hyperflex.AppSettingConstraint"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexBackupPolicySettings(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.BackupPolicySettings", "backup_data_store_size_unit": "BackupDataStoreSizeUnit 1", "backup_data_store_size": 32, "data_store_encryption_enabled": true, "local_snapshot_retention_count": 32, "snapshot_retention_count": 32, "class_id": "hyperflex.BackupPolicySettings", "replication_interval_in_minutes": 32, "backup_data_store_name": "BackupDataStoreName 1", "replication_pair_name_prefix": "ReplicationPairNamePrefix 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexEntityReference(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Id":"Id %d","Idtype":"Idtype %d","Name":"Name %d","Type":"Type %d","Confignum":32,"ObjectType":"hyperflex.EntityReference","ClassId":"hyperflex.EntityReference"}`
	...
	expectedOp := map[string]interface{}{"id": "Id 1", "idtype": "Idtype 1", "name": "Name 1", "type": "Type 1", "confignum": 32, "object_type": "hyperflex.EntityReference", "class_id": "hyperflex.EntityReference"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHealthCheckScriptInfo(t *testing.T) {
	p := models.HyperflexHealthCheckScriptInfo{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.HealthCheckScriptInfo","ObjectType":"hyperflex.HealthCheckScriptInfo","ScriptName":"ScriptName %d","Version":"Version %d","HyperflexVersion":"HyperflexVersion %d","AggregateScriptName":"AggregateScriptName %d","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptInput":"ScriptInput %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.HealthCheckScriptInfo", "object_type": "hyperflex.HealthCheckScriptInfo", "script_name": "ScriptName 1", "nr_version": "Version 1", "hyperflex_version": "HyperflexVersion 1", "aggregate_script_name": "AggregateScriptName 1", "script_execute_location": "ScriptExecuteLocation 1", "script_input": "ScriptInput 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxLicenseAuthorizationDetailsDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.HxLicenseAuthorizationDetailsDt","CommunicationDeadlineDate":"CommunicationDeadlineDate %d","EvaluationPeriodExpiredAt":"EvaluationPeriodExpiredAt %d","EvaluationPeriodRemaining":"EvaluationPeriodRemaining %d","ObjectType":"hyperflex.HxLicenseAuthorizationDetailsDt","LastCommunicationAttemptDate":"LastCommunicationAttemptDate %d","NextCommunicationAttemptDate":"NextCommunicationAttemptDate %d","Status":"Status %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.HxLicenseAuthorizationDetailsDt", "communication_deadline_date": "CommunicationDeadlineDate 1", "evaluation_period_expired_at": "EvaluationPeriodExpiredAt 1", "evaluation_period_remaining": "EvaluationPeriodRemaining 1", "object_type": "hyperflex.HxLicenseAuthorizationDetailsDt", "last_communication_attempt_date": "LastCommunicationAttemptDate 1", "next_communication_attempt_date": "NextCommunicationAttemptDate 1", "status": "Status 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHxPlatformDatastoreConfigDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HxPlatformDatastoreConfigDt","UsageType":"UsageType %d","ProvisionedCapacity":32,"ClassId":"hyperflex.HxPlatformDatastoreConfigDt","DataBlockSize":32,"SystemDatastore":true,"Name":"Name %d","NumMirrors":32,"NumStripesForLargeFiles":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.HxPlatformDatastoreConfigDt", "usage_type": "UsageType 1", "provisioned_capacity": 32, "class_id": "hyperflex.HxPlatformDatastoreConfigDt", "data_block_size": 32, "system_datastore": true, "name": "Name 1", "num_mirrors": 32, "num_stripes_for_large_files": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxRegistrationDetailsDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"InitialRegistrationDate":"InitialRegistrationDate %d","RegistrationFailedReason":"RegistrationFailedReason %d","ClassId":"hyperflex.HxRegistrationDetailsDt","VirtualAccount":"VirtualAccount %d","OutOfComplianceStartDate":"OutOfComplianceStartDate %d","LastRenewalAttemptDate":"LastRenewalAttemptDate %d","ObjectType":"hyperflex.HxRegistrationDetailsDt","Status":"Status %d","SmartAccount":"SmartAccount %d","NextRenewalAttemptDate":"NextRenewalAttemptDate %d","RegistrationExpireDate":"RegistrationExpireDate %d"}`
	...
	expectedOp := map[string]interface{}{"initial_registration_date": "InitialRegistrationDate 1", "registration_failed_reason": "RegistrationFailedReason 1", "class_id": "hyperflex.HxRegistrationDetailsDt", "virtual_account": "VirtualAccount 1", "out_of_compliance_start_date": "OutOfComplianceStartDate 1", "last_renewal_attempt_date": "LastRenewalAttemptDate 1", "object_type": "hyperflex.HxRegistrationDetailsDt", "status": "Status 1", "smart_account": "SmartAccount 1", "next_renewal_attempt_date": "NextRenewalAttemptDate 1", "registration_expire_date": "RegistrationExpireDate 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxResiliencyInfoDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SsdFailuresTolerable":32,"ResiliencyState":"ResiliencyState %d","HddFailuresTolerable":32,"NodeFailuresTolerable":32,"PolicyCompliance":"PolicyCompliance %d","DataReplicationFactor":"DataReplicationFactor %d","ObjectType":"hyperflex.HxResiliencyInfoDt","ClassId":"hyperflex.HxResiliencyInfoDt"}`
	...
	expectedOp := map[string]interface{}{"ssd_failures_tolerable": 32, "resiliency_state": "ResiliencyState 1", "hdd_failures_tolerable": 32, "node_failures_tolerable": 32, "policy_compliance": "PolicyCompliance 1", "data_replication_factor": "DataReplicationFactor 1", "object_type": "hyperflex.HxResiliencyInfoDt", "class_id": "hyperflex.HxResiliencyInfoDt"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexIpAddrRange(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"end_addr": "EndAddr 1", "gateway": "Gateway 1", "netmask": "Netmask 1", "start_addr": "StartAddr 1", "class_id": "hyperflex.IpAddrRange", "object_type": "hyperflex.IpAddrRange"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexLogicalAvailabilityZone(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.LogicalAvailabilityZone","AutoConfig":true,"ObjectType":"hyperflex.LogicalAvailabilityZone"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.LogicalAvailabilityZone", "auto_config": true, "object_type": "hyperflex.LogicalAvailabilityZone"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexReplicationPeerInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Status":"Status %d","ClassId":"hyperflex.ReplicationPeerInfo","ReplCip":"ReplCip %d","Dcip":"Dcip %d","StatusDetails":"StatusDetails %d","ObjectType":"hyperflex.ReplicationPeerInfo","Mcip":"Mcip %d"}`
	...
	expectedOp := map[string]interface{}{"status": "Status 1", "class_id": "hyperflex.ReplicationPeerInfo", "repl_cip": "ReplCip 1", "dcip": "Dcip 1", "status_details": "StatusDetails 1", "object_type": "hyperflex.ReplicationPeerInfo", "mcip": "Mcip 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.Summary","DeduplicationSavings":32.000000,"Name":"Name %d","ClusterAccessPolicy":"ClusterAccessPolicy %d","DataReplicationCompliance":"DataReplicationCompliance %d","Address":"Address %d","ObjectType":"hyperflex.Summary","Downtime":"Downtime %d","UsedCapacity":32,"FreeCapacity":32,"SpaceStatus":"SpaceStatus %d","Boottime":32,"Uptime":"Uptime %d","State":"State %d","TotalSavings":32.000000,"DataReplicationFactor":"DataReplicationFactor %d","TotalCapacity":32,"ResiliencyDetailsSize":32,"CompressionSavings":32.000000,"ActiveNodes":"ActiveNodes %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.Summary", "deduplication_savings": 32.000000, "name": "Name 1", "cluster_access_policy": "ClusterAccessPolicy 1", "data_replication_compliance": "DataReplicationCompliance 1", "address": "Address 1", "object_type": "hyperflex.Summary", "downtime": "Downtime 1", "used_capacity": 32, "free_capacity": 32, "space_status": "SpaceStatus 1", "boottime": 32, "uptime": "Uptime 1", "state": "State 1", "total_savings": 32.000000, "data_replication_factor": "DataReplicationFactor 1", "total_capacity": 32, "resiliency_details_size": 32, "compression_savings": 32.000000, "active_nodes": "ActiveNodes 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexWwxnPrefixRange(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.WwxnPrefixRange","EndAddr":"EndAddr %d","StartAddr":"StartAddr %d","ObjectType":"hyperflex.WwxnPrefixRange"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.WwxnPrefixRange", "end_addr": "EndAddr 1", "start_addr": "StartAddr 1", "object_type": "hyperflex.WwxnPrefixRange"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamClientMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"iam.ClientMeta","DeviceModel":"DeviceModel %d","UserAgent":"UserAgent %d","ObjectType":"iam.ClientMeta"}`
	...
	expectedOp := map[string]interface{}{"class_id": "iam.ClientMeta", "device_model": "DeviceModel 1", "user_agent": "UserAgent 1", "object_type": "iam.ClientMeta"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamEndPointPasswordProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"iam.EndPointPasswordProperties","NotificationPeriod":32,"GracePeriod":32,"EnablePasswordExpiry":true,"ClassId":"iam.EndPointPasswordProperties","PasswordHistory":32,"PasswordExpiryDuration":32,"ForceSendPassword":true,"EnforceStrongPassword":true}`
	...
	expectedOp := map[string]interface{}{"object_type": "iam.EndPointPasswordProperties", "notification_period": 32, "grace_period": 32, "enable_password_expiry": true, "class_id": "iam.EndPointPasswordProperties", "password_history": 32, "password_expiry_duration": 32, "force_send_password": true, "enforce_strong_password": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamLdapBaseProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"GroupAttribute":"GroupAttribute %d","BindMethod":"BindMethod %d","NestedGroupSearchDepth":32,"Timeout":32,"ClassId":"iam.LdapBaseProperties","IsPasswordSet":true,"Domain":"Domain %d","Attribute":"Attribute %d","ObjectType":"iam.LdapBaseProperties","EnableEncryption":true,"Filter":"Filter %d","BaseDn":"BaseDn %d","EnableGroupAuthorization":true,"BindDn":"BindDn %d"}`
	...
	expectedOp := map[string]interface{}{"group_attribute": "GroupAttribute 1", "bind_method": "BindMethod 1", "nested_group_search_depth": 32, "timeout": 32, "class_id": "iam.LdapBaseProperties", "is_password_set": true, "domain": "Domain 1", "attribute": "Attribute 1", "object_type": "iam.LdapBaseProperties", "enable_encryption": true, "filter": "Filter 1", "base_dn": "BaseDn 1", "enable_group_authorization": true, "bind_dn": "BindDn 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapInfraHardwareInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"infra.HardwareInfo","ClassId":"infra.HardwareInfo","CpuSpeed":32,"MemorySize":32,"CpuCores":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "infra.HardwareInfo", "class_id": "infra.HardwareInfo", "cpu_speed": 32, "memory_size": 32, "cpu_cores": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIppoolIpV4Block(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"class_id": "ippool.IpV4Block", "object_type": "ippool.IpV4Block", "size": 32, "to": "To 1", "from": "From 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapIppoolIpV4Config(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Netmask":"Netmask %d","PrimaryDns":"PrimaryDns %d","SecondaryDns":"SecondaryDns %d","ObjectType":"ippool.IpV4Config","ClassId":"ippool.IpV4Config","Gateway":"Gateway %d"}`
	...
	expectedOp := map[string]interface{}{"netmask": "Netmask 1", "primary_dns": "PrimaryDns 1", "secondary_dns": "SecondaryDns 1", "object_type": "ippool.IpV4Config", "class_id": "ippool.IpV4Config", "gateway": "Gateway 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapIppoolIpV6Block(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"class_id": "ippool.IpV6Block", "object_type": "ippool.IpV6Block", "size": 32, "from": "From 1", "to": "To 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIqnpoolIqnSuffixBlock(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"from": 32, "suffix": "Suffix 1", "to": 32, "class_id": "iqnpool.IqnSuffixBlock", "object_type": "iqnpool.IqnSuffixBlock", "size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesAddonConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.AddonConfiguration","ClassId":"kubernetes.AddonConfiguration","InstallStrategy":"InstallStrategy %d","Overrides":"Overrides %d","ReleaseName":"ReleaseName %d","ReleaseNamespace":"ReleaseNamespace %d","UpgradeStrategy":"UpgradeStrategy %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.AddonConfiguration", "class_id": "kubernetes.AddonConfiguration", "install_strategy": "InstallStrategy 1", "overrides": "Overrides 1", "release_name": "ReleaseName 1", "release_namespace": "ReleaseNamespace 1", "upgrade_strategy": "UpgradeStrategy 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesClusterCertificateConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"EtcdCert":"EtcdCert %d","ObjectType":"kubernetes.ClusterCertificateConfiguration","CaCert":"CaCert %d","FrontProxyCert":"FrontProxyCert %d","ClassId":"kubernetes.ClusterCertificateConfiguration","SaPublicKey":"SaPublicKey %d"}`
	...
	expectedOp := map[string]interface{}{"etcd_cert": "EtcdCert 1", "object_type": "kubernetes.ClusterCertificateConfiguration", "ca_cert": "CaCert 1", "front_proxy_cert": "FrontProxyCert 1", "class_id": "kubernetes.ClusterCertificateConfiguration", "sa_public_key": "SaPublicKey 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesClusterManagementConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.ClusterManagementConfig","SshUser":"SshUser %d","IsTacPasswdSet":true,"MasterVip":"MasterVip %d","LoadBalancerCount":32,"ObjectType":"kubernetes.ClusterManagementConfig"}`
	...
	expectedOp := map[string]interface{}{"class_id": "kubernetes.ClusterManagementConfig", "ssh_user": "SshUser 1", "is_tac_passwd_set": true, "master_vip": "MasterVip 1", "load_balancer_count": 32, "object_type": "kubernetes.ClusterManagementConfig"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesDaemonSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.DaemonSetStatus","ObservedGeneration":32,"UpdatedNumberScheduled":"UpdatedNumberScheduled %d","NumberAvailable":"NumberAvailable %d","DesiredNumberScheduled":32,"ObjectType":"kubernetes.DaemonSetStatus","CurrentNumberScheduled":32,"NumberReady":32,"NumberMisscheduled":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "kubernetes.DaemonSetStatus", "observed_generation": 32, "updated_number_scheduled": "UpdatedNumberScheduled 1", "number_available": "NumberAvailable 1", "desired_number_scheduled": 32, "object_type": "kubernetes.DaemonSetStatus", "current_number_scheduled": 32, "number_ready": 32, "number_misscheduled": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesDeploymentStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReadyReplicas":32,"ObjectType":"kubernetes.DeploymentStatus","ClassId":"kubernetes.DeploymentStatus","Replicas":32,"UpdatedReplicas":32,"AvailableReplicas":32,"ObservedGeneration":32}`
	...
	expectedOp := map[string]interface{}{"ready_replicas": 32, "object_type": "kubernetes.DeploymentStatus", "class_id": "kubernetes.DeploymentStatus", "replicas": 32, "updated_replicas": 32, "available_replicas": 32, "observed_generation": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesNodeInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ContainerRuntimeVersion":"ContainerRuntimeVersion %d","KubeletVersion":"KubeletVersion %d","Architecture":"Architecture %d","OperatingSystem":"OperatingSystem %d","ClassId":"kubernetes.NodeInfo","MachineId":"MachineId %d","BootId":"BootId %d","KernelVersion":"KernelVersion %d","KubeProxyVersion":"KubeProxyVersion %d","OsImage":"OsImage %d","ObjectType":"kubernetes.NodeInfo","SystemUuid":"SystemUuid %d"}`
	...
	expectedOp := map[string]interface{}{"container_runtime_version": "ContainerRuntimeVersion 1", "kubelet_version": "KubeletVersion 1", "architecture": "Architecture 1", "operating_system": "OperatingSystem 1", "class_id": "kubernetes.NodeInfo", "machine_id": "MachineId 1", "boot_id": "BootId 1", "kernel_version": "KernelVersion 1", "kube_proxy_version": "KubeProxyVersion 1", "os_image": "OsImage 1", "object_type": "kubernetes.NodeInfo", "system_uuid": "SystemUuid 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesObjectMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.ObjectMeta","CreationTimestamp":"CreationTimestamp %d","Name":"Name %d","Namespace":"Namespace %d","ResourceVersion":"ResourceVersion %d","Uuid":"Uuid %d","ObjectType":"kubernetes.ObjectMeta"}`
	...
	expectedOp := map[string]interface{}{"class_id": "kubernetes.ObjectMeta", "creation_timestamp": "CreationTimestamp 1", "name": "Name 1", "namespace": "Namespace 1", "resource_version": "ResourceVersion 1", "uuid": "Uuid 1", "object_type": "kubernetes.ObjectMeta"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesPodStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"HostIp":"HostIp %d","Phase":"Phase %d","PodIp":"PodIp %d","QosClass":"QosClass %d","StartTime":"StartTime %d","ClassId":"kubernetes.PodStatus","ObjectType":"kubernetes.PodStatus"}`
	...
	expectedOp := map[string]interface{}{"host_ip": "HostIp 1", "phase": "Phase 1", "pod_ip": "PodIp 1", "qos_class": "QosClass 1", "start_time": "StartTime 1", "class_id": "kubernetes.PodStatus", "object_type": "kubernetes.PodStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesProxyConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Hostname":"Hostname %d","IsPasswordSet":true,"Protocol":"Protocol %d","ObjectType":"kubernetes.ProxyConfig","Port":32,"ClassId":"kubernetes.ProxyConfig","Username":"Username %d"}`
	...
	expectedOp := map[string]interface{}{"hostname": "Hostname 1", "is_password_set": true, "protocol": "Protocol 1", "object_type": "kubernetes.ProxyConfig", "port": 32, "class_id": "kubernetes.ProxyConfig", "username": "Username 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesStatefulSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObservedGeneration":32,"AvailableReplicas":32,"ObjectType":"kubernetes.StatefulSetStatus","CurrentRevision":"CurrentRevision %d","ReadyReplicas":32,"UpdateRevision":"UpdateRevision %d","CollisionCount":32,"ClassId":"kubernetes.StatefulSetStatus","UpdatedReplicas":32,"Replicas":32}`
	...
	expectedOp := map[string]interface{}{"observed_generation": 32, "available_replicas": 32, "object_type": "kubernetes.StatefulSetStatus", "current_revision": "CurrentRevision 1", "ready_replicas": 32, "update_revision": "UpdateRevision 1", "collision_count": 32, "class_id": "kubernetes.StatefulSetStatus", "updated_replicas": 32, "replicas": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapMacpoolBlock(t *testing.T) {
	p := models.MacpoolBlock{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"macpool.Block","Size":32,"ClassId":"macpool.Block","From":"From %d","To":"To %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "macpool.Block", "size": 32, "class_id": "macpool.Block", "from": "From 1", "to": "To 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapMoBaseMo(t *testing.T) {
	p := models.MoBaseMo{}
	var d = &schema.ResourceData{}
	c := `{"Moid":"Moid %d","SharedScope":"SharedScope %d","ClassId":"mo.BaseMo","AccountMoid":"AccountMoid %d","ObjectType":"mo.BaseMo","DomainGroupMoid":"DomainGroupMoid %d"}`
	...
	expectedOp := map[string]interface{}{"moid": "Moid 1", "shared_scope": "SharedScope 1", "class_id": "mo.BaseMo", "account_moid": "AccountMoid 1", "object_type": "mo.BaseMo", "domain_group_moid": "DomainGroupMoid 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapMoVersionContext(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"mo.VersionContext","Version":"Version %d","VersionType":"VersionType %d","ObjectType":"mo.VersionContext"}`
	...
	expectedOp := map[string]interface{}{"class_id": "mo.VersionContext", "nr_version": "Version 1", "version_type": "VersionType 1", "object_type": "mo.VersionContext"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNetworkHyperFlexNetworkAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Ip":"Ip %d","ClassId":"network.HyperFlexNetworkAddress","ObjectType":"network.HyperFlexNetworkAddress","Address":"Address %d","Fqdn":"Fqdn %d"}`
	...
	expectedOp := map[string]interface{}{"ip": "Ip 1", "class_id": "network.HyperFlexNetworkAddress", "object_type": "network.HyperFlexNetworkAddress", "address": "Address 1", "fqdn": "Fqdn 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiaapiNewReleaseDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SoftwareDownloadLinkTitle":"SoftwareDownloadLinkTitle %d","ReleaseNoteLink":"ReleaseNoteLink %d","ObjectType":"niaapi.NewReleaseDetail","Version":"Version %d","Description":"Description %d","ReleaseNoteLinkTitle":"ReleaseNoteLinkTitle %d","Title":"Title %d","ClassId":"niaapi.NewReleaseDetail","Link":"Link %d","SoftwareDownloadLink":"SoftwareDownloadLink %d"}`
	...
	expectedOp := map[string]interface{}{"software_download_link_title": "SoftwareDownloadLinkTitle 1", "release_note_link": "ReleaseNoteLink 1", "object_type": "niaapi.NewReleaseDetail", "nr_version": "Version 1", "description": "Description 1", "release_note_link_title": "ReleaseNoteLinkTitle 1", "title": "Title 1", "class_id": "niaapi.NewReleaseDetail", "link": "Link 1", "software_download_link": "SoftwareDownloadLink 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryNetworkInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.NetworkInfo","ClassId":"niatelemetry.NetworkInfo","Hostname":"Hostname %d","ManagementtIp":"ManagementtIp %d","OutofbandIp":"OutofbandIp %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "niatelemetry.NetworkInfo", "class_id": "niatelemetry.NetworkInfo", "hostname": "Hostname 1", "managementt_ip": "ManagementtIp 1", "outofband_ip": "OutofbandIp 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryNvePacketCounters(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UcastOutpkts":32,"McastInpkts":32,"McastOutbytes":32,"UcastInpkts":32,"ClassId":"niatelemetry.NvePacketCounters","ObjectType":"niatelemetry.NvePacketCounters"}`
	...
	expectedOp := map[string]interface{}{"ucast_outpkts": 32, "mcast_inpkts": 32, "mcast_outbytes": 32, "ucast_inpkts": 32, "class_id": "niatelemetry.NvePacketCounters", "object_type": "niatelemetry.NvePacketCounters"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNveVni(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CpVniUp":32,"CpVniCount":32,"ClassId":"niatelemetry.NveVni","CpVniDown":32,"DpVniCount":32,"DpVniDown":32,"DpVniUp":32,"ObjectType":"niatelemetry.NveVni"}`
	...
	expectedOp := map[string]interface{}{"cp_vni_up": 32, "cp_vni_count": 32, "class_id": "niatelemetry.NveVni", "cp_vni_down": 32, "dp_vni_count": 32, "dp_vni_down": 32, "dp_vni_up": 32, "object_type": "niatelemetry.NveVni"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosBgpEvpn(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.NxosBgpEvpn","ClassId":"niatelemetry.NxosBgpEvpn","NxosEvpnMacCount":"NxosEvpnMacCount %d","TotalNetworks":32,"TotalPaths":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "niatelemetry.NxosBgpEvpn", "class_id": "niatelemetry.NxosBgpEvpn", "nxos_evpn_mac_count": "NxosEvpnMacCount 1", "total_networks": 32, "total_paths": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosBgpMvpn(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"TableVersion":32,"CapablePeers":32,"ConfiguredPeers":32,"TotalNetworks":32,"TotalPaths":32,"ObjectType":"niatelemetry.NxosBgpMvpn","ClassId":"niatelemetry.NxosBgpMvpn","MemoryUsed":32,"NumberOfCommunities":32,"NumberOfClusterLists":32}`
	...
	expectedOp := map[string]interface{}{"table_version": 32, "capable_peers": 32, "configured_peers": 32, "total_networks": 32, "total_paths": 32, "object_type": "niatelemetry.NxosBgpMvpn", "class_id": "niatelemetry.NxosBgpMvpn", "memory_used": 32, "number_of_communities": 32, "number_of_cluster_lists": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosVtp(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RunningVersion":"RunningVersion %d","V2Mode":"V2Mode %d","ClassId":"niatelemetry.NxosVtp","PruningMode":"PruningMode %d","OperMode":"OperMode %d","ObjectType":"niatelemetry.NxosVtp","TrapEnabled":"TrapEnabled %d","Version":32}`
	...
	expectedOp := map[string]interface{}{"running_version": "RunningVersion 1", "v2_mode": "V2Mode 1", "class_id": "niatelemetry.NxosVtp", "pruning_mode": "PruningMode 1", "oper_mode": "OperMode 1", "object_type": "niatelemetry.NxosVtp", "trap_enabled": "TrapEnabled 1", "nr_version": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetrySmartLicense(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AuthStatus":"AuthStatus %d","ClassId":"niatelemetry.SmartLicense","ObjectType":"niatelemetry.SmartLicense","LicenseUdi":"LicenseUdi %d","SmartAccount":"SmartAccount %d","ActiveMode":"ActiveMode %d"}`
	...
	expectedOp := map[string]interface{}{"auth_status": "AuthStatus 1", "class_id": "niatelemetry.SmartLicense", "object_type": "niatelemetry.SmartLicense", "license_udi": "LicenseUdi 1", "smart_account": "SmartAccount 1", "active_mode": "ActiveMode 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOnpremSchedule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DayOfMonth":32,"TimeZone":"TimeZone %d","WeekOfMonth":32,"RepeatInterval":32,"ObjectType":"onprem.Schedule","DayOfWeek":32,"MonthOfYear":32,"TimeOfDay":32,"ClassId":"onprem.Schedule"}`
	...
	expectedOp := map[string]interface{}{"day_of_month": 32, "time_zone": "TimeZone 1", "week_of_month": 32, "repeat_interval": 32, "object_type": "onprem.Schedule", "day_of_week": 32, "month_of_year": 32, "time_of_day": 32, "class_id": "onprem.Schedule"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapOnpremUpgradePhase(t *testing.T) {
	p := models.OnpremUpgradePhase{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"onprem.UpgradePhase","Name":"Name %d","ElapsedTime":32,"Message":"Message %d","Failed":true,"ClassId":"onprem.UpgradePhase"}`
	...
	expectedOp := map[string]interface{}{"object_type": "onprem.UpgradePhase", "name": "Name 1", "elapsed_time": 32, "message": "Message 1", "failed": true, "class_id": "onprem.UpgradePhase"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsAnswers(t *testing.T) {
	p := models.OsAnswers{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"os.Answers","IsRootPasswordSet":true,"Hostname":"Hostname %d","NetworkDevice":"NetworkDevice %d","ProductKey":"ProductKey %d","IsRootPasswordCrypted":true,"IpConfigType":"IpConfigType %d","IsAnswerFileSet":true,"ObjectType":"os.Answers","Nameserver":"Nameserver %d","Source":"Source %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "os.Answers", "is_root_password_set": true, "hostname": "Hostname 1", "network_device": "NetworkDevice 1", "product_key": "ProductKey 1", "is_root_password_crypted": true, "ip_config_type": "IpConfigType 1", "is_answer_file_set": true, "object_type": "os.Answers", "nameserver": "Nameserver 1", "nr_source": "Source 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsGlobalConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ConfigurationSource":"ConfigurationSource %d","WindowsEdition":"WindowsEdition %d","ClassId":"os.GlobalConfig","InstallMethod":"InstallMethod %d","ConfigurationFileName":"ConfigurationFileName %d","OsImageName":"OsImageName %d","ObjectType":"os.GlobalConfig","ScuImageName":"ScuImageName %d","InstallTargetType":"InstallTargetType %d"}`
	...
	expectedOp := map[string]interface{}{"configuration_source": "ConfigurationSource 1", "windows_edition": "WindowsEdition 1", "class_id": "os.GlobalConfig", "install_method": "InstallMethod 1", "configuration_file_name": "ConfigurationFileName 1", "os_image_name": "OsImageName 1", "object_type": "os.GlobalConfig", "scu_image_name": "ScuImageName 1", "install_target_type": "InstallTargetType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageAutomaticDriveGroup(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"storage.AutomaticDriveGroup","DriveType":"DriveType %d","MinimumDriveSize":32,"DrivesPerSpan":32,"ObjectType":"storage.AutomaticDriveGroup","NumDedicatedHotSpares":"NumDedicatedHotSpares %d","NumberOfSpans":32,"UseRemainingDrives":true}`
	...
	expectedOp := map[string]interface{}{"class_id": "storage.AutomaticDriveGroup", "drive_type": "DriveType 1", "minimum_drive_size": 32, "drives_per_span": 32, "object_type": "storage.AutomaticDriveGroup", "num_dedicated_hot_spares": "NumDedicatedHotSpares 1", "number_of_spans": 32, "use_remaining_drives": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapStorageBaseCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Available":32,"CapacityUtilization":32.000000,"Free":32,"Total":32,"Used":32,"ClassId":"storage.BaseCapacity","ObjectType":"storage.BaseCapacity"}`
	...
	expectedOp := map[string]interface{}{"available": 32, "capacity_utilization": 32.000000, "free": 32, "total": 32, "used": 32, "class_id": "storage.BaseCapacity", "object_type": "storage.BaseCapacity"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppEthernetPortLag(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"storage.NetAppEthernetPortLag","ClassId":"storage.NetAppEthernetPortLag","DistributionPolicy":"DistributionPolicy %d","Mode":"Mode %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "storage.NetAppEthernetPortLag", "class_id": "storage.NetAppEthernetPortLag", "distribution_policy": "DistributionPolicy 1", "mode": "Mode 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppHighAvailability(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"TakeoverState":"TakeoverState %d","Enabled":true,"GivebackState":"GivebackState %d","PartnerName":"PartnerName %d","PartnerUuid":"PartnerUuid %d","ObjectType":"storage.NetAppHighAvailability","ClassId":"storage.NetAppHighAvailability"}`
	...
	expectedOp := map[string]interface{}{"takeover_state": "TakeoverState 1", "enabled": true, "giveback_state": "GivebackState 1", "partner_name": "PartnerName 1", "partner_uuid": "PartnerUuid 1", "object_type": "storage.NetAppHighAvailability", "class_id": "storage.NetAppHighAvailability"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageNetAppPerformanceMetricsAverage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Throughput":32.000000,"ClassId":"storage.NetAppPerformanceMetricsAverage","ObjectType":"storage.NetAppPerformanceMetricsAverage","Iops":32.000000,"Latency":32.000000,"Period":32}`
	...
	expectedOp := map[string]interface{}{"throughput": 32.000000, "class_id": "storage.NetAppPerformanceMetricsAverage", "object_type": "storage.NetAppPerformanceMetricsAverage", "iops": 32.000000, "latency": 32.000000, "period": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapStorageNetAppStorageClusterEfficiency(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LogicalUsed":32,"ObjectType":"storage.NetAppStorageClusterEfficiency","ClassId":"storage.NetAppStorageClusterEfficiency","Ratio":32.000000,"Savings":32}`
	...
	expectedOp := map[string]interface{}{"logical_used": 32, "object_type": "storage.NetAppStorageClusterEfficiency", "class_id": "storage.NetAppStorageClusterEfficiency", "ratio": 32.000000, "savings": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageR0Drive(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DriveSlots":"DriveSlots %d","DriveSlotsList":"DriveSlotsList %d","ClassId":"storage.R0Drive","ObjectType":"storage.R0Drive","Enable":true}`
	...
	expectedOp := map[string]interface{}{"drive_slots": "DriveSlots 1", "drive_slots_list": "DriveSlotsList 1", "class_id": "storage.R0Drive", "object_type": "storage.R0Drive", "enable": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapUuidpoolUuidBlock(t *testing.T) {
	p := models.UuidpoolUuidBlock{}
	var d = &schema.ResourceData{}
	c := `{"Size":32,"ClassId":"uuidpool.UuidBlock","ObjectType":"uuidpool.UuidBlock","From":"From %d","To":"To %d"}`
	...
	expectedOp := map[string]interface{}{"size": 32, "class_id": "uuidpool.UuidBlock", "object_type": "uuidpool.UuidBlock", "from": "From 1", "to": "To 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCloudInitConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NetworkData":"NetworkData %d","NetworkDataBase64Encoded":true,"UserData":"UserData %d","UserDataBase64Encoded":true,"ClassId":"virtualization.CloudInitConfig","ObjectType":"virtualization.CloudInitConfig","ConfigType":"ConfigType %d"}`
	...
	expectedOp := map[string]interface{}{"network_data": "NetworkData 1", "network_data_base64_encoded": true, "user_data": "UserData 1", "user_data_base64_encoded": true, "class_id": "virtualization.CloudInitConfig", "object_type": "virtualization.CloudInitConfig", "config_type": "ConfigType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationComputeCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.ComputeCapacity","ClassId":"virtualization.ComputeCapacity","Capacity":32,"Free":32,"Used":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.ComputeCapacity", "class_id": "virtualization.ComputeCapacity", "capacity": 32, "free": 32, "used": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationCpuAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Reserved":32,"Total":32,"Used":32,"ObjectType":"virtualization.CpuAllocation","ClassId":"virtualization.CpuAllocation","Free":32}`
	...
	expectedOp := map[string]interface{}{"reserved": 32, "total": 32, "used": 32, "object_type": "virtualization.CpuAllocation", "class_id": "virtualization.CpuAllocation", "free": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationDiskStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"VolumeName":"VolumeName %d","VolumeVendor":"VolumeVendor %d","DownloadPercentage":"DownloadPercentage %d","Reason":"Reason %d","VolumeHandle":"VolumeHandle %d","ClassId":"virtualization.DiskStatus","State":"State %d","ObjectType":"virtualization.DiskStatus"}`
	...
	expectedOp := map[string]interface{}{"volume_name": "VolumeName 1", "volume_vendor": "VolumeVendor 1", "download_percentage": "DownloadPercentage 1", "reason": "Reason 1", "volume_handle": "VolumeHandle 1", "class_id": "virtualization.DiskStatus", "state": "State 1", "object_type": "virtualization.DiskStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationMemoryAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Used":32,"Free":32,"Reserved":32,"Total":32,"ObjectType":"virtualization.MemoryAllocation","ClassId":"virtualization.MemoryAllocation"}`
	...
	expectedOp := map[string]interface{}{"used": 32, "free": 32, "reserved": 32, "total": 32, "object_type": "virtualization.MemoryAllocation", "class_id": "virtualization.MemoryAllocation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationMemoryCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Capacity":32,"ClassId":"virtualization.MemoryCapacity","ObjectType":"virtualization.MemoryCapacity","Free":32,"Used":32}`
	...
	expectedOp := map[string]interface{}{"capacity": 32, "class_id": "virtualization.MemoryCapacity", "object_type": "virtualization.MemoryCapacity", "free": 32, "used": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationProductInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":"Version %d","Build":"Build %d","ProductName":"ProductName %d","ProductType":"ProductType %d","ProductVendor":"ProductVendor %d","ObjectType":"virtualization.ProductInfo","ClassId":"virtualization.ProductInfo"}`
	...
	expectedOp := map[string]interface{}{"nr_version": "Version 1", "build": "Build 1", "product_name": "ProductName 1", "product_type": "ProductType 1", "product_vendor": "ProductVendor 1", "object_type": "virtualization.ProductInfo", "class_id": "virtualization.ProductInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareDiscoveryProtocol(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareDiscoveryProtocol","ObjectType":"virtualization.VmwareDiscoveryProtocol","Operation":"Operation %d","Type":"Type %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.VmwareDiscoveryProtocol", "object_type": "virtualization.VmwareDiscoveryProtocol", "operation": "Operation 1", "type": "Type 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareRemoteDisplayInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RemoteDisplayVncPort":32,"RemoteDisplayPassword":"RemoteDisplayPassword %d","RemoteDisplayVncKey":"RemoteDisplayVncKey %d","ClassId":"virtualization.VmwareRemoteDisplayInfo","ObjectType":"virtualization.VmwareRemoteDisplayInfo"}`
	...
	expectedOp := map[string]interface{}{"remote_display_vnc_port": 32, "remote_display_password": "RemoteDisplayPassword 1", "remote_display_vnc_key": "RemoteDisplayVncKey 1", "class_id": "virtualization.VmwareRemoteDisplayInfo", "object_type": "virtualization.VmwareRemoteDisplayInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareTeamingAndFailover(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.VmwareTeamingAndFailover","Failback":true,"NetworkFailureDetection":"NetworkFailureDetection %d","Name":"Name %d","ClassId":"virtualization.VmwareTeamingAndFailover","NotifySwitches":true,"LoadBalancing":"LoadBalancing %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.VmwareTeamingAndFailover", "failback": true, "network_failure_detection": "NetworkFailureDetection 1", "name": "Name 1", "class_id": "virtualization.VmwareTeamingAndFailover", "notify_switches": true, "load_balancing": "LoadBalancing 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareVmCpuSocketInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NumSockets":32,"ClassId":"virtualization.VmwareVmCpuSocketInfo","ObjectType":"virtualization.VmwareVmCpuSocketInfo","CoresPerSocket":32,"NumCpus":32}`
	...
	expectedOp := map[string]interface{}{"num_sockets": 32, "class_id": "virtualization.VmwareVmCpuSocketInfo", "object_type": "virtualization.VmwareVmCpuSocketInfo", "cores_per_socket": 32, "num_cpus": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVirtualizationVmwareVmDiskCommitInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.VmwareVmDiskCommitInfo","ObjectType":"virtualization.VmwareVmDiskCommitInfo","UnCommittedDisk":32,"UnsharedDisk":32,"CommittedDisk":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.VmwareVmDiskCommitInfo", "object_type": "virtualization.VmwareVmDiskCommitInfo", "un_committed_disk": 32, "unshared_disk": 32, "committed_disk": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicArfsSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.ArfsSettings","ClassId":"vnic.ArfsSettings","Enabled":true}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.ArfsSettings", "class_id": "vnic.ArfsSettings", "enabled": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcErrorRecoverySettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.FcErrorRecoverySettings","ClassId":"vnic.FcErrorRecoverySettings","LinkDownTimeout":32,"PortDownTimeout":32,"Enabled":true,"IoRetryCount":32,"IoRetryTimeout":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.FcErrorRecoverySettings", "class_id": "vnic.FcErrorRecoverySettings", "link_down_timeout": 32, "port_down_timeout": 32, "enabled": true, "io_retry_count": 32, "io_retry_timeout": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcInterruptSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Mode":"Mode %d","ClassId":"vnic.FcInterruptSettings","ObjectType":"vnic.FcInterruptSettings"}`
	...
	expectedOp := map[string]interface{}{"mode": "Mode 1", "class_id": "vnic.FcInterruptSettings", "object_type": "vnic.FcInterruptSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcQueueSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.FcQueueSettings","ClassId":"vnic.FcQueueSettings","Count":32,"RingSize":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.FcQueueSettings", "class_id": "vnic.FcQueueSettings", "nr_count": 32, "ring_size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicFlogiSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.FlogiSettings","Retries":32,"Timeout":32,"ObjectType":"vnic.FlogiSettings"}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.FlogiSettings", "retries": 32, "timeout": 32, "object_type": "vnic.FlogiSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicPtpSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Enabled":true,"ObjectType":"vnic.PtpSettings","ClassId":"vnic.PtpSettings"}`
	...
	expectedOp := map[string]interface{}{"enabled": true, "object_type": "vnic.PtpSettings", "class_id": "vnic.PtpSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicRoceSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MemoryRegions":32,"ResourceGroups":32,"Version":32,"ObjectType":"vnic.RoceSettings","Enabled":true,"QueuePairs":32,"ClassId":"vnic.RoceSettings","ClassOfService":32}`
	...
	expectedOp := map[string]interface{}{"memory_regions": 32, "resource_groups": 32, "nr_version": 32, "object_type": "vnic.RoceSettings", "enabled": true, "queue_pairs": 32, "class_id": "vnic.RoceSettings", "class_of_service": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicRssHashSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.RssHashSettings","TcpIpv6Hash":true,"UdpIpv6Hash":true,"TcpIpv4Hash":true,"Ipv4Hash":true,"ObjectType":"vnic.RssHashSettings","UdpIpv4Hash":true,"Ipv6Hash":true,"TcpIpv6ExtHash":true,"Ipv6ExtHash":true}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.RssHashSettings", "tcp_ipv6_hash": true, "udp_ipv6_hash": true, "tcp_ipv4_hash": true, "ipv4_hash": true, "object_type": "vnic.RssHashSettings", "udp_ipv4_hash": true, "ipv6_hash": true, "tcp_ipv6_ext_hash": true, "ipv6_ext_hash": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicUsnicSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UsnicAdapterPolicy":"UsnicAdapterPolicy %d","Cos":32,"ObjectType":"vnic.UsnicSettings","ClassId":"vnic.UsnicSettings","Count":32}`
	...
	expectedOp := map[string]interface{}{"usnic_adapter_policy": "UsnicAdapterPolicy 1", "cos": 32, "object_type": "vnic.UsnicSettings", "class_id": "vnic.UsnicSettings", "nr_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicVlanSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.VlanSettings","ClassId":"vnic.VlanSettings","AllowedVlans":"AllowedVlans %d","DefaultVlan":32,"Mode":"Mode %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.VlanSettings", "class_id": "vnic.VlanSettings", "allowed_vlans": "AllowedVlans 1", "default_vlan": 32, "mode": "Mode 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicVmqSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.VmqSettings","ObjectType":"vnic.VmqSettings","Enabled":true,"MultiQueueSupport":true,"NumInterrupts":32,"NumSubVnics":32,"VmmqAdapterPolicy":"VmmqAdapterPolicy %d","NumVmqs":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.VmqSettings", "object_type": "vnic.VmqSettings", "enabled": true, "multi_queue_support": true, "num_interrupts": 32, "num_sub_vnics": 32, "vmmq_adapter_policy": "VmmqAdapterPolicy 1", "num_vmqs": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowCustomDataTypeProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.CustomDataTypeProperties","ExternalMeta":true,"Cloneable":true,"ObjectType":"workflow.CustomDataTypeProperties"}`
	...
	expectedOp := map[string]interface{}{"class_id": "workflow.CustomDataTypeProperties", "external_meta": true, "cloneable": true, "object_type": "workflow.CustomDataTypeProperties"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowInternalProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Internal":true,"Owner":"Owner %d","BaseTaskType":"BaseTaskType %d","ClassId":"workflow.InternalProperties","ObjectType":"workflow.InternalProperties"}`
	...
	expectedOp := map[string]interface{}{"internal": true, "owner": "Owner 1", "base_task_type": "BaseTaskType 1", "class_id": "workflow.InternalProperties", "object_type": "workflow.InternalProperties"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Cloneable":true,"ClassId":"workflow.Properties","ExternalMeta":true,"RetryDelay":32,"ObjectType":"workflow.Properties","StartsWorkflow":true,"TimeoutPolicy":"TimeoutPolicy %d","RetryCount":32,"RetryPolicy":"RetryPolicy %d","Timeout":32,"SupportStatus":"SupportStatus %d"}`
	...
	expectedOp := map[string]interface{}{"cloneable": true, "class_id": "workflow.Properties", "external_meta": true, "retry_delay": 32, "object_type": "workflow.Properties", "starts_workflow": true, "timeout_policy": "TimeoutPolicy 1", "retry_count": 32, "retry_policy": "RetryPolicy 1", "timeout": 32, "support_status": "SupportStatus 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowTaskLoopInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"workflow.TaskLoopInfo","Iteration":32,"LoopTaskLabel":"LoopTaskLabel %d","LoopTaskName":"LoopTaskName %d","LoopType":"LoopType %d","ClassId":"workflow.TaskLoopInfo"}`
	...
	expectedOp := map[string]interface{}{"object_type": "workflow.TaskLoopInfo", "iteration": 32, "loop_task_label": "LoopTaskLabel 1", "loop_task_name": "LoopTaskName 1", "loop_type": "LoopType 1", "class_id": "workflow.TaskLoopInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"EnableDebug":true,"ObjectType":"workflow.WorkflowProperties","ClassId":"workflow.WorkflowProperties","ExternalMeta":true,"Retryable":true,"SupportStatus":"SupportStatus %d","Cloneable":true}`
	...
	expectedOp := map[string]interface{}{"enable_debug": true, "object_type": "workflow.WorkflowProperties", "class_id": "workflow.WorkflowProperties", "external_meta": true, "retryable": true, "support_status": "SupportStatus 1", "cloneable": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapX509Certificate(t *testing.T) {
	p := models.X509Certificate{}
	var d = &schema.ResourceData{}
	c := `{"Sha256Fingerprint":"Sha256Fingerprint %d","ClassId":"x509.Certificate","PemCertificate":"PemCertificate %d","SignatureAlgorithm":"SignatureAlgorithm %d","ObjectType":"x509.Certificate"}`
	...
	expectedOp := map[string]interface{}{"sha256_fingerprint": "Sha256Fingerprint 1", "class_id": "x509.Certificate", "pem_certificate": "PemCertificate 1", "signature_algorithm": "SignatureAlgorithm 1", "object_type": "x509.Certificate"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
