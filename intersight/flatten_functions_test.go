...
func TestFlattenListApplianceCertRenewalPhase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","Failed":true,"Message":"Message %d","ClassId":"appliance.CertRenewalPhase","ObjectType":"appliance.CertRenewalPhase"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "failed": true, "message": "Message 1", "class_id": "appliance.CertRenewalPhase", "object_type": "appliance.CertRenewalPhase"}, {"name": "Name 2", "failed": true, "message": "Message 2", "class_id": "appliance.CertRenewalPhase", "object_type": "appliance.CertRenewalPhase"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCapabilityPortRange(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"capability.PortRange","ObjectType":"capability.PortRange","StartSlotId":32,"EndPortId":32,"EndSlotId":32,"StartPortId":32}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "capability.PortRange", "object_type": "capability.PortRange", "start_slot_id": 32, "end_port_id": 32, "end_slot_id": 32, "start_port_id": 32}, {"class_id": "capability.PortRange", "object_type": "capability.PortRange", "start_slot_id": 32, "end_port_id": 32, "end_slot_id": 32, "start_port_id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudCloudTag(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.CloudTag","ClassId":"cloud.CloudTag","Value":"Value %d","Key":"Key %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "cloud.CloudTag", "class_id": "cloud.CloudTag", "value": "Value 1", "key": "Key 1"}, {"object_type": "cloud.CloudTag", "class_id": "cloud.CloudTag", "value": "Value 2", "key": "Key 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudNetworkInterfaceAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"cloud.NetworkInterfaceAttachment","NetworkId":"NetworkId %d","ObjectType":"cloud.NetworkInterfaceAttachment","Identity":"Identity %d","NicIndex":32,"SubNetworkId":"SubNetworkId %d","MacAddress":"MacAddress %d","SubNetworkName":"SubNetworkName %d","IpForwardingEnabled":true,"NetworkName":"NetworkName %d"}`
...
func TestFlattenListCloudSecurityGroupRule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"StartPort":32,"SourceSecurityGroup":"SourceSecurityGroup %d","Protocol":"Protocol %d","Action":"Action %d","Identity":"Identity %d","EndPort":32,"Name":"Name %d","Index":32,"ObjectType":"cloud.SecurityGroupRule","Description":"Description %d","ClassId":"cloud.SecurityGroupRule","EtherType":"EtherType %d"}`
	...
	expectedOp := []map[string]interface{}{{"start_port": 32, "source_security_group": "SourceSecurityGroup 1", "protocol": "Protocol 1", "action": "Action 1", "identity": "Identity 1", "end_port": 32, "name": "Name 1", "index": 32, "object_type": "cloud.SecurityGroupRule", "description": "Description 1", "class_id": "cloud.SecurityGroupRule", "ether_type": "EtherType 1"}, {"start_port": 32, "source_security_group": "SourceSecurityGroup 2", "protocol": "Protocol 2", "action": "Action 2", "identity": "Identity 2", "end_port": 32, "name": "Name 2", "index": 32, "object_type": "cloud.SecurityGroupRule", "description": "Description 2", "class_id": "cloud.SecurityGroupRule", "ether_type": "EtherType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudTfcWorkspaceVariables(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Hcl":true,"Sensitive":true,"Description":"Description %d","ObjectType":"cloud.TfcWorkspaceVariables","Value":"Value %d","Key":"Key %d","Identity":"Identity %d","ClassId":"cloud.TfcWorkspaceVariables","Category":"Category %d"}`
	...
	expectedOp := []map[string]interface{}{{"hcl": true, "sensitive": true, "description": "Description 1", "object_type": "cloud.TfcWorkspaceVariables", "value": "Value 1", "key": "Key 1", "identity": "Identity 1", "class_id": "cloud.TfcWorkspaceVariables", "category": "Category 1"}, {"hcl": true, "sensitive": true, "description": "Description 2", "object_type": "cloud.TfcWorkspaceVariables", "value": "Value 2", "key": "Key 2", "identity": "Identity 2", "class_id": "cloud.TfcWorkspaceVariables", "category": "Category 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListCloudVolumeAttachment(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"DeviceName":"DeviceName %d","ObjectType":"cloud.VolumeAttachment","IsRoot":true,"AutoDelete":true,"ClassId":"cloud.VolumeAttachment","Index":32,"Identity":"Identity %d"}`
	...
	expectedOp := []map[string]interface{}{{"device_name": "DeviceName 1", "object_type": "cloud.VolumeAttachment", "is_root": true, "auto_delete": true, "class_id": "cloud.VolumeAttachment", "index": 32, "identity": "Identity 1"}, {"device_name": "DeviceName 2", "object_type": "cloud.VolumeAttachment", "is_root": true, "auto_delete": true, "class_id": "cloud.VolumeAttachment", "index": 32, "identity": "Identity 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListComputeIpAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"compute.IpAddress","Category":"Category %d","Name":"Name %d","Dn":"Dn %d","KvmVlan":32,"ObjectType":"compute.IpAddress","HttpPort":32,"KvmPort":32,"Address":"Address %d","Subnet":"Subnet %d","DefaultGateway":"DefaultGateway %d","HttpsPort":32,"Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "compute.IpAddress", "category": "Category 1", "name": "Name 1", "dn": "Dn 1", "kvm_vlan": 32, "object_type": "compute.IpAddress", "http_port": 32, "kvm_port": 32, "address": "Address 1", "subnet": "Subnet 1", "default_gateway": "DefaultGateway 1", "https_port": 32, "type": "Type 1"}, {"class_id": "compute.IpAddress", "category": "Category 2", "name": "Name 2", "dn": "Dn 2", "kvm_vlan": 32, "object_type": "compute.IpAddress", "http_port": 32, "kvm_port": 32, "address": "Address 2", "subnet": "Subnet 2", "default_gateway": "DefaultGateway 2", "https_port": 32, "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListContentParameter(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Secure":true,"ComplexType":"ComplexType %d","ClassId":"content.Parameter","Path":"Path %d","AcceptSingleValue":true,"ItemType":"ItemType %d","Name":"Name %d","ObjectType":"content.Parameter","Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"secure": true, "complex_type": "ComplexType 1", "class_id": "content.Parameter", "path": "Path 1", "accept_single_value": true, "item_type": "ItemType 1", "name": "Name 1", "object_type": "content.Parameter", "type": "Type 1"}, {"secure": true, "complex_type": "ComplexType 2", "class_id": "content.Parameter", "path": "Path 2", "accept_single_value": true, "item_type": "ItemType 2", "name": "Name 2", "object_type": "content.Parameter", "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListEquipmentIoCardIdentity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SwitchId":32,"IoCardMoid":"IoCardMoid %d","ModuleId":32,"NetworkElementMoid":"NetworkElementMoid %d","ClassId":"equipment.IoCardIdentity","ObjectType":"equipment.IoCardIdentity"}`
	...
	expectedOp := []map[string]interface{}{{"switch_id": 32, "io_card_moid": "IoCardMoid 1", "module_id": 32, "network_element_moid": "NetworkElementMoid 1", "class_id": "equipment.IoCardIdentity", "object_type": "equipment.IoCardIdentity"}, {"switch_id": 32, "io_card_moid": "IoCardMoid 2", "module_id": 32, "network_element_moid": "NetworkElementMoid 2", "class_id": "equipment.IoCardIdentity", "object_type": "equipment.IoCardIdentity"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricPortIdentifier(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SlotId":32,"AggregatePortId":32,"ClassId":"fabric.PortIdentifier","ObjectType":"fabric.PortIdentifier","PortId":32}`
	...
	expectedOp := []map[string]interface{}{{"slot_id": 32, "aggregate_port_id": 32, "class_id": "fabric.PortIdentifier", "object_type": "fabric.PortIdentifier", "port_id": 32}, {"slot_id": 32, "aggregate_port_id": 32, "class_id": "fabric.PortIdentifier", "object_type": "fabric.PortIdentifier", "port_id": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListFabricQosClass(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MulticastOptimize":true,"Cos":32,"ClassId":"fabric.QosClass","Weight":32,"BandwidthPercent":32,"Mtu":32,"PacketDrop":true,"ObjectType":"fabric.QosClass","AdminState":"AdminState %d","Name":"Name %d"}`
...
func TestFlattenListFirmwareBaseImpact(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ComputationError":"ComputationError %d","EndPoint":"EndPoint %d","ImpactType":"ImpactType %d","FirmwareVersion":"FirmwareVersion %d","Model":"Model %d","ObjectType":"firmware.BaseImpact","DomainName":"DomainName %d","VersionImpact":"VersionImpact %d","ClassId":"firmware.BaseImpact","TargetFirmwareVersion":"TargetFirmwareVersion %d","ComputationStatusDetail":"ComputationStatusDetail %d"}`
...
func TestFlattenListFirmwareFirmwareInventory(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Model":"Model %d","ClassId":"firmware.FirmwareInventory","Version":"Version %d","Category":"Category %d","Vendor":"Vendor %d","Label":"Label %d","ObjectType":"firmware.FirmwareInventory","UpdateUri":"UpdateUri %d"}`
...
func TestFlattenListHyperflexFeatureLimitEntry(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.FeatureLimitEntry","Name":"Name %d","Value":"Value %d","ClassId":"hyperflex.FeatureLimitEntry"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "hyperflex.FeatureLimitEntry", "name": "Name 1", "value": "Value 1", "class_id": "hyperflex.FeatureLimitEntry"}, {"object_type": "hyperflex.FeatureLimitEntry", "name": "Name 2", "value": "Value 2", "class_id": "hyperflex.FeatureLimitEntry"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexHealthCheckScriptInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.HealthCheckScriptInfo","HyperflexVersion":"HyperflexVersion %d","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptInput":"ScriptInput %d","ScriptName":"ScriptName %d","Version":"Version %d","ObjectType":"hyperflex.HealthCheckScriptInfo","AggregateScriptName":"AggregateScriptName %d"}`
...
func TestFlattenListHyperflexHxHostMountStatusDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"HostName":"HostName %d","Mounted":true,"Reason":"Reason %d","ClassId":"hyperflex.HxHostMountStatusDt","ObjectType":"hyperflex.HxHostMountStatusDt","Accessibility":"Accessibility %d"}`
...
func TestFlattenListHyperflexNetworkPort(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.NetworkPort","ClassId":"hyperflex.NetworkPort","Name":"Name %d","PortType":"PortType %d","Vlans":"Vlans %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "hyperflex.NetworkPort", "class_id": "hyperflex.NetworkPort", "name": "Name 1", "port_type": "PortType 1", "vlans": "Vlans 1"}, {"object_type": "hyperflex.NetworkPort", "class_id": "hyperflex.NetworkPort", "name": "Name 2", "port_type": "PortType 2", "vlans": "Vlans 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListHyperflexVmDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Bus":"Bus %d","Name":"Name %d","Type":"Type %d","VirtualDiskReference":"VirtualDiskReference %d","BootOrder":32,"ClassId":"hyperflex.VmDisk","ObjectType":"hyperflex.VmDisk"}`
	...
	expectedOp := []map[string]interface{}{{"bus": "Bus 1", "name": "Name 1", "type": "Type 1", "virtual_disk_reference": "VirtualDiskReference 1", "boot_order": 32, "class_id": "hyperflex.VmDisk", "object_type": "hyperflex.VmDisk"}, {"bus": "Bus 2", "name": "Name 2", "type": "Type 2", "virtual_disk_reference": "VirtualDiskReference 2", "boot_order": 32, "class_id": "hyperflex.VmDisk", "object_type": "hyperflex.VmDisk"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseKeysInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LicenseId":"LicenseId %d","Pid":"Pid %d","Count":32,"ClassId":"iaas.LicenseKeysInfo","ObjectType":"iaas.LicenseKeysInfo","ExpirationDate":"ExpirationDate %d"}`
	...
	expectedOp := []map[string]interface{}{{"license_id": "LicenseId 1", "pid": "Pid 1", "nr_count": 32, "class_id": "iaas.LicenseKeysInfo", "object_type": "iaas.LicenseKeysInfo", "expiration_date": "ExpirationDate 1"}, {"license_id": "LicenseId 2", "pid": "Pid 2", "nr_count": 32, "class_id": "iaas.LicenseKeysInfo", "object_type": "iaas.LicenseKeysInfo", "expiration_date": "ExpirationDate 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIaasLicenseUtilizationInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Label":"Label %d","LicensedLimit":"LicensedLimit %d","Sku":"Sku %d","ObjectType":"iaas.LicenseUtilizationInfo","ClassId":"iaas.LicenseUtilizationInfo","ActualUsed":32}`
	...
	expectedOp := []map[string]interface{}{{"label": "Label 1", "licensed_limit": "LicensedLimit 1", "sku": "Sku 1", "object_type": "iaas.LicenseUtilizationInfo", "class_id": "iaas.LicenseUtilizationInfo", "actual_used": 32}, {"label": "Label 2", "licensed_limit": "LicensedLimit 2", "sku": "Sku 2", "object_type": "iaas.LicenseUtilizationInfo", "class_id": "iaas.LicenseUtilizationInfo", "actual_used": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIppoolIpV6Block(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"To":"To %d","From":"From %d","ObjectType":"ippool.IpV6Block","Size":32,"ClassId":"ippool.IpV6Block"}`
	...
	expectedOp := []map[string]interface{}{{"to": "To 1", "from": "From 1", "object_type": "ippool.IpV6Block", "size": 32, "class_id": "ippool.IpV6Block"}, {"to": "To 2", "from": "From 2", "object_type": "ippool.IpV6Block", "size": 32, "class_id": "ippool.IpV6Block"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListIqnpoolIqnSuffixBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Suffix":"Suffix %d","ObjectType":"iqnpool.IqnSuffixBlock","Size":32,"ClassId":"iqnpool.IqnSuffixBlock","To":32,"From":32}`
	...
	expectedOp := []map[string]interface{}{{"suffix": "Suffix 1", "object_type": "iqnpool.IqnSuffixBlock", "size": 32, "class_id": "iqnpool.IqnSuffixBlock", "to": 32, "from": 32}, {"suffix": "Suffix 2", "object_type": "iqnpool.IqnSuffixBlock", "size": 32, "class_id": "iqnpool.IqnSuffixBlock", "to": 32, "from": 32}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesAddon(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.Addon","Name":"Name %d","ObjectType":"kubernetes.Addon"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "kubernetes.Addon", "name": "Name 1", "object_type": "kubernetes.Addon"}, {"class_id": "kubernetes.Addon", "name": "Name 2", "object_type": "kubernetes.Addon"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesNodeAddress(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Address":"Address %d","Type":"Type %d","ObjectType":"kubernetes.NodeAddress","ClassId":"kubernetes.NodeAddress"}`
	...
	expectedOp := []map[string]interface{}{{"address": "Address 1", "type": "Type 1", "object_type": "kubernetes.NodeAddress", "class_id": "kubernetes.NodeAddress"}, {"address": "Address 2", "type": "Type 2", "object_type": "kubernetes.NodeAddress", "class_id": "kubernetes.NodeAddress"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListKubernetesNodeGroupTaint(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Key":"Key %d","Value":"Value %d","ClassId":"kubernetes.NodeGroupTaint","ObjectType":"kubernetes.NodeGroupTaint","Effect":"Effect %d"}`
	...
	expectedOp := []map[string]interface{}{{"key": "Key 1", "value": "Value 1", "class_id": "kubernetes.NodeGroupTaint", "object_type": "kubernetes.NodeGroupTaint", "effect": "Effect 1"}, {"key": "Key 2", "value": "Value 2", "class_id": "kubernetes.NodeGroupTaint", "object_type": "kubernetes.NodeGroupTaint", "effect": "Effect 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMacpoolBlock(t *testing.T) {
	...
	expectedOp := []map[string]interface{}{{"from": "From 1", "class_id": "macpool.Block", "object_type": "macpool.Block", "size": 32, "to": "To 1"}, {"from": "From 2", "class_id": "macpool.Block", "object_type": "macpool.Block", "size": 32, "to": "To 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMemoryPersistentMemoryLogicalNamespace(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"memory.PersistentMemoryLogicalNamespace","Capacity":32,"Mode":"Mode %d","Name":"Name %d","SocketId":32,"SocketMemoryId":"SocketMemoryId %d","ObjectType":"memory.PersistentMemoryLogicalNamespace"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "memory.PersistentMemoryLogicalNamespace", "capacity": 32, "mode": "Mode 1", "name": "Name 1", "socket_id": 32, "socket_memory_id": "SocketMemoryId 1", "object_type": "memory.PersistentMemoryLogicalNamespace"}, {"class_id": "memory.PersistentMemoryLogicalNamespace", "capacity": 32, "mode": "Mode 2", "name": "Name 2", "socket_id": 32, "socket_memory_id": "SocketMemoryId 2", "object_type": "memory.PersistentMemoryLogicalNamespace"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaDisplayNameDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"meta.DisplayNameDefinition","ObjectType":"meta.DisplayNameDefinition","IncludeAncestor":true,"Name":"Name %d","Format":"Format %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "meta.DisplayNameDefinition", "object_type": "meta.DisplayNameDefinition", "include_ancestor": true, "name": "Name 1", "format": "Format 1"}, {"class_id": "meta.DisplayNameDefinition", "object_type": "meta.DisplayNameDefinition", "include_ancestor": true, "name": "Name 2", "format": "Format 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListMetaRelationshipDefinition(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"meta.RelationshipDefinition","ExportWithPeer":true,"Type":"Type %d","Export":true,"Name":"Name %d","ClassId":"meta.RelationshipDefinition","Collection":true,"ApiAccess":"ApiAccess %d","PeerSync":true,"PeerRelName":"PeerRelName %d"}`
...
func TestFlattenListNiaapiDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niaapi.Detail","ClassId":"niaapi.Detail","Filename":"Filename %d","Name":"Name %d","Chksum":"Chksum %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "niaapi.Detail", "class_id": "niaapi.Detail", "filename": "Filename 1", "name": "Name 1", "chksum": "Chksum 1"}, {"object_type": "niaapi.Detail", "class_id": "niaapi.Detail", "filename": "Filename 2", "name": "Name 2", "chksum": "Chksum 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryInterfaceElement(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"XcvrPresent":"XcvrPresent %d","ObjectType":"niatelemetry.InterfaceElement","ClassId":"niatelemetry.InterfaceElement","Name":"Name %d","OperState":"OperState %d"}`
	...
	expectedOp := []map[string]interface{}{{"xcvr_present": "XcvrPresent 1", "object_type": "niatelemetry.InterfaceElement", "class_id": "niatelemetry.InterfaceElement", "name": "Name 1", "oper_state": "OperState 1"}, {"xcvr_present": "XcvrPresent 2", "object_type": "niatelemetry.InterfaceElement", "class_id": "niatelemetry.InterfaceElement", "name": "Name 2", "oper_state": "OperState 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListNiatelemetryLogicalLink(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.LogicalLink","Uptime":"Uptime %d","LinkState":"LinkState %d","ObjectType":"niatelemetry.LogicalLink","DbId":32,"LinkAddr2":"LinkAddr2 %d","LinkType":"LinkType %d","IsPresent":true,"LinkAddr1":"LinkAddr1 %d"}`
...
func TestFlattenListOnpremImagePackage(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"FileSha":"FileSha %d","PackageType":"PackageType %d","ObjectType":"onprem.ImagePackage","Filename":"Filename %d","Version":"Version %d","Name":"Name %d","FileSize":32,"ClassId":"onprem.ImagePackage","FilePath":"FilePath %d"}`
	...
	expectedOp := []map[string]interface{}{{"file_sha": "FileSha 1", "package_type": "PackageType 1", "object_type": "onprem.ImagePackage", "filename": "Filename 1", "nr_version": "Version 1", "name": "Name 1", "file_size": 32, "class_id": "onprem.ImagePackage", "file_path": "FilePath 1"}, {"file_sha": "FileSha 2", "package_type": "PackageType 2", "object_type": "onprem.ImagePackage", "filename": "Filename 2", "nr_version": "Version 2", "name": "Name 2", "file_size": 32, "class_id": "onprem.ImagePackage", "file_path": "FilePath 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListOnpremUpgradePhase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"onprem.UpgradePhase","Failed":true,"Name":"Name %d","ClassId":"onprem.UpgradePhase","ElapsedTime":32,"Message":"Message %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "onprem.UpgradePhase", "failed": true, "name": "Name 1", "class_id": "onprem.UpgradePhase", "elapsed_time": 32, "message": "Message 1"}, {"object_type": "onprem.UpgradePhase", "failed": true, "name": "Name 2", "class_id": "onprem.UpgradePhase", "elapsed_time": 32, "message": "Message 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListPolicyinventoryJobInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PolicyName":"PolicyName %d","ClassId":"policyinventory.JobInfo","ObjectType":"policyinventory.JobInfo","ExecutionStatus":"ExecutionStatus %d","PolicyId":"PolicyId %d"}`
	...
	expectedOp := []map[string]interface{}{{"policy_name": "PolicyName 1", "class_id": "policyinventory.JobInfo", "object_type": "policyinventory.JobInfo", "execution_status": "ExecutionStatus 1", "policy_id": "PolicyId 1"}, {"policy_name": "PolicyName 2", "class_id": "policyinventory.JobInfo", "object_type": "policyinventory.JobInfo", "execution_status": "ExecutionStatus 2", "policy_id": "PolicyId 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSdwanNetworkConfigurationType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"sdwan.NetworkConfigurationType","ClassId":"sdwan.NetworkConfigurationType","PortGroup":"PortGroup %d","Vlan":32,"NetworkType":"NetworkType %d"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "sdwan.NetworkConfigurationType", "class_id": "sdwan.NetworkConfigurationType", "port_group": "PortGroup 1", "vlan": 32, "network_type": "NetworkType 1"}, {"object_type": "sdwan.NetworkConfigurationType", "class_id": "sdwan.NetworkConfigurationType", "port_group": "PortGroup 2", "vlan": 32, "network_type": "NetworkType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSdwanTemplateInputsType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","ObjectType":"sdwan.TemplateInputsType","Template":"Template %d","Editable":true,"Required":true,"ClassId":"sdwan.TemplateInputsType","Title":"Title %d","Value":"Value %d","Key":"Key %d"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "object_type": "sdwan.TemplateInputsType", "template": "Template 1", "editable": true, "required": true, "class_id": "sdwan.TemplateInputsType", "title": "Title 1", "value": "Value 1", "key": "Key 1"}, {"type": "Type 2", "object_type": "sdwan.TemplateInputsType", "template": "Template 2", "editable": true, "required": true, "class_id": "sdwan.TemplateInputsType", "title": "Title 2", "value": "Value 2", "key": "Key 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpTrap(t *testing.T) {
	p := []models.SnmpTrap{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"snmp.Trap","Enabled":true,"User":"User %d","Version":"Version %d","Port":32,"ObjectType":"snmp.Trap","Community":"Community %d","Destination":"Destination %d","Type":"Type %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "snmp.Trap", "enabled": true, "user": "User 1", "nr_version": "Version 1", "port": 32, "object_type": "snmp.Trap", "community": "Community 1", "destination": "Destination 1", "type": "Type 1"}, {"class_id": "snmp.Trap", "enabled": true, "user": "User 2", "nr_version": "Version 2", "port": 32, "object_type": "snmp.Trap", "community": "Community 2", "destination": "Destination 2", "type": "Type 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSnmpUser(t *testing.T) {
	p := []models.SnmpUser{}
	var d = &schema.ResourceData{}
	c := `{"PrivacyType":"PrivacyType %d","Name":"Name %d","SecurityLevel":"SecurityLevel %d","IsPrivacyPasswordSet":true,"ObjectType":"snmp.User","ClassId":"snmp.User","IsAuthPasswordSet":true,"AuthType":"AuthType %d"}`
	...
	expectedOp := []map[string]interface{}{{"privacy_type": "PrivacyType 1", "name": "Name 1", "security_level": "SecurityLevel 1", "is_privacy_password_set": true, "object_type": "snmp.User", "class_id": "snmp.User", "is_auth_password_set": true, "auth_type": "AuthType 1"}, {"privacy_type": "PrivacyType 2", "name": "Name 2", "security_level": "SecurityLevel 2", "is_privacy_password_set": true, "object_type": "snmp.User", "class_id": "snmp.User", "is_auth_password_set": true, "auth_type": "AuthType 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSoftwarerepositoryConstraintModels(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"softwarerepository.ConstraintModels","ClassId":"softwarerepository.ConstraintModels","MinVersion":"MinVersion %d","Name":"Name %d","PlatformRegex":"PlatformRegex %d"}`
...
func TestFlattenListStorageBaseInitiator(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","Wwn":"Wwn %d","ObjectType":"storage.BaseInitiator","ClassId":"storage.BaseInitiator","Iqn":"Iqn %d","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "wwn": "Wwn 1", "object_type": "storage.BaseInitiator", "class_id": "storage.BaseInitiator", "iqn": "Iqn 1", "name": "Name 1"}, {"type": "Type 2", "wwn": "Wwn 2", "object_type": "storage.BaseInitiator", "class_id": "storage.BaseInitiator", "iqn": "Iqn 2", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListSyslogRemoteClientBase(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"syslog.RemoteClientBase","Enabled":true,"Hostname":"Hostname %d","MinSeverity":"MinSeverity %d","Port":32,"Protocol":"Protocol %d","ClassId":"syslog.RemoteClientBase"}`
...
func TestFlattenListTamAction(t *testing.T) {
	p := []models.TamAction{}
	var d = &schema.ResourceData{}
	c := `{"AlertType":"AlertType %d","AffectedObjectType":"AffectedObjectType %d","Name":"Name %d","OperationType":"OperationType %d","ClassId":"tam.Action","ObjectType":"tam.Action","Type":"Type %d"}`
...
func TestFlattenListTamS3DataSource(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Type":"Type %d","S3Path":"S3Path %d","ClassId":"tam.S3DataSource","ObjectType":"tam.S3DataSource","Name":"Name %d"}`
	...
	expectedOp := []map[string]interface{}{{"type": "Type 1", "s3_path": "S3Path 1", "class_id": "tam.S3DataSource", "object_type": "tam.S3DataSource", "name": "Name 1"}, {"type": "Type 2", "s3_path": "S3Path 2", "class_id": "tam.S3DataSource", "object_type": "tam.S3DataSource", "name": "Name 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUcsdConnectorPack(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","State":"State %d","ClassId":"ucsd.ConnectorPack","ConnectorFeature":"ConnectorFeature %d","DownloadedVersion":"DownloadedVersion %d","ObjectType":"ucsd.ConnectorPack","Version":"Version %d"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "state": "State 1", "class_id": "ucsd.ConnectorPack", "connector_feature": "ConnectorFeature 1", "downloaded_version": "DownloadedVersion 1", "object_type": "ucsd.ConnectorPack", "nr_version": "Version 1"}, {"name": "Name 2", "state": "State 2", "class_id": "ucsd.ConnectorPack", "connector_feature": "ConnectorFeature 2", "downloaded_version": "DownloadedVersion 2", "object_type": "ucsd.ConnectorPack", "nr_version": "Version 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListUuidpoolUuidBlock(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"uuidpool.UuidBlock","To":"To %d","From":"From %d","Size":32,"ClassId":"uuidpool.UuidBlock"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "uuidpool.UuidBlock", "to": "To 1", "from": "From 1", "size": 32, "class_id": "uuidpool.UuidBlock"}, {"object_type": "uuidpool.UuidBlock", "to": "To 2", "from": "From 2", "size": 32, "class_id": "uuidpool.UuidBlock"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationNetworkInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","ClassId":"virtualization.NetworkInterface","AdaptorType":"AdaptorType %d","Bridge":"Bridge %d","ConnectAtPowerOn":true,"DirectPathIo":true,"MacAddress":"MacAddress %d","ObjectType":"virtualization.NetworkInterface"}`
	...
	expectedOp := []map[string]interface{}{{"name": "Name 1", "class_id": "virtualization.NetworkInterface", "adaptor_type": "AdaptorType 1", "bridge": "Bridge 1", "connect_at_power_on": true, "direct_path_io": true, "mac_address": "MacAddress 1", "object_type": "virtualization.NetworkInterface"}, {"name": "Name 2", "class_id": "virtualization.NetworkInterface", "adaptor_type": "AdaptorType 2", "bridge": "Bridge 2", "connect_at_power_on": true, "direct_path_io": true, "mac_address": "MacAddress 2", "object_type": "virtualization.NetworkInterface"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVirtualizationVirtualMachineDisk(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Order":32,"Type":"Type %d","ClassId":"virtualization.VirtualMachineDisk","ObjectType":"virtualization.VirtualMachineDisk","Bus":"Bus %d","Name":"Name %d","VirtualDiskReference":"VirtualDiskReference %d"}`
	...
	expectedOp := []map[string]interface{}{{"order": 32, "type": "Type 1", "class_id": "virtualization.VirtualMachineDisk", "object_type": "virtualization.VirtualMachineDisk", "bus": "Bus 1", "name": "Name 1", "virtual_disk_reference": "VirtualDiskReference 1"}, {"order": 32, "type": "Type 2", "class_id": "virtualization.VirtualMachineDisk", "object_type": "virtualization.VirtualMachineDisk", "bus": "Bus 2", "name": "Name 2", "virtual_disk_reference": "VirtualDiskReference 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListVmediaMapping(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vmedia.Mapping","FileLocation":"FileLocation %d","AuthenticationProtocol":"AuthenticationProtocol %d","VolumeName":"VolumeName %d","SanitizedFileLocation":"SanitizedFileLocation %d","RemoteFile":"RemoteFile %d","MountOptions":"MountOptions %d","MountProtocol":"MountProtocol %d","HostName":"HostName %d","IsPasswordSet":true,"ObjectType":"vmedia.Mapping","Username":"Username %d","DeviceType":"DeviceType %d","RemotePath":"RemotePath %d"}`
...
func TestFlattenListWorkflowApi(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Body":"Body %d","SkipOnCondition":"SkipOnCondition %d","Label":"Label %d","ErrorContentType":"ErrorContentType %d","StartDelay":32,"ClassId":"workflow.Api","ContentType":"ContentType %d","Description":"Description %d","Name":"Name %d","Timeout":32,"ObjectType":"workflow.Api","AssetTargetMoid":"AssetTargetMoid %d"}`
...
func TestFlattenListWorkflowBaseDataType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.BaseDataType","ObjectType":"workflow.BaseDataType","Label":"Label %d","Required":true,"Name":"Name %d","Description":"Description %d"}`
	...
	expectedOp := []map[string]interface{}{{"class_id": "workflow.BaseDataType", "object_type": "workflow.BaseDataType", "label": "Label 1", "required": true, "name": "Name 1", "description": "Description 1"}, {"class_id": "workflow.BaseDataType", "object_type": "workflow.BaseDataType", "label": "Label 2", "required": true, "name": "Name 2", "description": "Description 2"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListWorkflowParameterSet(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.ParameterSet","ObjectType":"workflow.ParameterSet","Name":"Name %d","Value":"Value %d","Condition":"Condition %d","ControlParameter":"ControlParameter %d"}`
...
func TestFlattenListWorkflowRollbackTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"workflow.RollbackTask","Description":"Description %d","Name":"Name %d","Version":32,"ClassId":"workflow.RollbackTask","CatalogMoid":"CatalogMoid %d","TaskMoid":"TaskMoid %d"}`
...
func TestFlattenListWorkflowRollbackWorkflowTask(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"workflow.RollbackWorkflowTask","RollbackCompleted":true,"TaskInfoMoid":"TaskInfoMoid %d","Name":"Name %d","ObjectType":"workflow.RollbackWorkflowTask","RefName":"RefName %d","Status":"Status %d","TaskPath":"TaskPath %d","Description":"Description %d","RollbackTaskName":"RollbackTaskName %d"}`
...
func TestFlattenListWorkflowUiInputFilter(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"workflow.UiInputFilter","Name":"Name %d","UserHelpMessage":"UserHelpMessage %d","ClassId":"workflow.UiInputFilter"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "workflow.UiInputFilter", "name": "Name 1", "user_help_message": "UserHelpMessage 1", "class_id": "workflow.UiInputFilter"}, {"object_type": "workflow.UiInputFilter", "name": "Name 2", "user_help_message": "UserHelpMessage 2", "class_id": "workflow.UiInputFilter"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenListX509Certificate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"x509.Certificate","Sha256Fingerprint":"Sha256Fingerprint %d","PemCertificate":"PemCertificate %d","SignatureAlgorithm":"SignatureAlgorithm %d","ClassId":"x509.Certificate"}`
	...
	expectedOp := []map[string]interface{}{{"object_type": "x509.Certificate", "sha256_fingerprint": "Sha256Fingerprint 1", "pem_certificate": "PemCertificate 1", "signature_algorithm": "SignatureAlgorithm 1", "class_id": "x509.Certificate"}, {"object_type": "x509.Certificate", "sha256_fingerprint": "Sha256Fingerprint 2", "pem_certificate": "PemCertificate 2", "signature_algorithm": "SignatureAlgorithm 2", "class_id": "x509.Certificate"}}
	for i := 0; i < len(expectedOp); i++ {
		err := compareMaps(expectedOp[i], ffOp[i], t)
		CheckError(t, err)
	}
...
func TestFlattenMapApplianceCertRenewalPhase(t *testing.T) {
	p := models.ApplianceCertRenewalPhase{}
	var d = &schema.ResourceData{}
	c := `{"Failed":true,"Message":"Message %d","ClassId":"appliance.CertRenewalPhase","ObjectType":"appliance.CertRenewalPhase","Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"failed": true, "message": "Message 1", "class_id": "appliance.CertRenewalPhase", "object_type": "appliance.CertRenewalPhase", "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetContractInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ContractNumber":"ContractNumber %d","ObjectType":"asset.ContractInformation","ClassId":"asset.ContractInformation","LineStatus":"LineStatus %d"}`
	...
	expectedOp := map[string]interface{}{"contract_number": "ContractNumber 1", "object_type": "asset.ContractInformation", "class_id": "asset.ContractInformation", "line_status": "LineStatus 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeploymentDeviceInformation(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"object_type": "asset.DeploymentDeviceInformation", "mlb_product_id": 32, "application_name": "ApplicationName 1", "description": "Description 1", "old_device_id": "OldDeviceId 1", "instance_id": "InstanceId 1", "item_type": "ItemType 1", "mlb_product_name": "MlbProductName 1", "old_instance_id": "OldInstanceId 1", "old_device_status_description": "OldDeviceStatusDescription 1", "class_id": "asset.DeploymentDeviceInformation", "old_device_status_id": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceInformation(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"product_type": "ProductType 1", "mlb_product_id": 32, "old_device_status_description": "OldDeviceStatusDescription 1", "class_id": "asset.DeviceInformation", "instance_id": "InstanceId 1", "unit_of_measure": "UnitOfMeasure 1", "old_instance_id": "OldInstanceId 1", "old_device_id": "OldDeviceId 1", "product_family": "ProductFamily 1", "old_device_status_id": 32, "application_name": "ApplicationName 1", "item_type": "ItemType 1", "mlb_offer_type": "MlbOfferType 1", "mlb_product_name": "MlbProductName 1", "object_type": "asset.DeviceInformation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapAssetDeviceStatistics(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Connected":32,"MembershipRatio":32.000000,"MemoryMirroringFactor":32.000000,"ObjectType":"asset.DeviceStatistics","ClassId":"asset.DeviceStatistics","ClusterName":"ClusterName %d"}`
	...
	expectedOp := map[string]interface{}{"connected": 32, "membership_ratio": 32.000000, "memory_mirroring_factor": 32.000000, "object_type": "asset.DeviceStatistics", "class_id": "asset.DeviceStatistics", "cluster_name": "ClusterName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapAssetGlobalUltimate(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"asset.GlobalUltimate","ObjectType":"asset.GlobalUltimate","Name":"Name %d","Id":"Id %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "asset.GlobalUltimate", "object_type": "asset.GlobalUltimate", "name": "Name 1", "id": "Id 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapAssetProductInformation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"asset.ProductInformation","ClassId":"asset.ProductInformation","Number":"Number %d","Description":"Description %d","Family":"Family %d","Group":"Group %d","SubType":"SubType %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "asset.ProductInformation", "class_id": "asset.ProductInformation", "number": "Number 1", "description": "Description 1", "family": "Family 1", "group": "Group 1", "sub_type": "SubType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCapabilitySwitchNetworkLimits(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"max_uncompressed_port_vlan_count": 32, "maximum_port_channel_members": 32, "maximum_active_traffic_monitoring_sessions": 32, "maximum_fc_port_channels": 32, "maximum_secondary_vlan_per_primary": 32, "maximum_ethernet_port_channels": 32, "maximum_fc_port_channel_members": 32, "minimum_active_fans": 32, "maximum_ethernet_uplink_ports": 32, "maximum_primary_vlan": 32, "class_id": "capability.SwitchNetworkLimits", "maximum_secondary_vlan": 32, "maximum_vlans": 32, "max_compressed_port_vlan_count": 32, "object_type": "capability.SwitchNetworkLimits", "maximum_igmp_groups": 32, "maximum_vifs": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCapabilitySwitchStorageLimits(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"MaximumVirtualFcInterfacesPerBladeServer":32,"ClassId":"capability.SwitchStorageLimits","ObjectType":"capability.SwitchStorageLimits","MaximumVsans":32,"MaximumZoneCount":32,"MaximumUserZoneCount":32,"MaximumVirtualFcInterfaces":32}`
	...
	expectedOp := map[string]interface{}{"maximum_virtual_fc_interfaces_per_blade_server": 32, "class_id": "capability.SwitchStorageLimits", "object_type": "capability.SwitchStorageLimits", "maximum_vsans": 32, "maximum_zone_count": 32, "maximum_user_zone_count": 32, "maximum_virtual_fc_interfaces": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudInstanceType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CpuSpeed":32,"ClassId":"cloud.InstanceType","ObjectType":"cloud.InstanceType","InstanceTypeId":"InstanceTypeId %d","Platform":"Platform %d","Cpus":32,"Name":"Name %d","Memory":32,"Architecture":"Architecture %d"}`
	...
	expectedOp := map[string]interface{}{"cpu_speed": 32, "class_id": "cloud.InstanceType", "object_type": "cloud.InstanceType", "instance_type_id": "InstanceTypeId 1", "platform": "Platform 1", "cpus": 32, "name": "Name 1", "memory": 32, "architecture": "Architecture 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCloudVolumeIopsInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ThroughputWriteLimit":32,"IopsReadLimit":32,"IopsWriteLimit":32,"ThroughputReadLimit":32,"ClassId":"cloud.VolumeIopsInfo","ObjectType":"cloud.VolumeIopsInfo"}`
	...
	expectedOp := map[string]interface{}{"throughput_write_limit": 32, "iops_read_limit": 32, "iops_write_limit": 32, "throughput_read_limit": 32, "class_id": "cloud.VolumeIopsInfo", "object_type": "cloud.VolumeIopsInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCloudVolumeType(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"cloud.VolumeType","Name":"Name %d","VolumeTypeId":"VolumeTypeId %d","ClassId":"cloud.VolumeType"}`
	...
	expectedOp := map[string]interface{}{"object_type": "cloud.VolumeType", "name": "Name 1", "volume_type_id": "VolumeTypeId 1", "class_id": "cloud.VolumeType"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapCommIpV4Interface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Gateway":"Gateway %d","IpAddress":"IpAddress %d","Netmask":"Netmask %d","ObjectType":"comm.IpV4Interface","ClassId":"comm.IpV4Interface"}`
	...
	expectedOp := map[string]interface{}{"gateway": "Gateway 1", "ip_address": "IpAddress 1", "netmask": "Netmask 1", "object_type": "comm.IpV4Interface", "class_id": "comm.IpV4Interface"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapCommIpV6Interface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Prefix":"Prefix %d","Gateway":"Gateway %d","ObjectType":"comm.IpV6Interface","ClassId":"comm.IpV6Interface","IpAddress":"IpAddress %d"}`
	...
	expectedOp := map[string]interface{}{"prefix": "Prefix 1", "gateway": "Gateway 1", "object_type": "comm.IpV6Interface", "class_id": "comm.IpV6Interface", "ip_address": "IpAddress 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapComputeStoragePhysicalDriveOperation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"AdminAction":"AdminAction %d","ClassId":"compute.StoragePhysicalDriveOperation","ObjectType":"compute.StoragePhysicalDriveOperation","ControllerId":"ControllerId %d"}`
	...
	expectedOp := map[string]interface{}{"admin_action": "AdminAction 1", "class_id": "compute.StoragePhysicalDriveOperation", "object_type": "compute.StoragePhysicalDriveOperation", "controller_id": "ControllerId 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareDirectDownload(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IsPasswordSet":true,"ObjectType":"firmware.DirectDownload","Upgradeoption":"Upgradeoption %d","Username":"Username %d","ImageSource":"ImageSource %d","ClassId":"firmware.DirectDownload"}`
	...
	expectedOp := map[string]interface{}{"is_password_set": true, "object_type": "firmware.DirectDownload", "upgradeoption": "Upgradeoption 1", "username": "Username 1", "image_source": "ImageSource 1", "class_id": "firmware.DirectDownload"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapFirmwareNetworkShare(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"firmware.NetworkShare","MapType":"MapType %d","ClassId":"firmware.NetworkShare","Username":"Username %d","IsPasswordSet":true,"Upgradeoption":"Upgradeoption %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "firmware.NetworkShare", "map_type": "MapType 1", "class_id": "firmware.NetworkShare", "username": "Username 1", "is_password_set": true, "upgradeoption": "Upgradeoption 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexAlarmSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Critical":32,"ObjectType":"hyperflex.AlarmSummary","ClassId":"hyperflex.AlarmSummary","Warning":32}`
	...
	expectedOp := map[string]interface{}{"critical": 32, "object_type": "hyperflex.AlarmSummary", "class_id": "hyperflex.AlarmSummary", "warning": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexBondState(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Mode":"Mode %d","ActiveSlave":"ActiveSlave %d","ObjectType":"hyperflex.BondState","ClassId":"hyperflex.BondState"}`
	...
	expectedOp := map[string]interface{}{"mode": "Mode 1", "active_slave": "ActiveSlave 1", "object_type": "hyperflex.BondState", "class_id": "hyperflex.BondState"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexDiskStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.DiskStatus","DownloadPercentage":"DownloadPercentage %d","State":"State %d","VolumeHandle":"VolumeHandle %d","VolumeName":"VolumeName %d","ObjectType":"hyperflex.DiskStatus"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.DiskStatus", "download_percentage": "DownloadPercentage 1", "state": "State 1", "volume_handle": "VolumeHandle 1", "volume_name": "VolumeName 1", "object_type": "hyperflex.DiskStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHealthCheckScriptInfo(t *testing.T) {
	p := models.HyperflexHealthCheckScriptInfo{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"hyperflex.HealthCheckScriptInfo","AggregateScriptName":"AggregateScriptName %d","Version":"Version %d","ClassId":"hyperflex.HealthCheckScriptInfo","HyperflexVersion":"HyperflexVersion %d","ScriptExecuteLocation":"ScriptExecuteLocation %d","ScriptInput":"ScriptInput %d","ScriptName":"ScriptName %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "hyperflex.HealthCheckScriptInfo", "aggregate_script_name": "AggregateScriptName 1", "nr_version": "Version 1", "class_id": "hyperflex.HealthCheckScriptInfo", "hyperflex_version": "HyperflexVersion 1", "script_execute_location": "ScriptExecuteLocation 1", "script_input": "ScriptInput 1", "script_name": "ScriptName 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexHxPlatformDatastoreConfigDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Name":"Name %d","ProvisionedCapacity":32,"SystemDatastore":true,"ClassId":"hyperflex.HxPlatformDatastoreConfigDt","ObjectType":"hyperflex.HxPlatformDatastoreConfigDt","NumMirrors":32,"NumStripesForLargeFiles":32,"DataBlockSize":32}`
	...
	expectedOp := map[string]interface{}{"name": "Name 1", "provisioned_capacity": 32, "system_datastore": true, "class_id": "hyperflex.HxPlatformDatastoreConfigDt", "object_type": "hyperflex.HxPlatformDatastoreConfigDt", "num_mirrors": 32, "num_stripes_for_large_files": 32, "data_block_size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxRegistrationDetailsDt(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.HxRegistrationDetailsDt", "object_type": "hyperflex.HxRegistrationDetailsDt", "registration_failed_reason": "RegistrationFailedReason 1", "smart_account": "SmartAccount 1", "status": "Status 1", "virtual_account": "VirtualAccount 1", "registration_expire_date": "RegistrationExpireDate 1", "next_renewal_attempt_date": "NextRenewalAttemptDate 1", "out_of_compliance_start_date": "OutOfComplianceStartDate 1", "initial_registration_date": "InitialRegistrationDate 1", "last_renewal_attempt_date": "LastRenewalAttemptDate 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapHyperflexHxResiliencyInfoDt(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PolicyCompliance":"PolicyCompliance %d","DataReplicationFactor":"DataReplicationFactor %d","SsdFailuresTolerable":32,"ClassId":"hyperflex.HxResiliencyInfoDt","ObjectType":"hyperflex.HxResiliencyInfoDt","NodeFailuresTolerable":32,"HddFailuresTolerable":32,"ResiliencyState":"ResiliencyState %d"}`
	...
	expectedOp := map[string]interface{}{"policy_compliance": "PolicyCompliance 1", "data_replication_factor": "DataReplicationFactor 1", "ssd_failures_tolerable": 32, "class_id": "hyperflex.HxResiliencyInfoDt", "object_type": "hyperflex.HxResiliencyInfoDt", "node_failures_tolerable": 32, "hdd_failures_tolerable": 32, "resiliency_state": "ResiliencyState 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexReplicationPeerInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"hyperflex.ReplicationPeerInfo","ReplCip":"ReplCip %d","Status":"Status %d","Mcip":"Mcip %d","ObjectType":"hyperflex.ReplicationPeerInfo","StatusDetails":"StatusDetails %d","Dcip":"Dcip %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "hyperflex.ReplicationPeerInfo", "repl_cip": "ReplCip 1", "status": "Status 1", "mcip": "Mcip 1", "object_type": "hyperflex.ReplicationPeerInfo", "status_details": "StatusDetails 1", "dcip": "Dcip 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapHyperflexSummary(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SpaceStatus":"SpaceStatus %d","TotalCapacity":32,"Address":"Address %d","TotalSavings":32.000000,"Name":"Name %d","State":"State %d","Downtime":"Downtime %d","DeduplicationSavings":32.000000,"DataReplicationFactor":"DataReplicationFactor %d","ResiliencyDetailsSize":32,"ClassId":"hyperflex.Summary","ObjectType":"hyperflex.Summary","ActiveNodes":"ActiveNodes %d","FreeCapacity":32,"ClusterAccessPolicy":"ClusterAccessPolicy %d","DataReplicationCompliance":"DataReplicationCompliance %d","CompressionSavings":32.000000,"Boottime":32,"Uptime":"Uptime %d","UsedCapacity":32}`
	...
	expectedOp := map[string]interface{}{"space_status": "SpaceStatus 1", "total_capacity": 32, "address": "Address 1", "total_savings": 32.000000, "name": "Name 1", "state": "State 1", "downtime": "Downtime 1", "deduplication_savings": 32.000000, "data_replication_factor": "DataReplicationFactor 1", "resiliency_details_size": 32, "class_id": "hyperflex.Summary", "object_type": "hyperflex.Summary", "active_nodes": "ActiveNodes 1", "free_capacity": 32, "cluster_access_policy": "ClusterAccessPolicy 1", "data_replication_compliance": "DataReplicationCompliance 1", "compression_savings": 32.000000, "boottime": 32, "uptime": "Uptime 1", "used_capacity": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamEndPointPasswordProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NotificationPeriod":32,"PasswordHistory":32,"PasswordExpiryDuration":32,"EnforceStrongPassword":true,"ForceSendPassword":true,"GracePeriod":32,"ObjectType":"iam.EndPointPasswordProperties","ClassId":"iam.EndPointPasswordProperties","EnablePasswordExpiry":true}`
	...
	expectedOp := map[string]interface{}{"notification_period": 32, "password_history": 32, "password_expiry_duration": 32, "enforce_strong_password": true, "force_send_password": true, "grace_period": 32, "object_type": "iam.EndPointPasswordProperties", "class_id": "iam.EndPointPasswordProperties", "enable_password_expiry": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIamLdapBaseProperties(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"object_type": "iam.LdapBaseProperties", "bind_method": "BindMethod 1", "enable_encryption": true, "base_dn": "BaseDn 1", "enable_group_authorization": true, "domain": "Domain 1", "class_id": "iam.LdapBaseProperties", "is_password_set": true, "attribute": "Attribute 1", "filter": "Filter 1", "nested_group_search_depth": 32, "group_attribute": "GroupAttribute 1", "bind_dn": "BindDn 1", "timeout": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapInfraHardwareInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CpuCores":32,"CpuSpeed":32,"MemorySize":32,"ClassId":"infra.HardwareInfo","ObjectType":"infra.HardwareInfo"}`
	...
	expectedOp := map[string]interface{}{"cpu_cores": 32, "cpu_speed": 32, "memory_size": 32, "class_id": "infra.HardwareInfo", "object_type": "infra.HardwareInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIppoolIpV4Config(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Gateway":"Gateway %d","Netmask":"Netmask %d","PrimaryDns":"PrimaryDns %d","SecondaryDns":"SecondaryDns %d","ClassId":"ippool.IpV4Config","ObjectType":"ippool.IpV4Config"}`
	...
	expectedOp := map[string]interface{}{"gateway": "Gateway 1", "netmask": "Netmask 1", "primary_dns": "PrimaryDns 1", "secondary_dns": "SecondaryDns 1", "class_id": "ippool.IpV4Config", "object_type": "ippool.IpV4Config"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapIppoolIpV6Block(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"to": "To 1", "from": "From 1", "object_type": "ippool.IpV6Block", "size": 32, "class_id": "ippool.IpV6Block"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapIppoolIpV6Config(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"ippool.IpV6Config","Gateway":"Gateway %d","Prefix":32,"PrimaryDns":"PrimaryDns %d","SecondaryDns":"SecondaryDns %d","ObjectType":"ippool.IpV6Config"}`
	...
	expectedOp := map[string]interface{}{"class_id": "ippool.IpV6Config", "gateway": "Gateway 1", "prefix": 32, "primary_dns": "PrimaryDns 1", "secondary_dns": "SecondaryDns 1", "object_type": "ippool.IpV6Config"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapIqnpoolIqnSuffixBlock(t *testing.T) {
	p := models.IqnpoolIqnSuffixBlock{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"iqnpool.IqnSuffixBlock","To":32,"From":32,"Suffix":"Suffix %d","ObjectType":"iqnpool.IqnSuffixBlock","Size":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "iqnpool.IqnSuffixBlock", "to": 32, "from": 32, "suffix": "Suffix 1", "object_type": "iqnpool.IqnSuffixBlock", "size": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesAddonConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Overrides":"Overrides %d","UpgradeStrategy":"UpgradeStrategy %d","ClassId":"kubernetes.AddonConfiguration","ObjectType":"kubernetes.AddonConfiguration","InstallStrategy":"InstallStrategy %d","ReleaseName":"ReleaseName %d","ReleaseNamespace":"ReleaseNamespace %d"}`
	...
	expectedOp := map[string]interface{}{"overrides": "Overrides 1", "upgrade_strategy": "UpgradeStrategy 1", "class_id": "kubernetes.AddonConfiguration", "object_type": "kubernetes.AddonConfiguration", "install_strategy": "InstallStrategy 1", "release_name": "ReleaseName 1", "release_namespace": "ReleaseNamespace 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesClusterCertificateConfiguration(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"kubernetes.ClusterCertificateConfiguration","SaPublicKey":"SaPublicKey %d","FrontProxyCert":"FrontProxyCert %d","ObjectType":"kubernetes.ClusterCertificateConfiguration","EtcdCert":"EtcdCert %d","CaCert":"CaCert %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "kubernetes.ClusterCertificateConfiguration", "sa_public_key": "SaPublicKey 1", "front_proxy_cert": "FrontProxyCert 1", "object_type": "kubernetes.ClusterCertificateConfiguration", "etcd_cert": "EtcdCert 1", "ca_cert": "CaCert 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesDaemonSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NumberAvailable":"NumberAvailable %d","UpdatedNumberScheduled":"UpdatedNumberScheduled %d","CurrentNumberScheduled":32,"ClassId":"kubernetes.DaemonSetStatus","ObjectType":"kubernetes.DaemonSetStatus","ObservedGeneration":32,"DesiredNumberScheduled":32,"NumberReady":32,"NumberMisscheduled":32}`
	...
	expectedOp := map[string]interface{}{"number_available": "NumberAvailable 1", "updated_number_scheduled": "UpdatedNumberScheduled 1", "current_number_scheduled": 32, "class_id": "kubernetes.DaemonSetStatus", "object_type": "kubernetes.DaemonSetStatus", "observed_generation": 32, "desired_number_scheduled": 32, "number_ready": 32, "number_misscheduled": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesNodeInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Architecture":"Architecture %d","MachineId":"MachineId %d","SystemUuid":"SystemUuid %d","BootId":"BootId %d","KernelVersion":"KernelVersion %d","ContainerRuntimeVersion":"ContainerRuntimeVersion %d","KubeProxyVersion":"KubeProxyVersion %d","KubeletVersion":"KubeletVersion %d","ObjectType":"kubernetes.NodeInfo","ClassId":"kubernetes.NodeInfo","OperatingSystem":"OperatingSystem %d","OsImage":"OsImage %d"}`
...
func TestFlattenMapKubernetesObjectMeta(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ResourceVersion":"ResourceVersion %d","Uuid":"Uuid %d","ClassId":"kubernetes.ObjectMeta","ObjectType":"kubernetes.ObjectMeta","CreationTimestamp":"CreationTimestamp %d","Name":"Name %d","Namespace":"Namespace %d"}`
	...
	expectedOp := map[string]interface{}{"resource_version": "ResourceVersion 1", "uuid": "Uuid 1", "class_id": "kubernetes.ObjectMeta", "object_type": "kubernetes.ObjectMeta", "creation_timestamp": "CreationTimestamp 1", "name": "Name 1", "namespace": "Namespace 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesPodStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"kubernetes.PodStatus","QosClass":"QosClass %d","StartTime":"StartTime %d","HostIp":"HostIp %d","Phase":"Phase %d","PodIp":"PodIp %d","ClassId":"kubernetes.PodStatus"}`
	...
	expectedOp := map[string]interface{}{"object_type": "kubernetes.PodStatus", "qos_class": "QosClass 1", "start_time": "StartTime 1", "host_ip": "HostIp 1", "phase": "Phase 1", "pod_ip": "PodIp 1", "class_id": "kubernetes.PodStatus"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapKubernetesProxyConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Protocol":"Protocol %d","ClassId":"kubernetes.ProxyConfig","ObjectType":"kubernetes.ProxyConfig","Hostname":"Hostname %d","Port":32,"Username":"Username %d","IsPasswordSet":true}`
	...
	expectedOp := map[string]interface{}{"protocol": "Protocol 1", "class_id": "kubernetes.ProxyConfig", "object_type": "kubernetes.ProxyConfig", "hostname": "Hostname 1", "port": 32, "username": "Username 1", "is_password_set": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapKubernetesStatefulSetStatus(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UpdateRevision":"UpdateRevision %d","ClassId":"kubernetes.StatefulSetStatus","ObjectType":"kubernetes.StatefulSetStatus","CurrentRevision":"CurrentRevision %d","UpdatedReplicas":32,"ObservedGeneration":32,"CollisionCount":32,"ReadyReplicas":32,"Replicas":32,"AvailableReplicas":32}`
	...
	expectedOp := map[string]interface{}{"update_revision": "UpdateRevision 1", "class_id": "kubernetes.StatefulSetStatus", "object_type": "kubernetes.StatefulSetStatus", "current_revision": "CurrentRevision 1", "updated_replicas": 32, "observed_generation": 32, "collision_count": 32, "ready_replicas": 32, "replicas": 32, "available_replicas": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapMacpoolBlock(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"object_type": "macpool.Block", "size": 32, "to": "To 1", "from": "From 1", "class_id": "macpool.Block"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiaapiNewReleaseDetail(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ReleaseNoteLink":"ReleaseNoteLink %d","SoftwareDownloadLink":"SoftwareDownloadLink %d","Title":"Title %d","ObjectType":"niaapi.NewReleaseDetail","Description":"Description %d","Version":"Version %d","SoftwareDownloadLinkTitle":"SoftwareDownloadLinkTitle %d","ClassId":"niaapi.NewReleaseDetail","Link":"Link %d","ReleaseNoteLinkTitle":"ReleaseNoteLinkTitle %d"}`
...
func TestFlattenMapNiatelemetryBootflashDetails(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.BootflashDetails","Serial":"Serial %d","FwRev":"FwRev %d","ModelType":"ModelType %d","ClassId":"niatelemetry.BootflashDetails"}`
	...
	expectedOp := map[string]interface{}{"object_type": "niatelemetry.BootflashDetails", "serial": "Serial 1", "fw_rev": "FwRev 1", "model_type": "ModelType 1", "class_id": "niatelemetry.BootflashDetails"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryInterface(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"InterfaceUpCount":32,"ObjectType":"niatelemetry.Interface","ClassId":"niatelemetry.Interface","InterfaceDownCount":32}`
	...
	expectedOp := map[string]interface{}{"interface_up_count": 32, "object_type": "niatelemetry.Interface", "class_id": "niatelemetry.Interface", "interface_down_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapNiatelemetryNvePacketCounters(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"niatelemetry.NvePacketCounters","McastInpkts":32,"McastOutbytes":32,"UcastInpkts":32,"UcastOutpkts":32,"ObjectType":"niatelemetry.NvePacketCounters"}`
	...
	expectedOp := map[string]interface{}{"class_id": "niatelemetry.NvePacketCounters", "mcast_inpkts": 32, "mcast_outbytes": 32, "ucast_inpkts": 32, "ucast_outpkts": 32, "object_type": "niatelemetry.NvePacketCounters"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNveVni(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.NveVni","CpVniCount":32,"CpVniDown":32,"CpVniUp":32,"DpVniCount":32,"DpVniDown":32,"ClassId":"niatelemetry.NveVni","DpVniUp":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "niatelemetry.NveVni", "cp_vni_count": 32, "cp_vni_down": 32, "cp_vni_up": 32, "dp_vni_count": 32, "dp_vni_down": 32, "class_id": "niatelemetry.NveVni", "dp_vni_up": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosBgpMvpn(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"TableVersion":32,"NumberOfCommunities":32,"ConfiguredPeers":32,"TotalNetworks":32,"CapablePeers":32,"TotalPaths":32,"MemoryUsed":32,"NumberOfClusterLists":32,"ClassId":"niatelemetry.NxosBgpMvpn","ObjectType":"niatelemetry.NxosBgpMvpn"}`
	...
	expectedOp := map[string]interface{}{"table_version": 32, "number_of_communities": 32, "configured_peers": 32, "total_networks": 32, "capable_peers": 32, "total_paths": 32, "memory_used": 32, "number_of_cluster_lists": 32, "class_id": "niatelemetry.NxosBgpMvpn", "object_type": "niatelemetry.NxosBgpMvpn"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetryNxosVtp(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"niatelemetry.NxosVtp","Version":32,"OperMode":"OperMode %d","ClassId":"niatelemetry.NxosVtp","PruningMode":"PruningMode %d","RunningVersion":"RunningVersion %d","TrapEnabled":"TrapEnabled %d","V2Mode":"V2Mode %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "niatelemetry.NxosVtp", "nr_version": 32, "oper_mode": "OperMode 1", "class_id": "niatelemetry.NxosVtp", "pruning_mode": "PruningMode 1", "running_version": "RunningVersion 1", "trap_enabled": "TrapEnabled 1", "v2_mode": "V2Mode 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapNiatelemetrySmartLicense(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"LicenseUdi":"LicenseUdi %d","SmartAccount":"SmartAccount %d","ActiveMode":"ActiveMode %d","AuthStatus":"AuthStatus %d","ClassId":"niatelemetry.SmartLicense","ObjectType":"niatelemetry.SmartLicense"}`
	...
	expectedOp := map[string]interface{}{"license_udi": "LicenseUdi 1", "smart_account": "SmartAccount 1", "active_mode": "ActiveMode 1", "auth_status": "AuthStatus 1", "class_id": "niatelemetry.SmartLicense", "object_type": "niatelemetry.SmartLicense"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapOnpremSchedule(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"WeekOfMonth":32,"RepeatInterval":32,"TimeOfDay":32,"ObjectType":"onprem.Schedule","DayOfMonth":32,"TimeZone":"TimeZone %d","DayOfWeek":32,"MonthOfYear":32,"ClassId":"onprem.Schedule"}`
	...
	expectedOp := map[string]interface{}{"week_of_month": 32, "repeat_interval": 32, "time_of_day": 32, "object_type": "onprem.Schedule", "day_of_month": 32, "time_zone": "TimeZone 1", "day_of_week": 32, "month_of_year": 32, "class_id": "onprem.Schedule"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapOnpremUpgradePhase(t *testing.T) {
	p := models.OnpremUpgradePhase{}
	var d = &schema.ResourceData{}
	c := `{"ClassId":"onprem.UpgradePhase","ElapsedTime":32,"Message":"Message %d","ObjectType":"onprem.UpgradePhase","Failed":true,"Name":"Name %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "onprem.UpgradePhase", "elapsed_time": 32, "message": "Message 1", "object_type": "onprem.UpgradePhase", "failed": true, "name": "Name 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsAnswers(t *testing.T) {
	p := models.OsAnswers{}
	var d = &schema.ResourceData{}
	c := `{"IpConfigType":"IpConfigType %d","IsRootPasswordSet":true,"IsAnswerFileSet":true,"ProductKey":"ProductKey %d","Source":"Source %d","ObjectType":"os.Answers","ClassId":"os.Answers","Hostname":"Hostname %d","Nameserver":"Nameserver %d","IsRootPasswordCrypted":true}`
	...
	expectedOp := map[string]interface{}{"ip_config_type": "IpConfigType 1", "is_root_password_set": true, "is_answer_file_set": true, "product_key": "ProductKey 1", "nr_source": "Source 1", "object_type": "os.Answers", "class_id": "os.Answers", "hostname": "Hostname 1", "nameserver": "Nameserver 1", "is_root_password_crypted": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapOsGlobalConfig(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"os.GlobalConfig","OsImageName":"OsImageName %d","ConfigurationSource":"ConfigurationSource %d","ObjectType":"os.GlobalConfig","InstallMethod":"InstallMethod %d","ConfigurationFileName":"ConfigurationFileName %d","InstallTargetType":"InstallTargetType %d","ScuImageName":"ScuImageName %d","WindowsEdition":"WindowsEdition %d"}`
...
func TestFlattenMapPolicyConfigResultContext(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"policy.ConfigResultContext","ObjectType":"policy.ConfigResultContext","EntityMoid":"EntityMoid %d","EntityName":"EntityName %d","EntityType":"EntityType %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "policy.ConfigResultContext", "object_type": "policy.ConfigResultContext", "entity_moid": "EntityMoid 1", "entity_name": "EntityName 1", "entity_type": "EntityType 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapStorageAutomaticDriveGroup(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"storage.AutomaticDriveGroup","NumberOfSpans":32,"MinimumDriveSize":32,"ClassId":"storage.AutomaticDriveGroup","UseRemainingDrives":true,"DriveType":"DriveType %d","DrivesPerSpan":32,"NumDedicatedHotSpares":"NumDedicatedHotSpares %d"}`
	...
	expectedOp := map[string]interface{}{"object_type": "storage.AutomaticDriveGroup", "number_of_spans": 32, "minimum_drive_size": 32, "class_id": "storage.AutomaticDriveGroup", "use_remaining_drives": true, "drive_type": "DriveType 1", "drives_per_span": 32, "num_dedicated_hot_spares": "NumDedicatedHotSpares 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapStorageBaseCapacity(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Available":32,"CapacityUtilization":32.000000,"Free":32,"Total":32,"Used":32,"ObjectType":"storage.BaseCapacity","ClassId":"storage.BaseCapacity"}`
	...
	expectedOp := map[string]interface{}{"available": 32, "capacity_utilization": 32.000000, "free": 32, "total": 32, "used": 32, "object_type": "storage.BaseCapacity", "class_id": "storage.BaseCapacity"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapUuidpoolUuidBlock(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"object_type": "uuidpool.UuidBlock", "to": "To 1", "from": "From 1", "size": 32, "class_id": "uuidpool.UuidBlock"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationCpuInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.CpuInfo","Speed":32,"Vendor":"Vendor %d","Cores":32,"Description":"Description %d","Sockets":32,"ClassId":"virtualization.CpuInfo"}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.CpuInfo", "speed": 32, "vendor": "Vendor 1", "cores": 32, "description": "Description 1", "sockets": 32, "class_id": "virtualization.CpuInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationMemoryAllocation(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Free":32,"Reserved":32,"Total":32,"Used":32,"ClassId":"virtualization.MemoryAllocation","ObjectType":"virtualization.MemoryAllocation"}`
	...
	expectedOp := map[string]interface{}{"free": 32, "reserved": 32, "total": 32, "used": 32, "class_id": "virtualization.MemoryAllocation", "object_type": "virtualization.MemoryAllocation"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationProductInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"virtualization.ProductInfo","ObjectType":"virtualization.ProductInfo","Build":"Build %d","ProductName":"ProductName %d","ProductType":"ProductType %d","ProductVendor":"ProductVendor %d","Version":"Version %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "virtualization.ProductInfo", "object_type": "virtualization.ProductInfo", "build": "Build 1", "product_name": "ProductName 1", "product_type": "ProductType 1", "product_vendor": "ProductVendor 1", "nr_version": "Version 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareRemoteDisplayInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RemoteDisplayVncKey":"RemoteDisplayVncKey %d","RemoteDisplayVncPort":32,"RemoteDisplayPassword":"RemoteDisplayPassword %d","ObjectType":"virtualization.VmwareRemoteDisplayInfo","ClassId":"virtualization.VmwareRemoteDisplayInfo"}`
	...
	expectedOp := map[string]interface{}{"remote_display_vnc_key": "RemoteDisplayVncKey 1", "remote_display_vnc_port": 32, "remote_display_password": "RemoteDisplayPassword 1", "object_type": "virtualization.VmwareRemoteDisplayInfo", "class_id": "virtualization.VmwareRemoteDisplayInfo"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareTeamingAndFailover(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"NotifySwitches":true,"Name":"Name %d","ClassId":"virtualization.VmwareTeamingAndFailover","ObjectType":"virtualization.VmwareTeamingAndFailover","NetworkFailureDetection":"NetworkFailureDetection %d","Failback":true,"LoadBalancing":"LoadBalancing %d"}`
	...
	expectedOp := map[string]interface{}{"notify_switches": true, "name": "Name 1", "class_id": "virtualization.VmwareTeamingAndFailover", "object_type": "virtualization.VmwareTeamingAndFailover", "network_failure_detection": "NetworkFailureDetection 1", "failback": true, "load_balancing": "LoadBalancing 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVirtualizationVmwareVmCpuSocketInfo(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"virtualization.VmwareVmCpuSocketInfo","ClassId":"virtualization.VmwareVmCpuSocketInfo","NumCpus":32,"NumSockets":32,"CoresPerSocket":32}`
	...
	expectedOp := map[string]interface{}{"object_type": "virtualization.VmwareVmCpuSocketInfo", "class_id": "virtualization.VmwareVmCpuSocketInfo", "num_cpus": 32, "num_sockets": 32, "cores_per_socket": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicCdn(t *testing.T) {
	p := models.VnicCdn{}
	var d = &schema.ResourceData{}
	c := `{"ObjectType":"vnic.Cdn","Value":"Value %d","Source":"Source %d","ClassId":"vnic.Cdn"}`
	...
	expectedOp := map[string]interface{}{"object_type": "vnic.Cdn", "value": "Value 1", "nr_source": "Source 1", "class_id": "vnic.Cdn"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicEthInterruptSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"CoalescingTime":32,"CoalescingType":"CoalescingType %d","Count":32,"Mode":"Mode %d","ObjectType":"vnic.EthInterruptSettings","ClassId":"vnic.EthInterruptSettings"}`
	...
	expectedOp := map[string]interface{}{"coalescing_time": 32, "coalescing_type": "CoalescingType 1", "nr_count": 32, "mode": "Mode 1", "object_type": "vnic.EthInterruptSettings", "class_id": "vnic.EthInterruptSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcErrorRecoverySettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"IoRetryTimeout":32,"LinkDownTimeout":32,"ClassId":"vnic.FcErrorRecoverySettings","ObjectType":"vnic.FcErrorRecoverySettings","PortDownTimeout":32,"Enabled":true,"IoRetryCount":32}`
	...
	expectedOp := map[string]interface{}{"io_retry_timeout": 32, "link_down_timeout": 32, "class_id": "vnic.FcErrorRecoverySettings", "object_type": "vnic.FcErrorRecoverySettings", "port_down_timeout": 32, "enabled": true, "io_retry_count": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicFcInterruptSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.FcInterruptSettings","Mode":"Mode %d","ObjectType":"vnic.FcInterruptSettings"}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.FcInterruptSettings", "mode": "Mode 1", "object_type": "vnic.FcInterruptSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicPlacementSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"PciLink":32,"SwitchId":"SwitchId %d","Uplink":32,"Id":"Id %d","ClassId":"vnic.PlacementSettings","ObjectType":"vnic.PlacementSettings"}`
	...
	expectedOp := map[string]interface{}{"pci_link": 32, "switch_id": "SwitchId 1", "uplink": 32, "id": "Id 1", "class_id": "vnic.PlacementSettings", "object_type": "vnic.PlacementSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicRoceSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"Version":32,"ObjectType":"vnic.RoceSettings","QueuePairs":32,"ResourceGroups":32,"ClassOfService":32,"Enabled":true,"ClassId":"vnic.RoceSettings","MemoryRegions":32}`
	...
	expectedOp := map[string]interface{}{"nr_version": 32, "object_type": "vnic.RoceSettings", "queue_pairs": 32, "resource_groups": 32, "class_of_service": 32, "enabled": true, "class_id": "vnic.RoceSettings", "memory_regions": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicRssHashSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"UdpIpv6Hash":true,"Ipv4Hash":true,"UdpIpv4Hash":true,"TcpIpv6Hash":true,"ObjectType":"vnic.RssHashSettings","TcpIpv6ExtHash":true,"Ipv6ExtHash":true,"ClassId":"vnic.RssHashSettings","TcpIpv4Hash":true,"Ipv6Hash":true}`
	...
	expectedOp := map[string]interface{}{"udp_ipv6_hash": true, "ipv4_hash": true, "udp_ipv4_hash": true, "tcp_ipv6_hash": true, "object_type": "vnic.RssHashSettings", "tcp_ipv6_ext_hash": true, "ipv6_ext_hash": true, "class_id": "vnic.RssHashSettings", "tcp_ipv4_hash": true, "ipv6_hash": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicScsiQueueSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.ScsiQueueSettings","Count":32,"RingSize":32,"ObjectType":"vnic.ScsiQueueSettings"}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.ScsiQueueSettings", "nr_count": 32, "ring_size": 32, "object_type": "vnic.ScsiQueueSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicTcpOffloadSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RxChecksum":true,"TxChecksum":true,"LargeReceive":true,"LargeSend":true,"ClassId":"vnic.TcpOffloadSettings","ObjectType":"vnic.TcpOffloadSettings"}`
	...
	expectedOp := map[string]interface{}{"rx_checksum": true, "tx_checksum": true, "large_receive": true, "large_send": true, "class_id": "vnic.TcpOffloadSettings", "object_type": "vnic.TcpOffloadSettings"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapVnicVlanSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.VlanSettings","ObjectType":"vnic.VlanSettings","DefaultVlan":32,"Mode":"Mode %d","AllowedVlans":"AllowedVlans %d"}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.VlanSettings", "object_type": "vnic.VlanSettings", "default_vlan": 32, "mode": "Mode 1", "allowed_vlans": "AllowedVlans 1"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapVnicVmqSettings(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"ClassId":"vnic.VmqSettings","ObjectType":"vnic.VmqSettings","NumVmqs":32,"VmmqAdapterPolicy":"VmmqAdapterPolicy %d","NumSubVnics":32,"Enabled":true,"MultiQueueSupport":true,"NumInterrupts":32}`
	...
	expectedOp := map[string]interface{}{"class_id": "vnic.VmqSettings", "object_type": "vnic.VmqSettings", "num_vmqs": 32, "vmmq_adapter_policy": "VmmqAdapterPolicy 1", "num_sub_vnics": 32, "enabled": true, "multi_queue_support": true, "num_interrupts": 32}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"RetryDelay":32,"RetryPolicy":"RetryPolicy %d","TimeoutPolicy":"TimeoutPolicy %d","RetryCount":32,"ClassId":"workflow.Properties","ObjectType":"workflow.Properties","SupportStatus":"SupportStatus %d","Timeout":32,"ExternalMeta":true}`
	...
	expectedOp := map[string]interface{}{"retry_delay": 32, "retry_policy": "RetryPolicy 1", "timeout_policy": "TimeoutPolicy 1", "retry_count": 32, "class_id": "workflow.Properties", "object_type": "workflow.Properties", "support_status": "SupportStatus 1", "timeout": 32, "external_meta": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
...
func TestFlattenMapWorkflowWorkflowProperties(t *testing.T) {
	...
	var d = &schema.ResourceData{}
	c := `{"SupportStatus":"SupportStatus %d","ClassId":"workflow.WorkflowProperties","ObjectType":"workflow.WorkflowProperties","Cloneable":true,"EnableDebug":true,"ExternalMeta":true,"Retryable":true}`
	...
	expectedOp := map[string]interface{}{"support_status": "SupportStatus 1", "class_id": "workflow.WorkflowProperties", "object_type": "workflow.WorkflowProperties", "cloneable": true, "enable_debug": true, "external_meta": true, "retryable": true}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
func TestFlattenMapX509Certificate(t *testing.T) {
	...
	expectedOp := map[string]interface{}{"object_type": "x509.Certificate", "sha256_fingerprint": "Sha256Fingerprint 1", "pem_certificate": "PemCertificate 1", "signature_algorithm": "SignatureAlgorithm 1", "class_id": "x509.Certificate"}
	err = compareMaps(expectedOp, ffOp, t)
	CheckError(t, err)
}
