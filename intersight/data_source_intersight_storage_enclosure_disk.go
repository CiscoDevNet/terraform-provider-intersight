...
func dataSourceStorageEnclosureDisk() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStorageEnclosureDiskRead,
...
func dataSourceStorageEnclosureDiskRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStorageEnclosureDiskWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageEnclosureDiskList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageEnclosureDisk: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageEnclosureDiskWithDefaults()
			...
			if err := d.Set("physical_disk", flattenMapStoragePhysicalDiskRelationship(s.GetPhysicalDisk(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property PhysicalDisk: %+v", err)
			}
			...
			if err := d.Set("sas_address1", (s.GetSasAddress1())); err != nil {
				return fmt.Errorf("error occurred while setting property SasAddress1: %+v", err)
			}
			if err := d.Set("sas_address2", (s.GetSasAddress2())); err != nil {
				return fmt.Errorf("error occurred while setting property SasAddress2: %+v", err)
			}
