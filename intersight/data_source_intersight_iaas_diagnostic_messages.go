...
func dataSourceIaasDiagnosticMessages() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceIaasDiagnosticMessagesRead,
...
func dataSourceIaasDiagnosticMessagesRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIaasDiagnosticMessagesWithDefaults()
	...
	res, _, err := conn.ApiClient.IaasApi.GetIaasDiagnosticMessagesList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IaasDiagnosticMessages: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIaasDiagnosticMessagesWithDefaults()
			...
			if err := d.Set("last_checked", (s.GetLastChecked())); err != nil {
				return fmt.Errorf("error occurred while setting property LastChecked: %+v", err)
			}
