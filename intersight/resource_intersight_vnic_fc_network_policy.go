...
func resourceVnicFcNetworkPolicy() *schema.Resource {
	return &schema.Resource{
		Create: resourceVnicFcNetworkPolicyCreate,
		Read:   resourceVnicFcNetworkPolicyRead,
		Update: resourceVnicFcNetworkPolicyUpdate,
		Delete: resourceVnicFcNetworkPolicyDelete,
			...
			"vsan_settings": {
				...
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"id": {
							Description: "Default VLAN of the virtual interface. Setting the value to 0 is equivalent to None and will not associate any Default VLAN to the traffic on the virtual interface.",
							Type:        schema.TypeInt,
							Optional:    true,
						},
						"object_type": {
...
func resourceVnicFcNetworkPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	return resourceVnicFcNetworkPolicyRead(d, meta)
}

func resourceVnicFcNetworkPolicyRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
...
func resourceVnicFcNetworkPolicyUpdate(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewVnicFcNetworkPolicyWithDefaults()
...
func resourceVnicFcNetworkPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
