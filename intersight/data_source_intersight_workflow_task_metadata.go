...
func dataSourceWorkflowTaskMetadata() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceWorkflowTaskMetadataRead,
...
func dataSourceWorkflowTaskMetadataRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.WorkflowApi.GetWorkflowTaskMetadataList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to WorkflowTaskMetadata: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
