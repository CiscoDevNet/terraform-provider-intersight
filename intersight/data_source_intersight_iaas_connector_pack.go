...
func dataSourceIaasConnectorPackRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIaasConnectorPackWithDefaults()
	...
	res, _, err := conn.ApiClient.IaasApi.GetIaasConnectorPackList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to IaasConnectorPack: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIaasConnectorPackWithDefaults()
