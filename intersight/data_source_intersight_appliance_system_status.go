...
func dataSourceApplianceSystemStatus() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceApplianceSystemStatusRead,
...
func dataSourceApplianceSystemStatusRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewApplianceSystemStatusWithDefaults()
	...
	res, _, err := conn.ApiClient.ApplianceApi.GetApplianceSystemStatusList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ApplianceSystemStatus: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewApplianceSystemStatusWithDefaults()
			...
			if err := d.Set("app_statuses", flattenListApplianceAppStatusRelationship(s.GetAppStatuses(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property AppStatuses: %+v", err)
			}
			...
			if err := d.Set("group_statuses", flattenListApplianceGroupStatusRelationship(s.GetGroupStatuses(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property GroupStatuses: %+v", err)
			}
			...
			if err := d.Set("system_info", flattenMapApplianceSystemInfoRelationship(s.GetSystemInfo(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property SystemInfo: %+v", err)
			}
