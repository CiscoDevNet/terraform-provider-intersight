...
func dataSourceApplianceNodeStatus() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceApplianceNodeStatusRead,
...
func dataSourceApplianceNodeStatusRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewApplianceNodeStatusWithDefaults()
	...
	res, _, err := conn.ApiClient.ApplianceApi.GetApplianceNodeStatusList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ApplianceNodeStatus: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewApplianceNodeStatusWithDefaults()
			...
			if err := d.Set("cpu_usage", (s.GetCpuUsage())); err != nil {
				return fmt.Errorf("error occurred while setting property CpuUsage: %+v", err)
			}

			if err := d.Set("file_system_statuses", flattenListApplianceFileSystemStatusRelationship(s.GetFileSystemStatuses(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property FileSystemStatuses: %+v", err)
			}
			if err := d.Set("mem_usage", (s.GetMemUsage())); err != nil {
				return fmt.Errorf("error occurred while setting property MemUsage: %+v", err)
			}
			...
			if err := d.Set("node_info", flattenMapApplianceNodeInfoRelationship(s.GetNodeInfo(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property NodeInfo: %+v", err)
			}
			if err := d.Set("node_state", (s.GetNodeState())); err != nil {
				return fmt.Errorf("error occurred while setting property NodeState: %+v", err)
			}
