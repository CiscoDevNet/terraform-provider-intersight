...
func resourceTamAdvisoryInstance() *schema.Resource {
	return &schema.Resource{
		Create: resourceTamAdvisoryInstanceCreate,
		Read:   resourceTamAdvisoryInstanceRead,
...
func resourceTamAdvisoryInstanceRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
...
func resourceTamAdvisoryInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
