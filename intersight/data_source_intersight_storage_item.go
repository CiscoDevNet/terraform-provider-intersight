...
func dataSourceStorageItem() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStorageItemRead,
...
func dataSourceStorageItemRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStorageItemWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageItemList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageItem: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageItemWithDefaults()
			...
			if err := d.Set("alarm_type", (s.GetAlarmType())); err != nil {
				return fmt.Errorf("error occurred while setting property AlarmType: %+v", err)
			}
			...
			if err := d.Set("used", (s.GetUsed())); err != nil {
				return fmt.Errorf("error occurred while setting property Used: %+v", err)
			}
			d.SetId(s.GetMoid())
		}
	}
