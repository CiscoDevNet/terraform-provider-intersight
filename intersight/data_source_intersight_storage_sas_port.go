...
func dataSourceStorageSasPortRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStorageSasPortWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageSasPortList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageSasPort: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageSasPortWithDefaults()
			...
			if err := d.Set("end_point_id", (s.GetEndPointId())); err != nil {
				return fmt.Errorf("error occurred while setting property EndPointId: %+v", err)
			}
			...
			if err := d.Set("link_description", (s.GetLinkDescription())); err != nil {
				return fmt.Errorf("error occurred while setting property LinkDescription: %+v", err)
			}
