...
func dataSourceSoftwareApplianceDistributable() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceSoftwareApplianceDistributableRead,
...
func dataSourceSoftwareApplianceDistributableRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewSoftwareApplianceDistributableWithDefaults()
	...
	res, _, err := conn.ApiClient.SoftwareApi.GetSoftwareApplianceDistributableList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to SoftwareApplianceDistributable: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewSoftwareApplianceDistributableWithDefaults()
