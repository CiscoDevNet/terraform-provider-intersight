...
func dataSourceInventoryDeviceInfo() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceInventoryDeviceInfoRead,
...
func dataSourceInventoryDeviceInfoRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewInventoryDeviceInfoWithDefaults()
	...
	res, _, err := conn.ApiClient.InventoryApi.GetInventoryDeviceInfoList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to InventoryDeviceInfo: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewInventoryDeviceInfoWithDefaults()
			...
			if err := d.Set("control_action", (s.GetControlAction())); err != nil {
				return fmt.Errorf("error occurred while setting property ControlAction: %+v", err)
			}
			if err := d.Set("error_state", (s.GetErrorState())); err != nil {
				return fmt.Errorf("error occurred while setting property ErrorState: %+v", err)
			}

			if err := d.Set("job_info", flattenListPolicyinventoryJobInfo(s.GetJobInfo(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property JobInfo: %+v", err)
			}
			...
			if err := d.Set("profile_mo_id", (s.GetProfileMoId())); err != nil {
				return fmt.Errorf("error occurred while setting property ProfileMoId: %+v", err)
			}
