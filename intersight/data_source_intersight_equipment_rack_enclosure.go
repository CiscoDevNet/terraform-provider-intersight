...
func dataSourceEquipmentRackEnclosure() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceEquipmentRackEnclosureRead,
...
func dataSourceEquipmentRackEnclosureRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewEquipmentRackEnclosureWithDefaults()
	...
	res, _, err := conn.ApiClient.EquipmentApi.GetEquipmentRackEnclosureList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to EquipmentRackEnclosure: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewEquipmentRackEnclosureWithDefaults()
			...
			if err := d.Set("enclosure_id", (s.GetEnclosureId())); err != nil {
				return fmt.Errorf("error occurred while setting property EnclosureId: %+v", err)
			}
			...
			if err := d.Set("slots", flattenListEquipmentRackEnclosureSlotRelationship(s.GetSlots(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property Slots: %+v", err)
			}
