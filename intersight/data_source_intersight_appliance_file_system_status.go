...
func dataSourceApplianceFileSystemStatus() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceApplianceFileSystemStatusRead,
...
func dataSourceApplianceFileSystemStatusRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewApplianceFileSystemStatusWithDefaults()
	...
	res, _, err := conn.ApiClient.ApplianceApi.GetApplianceFileSystemStatusList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to ApplianceFileSystemStatus: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewApplianceFileSystemStatusWithDefaults()
			...
			if err := d.Set("mountpoint", (s.GetMountpoint())); err != nil {
				return fmt.Errorf("error occurred while setting property Mountpoint: %+v", err)
			}

			if err := d.Set("node_status", flattenMapApplianceNodeStatusRelationship(s.GetNodeStatus(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property NodeStatus: %+v", err)
			}
			...
			if err := d.Set("usage", (s.GetUsage())); err != nil {
				return fmt.Errorf("error occurred while setting property Usage: %+v", err)
			}
			d.SetId(s.GetMoid())
		}
	}
