...
func dataSourceVnicScpStatus() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceVnicScpStatusRead,
...
func dataSourceVnicScpStatusRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewVnicScpStatusWithDefaults()
	...
	res, _, err := conn.ApiClient.VnicApi.GetVnicScpStatusList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to VnicScpStatus: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewVnicScpStatusWithDefaults()
			...
			if err := d.Set("vhba_info", flattenListVnicVifStatus(s.GetVhbaInfo(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property VhbaInfo: %+v", err)
			}
			d.SetId(s.GetMoid())
		}
	}
