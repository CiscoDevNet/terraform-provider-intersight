...
func dataSourceStoragePureProtectionGroup() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStoragePureProtectionGroupRead,
...
func dataSourceStoragePureProtectionGroupRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStoragePureProtectionGroupWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStoragePureProtectionGroupList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StoragePureProtectionGroup: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStoragePureProtectionGroupWithDefaults()
			...
			if err := d.Set("host_groups", flattenListStoragePureHostGroupRelationship(s.GetHostGroups(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property HostGroups: %+v", err)
			}
			...
			if err := d.Set("replication_enabled", (s.GetReplicationEnabled())); err != nil {
				return fmt.Errorf("error occurred while setting property ReplicationEnabled: %+v", err)
			}
			...
			if err := d.Set("snapshot_enabled", (s.GetSnapshotEnabled())); err != nil {
				return fmt.Errorf("error occurred while setting property SnapshotEnabled: %+v", err)
			}
			...
			if err := d.Set("targets", (s.GetTargets())); err != nil {
				return fmt.Errorf("error occurred while setting property Targets: %+v", err)
			}
