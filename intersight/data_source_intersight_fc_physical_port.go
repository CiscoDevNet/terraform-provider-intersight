...
func dataSourceFcPhysicalPort() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFcPhysicalPortRead,
...
func dataSourceFcPhysicalPortRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFcPhysicalPortWithDefaults()
	...
	res, _, err := conn.ApiClient.FcApi.GetFcPhysicalPortList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FcPhysicalPort: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFcPhysicalPortWithDefaults()
			...
			if err := d.Set("b2b_credit", (s.GetB2bCredit())); err != nil {
				return fmt.Errorf("error occurred while setting property B2bCredit: %+v", err)
			}
			...
			if err := d.Set("max_speed", (s.GetMaxSpeed())); err != nil {
				return fmt.Errorf("error occurred while setting property MaxSpeed: %+v", err)
			}
