...
func dataSourceStorageHitachiController() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceStorageHitachiControllerRead,
...
func dataSourceStorageHitachiControllerRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewStorageHitachiControllerWithDefaults()
	...
	res, _, err := conn.ApiClient.StorageApi.GetStorageHitachiControllerList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to StorageHitachiController: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewStorageHitachiControllerWithDefaults()
			...
			if err := d.Set("operational_mode", (s.GetOperationalMode())); err != nil {
				return fmt.Errorf("error occurred while setting property OperationalMode: %+v", err)
			}
