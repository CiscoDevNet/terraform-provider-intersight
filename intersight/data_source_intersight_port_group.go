...
func dataSourcePortGroupRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewPortGroupWithDefaults()
	...
	res, _, err := conn.ApiClient.PortApi.GetPortGroupList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to PortGroup: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewPortGroupWithDefaults()
			...
			if err := d.Set("equipment_shared_io_module", flattenMapEquipmentSharedIoModuleRelationship(s.GetEquipmentSharedIoModule(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property EquipmentSharedIoModule: %+v", err)
			}
			...
			if err := d.Set("sub_groups", flattenListPortSubGroupRelationship(s.GetSubGroups(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property SubGroups: %+v", err)
			}
