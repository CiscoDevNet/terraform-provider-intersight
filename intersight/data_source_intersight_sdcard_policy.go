...
func dataSourceSdcardPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceSdcardPolicyRead,
...
func dataSourceSdcardPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.SdcardApi.GetSdcardPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to SdcardPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewSdcardPolicyWithDefaults()
			...
			if err := d.Set("partitions", flattenListSdcardPartition(s.GetPartitions(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property Partitions: %+v", err)
			}
