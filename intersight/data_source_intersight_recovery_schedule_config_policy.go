...
func dataSourceRecoveryScheduleConfigPolicy() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceRecoveryScheduleConfigPolicyRead,
...
func dataSourceRecoveryScheduleConfigPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.RecoveryApi.GetRecoveryScheduleConfigPolicyList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to RecoveryScheduleConfigPolicy: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewRecoveryScheduleConfigPolicyWithDefaults()
