...
func dataSourceFirmwareSasExpanderDescriptor() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFirmwareSasExpanderDescriptorRead,
...
func dataSourceFirmwareSasExpanderDescriptorRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFirmwareSasExpanderDescriptorWithDefaults()
	...
	res, _, err := conn.ApiClient.FirmwareApi.GetFirmwareSasExpanderDescriptorList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	var s = &models.FirmwareSasExpanderDescriptorList{}
	err = json.Unmarshal(x, s)
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FirmwareSasExpanderDescriptor: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFirmwareSasExpanderDescriptorWithDefaults()
