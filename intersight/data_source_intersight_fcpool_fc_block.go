...
func dataSourceFcpoolFcBlock() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceFcpoolFcBlockRead,
...
func dataSourceFcpoolFcBlockRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewFcpoolFcBlockWithDefaults()
	...
	res, _, err := conn.ApiClient.FcpoolApi.GetFcpoolFcBlockList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to FcpoolFcBlock: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewFcpoolFcBlockWithDefaults()
			...
			if err := d.Set("id_block", flattenMapFcpoolBlock(s.GetIdBlock(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property IdBlock: %+v", err)
			}
