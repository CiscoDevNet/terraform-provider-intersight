...
func dataSourceBootVmediaDevice() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceBootVmediaDeviceRead,
...
func dataSourceBootVmediaDeviceRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewBootVmediaDeviceWithDefaults()
	...
	res, _, err := conn.ApiClient.BootApi.GetBootVmediaDeviceList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to BootVmediaDevice: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewBootVmediaDeviceWithDefaults()
			...
			if err := d.Set("compute_physical", flattenMapComputePhysicalRelationship(s.GetComputePhysical(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property ComputePhysical: %+v", err)
			}
