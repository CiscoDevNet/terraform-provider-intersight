...
func dataSourceBootPchStorageDevice() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceBootPchStorageDeviceRead,
...
func dataSourceBootPchStorageDeviceRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewBootPchStorageDeviceWithDefaults()
	...
	res, _, err := conn.ApiClient.BootApi.GetBootPchStorageDeviceList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to BootPchStorageDevice: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewBootPchStorageDeviceWithDefaults()
