...
func dataSourceIppoolShadowPoolRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewIppoolShadowPoolWithDefaults()
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewIppoolShadowPoolWithDefaults()
			...
			if err := d.Set("v4_block_heads", flattenListIppoolShadowBlockRelationship(s.GetV4BlockHeads(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property V4BlockHeads: %+v", err)
			}
