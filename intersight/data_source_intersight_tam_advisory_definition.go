...
func dataSourceTamAdvisoryDefinition() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceTamAdvisoryDefinitionRead,
...
func dataSourceTamAdvisoryDefinitionRead(d *schema.ResourceData, meta interface{}) error {
	...
	res, _, err := conn.ApiClient.TamApi.GetTamAdvisoryDefinitionList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to TamAdvisoryDefinition: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewTamAdvisoryDefinitionWithDefaults()
