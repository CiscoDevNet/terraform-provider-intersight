...
func dataSourceHyperflexHxapVirtualDisk() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceHyperflexHxapVirtualDiskRead,
...
func dataSourceHyperflexHxapVirtualDiskRead(d *schema.ResourceData, meta interface{}) error {
	log.SetFlags(log.LstdFlags | log.Lshortfile)
	log.Printf("%v", meta)
	conn := meta.(*Config)
	var o = models.NewHyperflexHxapVirtualDiskWithDefaults()
	...
	res, _, err := conn.ApiClient.HyperflexApi.GetHyperflexHxapVirtualDiskList(conn.ctx).Filter(getRequestParams(data)).Execute()
	if err != nil {
	...
	if err != nil {
		return fmt.Errorf("error occurred while unmarshalling response to HyperflexHxapVirtualDisk: %+v", err)
	}
	result := s.GetResults()
	if result == nil {
	...
	case reflect.Slice:
		...
		for i := 0; i < r.Len(); i++ {
			var s = models.NewHyperflexHxapVirtualDiskWithDefaults()
			oo, _ := json.Marshal(r.Index(i).Interface())
			if err = json.Unmarshal(oo, s); err != nil {
				return fmt.Errorf("error occurred while unmarshalling result at index %+v: %+v", i, err)
			}
			if err := d.Set("access_mode", (s.GetAccessMode())); err != nil {
				return fmt.Errorf("error occurred while setting property AccessMode: %+v", err)
			}
			...
			if err := d.Set("source_file_path", (s.GetSourceFilePath())); err != nil {
				return fmt.Errorf("error occurred while setting property SourceFilePath: %+v", err)
			}
			if err := d.Set("source_virtual_disk", (s.GetSourceVirtualDisk())); err != nil {
				return fmt.Errorf("error occurred while setting property SourceVirtualDisk: %+v", err)
			}
			...
			if err := d.Set("virtual_machine", flattenMapHyperflexHxapVirtualMachineRelationship(s.GetVirtualMachine(), d)); err != nil {
				return fmt.Errorf("error occurred while setting property VirtualMachine: %+v", err)
			}
			d.SetId(s.GetMoid())
		}
	}
