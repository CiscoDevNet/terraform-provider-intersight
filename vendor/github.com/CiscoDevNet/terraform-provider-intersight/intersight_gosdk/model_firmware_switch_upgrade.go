...
// FirmwareSwitchUpgrade The SwitchUpgrade object is designed for upgrading firmware on Fabric Interconnects, streamlining the process of downloading and applying firmware images from cloud repositories or network shares. #### Purpose SwitchUpgrade simplifies the firmware upgrade process for Fabric Interconnects, enhancing network performance and security by facilitating timely updates. This provides options for both direct download and network share-based upgrades for UCSM devices. IMM Fabric Interconnect supports direct download. #### Key Concepts - **Fabric Interconnect Optimization:** Tailored to address the specific needs of Fabric Interconnects, ensuring efficient and effective firmware upgrades. - **Evacuation Management:** Includes features to manage fabric evacuation during upgrades, enhancing operational continuity and minimizing disruptions. - **Flexible Deployment:** Supports both direct download from Cisco sources and network share-based upgrades, catering to varied infrastructure configurations. - **Security and Compliance:** Implements rigorous access and licensing controls, ensuring upgrades are performed securely and in compliance with organizational policies.
type FirmwareSwitchUpgrade struct {
	...
	// Name of the Fabric Interconnect on which the firmware upgrade operation is performed.
...
func (o *FirmwareSwitchUpgrade) UnmarshalJSON(data []byte) (err error) {
	...
	if err == nil {
		...
		varFirmwareSwitchUpgrade.SwitchName = varFirmwareSwitchUpgradeWithoutEmbeddedStruct.SwitchName
