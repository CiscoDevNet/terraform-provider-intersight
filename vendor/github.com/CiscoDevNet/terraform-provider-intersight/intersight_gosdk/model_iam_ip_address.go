...
// IamIpAddress IpAddress represents a single allowlisted IP, CIDR block, or range used by IpAccessManagement. #### Purpose It models an individual entry in the IP access policy to refine allowed sources. #### Key Concepts - **Entry Granularity:** Supports point IPs, CIDR, and address ranges. - **Descriptive Policy:** Accommodates human-readable rationale for entries. - **Policy Membership:** Always associated with an IpAccessManagement policy. - **Enforcement-ready:** Used directly in access control evaluations.
