...
// AaaRetentionPolicy # Overview  The RetentionPolicy object governs audit log retention policies at the account level,    specifying the duration for which audit logs are preserved before automatic deletion.  ## Purpose  A RetentionPolicy defines the time frame for audit log retention,    ensuring logs are kept for an appropriate duration to meet organizational and regulatory requirements.    This facilitates systematic management of log retention, balancing storage needs with compliance obligations. ## Key Concepts - **Policy Management** – Allows administrators to define and update retention periods, providing flexibility for changing compliance needs. - **Automatic Cleanup** – Enables automatic log deletion based on retention periods, reducing manual overhead and ensuring efficient storage use. - **Access Control** – Privilege sets govern the creation, reading, and updating of RetentionPolicies, ensuring only authorized users can modify configurations. - **Account Association** – Each RetentionPolicy is linked to specific accounts, enabling tailored retention strategies for different organizational units.
...
// GetDefaultClassId returns the default value "aaa.RetentionPolicy" of the ClassId field.
func (o *AaaRetentionPolicy) GetDefaultClassId() interface{} {
	return "aaa.RetentionPolicy"
}
...
// GetDefaultObjectType returns the default value "aaa.RetentionPolicy" of the ObjectType field.
func (o *AaaRetentionPolicy) GetDefaultObjectType() interface{} {
	return "aaa.RetentionPolicy"
}
