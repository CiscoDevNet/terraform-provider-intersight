...
func (dst *EquipmentFanResponse) UnmarshalJSON(data []byte) error {
	...
	// check if the discriminator value is 'equipment.Fan.List'
	if jsonDict["ObjectType"] == "equipment.Fan.List" {
		// try to unmarshal JSON data into EquipmentFanList
		err = json.Unmarshal(data, &dst.EquipmentFanList)
		if err == nil {
			return nil // data stored in dst.EquipmentFanList, return on the first match
		} else {
			dst.EquipmentFanList = nil
			return fmt.Errorf("Failed to unmarshal EquipmentFanResponse as EquipmentFanList: %s", err.Error())
		}
	}
	...
	if jsonDict["ObjectType"] == "mo.AggregateTransform" {
		...
		if err == nil {
			...
			return fmt.Errorf("Failed to unmarshal EquipmentFanResponse as MoAggregateTransform: %s", err.Error())
		}
	}
	...
	if jsonDict["ObjectType"] == "mo.DocumentCount" {
		...
		if err == nil {
			...
			return fmt.Errorf("Failed to unmarshal EquipmentFanResponse as MoDocumentCount: %s", err.Error())
		}
	}
	...
	if jsonDict["ObjectType"] == "mo.TagSummary" {
		...
		if err == nil {
			...
			return fmt.Errorf("Failed to unmarshal EquipmentFanResponse as MoTagSummary: %s", err.Error())
		}
	}
...
func (src EquipmentFanResponse) MarshalJSON() ([]byte, error) {
	if src.EquipmentFanList != nil {
		return json.Marshal(&src.EquipmentFanList)
	}
...
type NullableEquipmentFanResponse struct {
	value *EquipmentFanResponse
	isSet bool
}

func (v NullableEquipmentFanResponse) Get() *EquipmentFanResponse {
	return v.value
}

func (v *NullableEquipmentFanResponse) Set(val *EquipmentFanResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableEquipmentFanResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableEquipmentFanResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableEquipmentFanResponse(val *EquipmentFanResponse) *NullableEquipmentFanResponse {
	return &NullableEquipmentFanResponse{value: val, isSet: true}
}

func (v NullableEquipmentFanResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableEquipmentFanResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
