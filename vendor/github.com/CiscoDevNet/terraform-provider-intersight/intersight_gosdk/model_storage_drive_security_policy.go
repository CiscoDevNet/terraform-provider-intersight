...
// StorageDriveSecurityPolicy The drive security policy models the KMIP server related configuration that can be applied on multiple servers. Storage controller encryption can be enabled through this policy using remote keys from a KMIP server.
...
func (o *StorageDriveSecurityPolicy) UnmarshalJSON(bytes []byte) (err error) {
	...
	err = json.Unmarshal(bytes, &varStorageDriveSecurityPolicyWithoutEmbeddedStruct)
	if err == nil {
	...
	err = json.Unmarshal(bytes, &varStorageDriveSecurityPolicy)
	if err == nil {
