...
// EtherNetworkPort The ether.NetworkPort object represents a network-facing port on a Fabric Extender (FEX) or I/O Module (IOM). These ports, also known as Network Interfaces (NIFs), provide the uplink connection from the FEX/IOM to the parent Fabric Interconnect. #### Purpose The NetworkPort object is used to model and inventory the physical uplink ports of a FEX or IOM. It captures the port's identity, its operational speed, and its connection status to the parent switch. It is essential for monitoring the health and bandwidth of the connection between the fabric extender and the core fabric. #### Key Concepts - **Uplink Port Model:** Represents a physical port used for uplinking a FEX or IOM to a Fabric Interconnect. - **Connectivity Status:** The operReason property provides specific details on the link's status, such as LinkMissing or LinkMisconnect.
