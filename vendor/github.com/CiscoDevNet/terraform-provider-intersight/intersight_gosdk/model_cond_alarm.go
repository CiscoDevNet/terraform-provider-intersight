...
// CondAlarm The Alarm object represents the alarms generated by either the managed system itself or by Intersight. It is used for identifying and responding to issues detected across various managed objects. #### Purpose The Alarm object is crucial for tracking the state and severity of issues, providing a structured approach to alarm management. It ensures that alarms are reported consistently and can be acknowledged or suppressed based on user needs. #### Key Concepts - **Severity:** Alarms are categorized by severity levels (Critical, Warning, Info, and Cleared), offering a standardized way to prioritize and respond to detected issues. - **Acknowledgement and Suppression:** Users can acknowledge alarms to exclude them from health status and overall counts, while suppression settings control visibility. - **Integrated Relationships:** Alarms are linked to definitions and affected managed objects, ensuring comprehensive tracking and management of issues.
...
func (o *CondAlarm) SetLocationDetailsNil() {
...
func (o *CondAlarm) UnmarshalJSON(data []byte) (err error) {
	...
	if err == nil {
		...
		varCondAlarm.LocationDetails = varCondAlarmWithoutEmbeddedStruct.LocationDetails
