...
// checks if the BootSdDevice type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BootSdDevice{}

// BootSdDevice The boot.SdDevice object represents an SD (Secure Digital) card as a configurable boot option within a server's boot policy. This typically refers to an internal SD card slot, often used for booting hypervisors. #### Purpose The SdDevice object provides a standardized way to include an SD card in a server's boot order. It allows administrators to define the SD card as a bootable source, set its priority, and manage its state (enabled/disabled) within a boot policy. It is particularly useful for embedded operating systems or hypervisor installations. #### Key Concepts - **Embedded OS Boot:** Defines an internal SD card as a bootable source, commonly used for lightweight hypervisors like VMware ESXi.  - **Ordered Boot Sequence:** The order property allows its position in the boot process to be controlled relative to other devices. - **State Management:** The state property allows the SD card to be easily enabled or disabled as a boot option within a policy. - **Boot Policy Component:** Acts as an element within a boot policy that is applied to a compute.Physical server object.
...
// GetDefaultClassId returns the default value "boot.SdDevice" of the ClassId field.
func (o *BootSdDevice) GetDefaultClassId() interface{} {
	return "boot.SdDevice"
}
...
// GetDefaultObjectType returns the default value "boot.SdDevice" of the ObjectType field.
func (o *BootSdDevice) GetDefaultObjectType() interface{} {
	return "boot.SdDevice"
}
...
func (o *BootSdDevice) SetComputePhysicalNil() {
...
func (o *BootSdDevice) SetInventoryDeviceInfoNil() {
...
func (o *BootSdDevice) SetRegisteredDeviceNil() {
...
func (o BootSdDevice) ToMap() (map[string]interface{}, error) {
...
func (o *BootSdDevice) UnmarshalJSON(data []byte) (err error) {
	...
	err = json.Unmarshal(data, &varBootSdDeviceWithoutEmbeddedStruct)
	if err == nil {
	...
	err = json.Unmarshal(data, &varBootSdDevice)
	if err == nil {
