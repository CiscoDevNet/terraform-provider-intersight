...
type FabricServerRole struct {
	...
	// Forward error correction configuration for server port. This configuration is required only for FEX Model N9K-C93180YC-FX3 connected with 25G speed ports on UCS-FI-6454/UCS-FI-64108 and should be set as Cl74. * `Auto` - Forward error correction option 'Auto'. * `Cl91` - Forward error correction option 'cl91'. * `Cl74` - Forward error correction option 'cl74'. * `rs-cons16` - Forward Error Correction option \"rs-cons16\". * `rs-ieee` - Forward Error Correction option \"rs-ieee\". * `Off` - Turn off Forward Error Correction.
