...
type AssetMeteringType struct {
	...
	// Metric type used to calculate metering for the device sent from the IB Contract. example  Node, vMemory, vCPU. * `None` - A default value to catch cases where metric type is not correctly detected. * `Node` - The metering of the device is on the basis of Power state. * `Storage` - The metering of the device is on the basis of used Storage. * `vMemory` - The metering of the device is on the basis of VM Memory. * `vCPU` - The metering of the device is on the basis of vCPU. * `vStorage` - The metering of the device is on the basis of used virtual Storage.
